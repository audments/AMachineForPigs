...
tWString ToFormat(ePixelFormat aPixelFormat) {
    if (aPixelFormat == ePixelFormat_DXT1)
        return _W("DXT1");
    if (aPixelFormat == ePixelFormat_DXT2)
        return _W("DXT2");
    if (aPixelFormat == ePixelFormat_DXT3)
        return _W("DXT3");
    if (aPixelFormat == ePixelFormat_DXT4)
        return _W("DXT4");
    if (aPixelFormat == ePixelFormat_DXT5)
        return _W("DXT5");
...
bool ConvertFile(const tWString &asFile) {
    ...
    sArg += _W(" -mf TRIANGLE");
    // sArg += _W(" -m ")+ cString::ToStringW(GetMipMapNum(pBitMap->GetWidth(), pBitMap->GetHeight())); //<- skip so
    // tool chooses! sArg += _W(" -sx _test_"); //Only when testing!
    sArg += _W(" -nologo");
    sArg += _W(" \"") + asFile + _W("\"");
