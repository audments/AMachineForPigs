...
void cParticleEditorWindowEmitterParams::OnInitLayout() {
    ...
    mpTFCategories =
        mpSet->CreateWidgetTabFrame(cVector3f(5, 5, 0.1f), GetSize() - cVector2f(10, 4), _W(""), mpBGFrame);
    ...
    pTab = mpTFCategories->AddTab(_W("Movement"));
    {
        ...
        mpInpCoordSys = CreateInputEnum(vPos + cVector3f(mpInpVelType->GetSize().x, 0, 0), _W("Coord System"), "",
                                        lstValues, pGroup);
        ...
        vPos = cVector3f(10, 20, 0.1f);
        mpInpSphVelMinAng =
            CreateInputVec2(vPos, _W("Min Angles(deg)"), "InputSphVelMinAngles", pGroup, 50, lstLabelXYZ);
        vPos.x += mpInpSphVelMinAng->GetSize().x + 20;
        mpInpSphVelMaxAng =
            CreateInputVec2(vPos, _W("Max Angles(deg)"), "InputSphVelMaxAngles", pGroup, 50, lstLabelXYZ);
        ...
        mpInpSpeedMul = CreateInputVec2(vPos + cVector3f(0, mpInpMinAcc->GetSize().y, 0), _W("Speed multiplier"),
                                        "InputSpeedMul", pGroup, 50, lstLabelMinMax);
        vPos = cVector3f(240, 10, 0.1f);
        mpInpMaxAcc = CreateInputVec3(vPos, _W("Max acceleration(m/s^2)"), "InputMaxAccel", pGroup, 50, lstLabelXYZ);
        mpInpMaxSpeed = CreateInputVec2(vPos + cVector3f(0, mpInpMaxAcc->GetSize().y, 0), _W("Maximum speed"),
                                        "InputMaxSpeed", pGroup, 50, lstLabelMinMax);
    }
    ...
    pTab = mpTFCategories->AddTab(_W("Rendering"));
    {
        ...
        pInp = mpInpMinSize;
        mpInpMaxSize = CreateInputVec2(vPos + cVector3f(pInp->GetSize().x + 20, 0, 0), _W("Max Size(m)"),
                                       "InputMaxSize", pGroup, 50, lstLabelXYZ);
        pInp = mpInpMaxSize;
        vPos.y += pInp->GetSize().y;
        ...
        mpInpMiddleRelSizeStart = CreateInputNumber(vPos + cVector3f(pInp->GetSize().x, 0, 0),
                                                    _W("Middle Relative start:"), "InputMidRelSize", pGroup);
        ...
        mpInpMiddleRelSizeLength = CreateInputNumber(vPos + cVector3f(pInp->GetSize().x, 0, 0),
                                                     _W("Middle Relative length:"), "InputMidRelLen", pGroup);
    ...
    pTab = mpTFCategories->AddTab(_W("Color"));
    {
        ...
        mpInpMiddleRelColStart = CreateInputNumber(vPos + cVector3f(mpInpStartRelCol->GetSize().x + 50, 0, 0),
                                                   _W("Middle relative start"), "InputMidRelStart", pGroup);
        ...
        mpInpMiddleRelColLength = CreateInputNumber(vPos + cVector3f(mpInpMiddleRelCol->GetSize().x + 41, 0, 0),
                                                    _W("Middle relative length"), "InputMidRelLen", pGroup);
    ...
    pTab = mpTFCategories->AddTab(_W("Life"));
    {
        ...
        vPos = cVector3f(10, 10, 0.1f);
        mpInpLifeSpan = CreateInputVec2(vPos, _W("Life Span(s)"), "InputLifeSpan", pGroup, 50, lstLabelMinMax,
    ...
    pTab = mpTFCategories->AddTab(_W("Material"));
    {
        ...
        mpInpMaterial->SetInitialPath(cString::AddSlashAtEndW(mpEditor->GetFolder(eEditorDir_Particles)) +
                                      _W("materials"));
    ...
    pTab = mpTFCategories->AddTab(_W("Rotation"));
    {
        ...
        vPos.y += 50;
        mpInpSpinVelRange = CreateInputVec2(vPos, _W("Particle Spin Velocity Range(rad/s)"), "InputSpinVelRange",
                                            pGroup, 50, lstLabelMinMax);
