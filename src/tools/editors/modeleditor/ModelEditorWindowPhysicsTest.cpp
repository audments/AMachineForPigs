...
cModelEditorWindowPhysicsTest::cModelEditorWindowPhysicsTest(iEditorBase *apEditor, iFrameBuffer *apFB)
    : iEditorWindow(apEditor, "Physics Test Window"),
      iEditorViewport(apEditor, apEditor->GetEngine()->GetScene()->CreateWorld("PhysicsTestWorld"), apFB) {
...
int cModelEditorWindowPhysicsTest::GetSelectedAnimation() { return mpInpAnimations->GetValue(); }
...
void cModelEditorWindowPhysicsTest::SetUpModel() {
    ...
    mpTestEntity = (cMeshEntity *)pLoader->LoadEntityFromElement(0, "TestEntity", pDoc, mpTestWorld, "", sFullPath,
...
void cModelEditorWindowPhysicsTest::OnInitLayout() {
    ...
    mpInpBuoyancySurfaceHeight = CreateInputNumber(vPos + cVector3f(0, mpInpBuoyancyDensity->GetSize().y + 5, 0),
                                                   _W("S.Height"), "", mpBGFrame, 50, 0.1f);
    ...
    vPos.x = mpInpBuoyancyDensity->GetPosition().x +
             cMath::Max(mpInpBuoyancyDensity->GetSize().x, mpInpBuoyancySurfaceHeight->GetSize().x) + 10;
    ...
    mpInpBuoyancyLinearViscosity =
        CreateInputNumber(vPos + cVector3f(0, mpInpBuoyancyAngularViscosity->GetSize().y + 5, 0), _W("B.L.viscosity"),
                          "", mpBGFrame, 50, 0.1f);
...
bool cModelEditorWindowPhysicsTest::OnViewportUpdate(const cGuiMessageData &aData) {
    ...
    mbViewMode =
        mbViewMode || pInput->GetKeyboard()->KeyIsDown(eKey_LeftSuper) || pInput->GetKeyboard()->KeyIsDown(eKey_Z);
#endif
    ...
    if (mBodyPicker.mpPickedBody) {
        // Get pos of start.
        mBodyPicker.mvPos = cMath::MatrixMul(mBodyPicker.mpPickedBody->GetLocalMatrix(), mBodyPicker.mvLocalPos);
        ...
        // Spring testing:
        cVector3f vForce = (vDragPos - mBodyPicker.mvPos) * 20 - (mBodyPicker.mpPickedBody->GetLinearVelocity() * 0.4f);

        mBodyPicker.mpPickedBody->AddForceAtPosition(vForce * mBodyPicker.mpPickedBody->GetMass(), mBodyPicker.mvPos);
    }
...
bool cModelEditorWindowPhysicsTest::OnViewportKeyPress(const cGuiMessageData &aData) { return true; }

//------------------------------------------------------------------------------------

bool cModelEditorWindowPhysicsTest::OnViewportKeyRelease(const cGuiMessageData &aData) { return true; }
