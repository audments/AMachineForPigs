...
cModelEditorActionMeshLoad::cModelEditorActionMeshLoad(cModelEditorWorld *apEditorWorld, const tString &asFilename)
    ...
    mlstOldSubMeshIDs =
        pWorld->GetSubMeshType()->GetSubMeshIDs();        // ((cModelEditorWorld*)mpEditorWorld)->GetSubMeshIDs();
...
void cModelEditorActionMeshLoad::DoModify() {
    ...
    tEntityDataVec temp;
    ((cModelEditorWorld *)mpEditorWorld)
        ->GetSubMeshType()
        ->SetMesh(msNewMeshFilename, true, temp, mlstSubMeshIDs, temp, mlstBoneIDs);
}
...
void cModelEditorActionMeshLoad::UndoModify() {
    ...
    ((cModelEditorWorld *)mpEditorWorld)
        ->GetSubMeshType()
        ->SetMesh(msOldMeshFilename, true, temp, mlstOldSubMeshIDs, temp, mlstOldBoneIDs);
