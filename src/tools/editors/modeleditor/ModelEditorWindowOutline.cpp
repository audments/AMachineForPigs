...
cModelEditorWindowOutlineHelper::cModelEditorWindowOutlineHelper(cModelEditorWindowOutline *apWindow, tIntList &alstIDs)
    : iEditorWindowPopUp(apWindow->GetEditor(), "Attach helper window", true, true, true, cVector2f(230, 400)),
      mlstTempIDs(alstIDs) {
...
cModelEditorWindowOutlineHelper::~cModelEditorWindowOutlineHelper() { hplDelete(mpFilter); }
...
void cModelEditorWindowOutlineHelper::ExcludeIDs(tIntList &alstIDs) { tIntListIt it = alstIDs.begin(); }
...
void cModelEditorWindowOutlineHelper::OnInitLayout() {
    ...
    for (int i = 0; i < 2; i++) {
        mvButtons[i] =
            mpSet->CreateWidgetButton(vPos + cVector3f(10.05f + i * 45, 0, 0), cVector2f(40, 25), _W(""), mpWindow);
...
void cModelEditorWindowOutlineHelper::OnSetActive(bool abX) { iEditorWindowPopUp::OnSetActive(abX); }
...
void cModelEditorWindowOutlineHelper::SetMultiSelectionEnabled(bool abX) { mpListObjects->SetAllowMultiSelection(abX); }
...
void cModelEditorWindowOutline::AddButtonAttachChild() {
    mpButtonAttachChild =
        mpSet->CreateWidgetButton(cVector3f(0, 60, 0), cVector2f(100, 25), _W("Attach Child"), mpGroupPanel);
...
void cModelEditorWindowOutline::AddButtonDetachParent() {
    mpButtonDetachParent =
        mpSet->CreateWidgetButton(cVector3f(0, 30, 0), cVector2f(100, 25), _W("Detach Parent"), mpGroupPanel);
...
void cModelEditorWindowOutline::AddButtonDetachChild() {
    mpButtonDetachChild =
        mpSet->CreateWidgetButton(cVector3f(0, 90, 0), cVector2f(100, 25), _W("Detach Child"), mpGroupPanel);
...
bool cModelEditorWindowOutline::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (apWidget == mpCheckBoxHideShapes) {
        ...
        if (pEnt)
            pAction = hplNew(cEditorActionEntityDetachChildren,
                             (mpEditor->GetEditorWorld(), pEnt->GetParent()->GetID(), lstIDs));
        ...
        // hplNew(cEditorActionJointAttachBody,(pEnt->GetID(), mpEditor->GetEditorWorld(),
        // eEditorJointAttachedBody_Parent, -1));
    } else if (apWidget == mpButtonDetachChild) {
        pAction = pEnt->CreateSetPropertyActionInt(eJointInt_ChildID, -1);
        // pAction = hplNew(cEditorActionJointAttachBody,(pEnt->GetID(), mpEditor->GetEditorWorld(),
        // eEditorJointAttachedBody_Child, -1));
    }
...
bool cModelEditorWindowOutline::AttachParentBodyCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pJoint->GetTypeID() == eEditorEntityType_Joint) {
        ...
        // mpEditor->AddAction(hplNew(cEditorActionJointAttachBody,(pJoint->GetID(), mpEditor->GetEditorWorld(),
        // eEditorJointAttachedBody_Parent, lBodyID)));
    }
...
bool cModelEditorWindowOutline::SetAttachmentsCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pEnt) {
        mpEditor->AddAction(
            hplNew(cEditorActionEntityAttachChildren, (mpEditor->GetEditorWorld(), pEnt->GetID(), mlstTempIDs)));
    }
    return true;
}
...
cModelEditorWindowOutline::CreateHelperWindow(tIntList &alstIDs, tGuiCallbackFunc apCallback, bool abMulti) {
