...
void cModelEditorWindowAnimations::OnInitLayout() {
    ...
    mpListAnimations =
        mpSet->CreateWidgetListBox(pLabel->GetLocalPosition() + cVector3f(0, 25, 1), cVector2f(200, 270), mpWindow);

    mpListAnimations->AddCallback(eGuiMessage_SelectionChange, this, kGuiCallback(InputCallback));

    ///////////////////////////////////////////
    // Add animation button
    mpBAddAnimation = mpSet->CreateWidgetButton(mpListAnimations->GetLocalPosition() + cVector3f(0, 275, 0),
                                                cVector2f(100, 25), _W("Add"), mpWindow);

    mpBAddAnimation->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(InputCallback));

    ///////////////////////////////////////////
    // Remove animation button
    mpBRemAnimation = mpSet->CreateWidgetButton(mpListAnimations->GetLocalPosition() + cVector3f(110, 275, 0),
                                                cVector2f(100, 25), _W("Remove"), mpWindow);
    ...
    cWidgetGroup *pGroupAnim =
        mpSet->CreateWidgetGroup(cVector3f(240, 60, 0.1f), cVector2f(400, 300), _W("Animation Data"), mpWindow);

    mpInpAnimName = CreateInputString(cVector3f(10, 10, 0.1f), _W("Name"), "", pGroupAnim);
    mpInpAnimFile = CreateInputFile(mpInpAnimName->GetPosition() + cVector3f(mpInpAnimName->GetSize().x + 20, 0, 0),
                                    _W("File"), "", pGroupAnim);
    mpInpAnimFile->SetBrowserType(eEditorResourceType_ModelAnim);
    mpInpAnimSpeed = CreateInputNumber(mpInpAnimName->GetPosition() + cVector3f(0, mpInpAnimName->GetSize().y + 10, 0),
                                       _W("Speed"), "", pGroupAnim);
    mpInpAnimSpecialEventTime =
        CreateInputNumber(mpInpAnimFile->GetPosition() + cVector3f(0, mpInpAnimFile->GetSize().y + 10, 0),
                          _W("Special Event Time"), "", pGroupAnim);
    ...
    mpListEvents =
        mpSet->CreateWidgetListBox(mpLEvent->GetLocalPosition() + cVector3f(0, 25, 1), cVector2f(110, 100), pGroupAnim);

    mpListEvents->AddCallback(eGuiMessage_SelectionChange, this, kGuiCallback(InputCallback));

    mpBAddEvent = mpSet->CreateWidgetButton(mpListEvents->GetLocalPosition() + cVector3f(0, 110, 2), cVector2f(55, 25),
                                            _W("Add"), pGroupAnim);

    mpBAddEvent->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(InputCallback));

    mpBRemEvent = mpSet->CreateWidgetButton(mpListEvents->GetLocalPosition() + cVector3f(60, 110, 2), cVector2f(55, 25),
                                            _W("Rem"), pGroupAnim);

    mpBRemEvent->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(InputCallback));

    mpGEvent = mpSet->CreateWidgetGroup(mpListEvents->GetLocalPosition() + cVector3f(130, 0, 0.1f), cVector2f(200, 130),
                                        _W("Event"), pGroupAnim);

    /////////////////////////////////
    // Animation Event Attributes
    mpInpEventTime = CreateInputNumber(cVector3f(15, 15, 0.1f), _W("Time"), "", mpGEvent);
    mpInpEventType = CreateInputEnum(mpInpEventTime->GetPosition() + cVector2f(0, mpInpEventTime->GetSize().y),
                                     _W("Type"), "", tWStringList(), mpGEvent);
    mpInpEventType->AddValue(_W("PlaySound"));
    mpInpEventType->AddValue(_W("Step"));
    mpInpEventValue = CreateInputString(mpInpEventType->GetPosition() + cVector2f(0, mpInpEventType->GetSize().y),
                                        _W("Value"), "", mpGEvent);
}
...
bool cModelEditorWindowAnimations::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    if (apWidget == mpBAddAnimation) {
        ...
        if (bAnimationsValid) {
            ...
            mpEditor->ShowMessageBox(_W("Warning!"), _W("Animations were not saved due to errors (blank fields)"),
                                     _W("OK"), _W(""), NULL, NULL);
