...
cAnimationEventWrapper::cAnimationEventWrapper() { mlIndex = mlIndices++; }
...
cAnimationWrapper::cAnimationWrapper() { mfSpeed = 1; }
...
void cAnimationWrapper::ClearEvents() { mvEvents.clear(); }
...
void cModelEditorWorld::Reset() {
    ...
    cEditorUserClassDefinition *pDef =
        mpEditor->GetClassDefinitionManager()->GetDefinition(eUserClassDefinition_Entity);
...
cMeshEntity *cModelEditorWorld::GetMesh() { return mpTypeSubMesh->GetMesh(); }

tString cModelEditorWorld::GetMeshFilename() { return mpTypeSubMesh->GetMeshFilename(); }
...
void cModelEditorWorld::SetMeshFromElement(cXmlElement *apMeshElement, cXmlElement *apBonesElement) {
    ...
    mpTypeSubMesh->SetMesh(apMeshElement->GetAttributeString("Filename"), true, vSubMeshData, tIntList(), vBoneData,
                           tIntList());
}
...
cXmlElement *cModelEditorWorld::GetWorldObjectsElement(cXmlElement *apWorldDataElement) { return apWorldDataElement; }
