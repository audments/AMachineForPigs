...
cModelEditor::~cModelEditor() { OnSaveConfig(); }
...
bool cModelEditor::MainMenu_ItemClick(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    // Menu Item "File.Import Mesh"
        ...
        ShowLoadFilePicker(mvLoadFilenames, msLastMeshPath, this, kGuiCallback(MeshImport_Callback),
                           _W("Collada Meshes"), tWStringList(1, _W("*.dae")));
    }
    /////////////////////////////////////////////////////////////////
    // Menu Item "Edit.Search Entity"
    else if (apWidget == mpMainMenuSearch) {
        cEditorWindowEntitySearch *pWin =
            cEditorWindowFactory::CreateSearchWindow(this, (cEditorEditModeSelect *)GetEditMode("Select"));
        pWin->SetActive(true);
    }
    /////////////////////////////////////////////////////////////////
...
void cModelEditor::OnInit() {
    ...
    cEntityCustomVarHelper::GetSubTypesByTypeName(GetCustomVarDefinition(eCustomVarDefinition_Entity),
    msDefaultTypeName, lstSubTypes, msDefaultSubTypeName);
    */
...
void cModelEditor::OnSetUpDirectories() {
    ...
    // mpDirHandler->AddLookUpDir(eDir_Maps, sWorkingDir + mpMainConfig->GetStringW("Directories", "MapsDir",
    // _W("maps")), true);
}
...
void cModelEditor::OnSaveConfig() {
    ...
    for (tWStringListIt it = mlstRecentFiles.begin(); it != mlstRecentFiles.end(); ++it) {
        ...
        if (cPlatform::FileExists(sRecent)) {
            mpLocalConfig->SetString("Misc", "RecentFile" + cString::ToString(++i),
                                     (const tString &)cString::To8Char(sRecent));
        }
    }
...
bool cModelEditor::MeshImport_Callback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (aData.mlVal == 1) {
        ...
        msImportedMeshFilename =
            cString::To16Char(cString::Sub(sFilename, 0, cString::GetLastStringPos(sFilename, "."))) + _W(".ent");
...
iEditorWorld *cModelEditor::CreateSpecificWorld() { return hplNew(cModelEditorWorld, (this)); }
