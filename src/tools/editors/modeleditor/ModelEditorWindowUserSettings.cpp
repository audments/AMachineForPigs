...
void cModelEditorWindowUserSettings::OnSetActive(bool abX) {
    ...
    if (abX) {
        ...
        if (pClass) {
            ...
            mpEditor->ShowMessageBox(_W("Warning"), _W("Model has no type, falling back to defaults"), _W("OK"), _W(""),
                                     NULL, NULL);
        }
    } else {
    }
...
void cModelEditorWindowUserSettings::OnInitLayout() {
    ...
    mpFrameVars = mpSet->CreateWidgetFrame(cVector3f(mpWindow->GetSize().x * 0.025f, vPos.y, vPos.z),
                                           cVector2f(mpWindow->GetSize().x * 0.95f, mpWindow->GetSize().y * 0.8f),
                                           false, mpWindow, true, true);
