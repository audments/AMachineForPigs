...
iPrimitiveMeshCreator::iPrimitiveMeshCreator(cEditorEditModePrimitives *apEditMode) { mpEditMode = apEditMode; }

//---------------------------------------------------------------------------

iEntityWrapperData *iPrimitiveMeshCreator::CreateData() { return mpType->CreateData(); }
...
cPrimitiveMeshCreatorPlane::cPrimitiveMeshCreatorPlane(cEditorEditModePrimitives *apEditMode)
    : iPrimitiveMeshCreator(apEditMode) {}
...
void cPrimitiveMeshCreatorPlane::OnViewportMouseUp(int alButtons) { StoreMousePosition(); }
...
void cPrimitiveMeshCreatorPlane::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
void cPrimitiveMeshCreatorPlane::Reset() { mvCorners.clear(); }
...
bool cPrimitiveMeshCreatorPlane::IsDoneCreating() {
    ...
    for (int i = 0; i < 3; ++i) {
        if (mvCorners[0].v[i] == mvCorners[1].v[i])
            lNumEqualCoords++;
    }
...
cEditorEditModePrimitives::cEditorEditModePrimitives(iEditorBase *apEditor, iEditorWorld *apEditorWorld)
    : iEditorEditModeObjectCreator(apEditor, "Primitives", apEditorWorld) {}

//---------------------------------------------------------------------------

cEditorEditModePrimitives::~cEditorEditModePrimitives() { STLDeleteAll(mvMeshCreators); }
...
void cEditorEditModePrimitives::OnViewportMouseUp(int alButtons) {
    if (alButtons & eGuiMouseButton_Right) {
        ...
        return;
    } else if ((alButtons & eGuiMouseButton_Left) == 0)
        return;
...
void cEditorEditModePrimitives::DrawPostGrid(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
                                             const cVector3f &avPos) {
...
iEditorWindow *cEditorEditModePrimitives::CreateSpecificWindow() { return hplNew(cEditorWindowPrimitives, (this)); }
