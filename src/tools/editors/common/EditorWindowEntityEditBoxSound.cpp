...
cEditorWindowEntityEditBoxSound::cEditorWindowEntityEditBoxSound(cEditorEditModeSelect *apEditMode,
                                                                 cEntityWrapperSound *apSound)
    : cEditorWindowEntityEditBox(apEditMode, apSound) {
    mpEntity = apSound;
    if (msLastPath == _W(""))
...
void cEditorWindowEntityEditBoxSound::AddPropertySetSound(cWidgetTab *apParentTab) {
    ...
    mpInpDistance = CreateInputVec2(vPos, _W("Distance"), "", apParentTab, 50, vMinMax,
...
bool cEditorWindowEntityEditBoxSound::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpSoundEntity) {
        ...
        int vProps[] = {eSoundFloat_MinDist, eSoundFloat_MaxDist};
