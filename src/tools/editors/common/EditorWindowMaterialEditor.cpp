...
static tString gsTextureTypeStrings[] = {"1D", "2D", "Rect", "Cube", "3D"};

static tString gsWrapTypeStrings[] = {"Repeat", "Clamp", "ClampToEdge", "ClampToBorder"};
...
static tString gsBlendModeStrings[] = {"None", "Add", "Mul", "Mulx2", "Alpha", "PremulAlpha",

                                       ""};

//-------------------------------------------------------------------

cEditorClassMaterial::cEditorClassMaterial(cEditorWindowMaterialEditor *apWindow) { mpWindow = apWindow; }
...
bool cTextureWrapper::CheckFileIsUpdated() {
    ...
    tWString sFullPath =
        this->mpMat->mpMatEditor->GetEditor()->GetEngine()->GetResources()->GetFileSearcher()->GetFilePath(
            cString::To8Char(msFile));
...
void cMaterialWrapper::Save(const tWString &asFilename) {
    ...
    if (mvUVAnimations.empty() == false) {
        ...
        tString sTypeStrings[] = {"Translate", "Sin", "Rotate"};
        tString sAxisStrings[] = {"X", "Y", "Z"};
...
void cMaterialWrapper::ClearUVAnims() { mvUVAnimations.clear(); }
...
cTextureUnitPanel::cTextureUnitPanel(cEditorWindowMaterialEditor *apWin, eMaterialTexture aUnit) {
    ...
    mpImgThumb =
        pSet->CreateWidgetImage("", cVector3f(0, 0, 0.01f), cVector2f(100), eGuiMaterial_Diffuse, false, pFThumb);
...
void cTextureUnitPanel::Update() {
    ...
    mpInpWrap->SetValue(
        mpWindow->GetEditor()->GetEngine()->GetResources()->GetMaterialManager()->GetWrap(mpTextureWrapper->GetWrap()),
        false);
...
void cTextureUnitPanel::UpdateTexture() { mpTextureWrapper->Update(); }
...
cEditorWindowMaterialEditor::cEditorWindowMaterialEditor(iEditorBase *apEditor, iFrameBuffer *apFB,
                                                         ...
                                                         bool abStandAlone)
    : iEditorWindowPopUp(apEditor, "Material Editor Window", abStandAlone == false, abStandAlone == false,
                         abStandAlone == false, cVector2f(900, 725)),
...
void cEditorWindowMaterialEditor::Command_New() { Reset(); }

//------------------------------------------------------------------------------------

void cEditorWindowMaterialEditor::Command_Open() {
    cGuiPopUpFilePicker *pPicker = mpSet->CreatePopUpLoadFilePicker(
        mvTempLoadedFiles, false, cString::GetFilePathW(msMatFilename), false, this, kGuiCallback(FilePickerLoad));
...
void cEditorWindowMaterialEditor::Command_SaveAs() {
    if (mpMaterial->IsSaveable() == false)
        return;
    mpSet->CreatePopUpSaveFilePicker(msMatFilename, _W("Materials"), _W("*.mat"), cString::GetFilePathW(msMatFilename),
                                     false, this, kGuiCallback(FilePickerSave));
}
...
void cEditorWindowMaterialEditor::Reset() {
    ...
    ///////////////////////////////////////////////////////
    // Reset preview pane
    const tWString sCubeMapPath =
        mpEditor->GetEngine()->GetResources()->GetFileSearcher()->GetFilePath("editor_cubemap_default.dds");
...
void cEditorWindowMaterialEditor::CreateMaterial(const tString &asMat) { SetPreviewUpdated(); }
...
void cEditorWindowMaterialEditor::OnInit() { mpMaterial = hplNew(cMaterialWrapper, (this)); }
...
void cEditorWindowMaterialEditor::OnInitLayout() {
    ...
    // Preview
    {
        ...
        iTexture *pTex =
            mpEditor->GetEngine()->GetGraphics()->CreateTexture("", eTextureType_Rect, eTextureUsage_RenderTarget);
        ...
        mpInpPreviewModel = CreateInputEnum(cVector3f(10, 15, 0.1f) + cVector3f(0, GetGuiViewportSize().y, 0),
                                            _W("Model"), "", tWStringList(), pGroup);
        ...
        // Rotate Model
        mpInpRotateModel =
            CreateInputBool(mpInpPreviewModel->GetPosition() + cVector3f(mpInpPreviewModel->GetSize().x + 10, 15, 0),
                            _W("Rotate Model"), "", pGroup);

        // Background Type
        mpInpBGType =
            CreateInputEnum(mpInpPreviewModel->GetPosition() + cVector3f(0, mpInpPreviewModel->GetSize().y + 10, 0),
                            _W("Background Type"), "", tWStringList(), pGroup);
        mpInpBGType->AddValue(_W("CubeMap"));
        mpInpBGType->AddValue(_W("Flat"));

        // Background CubeMap
        mpInpBGCubeMap = CreateInputFile(mpInpBGType->GetPosition() + cVector3f(mpInpBGType->GetSize().x + 10, 0, 0),
                                         _W("CubeMap file"), "", pGroup);
        mpInpBGCubeMap->SetBrowserSubType(eEditorTextureResourceType_CubeMap);
        // Background Color
        mpInpBGColor =
            CreateInputColorFrame(mpInpBGCubeMap->GetPosition() + cVector3f(mpInpBGCubeMap->GetSize().x + 10, 15, 0),
                                  _W("Background color"), "", pGroup);

        // Lights color
        mpInpFixedLightColor =
            CreateInputColorFrame(mpInpBGType->GetPosition() + cVector3f(0, mpInpBGType->GetSize().y + 10, 0),
                                  _W("Fixed light color"), "", pGroup);
        mpInpMovableLightColor = CreateInputColorFrame(mpInpFixedLightColor->GetPosition() +
                                                           cVector3f(mpInpFixedLightColor->GetSize().x + 10, 0, 0),
                                                       _W("Movable light color"), "", pGroup);
    }
    ...
    // Animations
    {
        pGroup = mpSet->CreateWidgetGroup(pGroup->GetLocalPosition() + cVector3f(0, pGroup->GetSize().y + 10, 0),
                                          cVector2f(420, 80), _W("UV Animations"), pParent);
        AddWidget(pGroup);
        ...
        // Add anim button
        mpBAddAnim = mpSet->CreateWidgetButton(vPos + cVector3f(0, mpInpUVAnimation->GetSize().y + 5, 0),
                                               cVector2f(50, 20), _W("Create"), pGroup);
        AddWidget(mpBAddAnim);
        mpBAddAnim->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(ButtonPressed));

        // Remove anim button
        mpBRemAnim = mpSet->CreateWidgetButton(vPos + cVector3f(55, mpInpUVAnimation->GetSize().y + 5, 0),
                                               cVector2f(50, 20), _W("Delete"), pGroup);
        AddWidget(mpBAddAnim);
    ...
    pGroup = mpSet->CreateWidgetGroup(pGroup->GetLocalPosition() + cVector3f(0, pGroup->GetSize().y + 10, 0),
                                      cVector2f(420, 380), _W("Texture Units"), pParent);
    AddWidget(pGroup);
    ...
    {
        pGroup = mpSet->CreateWidgetGroup(pGroup->GetLocalPosition() + cVector3f(0, pGroup->GetSize().y + 10, 0),
                                          cVector2f(880, 90), _W("Material Specific Variables"), pParent);
        AddWidget(pGroup);

        mpFMaterialVars =
            mpSet->CreateWidgetFrame(cVector3f(15, 10, 0.1f), cVector2f(860, 75), false, pGroup, false, true);
...
bool cEditorWindowMaterialEditor::OnViewportUpdate(const cGuiMessageData &aData) {
    ...
    if (mbRotationEnabled) {
        mmtxRotation =
            cMath::MatrixMul(cMath::MatrixRotate(cVector3f(0, 0.005f, 0), eEulerRotationOrder_XYZ), mmtxRotation);
...
bool cEditorWindowMaterialEditor::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    // Background Type
        ...
        if (mpInpBGType->GetValue() == 0) {
            ...
            if (cEditorHelper::LoadTextureResource(eEditorTextureResourceType_CubeMap,
                                                   cString::To8Char(mpInpBGCubeMap->GetValue()), &pTexture)) {
