...
cEntityIcon::cEntityIcon(iEntityWrapper *apParent, const tString &asIconGfxName) {
    ...
    cTextureManager *pTexMgr =
        apParent->GetEditorWorld()->GetEditor()->GetEngine()->GetResources()->GetTextureManager();
...
cEntityIcon::~cEntityIcon() {
    cTextureManager *pTexMgr =
        mpParent->GetEditorWorld()->GetEditor()->GetEngine()->GetResources()->GetTextureManager();

    for (int i = 0; i < 2; ++i)
        if (mvIconGfx[i])
            pTexMgr->Destroy(mvIconGfx[i]);
}
...
bool cEntityIcon::CheckRayIntersect(cEditorWindowViewport *apViewport, cVector3f *apIntersection,
...
void cEntityIcon::DrawIcon(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
                           iEditorEditMode *apEditMode, bool abIsSelected, const cVector3f &avPos, bool abIsActive,
...
cBoundingVolume *cEntityIcon::GetPickBV(cEditorWindowViewport *apViewport, const cVector3f &avSize) {
    ...
    if (apViewport == NULL) {
        ...
        cEditorHelper::GetViewSpacePosAndSize(apViewport->GetCamera(), mpParent->GetPosition(), avSize, vPosition,
                                              vSize);
