...
iTypeNode::iTypeNode(iEdWorld *apWorld, int alID, const tWString &asName, iTypeNode *apParentType) {
    ...
    if (mpParentType)
        mpParentType->AddChildType(this);
}
...
iTypeNode *iEdWorld::GetTypeByName(const tWString &asName) {
    return mpRootType ? mpRootType->GetTypeByName(asName) : NULL;
}

iTypeNode *iEdWorld::GetTypeByID(int alID) { return mpRootType ? mpRootType->GetTypeByID(alID) : NULL; }

iTypeNode *iEdWorld::GetTypeByXmlElement(cXmlElement *apElement) {
    return mpRootType ? mpRootType->GetTypeByXmlElement(apElement) : NULL;
}
