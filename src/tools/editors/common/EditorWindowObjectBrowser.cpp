...
iEditorObjectIndexEntryMeshObject::iEditorObjectIndexEntryMeshObject(iEditorObjectIndexDir *apDir)
    : iEditorObjectIndexEntry(apDir) {}
...
cEditorWindowObjectBrowser::cEditorWindowObjectBrowser(iEditorEditMode *apEditMode, const tWStringVec &avBaseDirs,
                                                       bool abAddCategoryHeaders)
...
bool cEditorWindowObjectBrowser::Input_OnTextBoxEnter(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
...
void cEditorWindowObjectBrowser::OnInitLayout() {
    ...
    mpButtonRefresh =
        mpSet->CreateWidgetButton(cVector3f(10, 170, 0.1f), cVector2f(170, 25), _W("Refresh"), mpSelectionGroup);
    ...
    mpLabelThumbnail =
        mpSet->CreateWidgetLabel(cVector3f(30, 15, 0.1f), cVector2f(50, 25), _W("Thumbnail"), mpInfoGroup);
...
void cEditorWindowObjectBrowser::WriteInvalidFileListToFile(tWString &asFolder, tWStringList &alstInvalidFiles) {
    ...
    tString sInvalidFileList = cString::To8Char(mpEditor->GetFolderFullPath(eEditorDir_Home)) +
    mpEditor->GetInvalidFilesListFilename(); tString sFolder = cString::ReplaceCharTo(cString::To8Char(asFolder), "/",
    "\\");
...
void cEditorWindowObjectBrowser::AddEntriesInDirToList(iEditorObjectIndexDir *apDir,
                                                       std::vector<iEditorObjectIndexEntryMeshObject *> &avEntries) {
    ...
    for (; itSubDirs != mapSubDirs.end(); ++itSubDirs) {
        ...
        if (mbAddCategoryHeaders && apDir->GetParentDir() == NULL) {
            cWidgetItem *pDirHeader =
                mpObjectList->AddItem((pSubDir->IsExpanded() ? _W("- ") : _W("+ ")) + pSubDir->GetDirName());
