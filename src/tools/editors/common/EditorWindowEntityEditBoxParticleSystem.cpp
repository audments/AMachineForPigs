...
cEditorWindowEntityEditBoxParticleSystem::cEditorWindowEntityEditBoxParticleSystem(cEditorEditModeSelect *apEditMode,
                                                                                   cEntityWrapperParticleSystem *apPS)
    : cEditorWindowEntityEditBox(apEditMode, apPS) {
    mpEntity = apPS;
}
...
void cEditorWindowEntityEditBoxParticleSystem::OnUpdate(float afTimeStep) {
    ...
    if (mpInputFile)
        mpInputFile->SetText(cString::To16Char(mpEntity->GetFile()));

    if (mpInpFadeAtDistance)
        mpInpFadeAtDistance->SetValue(mpEntity->GetFadeAtDistance(), false);
    if (mpInpColor)
        mpInpColor->SetValue(mpEntity->GetColor(), false);
    if (mpInpMinFadeDistanceStart)
        mpInpMinFadeDistanceStart->SetValue(mpEntity->GetMinFadeDistanceStart(), false);
    if (mpInpMinFadeDistanceEnd)
        mpInpMinFadeDistanceEnd->SetValue(mpEntity->GetMinFadeDistanceEnd(), false);
    if (mpInpMaxFadeDistanceStart)
        mpInpMaxFadeDistanceStart->SetValue(mpEntity->GetMaxFadeDistanceStart(), false);
    if (mpInpMaxFadeDistanceEnd)
        mpInpMaxFadeDistanceEnd->SetValue(mpEntity->GetMaxFadeDistanceEnd(), false);
}
...
bool cEditorWindowEntityEditBoxParticleSystem::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (apWidget == mpInputFile) {
        ...
        if (sPSFile != "" && pPS == NULL) {
            ...
            // pAction = hplNew( cEditorActionParticleSystemSetStringProperty,(lID, pEditorWorld,
            // eParticleSystemStringProperty_File, sPSFile));
            if (pPS)
                pEditorWorld->GetWorld()->DestroyParticleSystem(pPS);
        }
    }
...
bool cEditorWindowEntityEditBoxParticleSystem::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpFadeAtDistance) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(eParticleSystemBool_FadeAtDistance, mpInpFadeAtDistance->GetValue());
    } else if (apInput == mpInpMinFadeDistanceEnd) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eParticleSystemFloat_MinFadeDistanceEnd,
                                                         mpInpMinFadeDistanceEnd->GetValue());
    } else if (apInput == mpInpMinFadeDistanceStart) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eParticleSystemFloat_MinFadeDistanceStart,
                                                         mpInpMinFadeDistanceStart->GetValue());
    } else if (apInput == mpInpMaxFadeDistanceStart) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eParticleSystemFloat_MaxFadeDistanceStart,
                                                         mpInpMaxFadeDistanceStart->GetValue());
    } else if (apInput == mpInpMaxFadeDistanceEnd) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eParticleSystemFloat_MaxFadeDistanceEnd,
                                                         mpInpMaxFadeDistanceEnd->GetValue());
...
bool cEditorWindowEntityEditBoxParticleSystem::BrowseButton_OnPressed(iWidget *apWidget, const cGuiMessageData &aData) {
    if (apWidget == mpButtonPSBrowse) {
        mpEditor->ShowLoadFilePicker(mvLoadedFiles, msLastPath, this, kGuiCallback(FilePSCallback),
                                     _W("Particle Systems"), tWStringList(1, _W("*.ps")));
    }
    return true;
}
