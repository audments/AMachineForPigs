...
cEditorObjectIndexEntryEntity::cEditorObjectIndexEntryEntity(cEditorObjectIndexDirEntities *apDir)
    : iEditorObjectIndexEntryMeshObject(apDir) {}
...
cMeshEntity *cEditorObjectIndexEntryEntity::CreateTempEntity(cWorld *apWorld) {
    ...
    cMeshEntity *pEnt = pEditor->GetEngineEntityLoader()->LoadEntFile(-1, "TempObject_" + sSourceFile, sSourceFile,
                                                                      apWorld, false, false, false, false, false);
...
cEditorObjectIndexDirEntities::cEditorObjectIndexDirEntities(cEditorObjectIndexEntities *apIndex,
                                                             cEditorObjectIndexDirEntities *apParent)
    : iEditorObjectIndexDir(apIndex, apParent) {}
...
cEditorObjectIndexEntities::cEditorObjectIndexEntities(iEditorBase *apEditor, const tWString &asBaseFolder)
    : iEditorObjectIndex(apEditor, asBaseFolder, _W("enp"), "Entities", "Entity", true) {
...
cEditorWindowEntities::cEditorWindowEntities(cEditorEditModeEntities *apEditMode, const tWStringVec &avBaseDirs)
    : cEditorWindowObjectBrowser(apEditMode, avBaseDirs, false) {
...
bool cEditorWindowEntities::Input_OnTextBoxEnter(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
...
cMeshEntity *cEditorWindowEntities::CreatePreviewEntity(iEditorObjectIndexEntryMeshObject *apEntry) {
    ...
    cMeshEntity *pEntity =
        pLoader->LoadEntFile(0, "PreviewEntity", apEntry->GetFileName(), pWorld, false, false, false, false, false);
