...
void iEditorWorld::LoadWorldObjects(cXmlElement *apWorldObjectsElement) {
    ...
    for (int i = 0; i < (int)mvEntityCategories.size(); ++i) {
        ...
        if (pCurrentXmlCategory == NULL || CustomCategoryLoader(apWorldObjectsElement, pCurrentXmlCategory) == true)
            continue;
    ...
    if (mbShowLoadErrorPopUp) {
        mpEditor->ShowMessageBox(_W("Warning!"),
                                 _W("Found some errors while loading file. Check log for details. ") + msLoadErrorMsg,
                                 _W("OK"), _W(""), NULL, NULL);
...
iEntityWrapperType *iEditorWorld::GetEntityTypeByElement(cXmlElement *apElement) {
    ...
    Error("No appropriate type found for %s \"%s\" with ID %d\n", apElement->GetValue().c_str(),
          apElement->GetAttributeString("Name").c_str(), apElement->GetAttributeInt("ID"));
    if (pDefaultType) {
...
int iEditorWorld::GetEntityTypeNum() { return (int)mlstEntityTypes.size(); }
...
void iEditorWorld::RemoveClipPlane(int alIdx, bool abDestroy) { RemoveClipPlane(mvClipPlanes[alIdx], abDestroy); }
...
void iEditorWorld::AddTypeToCategory(int alCatID, int alType) { mvEntityCategoryTypes[alCatID].push_back(alType); }

//----------------------------------------------------------------------------

cXmlElement *iEditorWorld::GetParentElementForObject(std::vector<cXmlElement *> &avParentElements,
                                                     iEntityWrapper *apEnt) {
...
void iEditorWorld::SetFogCulling(bool abX) { mpWorld->SetFogCulling(abX); }

//----------------------------------------------------------------------------

void iEditorWorld::SetFogStart(float afX) { mpWorld->SetFogStart(afX); }

//----------------------------------------------------------------------------

void iEditorWorld::SetFogEnd(float afX) { mpWorld->SetFogEnd(afX); }

//----------------------------------------------------------------------------

void iEditorWorld::SetFogFalloffExp(float afX) { mpWorld->SetFogFalloffExp(afX); }

//----------------------------------------------------------------------------

void iEditorWorld::SetFogColor(const cColor &aX) { mpWorld->SetFogColor(aX); }

//----------------------------------------------------------------------------

bool iEditorWorld::GetFogCulling() { return mpWorld->GetFogCulling(); }

//----------------------------------------------------------------------------

float iEditorWorld::GetFogStart() { return mpWorld->GetFogStart(); }

//----------------------------------------------------------------------------

float iEditorWorld::GetFogEnd() { return mpWorld->GetFogEnd(); }

//----------------------------------------------------------------------------

float iEditorWorld::GetFogFalloffExp() { return mpWorld->GetFogFalloffExp(); }

//----------------------------------------------------------------------------

const cColor &iEditorWorld::GetFogColor() { return mpWorld->GetFogColor(); }
...
void iEditorWorld::SetUpEntityWrapper(iEntityWrapper *apEntity, const cVector3f &avPosition,
                                      const cVector3f &avRotation, const cVector3f &avScale) {
    if (apEntity == NULL)
        return;
...
void iEditorWorld::DestroyEntityWrapper(iEntityWrapper *apEntity, bool abRemoveFromWorld) {
    ...
    if (abRemoveFromWorld)
        RemoveObject(apEntity);
