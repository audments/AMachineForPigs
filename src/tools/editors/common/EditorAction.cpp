...
iEditorActionWorldModifier::iEditorActionWorldModifier(const tString &asName, iEditorWorld *apEditorWorld)
    : iEditorAction(asName) {
...
cEditorActionCompoundAction::~cEditorActionCompoundAction() { STLDeleteAll(mvSubActions); }
...
bool cEditorActionCompoundAction::IsEmpty() { return mvSubActions.empty(); }
...
cEditorActionUserDefinedObjectSetVariable::cEditorActionUserDefinedObjectSetVariable(iEditorWorld *apWorld, int alID,
    ...
    : iEditorActionWorldModifier("Set Object Var", apWorld) {
    mlID = alID;
    msVarName = asName;
    msNewValue = asValue;
...
void cEditorActionUserDefinedObjectSetVariable::DoModify() { Apply(msNewValue); }

//---------------------------------------------------------------------

void cEditorActionUserDefinedObjectSetVariable::UndoModify() { Apply(msOldValue); }
...
iEditorActionAggregateAddComponents::iEditorActionAggregateAddComponents(iEditorWorld *apEditorWorld, int alID,
                                                                         const tIntList &alstComponentIDs)
    : iEditorActionWorldModifier("Add entities to Compound Object", apEditorWorld) {
    mlID = alID;
...
iEditorActionAggregateAddComponents::~iEditorActionAggregateAddComponents() { STLDeleteAll(mvOldAggregateData); }
...
iEditorActionAggregateRemoveComponents::iEditorActionAggregateRemoveComponents(iEditorWorld *apEditorWorld,
                                                                               const tIntList &alstComponentIDs)
    : iEditorActionWorldModifier("Detach entities from compound object", apEditorWorld) {
    mvComponentIDs.insert(mvComponentIDs.begin(), alstComponentIDs.begin(), alstComponentIDs.end());
}

//---------------------------------------------------------------------

iEditorActionAggregateRemoveComponents::~iEditorActionAggregateRemoveComponents() { STLDeleteAll(mvOldAggregateData); }
...
void iEditorActionAggregateRemoveComponents::UndoModify() {
    ...
    for (int i = 0; i < (int)mvComponentIDs.size(); ++i) {
        iEntityWrapper *pEnt = mpEditorWorld->GetEntity(mvComponentIDs[i]);
        iEntityWrapperAggregate *pAggregate =
            (iEntityWrapperAggregate *)mpEditorWorld->GetEntity(mvOldAggregateData[i]->GetID());
        if (pAggregate == NULL)
