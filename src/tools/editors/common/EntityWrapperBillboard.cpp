...
cIconEntityBB::~cIconEntityBB() { DestroyBB(); }
...
void cIconEntityBB::Update() {
    ...
    if (mpEntity) {
        ...
        if (pParent->mbMatUpdated) {
            cMaterialManager *pManager =
                pParent->GetEditorWorld()->GetEditor()->GetEngine()->GetResources()->GetMaterialManager();
...
void cIconEntityBB::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions, bool abIsSelected,
                         bool abIsActive) {
...
cEntityWrapperTypeBillboard::cEntityWrapperTypeBillboard()
    : iEntityWrapperType(eEditorEntityType_Billboard, _W("Billboard"), "Billboard") {
...
iEntityWrapper *cEntityWrapperDataBillboard::CreateSpecificEntity() { return hplNew(cEntityWrapperBillboard, (this)); }
...
void cEntityWrapperBillboard::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
void cEntityWrapperBillboard::SaveToElement(cXmlElement* apElement)
{
    ...
    apElement->SetAttributeVector3f("Axis", cMath::MatrixMul(cMath::MatrixRotate(mvRotation, eEulerRotationOrder_XYZ),
cVector3f(0,1,0))); apElement->SetAttributeVector2f("BillboardSize", mvBillboardSize);
    apElement->SetAttributeFloat("BillboardOffset", mfBillboardOffset);
    apElement->SetAttributeString("MaterialFile",
mpEditorWorld->GetEditor()->GetFilePathRelativeToWorkingDir(msMatFile)); apElement->SetAttributeString("BillboardType",
msType); apElement->SetAttributeColor("BillboardColor", mCol); apElement->SetAttributeString("ConnectLight",
msConnectedLightName); apElement->SetAttributeBool("IsHalo", mbIsHalo);
...
if(mbBBUpdated)
{
    ...
    if(mpEngineEntity)
    {
        ...
        {
            cMaterialManager* pManager =
GetEditorWorld()->GetEditor()->GetEngine()->GetResources()->GetMaterialManager(); cMaterial* pMat =
pManager->CreateMaterial(msMatFile);
        ...
        {
            //cEntityWrapperLight* pLight =
(cEntityWrapperLight*)GetEditorWorld()->GetEntityByName(msConnectedLightName);
            //if(pLight)
            //	pLight->AddConnectedBillboard(this);
...
iEngineEntity *cEntityWrapperBillboard::CreateSpecificEngineEntity() { return hplNew(cIconEntityBB, (this)); }
