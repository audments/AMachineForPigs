...
cEditorWindowEntityEditFogArea::cEditorWindowEntityEditFogArea(cEditorEditModeSelect *apEditMode,
                                                               cEntityWrapperFogArea *apArea)
    : cEditorWindowEntityEditBox(apEditMode, apArea) {
    mpFogArea = apArea;
}
...
bool cEditorWindowEntityEditFogArea::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpColor) {
        ...
        pAction = mpEntity->CreateSetPropertyActionBool(eFogAreaBool_ShownBacksideWhenInside,
                                                        mpInpShowBacksideWhenInside->GetValue());
    } else if (apInput == mpInpShowBacksideWhenOutside) {
        pAction = mpEntity->CreateSetPropertyActionBool(eFogAreaBool_ShownBacksideWhenOutside,
                                                        mpInpShowBacksideWhenOutside->GetValue());
    } else
        return false;
