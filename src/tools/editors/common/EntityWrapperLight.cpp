...
iIconEntityLight::iIconEntityLight(iEntityWrapper *apParent, const tString &asIconFile)
    : iIconEntity(apParent, "Light" + asIconFile) {}
...
void iIconEntityLight::Update() {
    ...
    if (pParent->GetShadowsAffectDynamic())
        lFlags |= eObjectVariabilityFlag_Dynamic;
    if (pParent->GetShadowsAffectStatic())
...
void iEntityWrapperLight::SetShadowsAffectStatic(bool abX) { mbShadowsAffectStatic = abX; }

//------------------------------------------------------------------------------

void iEntityWrapperLight::SetShadowsAffectDynamic(bool abX) { mbShadowsAffectDynamic = abX; }
...
void iEntityWrapperLight::UpdateFlickerParams() {
    ...
    pLight->SetFlicker(mcolFlickerOffColor, mfFlickerOffRadius, mfFlickerOnMinLength, mfFlickerOnMaxLength,
                       msFlickerOnSound, msFlickerOnPS, mfFlickerOffMinLength, mfFlickerOffMaxLength, msFlickerOffSound,
                       msFlickerOffPS, mbFlickerFade, mfFlickerOnFadeMinLength, mfFlickerOnFadeMaxLength,
...
void iEntityWrapperLight::AddConnectedBillboard(cEntityWrapperBillboard *apBB) {
    if (apBB == NULL || apBB->GetEngineEntity() == NULL)
        return;

    std::list<cEntityWrapperBillboard *>::iterator it = find(mlstConnectedBBs.begin(), mlstConnectedBBs.end(), apBB);
    if (it != mlstConnectedBBs.end())
        return;

    mlstConnectedBBs.push_back(apBB);
    apBB->SetConnectedLight(this);

    ((iLight *)mpEngineEntity->GetEntity())
        ->AttachBillboard((cBillboard *)apBB->GetEngineEntity()->GetEntity(), apBB->GetBillboardColor());
}
...
void iEntityWrapperLight::SetGobo(const tString &asGoboFilename) {
    ...
    cEditorHelper::LoadTextureResource(eEditorTextureResourceType_2D, asGoboFilename, &pTex, msGoboAnimMode,
                                       mfGoboAnimFrameTime);
    if (pTex)
...
void iEntityWrapperLight::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
void iEntityWrapperLight::OnSetActive(bool abX) { mpEngineEntity->Update(); }
