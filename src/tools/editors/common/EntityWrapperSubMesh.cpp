...
bool cEngineEntitySubMesh::Create(const tString &asName) {
    ...
    if (mpEntity && pData) {
        ...
        pData->SetVec3f(eObjVec3f_Rotation,
                        cMath::MatrixToEulerAngles(mtxSubMeshMatrix.GetRotation(), eEulerRotationOrder_XYZ));
...
bool cEngineEntitySubMesh::CheckRayIntersect(cEditorWindowViewport *apViewport, cVector3f *apPos, tVector3fVec *apTri,
                                             float *apT) {
    if (iEngineEntity::CheckRayIntersect(apViewport, apPos, apTri) == false)
        return false;

    return cEditorHelper::CheckRaySubMeshEntityIntersect(apViewport->GetUnprojectedStart(),
                                                         apViewport->GetUnprojectedEnd(), (cSubMeshEntity *)mpEntity,
                                                         apPos, apT, NULL, apTri);
}
...
cEntityWrapperTypeSubMesh::cEntityWrapperTypeSubMesh()
    : iEntityWrapperType(eEditorEntityType_SubMesh, _W("SubMesh"), "SubMesh") {
...
bool cEntityWrapperTypeSubMesh::SetMesh(const tString &asFilename, bool abDeleteData, tEntityDataVec &avSubMeshData,
                                        const tIntList &alstSubMeshIDs, tEntityDataVec &avBoneData,
                                        const tIntList &alstBoneIDs) {
    //////////////////////////////////////////////
    ...
    if (pMesh == NULL) {
        if (asFilename != "")
            pEditor->ShowMessageBox(_W("Warning"), _W("Could not load mesh file: ") + cString::To16Char(asFilename),
                                    _W("Ok"), _W(""), NULL, NULL);
...
tString cEntityWrapperTypeSubMesh::GetMeshFilename() {
    if (mpMesh)
        return cString::To8Char(mpMesh->GetMesh()->GetFullPath());
...
void cEntityWrapperTypeSubMesh::AddSubMesh(cEntityWrapperSubMesh *apSubMesh) {
    if (apSubMesh)
        mvSubMeshes.push_back(apSubMesh);
}
...
iEntityWrapper *cEntityWrapperDataSubMesh::CreateSpecificEntity() { return hplNew(cEntityWrapperSubMesh, (this)); }
...
/*
bool cEntityWrapperSubMesh::CheckRayAccurateIntersection(const cVector3f& avRayStart, const cVector3f& avRayEnd,
cVector3f* apIntersection, tVector3fVec* apTriangle)
{
    return cEditorHelper::CheckRaySubMeshEntityIntersect(avRayStart, avRayEnd, mpSubMeshEntity, apIntersection, NULL,
NULL, apTriangle);
}
*/
...
iEngineEntity *cEntityWrapperSubMesh::CreateSpecificEngineEntity() { return hplNew(cEngineEntitySubMesh, (this)); }

/*
iEntity3D* cEntityWrapperSubMesh::CreateEngineEntity(iEditorWorld* apEditorWorld, const tString& asFilename, int
alSubMeshID, int alID)
{
