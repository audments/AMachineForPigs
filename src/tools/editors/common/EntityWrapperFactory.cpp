...
iEntityWrapper *cEntityWrapperFactory::CreateEntityWrapper(iEditorWorld *apEditorWorld, eEditorEntityType aType,
                                                           int alID, const tString &asName, const tString &asFilename,
                                                           const cVector3f &avPosition, const cVector3f &avRotation,
                                                           const cVector3f &avScale, int alEntitySubType) {
    ...
    case eEditorEntityType_BodyShape:
        pEntityWrapper = hplNew(cEntityWrapperBodyShape,(apEditorWorld, alID, asName,
    (eEditorBodyShape)alEntitySubType)); break; case eEditorEntityType_Body: pEntityWrapper =
    hplNew(cEntityWrapperBody,(apEditorWorld, alID, asName)); break; case eEditorEntityType_Joint:
        switch(alEntitySubType)
        {
...
iEntityWrapper *cEntityWrapperFactory::CreateEntityWrapperPrimitive(iEditorWorld *apEditorWorld, int alID,
                                                                    const tString &asName,
                                                                    tVector3fList &alstParameters) {
    if (apEditorWorld == NULL)
        return NULL;

    cEntityWrapperPrimitive *pEnt = hplNew(cEntityWrapperPrimitivePlane, (apEditorWorld, alID, ""));
    if (pEnt)
        pEnt->CreatePrimitiveFromParameters(alstParameters);
...
iEntityWrapper *cEntityWrapperFactory::CreateEntityWrapperFromEntityWrapperData(iEditorWorld *apEditorWorld,
                                                                                iEntityWrapperData *apData) {
    ...
    if (apData->mEntityType == eEditorEntityType_Primitive) {
        ...
        pEnt = cEntityWrapperFactory::CreateEntityWrapperPrimitive(apEditorWorld, pData->mlID, pData->msName,
                                                                   pData->mlstParameters);
    } else
        pEnt =
            cEntityWrapperFactory::CreateEntityWrapper(apEditorWorld, apData->mEntityType, apData->mlID, apData->msName,
                                                       apData->msFilename, 0, 0, 1, apData->mlSubType);

    if (pEnt)
        pEnt->LoadFromEntityWrapperData(apData);
...
iEntityWrapper *cEntityWrapperFactory::CreateEntityWrapperFromXMLElement(iEditorWorld *apEditorWorld,
    ...
    if (lType == eEditorEntityType_Primitive) {
        ...
        pEnt = cEntityWrapperFactory::CreateEntityWrapper(apEditorWorld, (eEditorEntityType)lType, lID, "", sFilename,
                                                          0, 0, 1, lSubType);
...
void cEntityWrapperFactory::DestroyEntityWrapper(iEntityWrapper *apEntity, bool abRemoveFromWorld) {
    ...
    if (abRemoveFromWorld && pWorld)
        pWorld->RemoveObject(apEntity);
...
void cEntityWrapperFactory::SetUpEntityWrapper(iEntityWrapper *apEntity, const cVector3f &avPosition,
