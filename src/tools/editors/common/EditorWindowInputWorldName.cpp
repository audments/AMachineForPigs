...
cEditorWindowInputWorldName::cEditorWindowInputWorldName(iEditorBase *apEditor)
    : iEditorWindowPopUp(apEditor, "Input name window", true, true, false, cVector2f(200, 130)) {}
...
void cEditorWindowInputWorldName::OnInitLayout() {
    ...
    for (int i = 0; i < 2; ++i) {
        mvButtons[i] =
            mpSet->CreateWidgetButton(cVector3f(10 + (float)i * 45, 80, 0.1f), cVector2f(40, 25), _W(""), mpWindow);
...
void cEditorWindowInputWorldName::OnUpdate() { iEditorWindowPopUp::OnUpdate(); }
