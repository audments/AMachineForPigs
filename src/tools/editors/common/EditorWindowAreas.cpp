...
void cEditorWindowAreas::OnInitLayout() {
    ...
    mpGroupTypeSelector =
        mpSet->CreateWidgetGroup(cVector3f(5, 8, 0.1f), cVector2f(190, 50), _W("Area Type"), mpBGFrame);
    AddWidget(mpGroupTypeSelector);

    mpComboBoxAreaType =
        mpSet->CreateWidgetComboBox(cVector3f(10, 10, 0.2f), cVector2f(170, 25), _W(""), mpGroupTypeSelector);
