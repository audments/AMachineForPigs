...
cEditorWindowEntitySearch::cEditorWindowEntitySearch(iEditorBase *apEditor)
    : iEditorWindowPopUp(apEditor, "Search Window", false, false, false, cVector2f(640, 410)) {
...
void cEditorWindowEntitySearch::OnInitLayout() {
    ...
    mpInputSearch = mpSet->CreateWidgetTextBox(vPos + cVector3f(0, -3, 0), cVector2f(200, 0), _W(""), mpWindow,
    ...
    mpComboBoxInside = mpSet->CreateWidgetComboBox(vPos + cVector3f(mpLabelInside->GetSize().x + 5, -3, 0),
                                                   cVector2f(100, 0), _W(""), mpWindow);
    ...
    for (int i = 0; i < 2; ++i) {
        cWidgetButton *pButton = mpSet->CreateWidgetButton(vPos + cVector3f(i * 60.0f, 0, 0), cVector2f(50, 20),
                                                           vButtonStrings[i], mpWindow);
        AddWidget(pButton);
...
void cEditorWindowEntitySearch::OnWorldModify() { mbMustUpdateList = true; }

//-------------------------------------------------------------------------------

void cEditorWindowEntitySearch::OnSelectionChange() { mbUpdateHighlights = true; }
