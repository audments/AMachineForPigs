...
cEditorEditModeSelectToolTranslate::cEditorEditModeSelectToolTranslate(cEditorEditModeSelect *apParent,
                                                                       cEditorSelection *apSelection)
    : cEditorEditModeSelectTool(eSelectToolMode_Translate, apParent, apSelection) {
    for (int i = 0; i < 4; ++i)
...
bool cEditorEditModeSelectToolTranslate::IsActive() { return mpSelection->CanTranslate(); }

//----------------------------------------------------------------

bool cEditorEditModeSelectToolTranslate::CheckRayIntersectsAxis(eSelectToolAxis aeAxis, const cVector3f &avRayStart,
...
void cEditorEditModeSelectToolTranslate::CheckMouseOverAxis() {
    ...
    for (int i = eSelectToolAxis_X; i < eSelectToolAxis_LastEnum; ++i) {
        ...
        bRayIntersectedAxis[i] = cMath::CheckAABBLineIntersection(mvAxisMin[i], mvAxisMax[i], mvTransformedRayStart,
                                                                  mvTransformedRayEnd, &vAxisIntersect[i], NULL) ||
                                 cMath::CheckAABBLineIntersection(mvHeadMin[i], mvHeadMax[i], mvTransformedRayStart,
                                                                  mvTransformedRayEnd, &vHeadIntersect[i], NULL);

        bRayIntersectedPlane[i] =
            cMath::CheckAABBLineIntersection(mvAxesPlaneBoxMin, mvAxesPlaneBoxMax[i], mvTransformedRayStart,
                                             mvTransformedRayEnd, &vPlaneIntersect[i], NULL);
    ...
    mvAxisMouseOver[eSelectToolAxis_X] =
        bRayIntersectedAxis[eSelectToolAxis_X] || bRayIntersectedPlane[0] || bRayIntersectedPlane[2];
    mvAxisMouseOver[eSelectToolAxis_Y] =
        bRayIntersectedAxis[eSelectToolAxis_Y] || bRayIntersectedPlane[0] || bRayIntersectedPlane[1];
    mvAxisMouseOver[eSelectToolAxis_Z] =
        bRayIntersectedAxis[eSelectToolAxis_Z] || bRayIntersectedPlane[1] || bRayIntersectedPlane[2];
}
...
void cEditorEditModeSelectToolTranslate::DrawAxes(cEditorWindowViewport *apViewport,
    ...
    cMatrixf mtxXAxisHeadTransform =
        cMath::MatrixMul(cMath::MatrixTranslate(cVector3f(afAxisLength, 0, 0)), mtxTransform);
    cMatrixf mtxYAxisHeadTransform = cMath::MatrixMul(
        cMath::MatrixTranslate(cVector3f(0, afAxisLength, 0)),
        cMath::MatrixMul(mtxTransform, cMath::MatrixRotate(cVector3f(0, 0, kPi2f), eEulerRotationOrder_XYZ)));
    cMatrixf mtxZAxisHeadTransform = cMath::MatrixMul(
        cMath::MatrixTranslate(cVector3f(0, 0, afAxisLength)),
