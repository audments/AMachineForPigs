...
class iEditorWorld {
  public:
    ...
    virtual bool CustomCategoryLoader(cXmlElement *apWorldObjectsElement, cXmlElement *apCategoryElement) {
        return false;
    }
    ...
    void SetUpEntityWrapper(iEntityWrapper *apEntity, const cVector3f &avPosition = 0, const cVector3f &avRotation = 0,
    ...
    cEditorClipPlane *GetClipPlane(int alIdx) {
        if (alIdx < 0 || alIdx > (int)mvClipPlanes.size())
            return NULL;
        else
    ...
    bool ClipPlaneNumUpdated() {
        bool bUpdated = mbClipPlaneNumUpdated;
        mbClipPlaneNumUpdated = false;
        return bUpdated;
    }
