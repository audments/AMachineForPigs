...
void cViewportCallback::OnPostSolidDraw(cRendererCallbackFunctions *apFunctions) {
    ...
    if (mpViewport->GetDrawGrid()) {
        ...
        if (pGrid)
            pGrid->Draw(apFunctions, mpViewport->GetGridCenter());
    }
    ...
    // apFunctions->GetLowLevelGfx()->DrawLine(mpViewport->GetVCamera()->GetTrackRefMousePos(),
    // mpViewport->GetVCamera()->GetTrackRefTargetPos()-mpViewport->GetVCamera()->GetTrackRefMousePos(),
    // cColor(1,0,0,1)); apFunctions->GetLowLevelGfx()->DrawLine(mpViewport->GetVCamera()->GetTrackRefTargetPos(),
    // mpViewport->GetVCamera()->GetTrackNewMousePos(), cColor(1,0,0,1));
    // apFunctions->GetLowLevelGfx()->DrawLine(mpViewport->GetVCamera()->GetTrackRefTargetPos(), vMousePos,
    // cColor(0,1,0,1));
...
cEditorWindowViewport::cEditorWindowViewport(iEditorBase *apEditor, iFrameBuffer *apFB, bool abAddViewMenu)
    : iEditorWindow(apEditor, "Viewport Window"),
      iEditorViewport(apEditor, apEditor->GetEditorWorld()->GetWorld(), apFB) {
    mbDrawGrid = true;
    mbDrawDebug = false;
    mbDrawAxes = true;
...
bool cEditorWindowViewport::IsFocused() { return mpEditor->GetFocusedViewport() == this; }
...
void cEditorWindowViewport::Load(cXmlElement *apElement) {
    ...
    if (fZoom == 0)
        fZoom = mCamera.GetZoomFunc()->GetZoomFromDistToTarget(
            (mCamera.GetCameraPosition() - mCamera.GetTargetPosition()).Length());
...
bool cEditorWindowViewport::OnViewportGotFocus(const cGuiMessageData &aData) { return false; }

//----------------------------------------------------------------

bool cEditorWindowViewport::OnViewportLostFocus(const cGuiMessageData &aData) { return false; }
...
bool cEditorWindowViewport::OnViewportMouseUp(const cGuiMessageData &aData) {
    ...
    if (mpEditor->GetFlags(eEditorFlag_ViewLocked) || mpEditor->GetFlags(eEditorFlag_ViewModeActive) == false)
...
bool cEditorWindowViewport::OnViewportMouseMove(const cGuiMessageData &aData) {
    ...
    if (mpSet->GetFocusedWidget() != mpImgViewport || mpEditor->GetFlags(eEditorFlag_PopUpActive))
        return false;
...
bool cEditorWindowViewport::OnViewportKeyRelease(const cGuiMessageData &aData) { return false; }
...
void cEditorWindowViewport::OnUpdate(float afTimeStep) { UpdateMenu(); }
...
void cEditorWindowViewport::OnSetActive(bool abX) { SetViewportActive(abX); }
