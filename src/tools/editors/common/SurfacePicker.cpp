...
void cSurfacePicker::Iterate() {
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        mvTriangle.push_back(
            cMath::MatrixMul(mpClosestMeshEntity->GetWorldMatrix(),
...
void cSurfacePicker::IterateRenderableNode(iRenderableContainerNode *apNode, const cVector3f &avStart,
                                           const cVector3f &avEnd, cBoundingVolume *apBV, int *apTriIndex,
                                           bool abAveragingNormal) {
    ...
    if (apNode->GetParent() != NULL) {
        ...
        if (abAveragingNormal == false &&
            cMath::CheckPointInAABBIntersection(avStart, apNode->GetMin(), apNode->GetMax()) == false) {
            float fT = 0;
...
bool cSurfacePicker::CheckObjectIntersection(iRenderable *apObject, const cVector3f &avStart, const cVector3f &avEnd,
                                             cBoundingVolume *apBV, int *apTriIndex, bool abAveragingNormal) {
    ...
    if (cMath::CheckPointInBVIntersection(avStart, *pObjectBV) == false) {
        if (cMath::CheckAABBLineIntersection(pObjectBV->GetMin(), pObjectBV->GetMax(), avStart, avEnd, &vIntersection,
                                             &fT) == false)
            return false;
        if (fT > mfMinT)
            return false;
    ...
    bool bIntersect = cMath::CheckLineTriVertexBufferIntersection(
        avStart, avEnd, mtxInvModel, apObject->GetVertexBuffer(), &vIntersection, &fT, &lTriIndex, true);
    if (bIntersect == false || fT > mfMinT)
        return false;
    ...
    for (int i = 0; i < (int)mvSubMeshesToAverage.size(); ++i) {
        ...
        for (int i = 0; i < pVB->GetIndexNum(); i += 3) {
            ...
            for (int j = 0; j < 3; ++j) {
                ...
                vTriangle.push_back(
                    cVector3f(pVertices[lPosIndex], pVertices[lPosIndex + 1], pVertices[lPosIndex + 2]));
            }
            ...
            vNormalsToAverage.push_back(cMath::MatrixMul3x3(mtxWorldMatrix, vTriNormal));
...
void cSurfacePicker::SetAverageVolumeSize(const cVector3f &avX) { mAvgVolume.SetSize(avX); }

void cSurfacePicker::SetAverageBackfacingAngleThreshold(float afAngle) { mfBackfacingAngleThreshold = cos(afAngle); }
