...
cBoxCreator::cBoxCreator(iEditorEditMode *apEditMode) : iShapeCreator(apEditMode) { mvDefaultSize = 1; }
...
void cBoxCreator::OnEditorUpdate() {
    ...
    if (mvPoints.size() < 2) {
        ...
        if (cMath::CheckPlaneLineIntersection(mBoxHeightPlane, pViewport->GetUnprojectedStart(),
                                              pViewport->GetUnprojectedEnd(), &mvTempPoint, NULL)) {
            mvDebugPos = mvTempPoint;
            for (int i = 0; i < 3; ++i)
                if (i != mPlaneNormal)
                    mvTempPoint.v[i] = mvPoints[1].v[i];
        }
        if (pGrid->GetSnapToGrid())
            mvTempPoint = pGrid->GetSnappedPosInSpace(mvTempPoint);
    }
}
...
void cBoxCreator::Reset() { mvPoints.clear(); }
