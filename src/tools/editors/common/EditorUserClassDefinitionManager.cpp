...
static eEditorVarCategory IdxToCat(int alX) { return (eEditorVarCategory)cMath::Pow2(alX); }

static int CatToIdx(eEditorVarCategory aCat) { return cMath::Log2ToInt(aCat); }

//------------------------------------------------------------------------------

cEditorVarGroup::cEditorVarGroup(const tString &asName) { msName = asName; }
...
cEditorUserClassType::~cEditorUserClassType() { STLDeleteAll(mvSubTypes); }
...
void cEditorUserClassType::DumpEditorSetupVars(tEditorVarVec &avVars) {
    if (mpBaseClass)
        ((cEditorUserClassBase *)mpBaseClass)->DumpEditorSetupVars(avVars);
...
int cEditorUserClassType::GetSubTypeNum() { return (int)mvSubTypes.size(); }
...
cEditorUserClassSubType::cEditorUserClassSubType(cEditorUserClassType *apParent)
    : cEditorUserClass(apParent->GetDefinition()) {
    mpParent = apParent;
}
...
cEditorUserClassInstance::cEditorUserClassInstance(cEditorUserClassSubType *apClass, eEditorVarCategory aCat)
    : cEditorClassInstance(apClass) {
    mCat = aCat;
}
...
cEditorVarInputPanel *cEditorUserClassInstance::CreateInputPanel(iEditorWindow *apWindow, iWidget *apParent,
                                                                 bool abRows) {
...
cEditorUserClassInputPanel::cEditorUserClassInputPanel(cEditorUserClassInstance *apInstance)
    : cEditorVarInputPanel(apInstance) {}
...
bool cEditorUserClassDefinition::Create(const tString &asFilename, int alLoadFlags) {
    ...
    ////////////////////////////////////////////
    // Check XML validity
    if (pDoc == NULL || pDoc->GetFirstElement("Types") == NULL) {
        int lRow = pDoc->GetErrorRow();
        int lCol = pDoc->GetErrorCol();
        pRes->DestroyXmlDocument(pDoc);

        tString sError =
            (pDoc == NULL) ? "file not found"
                           : ("error found in line " + cString::ToString(lRow) + ", column " + cString::ToString(lCol));
        FatalError("Failed compilation of Custom Variable definition %s: %s\n",
                   cString::GetFileName(asFilename).c_str(), sError.c_str());
...
bool cEditorUserClassDefinition::IsLoadableCategory(eEditorVarCategory aCat) { return (mlLoadFlags & aCat) != 0; }
...
int cEditorUserClassDefinition::GetTypeNum() { return (int)mvTypes.size(); }
...
int cEditorUserClassDefinition::GetGroupNum() { return (int)mvGroups.size(); }
...
cEditorUserClassDefinitionManager::~cEditorUserClassDefinitionManager() { STLDeleteAll(mvDefinitions); }
