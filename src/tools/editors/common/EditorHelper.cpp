...
bool cEditorHelper::CheckEntityAgainstRay(iEntityWrapper *apEntity, iEntityWrapper *apCurrentPickedEntity,
                                          ...
                                          const cVector3f &avRayStart, const cVector3f &avRayEnd, float *apfMinDistance,
...
bool cEditorHelper::CheckEntityAgainstBox(iEntityWrapper *apEntity, const cRect2l &aMouseRect,
                                          const cVector2l &avViewportSize, cFrustum *apFrustum) {
    /*
    if(apEntity==NULL)
        return false;

    cBoundingVolume* pBV =
    apEntity->GetPickBV(apEntity->GetEditorWorld()->GetEditor()->GetFocusedViewport()->GetCamera());
...
void cEditorHelper::CalculateRayBoundingBox(const cVector3f &avStart, const cVector3f &avEnd, cVector3f &avMin,
                                            cVector3f &avMax) {
    avMin = avEnd;
    avMax = avStart;
...
bool cEditorHelper::CheckScreenSpaceMouseMeshIntersect(cCamera *apCamera, const cVector2f &avMousePos,
    ...
    for (int i = 0; i < apMesh->GetSubMeshEntityNum(); ++i) {
        ...
        bIntersect = bIntersect || CheckScreenSpaceMouseSubMeshIntersect(
                                       GetInvYMousePos(avMousePos, avViewportSize), avRayStart, avRayEnd,
                                       avViewportSize, pSubMesh, mtxCameraViewMatrix, mtxCameraProjMatrix,
                                       mtxSubMeshWorldMatrix, &vMeshIntersectPos, apTriangle);
...
bool cEditorHelper::CheckScreenSpaceMouseSubMeshIntersect(const cVector2f &avInvYMousePos, const cVector3f &avRayStart,
                                                          const cVector3f &avRayEnd, const cVector2f &avViewportSize,
                                                          cSubMeshEntity *apSubMesh,
    ...
    bIntersect = CheckScreenSpaceMouseVertexBufferIntersect(avInvYMousePos, avRayStart, avRayEnd, avViewportSize,
                                                            pVtxBuffer, amtxCameraViewMatrix, amtxCameraProjMatrix,
                                                            amtxSubMeshWorldMatrix, &vSubMeshIntersectPos, apTriangle);
...
bool cEditorHelper::CheckScreenSpaceMouseVertexBufferIntersect(
    const cVector2f &avInvYMousePos, const cVector3f &avRayStart, const cVector3f &avRayEnd,
    const cVector2f &avViewportSize, iVertexBuffer *apVtxBuffer, const cMatrixf &amtxCameraViewMatrix,
    const cMatrixf &amtxCameraProjMatrix, const cMatrixf &amtxVtxBufferWorldMatrix, cVector3f *apIntersectionPos,
    ...
    if (cMath::CheckPointInFlatPolygon(avRayStart, avRayEnd, avInvYMousePos, avViewportSize, lIndexNum, pIndices,
                                       pVertices, lStride, amtxVtxBufferWorldMatrix, amtxCameraViewMatrix,
                                       amtxCameraProjMatrix, NULL, &vVtxBufferIntersectionPos, &vTriangle)) {
        bIntersect = true;
...
bool cEditorHelper::CheckRayMeshEntityIntersect(const cVector3f &avRayStart, const cVector3f &avRayEnd,
                                                cMeshEntity *apObject, cVector3f *apIntersectionPos,
    ...
    if (bIntersected) {
        ...
        if (apTriangle) {
            ...
            for (int i = 0; i < 3; ++i) {
                ...
                apTriangle->push_back(cMath::MatrixMul(
                    *pSubMesh->GetModelMatrix(NULL),
...
bool cEditorHelper::CheckRaySubMeshEntityIntersect(const cVector3f &avRayStart, const cVector3f &avRayEnd,
                                                   cSubMeshEntity *apObject, cVector3f *apIntersection, float *apT,
                                                   unsigned int *apTriangleIdx, tVector3fVec *apTriangle) {
    iVertexBuffer *pVB = apObject->GetSubMesh()->GetVertexBuffer();
    cMatrixf mtxInvWorld = cMath::MatrixInverse(apObject->GetWorldMatrix());
    int lTriIndex = -1;

    if (cMath::CheckLineTriVertexBufferIntersection(avRayStart, avRayEnd, mtxInvWorld, pVB, apIntersection, apT,
                                                    &lTriIndex)) {
        if (apTriangleIdx)
...
void cEditorHelper::GetTrianglesInsideClipPlanes(std::vector<cPlanef> &avPlanes, tEntityWrapperList &alstEntities,
                                                 const cVector3f avBaseNormal, tVector3fVec &avTriangles,
                                                 tVector3fVec &avNormals) {
    /*
    avTriangles.clear();
    avNormals.clear();
    ...
    for(;it!=alstEntities.end();++it)
    {
        ...
        for(int i=0;i<pMeshEnt->GetSubMeshEntityNum();++i)
        {
            ...
            iVertexBuffer* pVBCopy =
    pSubMeshEnt->GetSubMesh()->GetVertexBuffer()->CreateCopy(eVertexBufferType_Hardware, eVertexBufferUsageType_Static,
    eVertexElementFlag_Position|eVertexElementFlag_Normal); pVBCopy->Transform(mtxWorldMatrix); float* pPositions =
    pVBCopy->GetFloatArray(eVertexBufferElement_Position); float* pNormals =
    pVBCopy->GetFloatArray(eVertexBufferElement_Normal); unsigned int* pIndices = pVBCopy->GetIndices(); for(int
    j=0;j<pVBCopy->GetIndexNum();j+=3)
            {
                tVector3fVec vTriangle;
...
void cEditorHelper::GetTrianglesIntersectingSphere(const cVector3f &avSphereCenter, float afRadius,
                                                   tSubMeshEntityVec &avSubMeshes, const cVector3f &avBaseNormal,
    ...
    for (; it != avSubMeshes.end(); ++it) {
        ...
        if (cMath::CheckBVIntersection(*pObjectBV, bv) == false)
            continue;
        ...
        for (int j = 0; j < pVB->GetIndexNum(); j += 3) {
            ...
            for (int k = 0; k < 3; ++k) {
                ...
                vTriVertices.push_back(
                    cVector3f(pPositions[pPosIndex], pPositions[pPosIndex + 1], pPositions[pPosIndex + 2]));
                vVertexNormals.push_back(
                    cVector3f(pNormals[pNrmIndex], pNormals[pNrmIndex + 1], pNormals[pNrmIndex + 2]));
            }
            ...
            if (bAddTriangle == false) {
                for (int k = 0; k < 2; ++k) {
                    if (cMath::CheckSphereLineIntersection(vTransSphCenter, afRadius, vTriVertices[k],
                                                           vTriVertices[k + 1], NULL, NULL, NULL, NULL)) {
                        bAddTriangle = true;
                        break;
                    }
                }
                if (bAddTriangle == false &&
                    cMath::CheckSphereLineIntersection(vTransSphCenter, afRadius, vTriVertices[2], vTriVertices[0],
                                                       NULL, NULL, NULL, NULL))
                    bAddTriangle = true;
            }
            // Check if inside
            if (bAddTriangle == false) {
                ...
                if (cMath::CheckPlaneLineIntersection(triPlane, vTransSphCenter,
                                                      vTransSphCenter - vTriNormal * afRadius, &vCenterProjectedOnPlane,
                                                      NULL)) {
                    bAddTriangle = true;
                }
            }
...
void cEditorHelper::GetIDsFromEntityList(const tEntityWrapperList &alstEntities, tIntList &alstEntityIDs,
                                         bool abClearList) {
    if (abClearList)
        alstEntityIDs.clear();
...
bool cEditorHelper::LoadEntityFile(int alID, const tString &asName, const tString &asFilename, cMeshEntity **apEntity,
                                   bool abLoadAsTemp) {
    if (asFilename == "")
        return false;
    ...
    pEntity = (cMeshEntity *)mpEditor->GetEngineEntityLoader()->Load(asName, alID, true, pDoc, cMatrixf::Identity,
                                                                     cVector3f(1), pWorld, asFilename, sFullPath, NULL);
...
bool cEditorHelper::LoadTextureResource(eEditorTextureResourceType aTexType, const tString &asFile,
                                        iTexture **apTexture, const tString &asAnimMode, float afFrameTime) {
    if (asFile == "")
        return false;
...
bool cEditorHelper::LoadResourceFile(eEditorResourceType aResType, const tString &asFile, void **apEditorResource,
                                     int alID) {
    if (asFile == "")
        return false;
...
void cEditorHelper::DrawBillboard(iTexture *apGfx, const cVector3f &avWorldPosition, const cVector2f &avSize,
                                  const cColor &aColor, cEditorWindowViewport *apViewport,
    ...
    // Any fixes for this?
    // Yeah, need to inverse the vertex order somehow. It is possible to have a static variable with a vertex array for
    // this.
    ...
    apFunctions->DrawQuad(vViewSpacePos - cVector3f(vViewSpaceSize.x, vViewSpaceSize.y, 0) * 0.5f,
                          cVector2f(vViewSpaceSize.x, vViewSpaceSize.y), cVector2f(0, 1), cVector2f(1, 0), false,
                          aColor);
...
void cEditorHelper::DrawPyramid(cRendererCallbackFunctions *apFunctions, const cVector3f &avBaseCenter,
                                const cVector3f &avTop, float afHalfWidth, const cColor &aColor) {
...
iVertexBuffer *cEditorHelper::CreatePyramidVtxBuffer(cGraphics *apGfx, const cVector3f &avBaseCenter,
                                                     const cVector3f &avTip, float afHalfWidth, const cColor &aColor) {
    iVertexBuffer *pVB = apGfx->GetLowLevel()->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri,
...
void cEditorHelper::GetViewSpacePosAndSize(cCamera *apCamera, const cVector3f &avWorldPosition,
                                           const cVector3f &avWorldSize, cVector3f &avViewSpacePosition,
                                           cVector3f &avViewSpaceSize) {
    if (apCamera == NULL) {
