...
const cMatrixf &iEngineEntity::GetWorldMatrix() { return mpEntity->GetWorldMatrix(); }
...
bool iEngineEntity::CheckRayBoundingVolumeIntersect(cEditorWindowViewport *apViewport, cVector3f *apPos, float *apT) {
    ...
    return cMath::CheckAABBLineIntersection(pBV->GetMin(), pBV->GetMax(), apViewport->GetUnprojectedStart(),
                                            apViewport->GetUnprojectedEnd(), apPos, apT);
}
...
bool iEngineEntity::CheckRayIntersect(cEditorWindowViewport *apViewport, cVector3f *apPos, tVector3fVec *apTriangle,
                                      float *apT) {
...
bool iEngineEntityMesh::CheckRayIntersect(cEditorWindowViewport *apViewport, cVector3f *apPos, tVector3fVec *apTriangle,
                                          float *apT) {
    if (iEngineEntity::CheckRayIntersect(apViewport, apPos, apTriangle, apT) == false)
        return false;

    return cEditorHelper::CheckRayMeshEntityIntersect(apViewport->GetUnprojectedStart(),
                                                      apViewport->GetUnprojectedEnd(), (cMeshEntity *)mpEntity, apPos,
                                                      apTriangle, apT);
}
...
void iEngineEntityMesh::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
void iEngineEntityMesh::DrawProgram(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
void iEngineEntityMesh::SetIsOccluder(bool abX) { ((cMeshEntity *)mpEntity)->SetIsOccluder(abX); }

//-----------------------------------------------------------------------

void iEngineEntityMesh::SetCoverage(float afX) { ((cMeshEntity *)mpEntity)->SetCoverageAmount(afX); }
...
cEngineEntityLoadedMesh::cEngineEntityLoadedMesh(iEntityWrapper *apParent, const tString &asFilename)
    : iEngineEntityMesh(apParent) {
...
cEngineEntityLoadedMeshAggregate::cEngineEntityLoadedMeshAggregate(iEntityWrapper *apParent, const tString &asFilename)
    : iEngineEntityMesh(apParent) {
...
bool cEngineEntityLoadedMeshAggregate::Create(const tString &asName) {
    ...
    mpEntity =
        pLoader->LoadEntFile(mpParent->GetID(), asName, msFilename, pWorld->GetWorld(), false, true, true, false, true);
...
cEngineEntityGeneratedMesh::cEngineEntityGeneratedMesh(iEntityWrapper *apParent, cMesh *apMesh)
    : iEngineEntityMesh(apParent) {
    SetMesh(apMesh);
}
...
iVertexBuffer *cEngineEntityGeneratedMesh::GetVertexBuffer() { return mpMesh->GetSubMesh(0)->GetVertexBuffer(); }
...
bool iIconEntity::Create(const tString &asName) { return true; }
...
bool iIconEntity::CheckRayIntersect(cEditorWindowViewport *apViewport, cVector3f *apPos, tVector3fVec *apTriangle,
                                    float *apT) {
    return mpIcon && mpIcon->CheckRayIntersect(apViewport, apPos, apTriangle, apT);
}

void iIconEntity::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions, bool abIsSelected,
                       bool abIsActive, const cColor &aHighlightCol) {
    if (mpIcon)
        mpIcon->DrawIcon(apViewport, apFunctions, NULL, abIsSelected, mpParent->GetPosition(), mpParent->IsActive());
}
