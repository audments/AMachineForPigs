...
cEditorWindowEntityEditBoxStaticObject::cEditorWindowEntityEditBoxStaticObject(cEditorEditModeSelect *apEditMode,
                                                                               cEntityWrapperStaticObject *apObject)
...
bool cEditorWindowEntityEditBoxStaticObject::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpMeshFile) {
        pAction = mpEntity->CreateSetPropertyActionString(eStaticObjectStr_Filename,
                                                          cString::To8Char(mpInpMeshFile->GetFullPath()));
...
void cEditorWindowEntityEditBoxStaticObject::OnUpdate(float afTimeStep) {
    ...
    mpInpMeshFile->GetInputWidget()->SetToolTip(
        mpEditor->GetFilePathRelativeToWorkingDirW(mpInpMeshFile->GetFullPath()));
