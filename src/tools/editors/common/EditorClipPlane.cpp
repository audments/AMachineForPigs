...
void cEditorClipPlane::Draw(cRendererCallbackFunctions *apFunctions, const cVector3f &avPos) {
    ...
    apFunctions->GetLowLevelGfx()->DrawBoxMinMax(GetProjectedPosOnPlane(vCenter - 20),
                                                 GetProjectedPosOnPlane(vCenter + 20), cColor(1, 1));
    // cEditorHelper::DrawPyramid(apFunctions, vCenter, vCenter + vPlaneNormal*fSign[mbCullingOnPositiveSide]*5, 1,
    // cColor(1,0,0,1));
    apFunctions->GetLowLevelGfx()->DrawLine(vCenter, vCenter + vPlaneNormal * fSign[mbCullingOnPositiveSide] * 5,
                                            cColor(1, 1));
}
...
void cEditorClipPlane::OnPlaneModified() { mpWorld->SetClipPlanesUpdated(); }
