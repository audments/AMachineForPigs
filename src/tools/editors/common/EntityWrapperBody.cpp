...
cEntityWrapperTypeBody::cEntityWrapperTypeBody()
    : iEntityWrapperTypeAggregate(eEditorEntityType_Body, _W("Body"), "Body", "Shape", false) {
...
iEntityWrapperData *cEntityWrapperTypeBody::CreateSpecificData() { return hplNew(cEntityWrapperDataBody, (this)); }
...
iEntityWrapper *cEntityWrapperDataBody::CreateSpecificEntity() { return hplNew(cEntityWrapperBody, (this)); }
...
cEntityWrapperBody::cEntityWrapperBody(iEntityWrapperData *apData)
    : iEntityWrapperAggregate(apData)
...
bool cEntityWrapperBody::GetProperty(int alPropID, float &afX) {
    ...
    case eBodyFloat_Mass:
        ...
        break;
    case eBodyFloat_AngularDamping:
        afX = GetAngularDamping();
        break;
    case eBodyFloat_LinearDamping:
        afX = GetLinearDamping();
        break;
    case eBodyFloat_MaxAngularSpeed:
        afX = GetMaxAngularSpeed();
        break;
    case eBodyFloat_MaxLinearSpeed:
        afX = GetMaxLinearSpeed();
        break;
    case eBodyFloat_BuoyancyDensityMul:
        afX = GetBuoyancyDensityMul();
        break;
    default:
...
bool cEntityWrapperBody::GetProperty(int alPropID, bool &abX) {
    ...
    case eBodyBool_BlockSound:
        ...
        break;
    case eBodyBool_ContinuousCollision:
        abX = HasContinuousCollision();
        break;
    case eBodyBool_CanAttachCharacter:
        abX = CanAttachCharacter();
        break;
    case eBodyBool_PushedByCharacterGravity:
        abX = IsPushedByCharacterGravity();
        break;
    case eBodyBool_CollideCharacter:
        abX = CollidesCharacter();
        break;
    case eBodyBool_CollideNonCharacter:
        abX = CollidesNonCharacter();
        break;
    case eBodyBool_Volatile:
        abX = IsVolatile();
        break;
    case eBodyBool_UseSurfaceEffects:
        abX = UsesSurfaceEffects();
        break;
    case eBodyBool_HasGravity:
        abX = HasGravity();
        break;
...
void cEntityWrapperBody::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
    ...
    if (abIsSelected) {
        ...
        for (; itShapes != mlstComponents.end(); ++itShapes) {
            ...
            // apFunctions->GetLowLevelGfx()->DrawBoxMinMax(pShape->GetPickBV()->GetMin(),
            // pShape->GetPickBV()->GetMax(), cColor(1));
...
cEditorWindowEntityEditBox *cEntityWrapperBody::CreateEditBox(cEditorEditModeSelect *apEditMode) {
    cEditorWindowEntityEditBox *pEditBox =
        (cEditorWindowEntityEditBox *)hplNew(cEditorWindowEntityEditBoxBody, (apEditMode, this));
...
void cEntityWrapperBody::RemoveJointHelper(iEntityWrapperJoint *apJoint, bool abModifyJoint) {
    if (abModifyJoint) {
        if (apJoint->GetParentBody() == this)
            apJoint->SetParentBody(NULL, false);
        if (apJoint->GetChildBody() == this)
            apJoint->SetChildBody(NULL, false);
    }
}
