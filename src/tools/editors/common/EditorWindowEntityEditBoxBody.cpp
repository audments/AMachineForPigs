...
cEditorWindowEntityEditBoxBody::cEditorWindowEntityEditBoxBody(cEditorEditModeSelect *apEditMode,
                                                               cEntityWrapperBody *apObject)
...
void cEditorWindowEntityEditBoxBody::AddPropertySetBody(cWidgetTab *apParentTab) {
    ...
    vPos.y += fAddNear;
    mpInputMass = mpSet->CreateWidgetTextBox(vPos, cVector2f(120, 25), _W(""), apParentTab,
    ...
    vPos.y += fAddNear;
    mpInputAngularDamping =
        mpSet->CreateWidgetTextBox(vPos, vSize, _W(""), apParentTab, eWidgetTextBoxInputType_Numeric, 0.5f);
    ...
    mpCheckBoxPushedByCharacterGravity =
        mpSet->CreateWidgetCheckBox(vPos, 0, _W("Pushed by character gravity"), apParentTab);
...
void cEditorWindowEntityEditBoxBody::OnUpdate(float afTimeStep) {
    ...
    if (mpComboBoxMaterial) {
        ...
        for (int i = 0; i < (int)mpComboBoxMaterial->GetItemNum(); ++i) {
            ...
            if (sComboBoxMaterial == sMaterial || lMaterialIdx < 0 && sComboBoxMaterial == sDefaultMaterial) {
                lMaterialIdx = i;
            }
        }
        if (lMaterialIdx < 0)
            lMaterialIdx = 0;
        if (sMaterial == "")
            mpEntity->SetMaterial(cString::To8Char(mpComboBoxMaterial->GetItemText(lMaterialIdx)));

        mpComboBoxMaterial->SetSelectedItem(lMaterialIdx, false, false);
    }
    if (mpInputMass)
        mpInputMass->SetNumericValue(mpEntity->GetMass());
    if (mpInputAngularDamping)
        mpInputAngularDamping->SetNumericValue(mpEntity->GetAngularDamping());
    if (mpInputLinearDamping)
        mpInputLinearDamping->SetNumericValue(mpEntity->GetLinearDamping());
    if (mpInputMaxAngularSpeed)
        mpInputMaxAngularSpeed->SetNumericValue(mpEntity->GetMaxAngularSpeed());
    if (mpInputMaxLinearSpeed)
        mpInputMaxLinearSpeed->SetNumericValue(mpEntity->GetMaxLinearSpeed());
    mpInpBuoyancyDensityMul->SetValue(mpEntity->GetBuoyancyDensityMul(), false);

    if (mpCheckBoxBlocksSound)
        mpCheckBoxBlocksSound->SetChecked(mpEntity->BlocksSound(), false);
    if (mpCheckBoxContinuousCollision)
        mpCheckBoxContinuousCollision->SetChecked(mpEntity->HasContinuousCollision(), false);
    if (mpCheckBoxCanAttachCharacter)
        mpCheckBoxCanAttachCharacter->SetChecked(mpEntity->CanAttachCharacter(), false);
    if (mpCheckBoxPushedByCharacterGravity)
        mpCheckBoxPushedByCharacterGravity->SetChecked(mpEntity->IsPushedByCharacterGravity(), false);
    if (mpCheckBoxCollideCharacter)
        mpCheckBoxCollideCharacter->SetChecked(mpEntity->CollidesCharacter(), false);
    if (mpCheckBoxCollideNonCharacter)
        mpCheckBoxCollideNonCharacter->SetChecked(mpEntity->CollidesNonCharacter(), false);
    if (mpCheckBoxVolatile)
        mpCheckBoxVolatile->SetChecked(mpEntity->IsVolatile(), false);
    if (mpInpUseSurfaceEffects)
        mpInpUseSurfaceEffects->SetValue(mpEntity->UsesSurfaceEffects(), false);
...
bool cEditorWindowEntityEditBoxBody::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (apWidget == mpComboBoxMaterial) {
        pAction =
            mpEntity->CreateSetPropertyActionString(eBodyStr_Material, cString::To8Char(mpComboBoxMaterial->GetText()));
    }
    ///////////////////////////////////////////
    // Mass
    ...
    // Angular Damping
        ...
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eBodyFloat_AngularDamping, mpInputAngularDamping->GetNumericValue());
    }
    ///////////////////////////////////////////
    // Linear Damping
    else if (apWidget == mpInputLinearDamping) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eBodyFloat_LinearDamping, mpInputLinearDamping->GetNumericValue());
    }
    ///////////////////////////////////////////
    // Max Angular Speed
    else if (apWidget == mpInputMaxAngularSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eBodyFloat_MaxAngularSpeed,
                                                         mpInputMaxAngularSpeed->GetNumericValue());
    }
    ///////////////////////////////////////////
    // Max Linear Speed
    else if (apWidget == mpInputMaxLinearSpeed) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eBodyFloat_MaxLinearSpeed, mpInputMaxLinearSpeed->GetNumericValue());
    }
    ///////////////////////////////////////////
    // Block sound
    ...
    // Continuous collision
        ...
        pAction = mpEntity->CreateSetPropertyActionBool(eBodyBool_ContinuousCollision,
                                                        mpCheckBoxContinuousCollision->IsChecked());
    }
    ///////////////////////////////////////////
    // Can attach character
    else if (apWidget == mpCheckBoxCanAttachCharacter) {
        pAction = mpEntity->CreateSetPropertyActionBool(eBodyBool_CanAttachCharacter,
                                                        mpCheckBoxCanAttachCharacter->IsChecked());
    }
    ///////////////////////////////////////////
    // Pushed by character gravity
    else if (apWidget == mpCheckBoxPushedByCharacterGravity) {
        pAction = mpEntity->CreateSetPropertyActionBool(eBodyBool_PushedByCharacterGravity,
                                                        mpCheckBoxPushedByCharacterGravity->IsChecked());
    }
    ///////////////////////////////////////////
    // Collide character
    else if (apWidget == mpCheckBoxCollideCharacter) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(eBodyBool_CollideCharacter, mpCheckBoxCollideCharacter->IsChecked());
    }
    ///////////////////////////////////////////
    // Collide non character
    else if (apWidget == mpCheckBoxCollideNonCharacter) {
        pAction = mpEntity->CreateSetPropertyActionBool(eBodyBool_CollideNonCharacter,
                                                        mpCheckBoxCollideNonCharacter->IsChecked());
    }
    ///////////////////////////////////////////
    // Volatile
...
bool cEditorWindowEntityEditBoxBody::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpBuoyancyDensityMul) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eBodyFloat_BuoyancyDensityMul, mpInpBuoyancyDensityMul->GetValue());
    } else if (apInput == mpInpUseSurfaceEffects) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(eBodyBool_UseSurfaceEffects, mpInpUseSurfaceEffects->GetValue());
