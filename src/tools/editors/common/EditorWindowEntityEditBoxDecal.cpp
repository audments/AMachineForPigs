...
cEditorWindowEntityEditBoxDecal::cEditorWindowEntityEditBoxDecal(cEditorEditModeSelect *apEditMode,
                                                                 cEntityWrapperDecal *apObject)
...
void cEditorWindowEntityEditBoxDecal::AddDecalPropertySet(cWidgetTab *apParentTab) {
    ...
    for (int i = 0; i < eSurfaceType_LastEnum; ++i) {
        cWidgetButton *pButton = mpSet->CreateWidgetButton(vPos + cVector3f(25, 0, 0) * (float)i, cVector2f(20),
                                                           vButtonText[i], apParentTab);
        AddWidget(pButton);
...
bool cEditorWindowEntityEditBoxDecal::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    for (int i = 0; i < (int)this->mvBSurfaceTypes.size(); ++i) {
        if (apWidget == mvBSurfaceTypes[i]) {
            mpEditor->AddAction(mpEntity->CreateSetPropertyActionBool(lPropID[apWidget->GetUserValue()],
                                                                      mvBSurfaceTypes[i]->IsPressed()));
        }
    }
...
bool cEditorWindowEntityEditBoxDecal::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpMaterial) {
        if (mpInpMaterial->GetValue() == _W("") ||
            cEditorHelper::LoadResourceFile(eEditorResourceType_Material, cString::To8Char(mpInpMaterial->GetValue()),
                                            NULL))
        ...
        pAction = mpEntity->CreateSetPropertyActionVector2f(eDecalVec2f_SubDivs,
                                                            cVector2f((float)mpInpSubDivisions->GetValue() + 1));
