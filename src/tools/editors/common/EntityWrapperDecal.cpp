...
iEntityWrapperData *cEntityWrapperTypeDecal::CreateSpecificData() { return hplNew(cEntityWrapperDataDecal, (this)); }
...
bool cEntityWrapperDataDecal::Load(cXmlElement *apElement) {
    ...
    if (lFileIndex == -1) {
        tString sMat =
            cString::To8Char(pWorld->GetEditor()->GetEngine()->GetResources()->GetFileSearcher()->GetFilePath(
                GetString(eDecalStr_Material)));
    ...
    if (fOffset < fOffsetMin || fOffset > fOffsetMax) {
    ...
    mpMesh =
        cEngineFileLoading::LoadDecalMeshHelper(apElement->GetFirstElement("DecalMesh"), pEng->GetGraphics(),
                                                pEng->GetResources(), GetName(), sMaterial, GetColor(eDecalCol_Color));
...
void cEntityWrapperDecal::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
iVertexBuffer *
cEntityWrapperDecal::BuildDecalVertexBuffer(cWorld *apWorld, cDecalCreator *apCreator, const cVector3f &avPos,
                                            const cVector3f &avSize, float afOffset, const cVector3f &avRight,
                                            const cVector3f &avUp, const cVector3f &avFwd, const tString &asMaterial,
                                            const cColor &aCol, const cVector2l &avSubDivs, int alSubDiv, int alMaxTris,
    ...
    // Debug
    // Log("[Decal] Pos:%s Size:%s Right:%s Up:%s Forward:%s\n", avPos.ToFileString().c_str(),
    // avSize.ToFileString().c_str(), 														  avRight.ToFileString().c_str(), avUp.ToFileString().c_str(),
    //avFwd.ToFileString().c_str());
    ...
    apCreator->SetMaxTrianglesPerDecal((alMaxTris >= 0) ? alMaxTris : cEntityWrapperTypeDecal::GetGlobalMaxTriangles());
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        IterateRenderableNode(pContainers[i]->GetRoot(), apCreator, abAffectStaticObject, abAffectPrimitive,
                              abAffectEntity);
    }
...
cEntityIcon *cEntityWrapperDecal::CreateIcon() { return hplNew(cEntityIcon, (this, "Decal")); }
...
cMesh *cEntityWrapperDecal::CreateDecalMesh() {
    ...
    iVertexBuffer *pVB = BuildDecalVertexBuffer(
        pWorld->GetWorld(), pCreator, mvPosition, mvScale, mfOffset, mtxOrientation.GetRight(), mtxOrientation.GetUp(),
        mtxOrientation.GetForward(), msMaterial, mColor, mvSubDivisions, mlCurrentSubDiv, mlMaxTriangles,
...
void cEntityWrapperDecal::IterateRenderableNode(iRenderableContainerNode *apNode, cDecalCreator *apCreator,
                                                bool abAffectStaticObject, bool abAffectPrimitive,
                                                bool abAffectEntity) {
    apNode->UpdateBeforeUse();

    if (apNode->GetParent() != NULL) {
        cBoundingVolume *pDecalBV = apCreator->GetDecalBoundingVolume();
        if (cMath::CheckAABBIntersection(apNode->GetMin(), apNode->GetMax(), pDecalBV->GetMin(), pDecalBV->GetMax()) ==
            false)
            return;
    }
    ...
    if (apNode->HasChildNodes()) {
        ...
        for (; childIt != apNode->GetChildNodeList()->end(); ++childIt) {
            ...
            IterateRenderableNode(pChildNode, apCreator, abAffectStaticObject, abAffectPrimitive, abAffectEntity);
        }
    }
}
