...
cEntityWrapperTypeEntity::cEntityWrapperTypeEntity(cEditorUserClassSubType *apType)
    : iEntityWrapperTypeUserDefinedEntity(eEditorEntityType_Entity, _W("Entity"), "Entity", apType) {
...
bool cEntityWrapperTypeEntity::IsAppropriateType(cXmlElement *apElement) {
    ...
    if (lFileIndex == -1) {
        ...
        lFileIndex =
            mpWorld->AddFilenameToIndex("Entities", cString::To8Char(pRes->GetFileSearcher()->GetFilePath(sFilename)));
    }
        ...
        if (sSubType != "") {
            ...
            if (msLastCheckedSubType != "")
                Log("Inconsistency found in file %s : no subtype %s in type %s\n", sFilename.c_str(),
                    msLastCheckedSubType.c_str(), sType.c_str());
...
const tString &cEntityWrapperTypeEntity::GetUserTypeName() { return GetUserType()->GetParent()->GetName(); }

//------------------------------------------------------------------------------

const tString &cEntityWrapperTypeEntity::GetUserSubTypeName() { return GetUserType()->GetName(); }
...
iEntityWrapperData *cEntityWrapperTypeEntity::CreateSpecificData() { return hplNew(cEntityWrapperDataEntity, (this)); }

cEntityWrapperDataEntity::cEntityWrapperDataEntity(iEntityWrapperType *apType)
    : iEntityWrapperDataUserDefinedEntity(apType, eUserClassDefinition_Entity) {}
...
bool cEntityWrapperDataEntity::Load(cXmlElement *apElement) {
    ...
    if (lFileIndex == -1) {
        tWString sPath = pWorld->GetEditor()->GetEngine()->GetResources()->GetFileSearcher()->GetFilePath(
            apElement->GetAttributeString("Filename"));
...
bool cEntityWrapperEntity::IsAffectedByDecal(bool abAffectsStaticObject, bool abAffectsPrimitive,
                                             bool abAffectsEntity) {
    return abAffectsEntity && mbAffectedByDecal && IsVisible() && IsActive() && IsCulledByClipPlanes() == false;
}

//---------------------------------------------------------------------------

void cEntityWrapperEntity::UpdateEntity() { iEntityWrapper::UpdateEntity(); }
...
void cEntityWrapperEntity::OnSetActive(bool abX) { mpEngineEntity->Update(); }
...
void cEntityWrapperEntity::SetFilename(const tString &asX) {
    ...
    cEditorEditModeEntities *pEditMode =
        (cEditorEditModeEntities *)GetEditorWorld()->GetEditor()->GetEditMode("Entities");
...
bool cEntityWrapperEntity::SetEntityType(iEntityWrapperType *apType) {
    if (apType == NULL || apType->GetID() != eEditorEntityType_Entity)
        return false;
