...
cEditorWindowSoundBrowser::cEditorWindowSoundBrowser(iEditorBase *apEditor, const tWString &asStartDir,
                                                     tWString &asFilename, void *apCallbackObject,
                                                     tGuiCallbackFunc apCallback)
    : iEditorWindowPopUp(apEditor, "Sound Browser", true, true, true, cVector2f(570, 430)),
      iFileBrowser(asFilename.empty() ? asStartDir : cString::GetFilePathW(asFilename)), msFilename(asFilename) {
    mpEditor = apEditor;
...
void cEditorWindowSoundBrowser::OnInitLayout() {
    ...
    mpCBCurrentDirectory->AddCallback(eGuiMessage_SelectionChange, this,
