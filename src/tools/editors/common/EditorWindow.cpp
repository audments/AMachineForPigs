...
iEditorWindow::~iEditorWindow() { STLDeleteAll(mlstInputs); }
...
const cVector3f &iEditorWindow::GetPosition() { return mvPos; }
...
bool iEditorWindow::IsActive() { return mbEnabled && mbVisible; }
...
cEditorInputBool *iEditorWindow::CreateInputBool(const cVector3f &avPos, const tWString &asLabel, const tString &asName,
                                                 iWidget *apParent) {
...
cEditorInputEnum *iEditorWindow::CreateInputEnum(const cVector3f &avPos, const tWString &asLabel, const tString &asName,
                                                 const tWStringList &alstValues, iWidget *apParent, float afBoxWidth) {
...
cEditorInputText *iEditorWindow::CreateInputString(const cVector3f &avPos, const tWString &asLabel,
                                                   const tString &asName, iWidget *apParent, float afBoxWidth) {
    cEditorInputText *pInput =
        hplNew(cEditorInputText, (this, avPos, asLabel, asName, afBoxWidth, 1, false, false, apParent));
...
cEditorInputFile *iEditorWindow::CreateInputFile(const cVector3f &avPos, const tWString &asLabel, const tString &asName,
                                                 iWidget *apParent, float afBoxWidth) {
...
cEditorInputNumber *iEditorWindow::CreateInputNumber(const cVector3f &avPos, const tWString &asLabel,
                                                     const tString &asName, iWidget *apParent, float afBoxWidth,
                                                     float afNumericAdd) {
    cEditorInputNumber *pInput =
        hplNew(cEditorInputNumber, (this, avPos, asLabel, asName, afBoxWidth, apParent, afNumericAdd));
    pInput->SetLayoutStyle(eEditorInputLayoutStyle_ColumnLabelOnTop);
    SetUpInput(pInput);
...
cEditorInputVec2 *iEditorWindow::CreateInputVec2(const cVector3f &avPos, const tWString &asLabel, const tString &asName,
                                                 iWidget *apParent, float afBoxWidth, const tWStringList &alstLabels,
                                                 eEditorInputLayoutStyle aStyle, float afNumericAdd) {
    cEditorInputVec2 *pInput =
        hplNew(cEditorInputVec2, (this, avPos, asLabel, asName, afBoxWidth, alstLabels, apParent, afNumericAdd));
...
cEditorInputVec3 *iEditorWindow::CreateInputVec3(const cVector3f &avPos, const tWString &asLabel, const tString &asName,
                                                 iWidget *apParent, float afBoxWidth, const tWStringList &alstLabels,
                                                 eEditorInputLayoutStyle aStyle, float afNumericAdd) {
    cEditorInputVec3 *pInput =
        hplNew(cEditorInputVec3, (this, avPos, asLabel, asName, afBoxWidth, alstLabels, apParent, afNumericAdd));
    pInput->SetLayoutStyle(aStyle);
    SetUpInput(pInput);
...
cEditorInputColorFrame *iEditorWindow::CreateInputColorFrame(const cVector3f &avPos, const tWString &asLabel,
...
iEditModeWindow::iEditModeWindow(iEditorEditMode *apEditMode)
    : iEditorWindow(apEditMode->GetEditor(), apEditMode->GetName() + " Edit Mode Window") {
...
void iEditModeObjectCreatorWindow::AddRotateControls() {
    ...
    mpInpMinRotate = CreateInputVec3(vPos, _W("Min rotate"), "", mpGRotate, 50, tWStringList(),
                                     eEditorInputLayoutStyle_RowLabelOnTop, 15);
    vPos.y += mpInpMinRotate->GetSize().y + 5;
    mpInpMaxRotate = CreateInputVec3(vPos, _W("Max rotate"), "", mpGRotate, 50, tWStringList(),
                                     eEditorInputLayoutStyle_RowLabelOnTop, 15);
    ...
    if (mvShortcuts.empty()) {
        mvShortcuts.push_back(
            mpSet->AddGlobalShortcut(0, eKey_Q, mpInpMinRotate->GetInputWidget(1), eGuiMessage_TextBoxValueDown));
        mvShortcuts.push_back(
            mpSet->AddGlobalShortcut(0, eKey_W, mpInpMinRotate->GetInputWidget(1), eGuiMessage_TextBoxValueUp));
    }
}
...
void iEditModeObjectCreatorWindow::AddScaleControls() {
    ...
    mpInpMinScale = CreateInputVec3(vPos, _W("Min scale"), "", mpGScale, 50, tWStringList(),
                                    eEditorInputLayoutStyle_RowLabelOnTop, 0.25f);
    vPos.y += mpInpMinScale->GetSize().y + 5;
    mpInpMaxScale = CreateInputVec3(vPos, _W("Max scale"), "", mpGScale, 50, tWStringList(),
                                    eEditorInputLayoutStyle_RowLabelOnTop, 0.25f);
...
iEditorWindowPopUp::iEditorWindowPopUp(iEditorBase *apEditor, const tString &asName, bool abGetEditorAttention,
                                       bool abGetGuiAttention, bool abDestroyOnDisable, const cVector2f &avSize)
    : iEditorWindow(apEditor, asName) {
    mvSize = avSize;
...
void iEditorWindowPopUp::OnSetActive(bool abX) {
    ...
    if (abX) {
        ...
        if (mbGetGuiAttention) {
            if (mpSet->GetAttentionWidget() == mpBGFrame)
