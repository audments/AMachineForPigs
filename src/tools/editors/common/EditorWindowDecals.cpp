...
cEditorWindowDecals::cEditorWindowDecals(cEditorEditModeDecals *apEditMode)
...
void cEditorWindowDecals::Reset() { ResetInputs(); }
...
cVector3f cEditorWindowDecals::GetSize() {
    ...
    float fDecalDepth =
        mpInpCustomDepthActive->GetValue() ? mpInpCustomDepth->GetValue() : cMath::Max(vSize2D.x, vSize2D.y);
...
cColor cEditorWindowDecals::GetColor() { return mpInpColor->GetValue(); }
...
void cEditorWindowDecals::OnInitLayout() {
    ...
    mpInpMaxAngle = CreateInputNumber(vPos + cVector3f(mpInpMinAngle->GetSize().x + 5, 0, 0), _W("Max Angle"), "",
                                      mpBGFrame, 50, 15);
    ...
    mpInpMinSize = CreateInputVec2(vPos, _W("Min Size"), "", mpBGFrame, 40, tWStringList(),
                                   eEditorInputLayoutStyle_RowLabelOnTop, 0.5f);
    mpInpMinSize->SetLowerBound(true, 0);
    mpInpMaxSize = CreateInputVec2(vPos + cVector3f(mpInpMinSize->GetSize().x + 5, 0, 0), _W("Max Size"), "", mpBGFrame,
                                   40, tWStringList(), eEditorInputLayoutStyle_RowLabelOnTop, 0.5f);
    ...
    mpInpCustomDepth = CreateInputNumber(vPos + cVector3f(mpInpCustomDepthActive->GetSize().x + 5, -3, 0), _W(""), "",
                                         mpBGFrame, 50, 0.5f);
...
bool cEditorWindowDecals::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
