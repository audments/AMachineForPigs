...
cThumbnailRenderCallback::cThumbnailRenderCallback() { mpObject = NULL; }
...
void cEditorThumbnailBuilder::BuildThumbnailFromMeshEntity(cMeshEntity *apEntity, const tWString &asDestName) {
    ...
    mpViewport->GetRenderer()->Render(1.0f / 60.0f, pCamera->GetFrustum(), pWorld, mpViewport->GetRenderSettings(),
                                      mpViewport->GetRenderTarget(), false, mpViewport->GetRendererCallbackList());
    ...
    mpEditor->GetEngine()->GetResources()->GetBitmapLoaderHandler()->SaveBitmap(
        pBmp, GetThumbnailNameFromFileW(cString::To16Char(apEntity->GetSourceFile())), 0);
    hplDelete(pBmp);
...
void cEditorThumbnailBuilder::BuildThumbnailFromMesh(const tWString &asMeshFilename, const tWString &asDestName) {
    ...
    cBitmap *pBmp = pGfx->CopyFrameBufferToBitmap();
    mpEditor->GetEngine()->GetResources()->GetBitmapLoaderHandler()->SaveBitmap(
        pBmp, GetThumbnailNameFromFileW(asMeshFilename), 0);
    hplDelete(pBmp);
...
void cEditorThumbnailBuilder::FocusCameraOnEntity(cMeshEntity *apEntity) {
    ...
    if (fDist < meshBV.GetSize().y)
        fDist = meshBV.GetSize().y;
    if (fDist < meshBV.GetSize().z)
        fDist = meshBV.GetSize().z;
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        if (fAngle > fLargestAngle)
            fLargestAngle = fAngle;
    }
...
void cEditorThumbnailBuilder::VtxBufferAddNormals(const cMatrixf a_mtxTransform, iVertexBuffer *apVtxBuffer,
                                                  cVector3f &avVecSum, float &afCount) {
    ...
    for (int tri = 0; tri < lIndexNum; tri += 3) {
        ...
        float fTriSide[3] = {cMath::Vector3Dist(vPos[0], vPos[1]), cMath::Vector3Dist(vPos[1], vPos[2]),
                             cMath::Vector3Dist(vPos[2], vPos[0])};
        ...
        if (vNormal.y < 0)
            vNormal = vNormal * 0.45f;

        // In case both sides have equal amount of polygons
        if (vNormal.x < 0)
            vNormal = vNormal * 0.85f;
        else if (vNormal.z < 0)
            vNormal = vNormal * 0.8f;
