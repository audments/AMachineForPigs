...
cEditorWindowEntityEditBoxArea::cEditorWindowEntityEditBoxArea(cEditorEditModeSelect *apEditMode,
                                                               cEntityWrapperArea *apArea)
    : cEditorWindowEntityEditBoxUserDefinedEntity(apEditMode, apArea) {
    mpEntity = apArea;
}
...
bool cEditorWindowEntityEditBoxArea::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpType) {
        pAction = hplNew(cEditorActionAreaSetType,
                         (pWorld, lID, cString::To8Char(mpInpType->GetEnumValue(mpInpType->GetValue()))));
