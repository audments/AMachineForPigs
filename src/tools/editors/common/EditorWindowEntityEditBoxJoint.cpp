...
cEditorWindowEntityEditBoxJoint::cEditorWindowEntityEditBoxJoint(cEditorEditModeSelect *apEditMode,
                                                                 iEntityWrapperJoint *apObject)
...
void cEditorWindowEntityEditBoxJoint::AddPropertySetJointBase(cWidgetTab *apParentTab) {
    ...
    for (int i = 0; i < 2; ++i) {
        mvButtons[i] =
            mpSet->CreateWidgetButton(vPos + cVector3f(0, (float)i * 30, 0), cVector2f(160, 25), _W(""), apParentTab);
    ...
    mpInputLimitStepCount =
        mpSet->CreateWidgetTextBox(vPos, cVector2f(120, 25), _W(""), apParentTab, eWidgetTextBoxInputType_Numeric, 1);
    ...
    vPos.y += fAddNearLabel;
    mpInputBreakForce = mpSet->CreateWidgetTextBox(vPos, cVector2f(120, 25), _W(""), apParentTab,
                                                   eWidgetTextBoxInputType_Numeric, 0.5f);
...
void cEditorWindowEntityEditBoxJoint::AddPropertySetJointSounds(cWidgetTab *apParentTab) {
    ...
    mpButtonMoveSoundBrowse =
        mpSet->CreateWidgetButton(vPos + cVector3f(123, 0, 0), cVector2f(20), _W("..."), apParentTab);
    ...
    mpInputMinMoveSpeed =
        mpSet->CreateWidgetTextBox(vPos, cVector2f(70, 25), _W(""), apParentTab, eWidgetTextBoxInputType_Numeric, 0.5f);
...
void cEditorWindowEntityEditBoxJoint::AddPropertySetBall(cWidgetTab *apParentTab) {
    ...
    vPos.y += fAddNearLabel;
    mpInputMaxConeAngle =
        mpSet->CreateWidgetTextBox(vPos, cVector2f(100, 25), _W(""), apParentTab, eWidgetTextBoxInputType_Numeric);
...
void cEditorWindowEntityEditBoxJoint::AddPropertySetScrewSlider(cWidgetTab *apParentTab) {
    ...
    /*for(int i=0; i<3; ++i)
    {
        mvInputPin[i] = mpSet->CreateWidgetTextBox(vPos, cVector2f(100,25), _W(""), apParentTab,
    eWidgetTextBoxInputType_Numeric); mvInputPin[i]->AddCallback(eGuiMessage_TextBoxEnter, this,
    kGuiCallback(InputCallback)); AddWidget(mvInputPin[i]);
...
bool cEditorWindowEntityEditBoxJoint::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (apWidget == mpCheckBoxStickyMinLimit) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(eJointBool_StickyMinLimit, mpCheckBoxStickyMinLimit->IsChecked());
    }
    //////////////////////////////////
    // Sticky Min Limit
    if (apWidget == mpCheckBoxStickyMaxLimit) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(eJointBool_StickyMaxLimit, mpCheckBoxStickyMaxLimit->IsChecked());
    }
    //////////////////////////////////
    // Move Sound
    ...
    // Min Limit Min Speed
        ...
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MinLimitMinSpeed,
                                                         mpInputMinLimitMinSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Min Limit Max Speed
    else if (apWidget == mpInputMinLimitMaxSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MinLimitMaxSpeed,
                                                         mpInputMinLimitMaxSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Max Limit Min Speed
    else if (apWidget == mpInputMaxLimitMinSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MaxLimitMinSpeed,
                                                         mpInputMaxLimitMinSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Max Limit Max Speed
    else if (apWidget == mpInputMaxLimitMaxSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MaxLimitMaxSpeed,
                                                         mpInputMaxLimitMaxSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Min Move Speed
    else if (apWidget == mpInputMinMoveSpeed) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eJointFloat_MinMoveSpeed, mpInputMinMoveSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Min Move Freq
    else if (apWidget == mpInputMinMoveFreq) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eJointFloat_MinMoveFreq, mpInputMinMoveFreq->GetNumericValue());
    }
    //////////////////////////////////
    // Min Move Freq Speed
    else if (apWidget == mpInputMinMoveFreqSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MinMoveFreqSpeed,
                                                         mpInputMinMoveFreqSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Min Move Volume
    else if (apWidget == mpInputMinMoveVolume) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eJointFloat_MinMoveVolume, mpInputMinMoveVolume->GetNumericValue());
    }
    //////////////////////////////////
    // Max Move Speed
    else if (apWidget == mpInputMaxMoveSpeed) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eJointFloat_MaxMoveSpeed, mpInputMaxMoveSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Max Move Freq
    else if (apWidget == mpInputMaxMoveFreq) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eJointFloat_MaxMoveFreq, mpInputMaxMoveFreq->GetNumericValue());
    }
    //////////////////////////////////
    // Max Move Freq Speed
    else if (apWidget == mpInputMaxMoveFreqSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MaxMoveFreqSpeed,
                                                         mpInputMaxMoveFreqSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Max Move Volume
    else if (apWidget == mpInputMaxMoveVolume) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eJointFloat_MaxMoveVolume, mpInputMaxMoveVolume->GetNumericValue());
    }
    //////////////////////////////////
    // Middle Move Speed
    else if (apWidget == mpInputMiddleMoveSpeed) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MiddleMoveSpeed,
                                                         mpInputMiddleMoveSpeed->GetNumericValue());
    }
    //////////////////////////////////
    // Middle Move Volume
    else if (apWidget == mpInputMiddleMoveVolume) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointFloat_MiddleMoveVolume,
                                                         mpInputMiddleMoveVolume->GetNumericValue());
    }
    //////////////////////////////////
    // Limit Step Count
    else if (apWidget == mpInputLimitStepCount) {
        pAction = mpEntity->CreateSetPropertyActionInt(eJointInt_LimitAutoSleepNumSteps,
                                                       (int)mpInputLimitStepCount->GetNumericValue());
    }
    //////////////////////////////////
    // Collide bodies
    ...
    // Ball : Max Cone Angle
        ...
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointBallFloat_MaxConeAngle,
                                                         mpInputMaxConeAngle->GetNumericValue());
    }
    //////////////////////////////////
    // Ball : Max Twist Angle
    else if (apWidget == mpInputMaxTwistAngle) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eJointBallFloat_MaxTwistAngle,
                                                         mpInputMaxTwistAngle->GetNumericValue());
    }
    /*
    //////////////////////////////////
    // Ball : Cone Pin
...
void cEditorWindowEntityEditBoxJoint::AttachBodyToJoint(int alBodyType) {
    ...
    if (pNewBody != NULL && pNewBody == pOtherBody) {
        mpSet->CreatePopUpMessageBox(_W("Warning"), _W("Parent and child bodies must be different"), _W("Continue"),
                                     _W(""), this, NULL);
    } else {
        int lBodyID = -1;
        if (pNewBody)
            lBodyID = pNewBody->GetID();

        iEditorAction *pAction = mpEntity->CreateSetPropertyActionInt(alBodyType, lBodyID);
        // iEditorAction* pAction = hplNew( cEditorActionJointAttachBody,(mpEntity->GetID(),
        // mpEditMode->GetEditorWorld(), aBody, lBodyID));
        mpEditor->AddAction(pAction);
    }
...
bool cEditorWindowEntityEditBoxJoint::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpMoveType) {
        pAction = mpEntity->CreateSetPropertyActionString(
            eJointStr_MoveType, cString::To8Char(mpInpMoveType->GetEnumValue(mpInpMoveType->GetValue())));
        // pAction = hplNew(cEditorActionJointSetStringProperty,(lID, pWorld, eEditorJointStringProperty_MoveType,
        // cString::To8Char(mpInpMoveType->GetEnumValue(mpInpMoveType->GetValue()))));
    }
