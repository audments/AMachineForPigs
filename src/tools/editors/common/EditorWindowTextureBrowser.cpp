...
cEditorObjectIndexEntryTexture::cEditorObjectIndexEntryTexture(cEditorObjectIndexTextures *apIndex)
    : iEditorObjectIndexEntry(apIndex->GetRootDir()) {}
...
cEditorObjectIndexDirTextures::cEditorObjectIndexDirTextures(cEditorObjectIndexTextures *apIndex)
    : iEditorObjectIndexDir(apIndex, NULL) {}
...
cEditorObjectIndexTextures::cEditorObjectIndexTextures(iEditorBase *apEditor, const tWString &asBaseDir)
    : iEditorObjectIndex(apEditor, asBaseDir, _W("tls"), "Textures", "Texture", false) {
...
cTextureBrowserIcon::cTextureBrowserIcon(cEditorWindowTextureBrowser *apBrowser, iWidget *apWidget,
    ...
    mpTexture =
        mpSet->CreateWidgetImage("", cVector3f(65, 65, 0.1f) - cVector3f(lWidth * fScale, lHeight * fScale, 0) * 0.5f,
                                 cVector2f(lWidth * fScale, lHeight * fScale), eGuiMaterial_Diffuse, false, mpIconBG);
...
cTextureBrowserIcon::~cTextureBrowserIcon() {
    if (mpSet->IsDestroyingSet() == false) {
        if (mpIconBG)
            mpSet->DestroyWidget(mpIconBG);
        if (mpLabelName)
            mpSet->DestroyWidget(mpLabelName);
        if (mpLabelSize)
            mpSet->DestroyWidget(mpLabelSize);
        if (mpLabelType)
            mpSet->DestroyWidget(mpLabelType);
        if (mpTexture) {
...
void cTextureBrowserIcon::SetSelected(bool abX) { mpIconBG->SetDrawBackground(abX); }
...
cEditorWindowTextureBrowser::cEditorWindowTextureBrowser(iEditorBase *apEditor, eEditorTextureResourceType aType,
                                                         const tWString &asStartDir, tWString &asTextureFilename,
                                                         void *apCallbackObject, tGuiCallbackFunc apCallback,
                                                         const tWStringList &alstFilters)
    : iEditorWindowPopUp(apEditor, "Texture Browser", true, true, true, cVector2f(800, 550)),
...
void cEditorWindowTextureBrowser::OnInitLayout() {
    ...
    mpComboBoxCurrentDirectory =
        mpSet->CreateWidgetComboBox(cVector3f(5, 30, 10), cVector2f(200, 25), _W(""), mpWindow);
    mpComboBoxCurrentDirectory->AddCallback(eGuiMessage_SelectionChange, this,
                                            kGuiCallback(CurrentDirectory_OnSelectionChange));
    ...
    mpListDirectories->AddCallback(eGuiMessage_SelectionDoubleClick, this,
                                   kGuiCallback(DirectoryList_OnSelectionDblClick));
    ...
    mpLabelTextureFilename =
        mpSet->CreateWidgetTextBox(cVector3f(175, 490, 0.1f), cVector2f(375, 25), _W(""), mpWindow);
    ...
    for (; itEntries != mapEntries.end(); ++itEntries) {
        ...
        if (mlstFilters.empty())
            ...
            for (; itFilters != mlstFilters.end(); ++itFilters) {
                if (cString::ToLowerCaseW(cString::GetFileExtW(cString::To16Char(pEntry->GetFileName()))) ==
                    *itFilters) {
                    bValid = true;
                    break;
                }
        ...
        cTextureBrowserIcon *pIcon =
            hplNew(cTextureBrowserIcon, (this, mpFrameTextures, (cEditorObjectIndexEntryTexture *)pEntry));
