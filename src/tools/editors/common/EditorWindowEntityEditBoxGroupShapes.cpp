...
cEditorWindowEntityEditBoxGroupShapes::cEditorWindowEntityEditBoxGroupShapes(cEditorEditModeSelect *apEditMode,
                                                                             const tEntityWrapperList &alstEntities)
    : cEditorWindowEntityEditBoxGroup(apEditMode, alstEntities) {}
...
void cEditorWindowEntityEditBoxGroupShapes::Create() {
    ...
    for (int i = 0; i < 2; ++i) {
        *vButtons[i] =
            mpSet->CreateWidgetButton(vPos + cVector3f(0, (float)i * 30, 0), cVector2f(160, 25), _W(""), mpBGFrame);
