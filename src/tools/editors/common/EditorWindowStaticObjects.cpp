...
cEditorObjectIndexEntryStaticObject::cEditorObjectIndexEntryStaticObject(cEditorObjectIndexDirStaticObjects *apDir)
    : iEditorObjectIndexEntryMeshObject(apDir) {}

//-------------------------------------------------------------------
//-------------------------------------------------------------------

cEditorObjectIndexDirStaticObjects::cEditorObjectIndexDirStaticObjects(cEditorObjectIndexStaticObjects *apIndex,
                                                                       cEditorObjectIndexDirStaticObjects *apParent)
...
cEditorObjectIndexStaticObjects::cEditorObjectIndexStaticObjects(iEditorBase *apEditor, const tWString &asBaseFolder)
    : iEditorObjectIndex(apEditor, asBaseFolder, _W("sop"), "StaticObjects", "StaticObject", true) {
...
cEditorWindowStaticObjects::cEditorWindowStaticObjects(cEditorEditModeStaticObjects *apEditMode,
    ...
    : cEditorWindowObjectBrowser(apEditMode, avBaseDirs, true) {
    mbScaleUpdated = true;
...
cMeshEntity *cEditorWindowStaticObjects::CreatePreviewEntity(iEditorObjectIndexEntryMeshObject *apEntry) {
    ...
    cMesh *pMesh = mpEditor->GetEngine()->GetResources()->GetMeshManager()->CreateMesh(
        cString::To8Char(apEntry->GetFileNameRelPath()));
    if (pMesh)
