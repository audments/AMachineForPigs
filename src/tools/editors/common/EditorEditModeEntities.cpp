...
cEditorEditModeEntities::cEditorEditModeEntities(iEditorBase *apEditor, iEditorWorld *apEditorWorld)
    : iEditorEditModeObjectCreator(apEditor, "Entities", apEditorWorld) {}
...
void cEditorEditModeEntities::DrawObjectPreview(cEditorWindowViewport *apViewport,
                                                cRendererCallbackFunctions *apFunctions, const cMatrixf &amtxTransform,
                                                bool abPreCreationActive) {
...
iEntityWrapperType *cEditorEditModeEntities::GetTypeFromEntFile(const tString &asFile) {
    ...
    Error("EditModeEntities : Could not find type %s %s, either %s is badly saved, or was saved using a different "
          "EntityTypes.cfg file.",
          sType.c_str(), sSubType.c_str(), asFile.c_str());
    cEntityWrapperTypeEntity *pType = (cEntityWrapperTypeEntity *)iEditorEditModeObjectCreator::GetType(0);
