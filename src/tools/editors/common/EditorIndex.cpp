...
iEditorObjectIndex::iEditorObjectIndex(iEditorBase *apEditor, const tWString &asBaseFolder, const tWString &asIndexExt,
                                       const tString &asIndexTypeName, const tString &asEntryTypeName,
...
bool iEditorObjectIndex::Refresh() {
    ...
    if (mbRefreshThumbnails) {
        mbRefreshThumbnails = false;
        mpEditor->GetEngine()->GetResources()->AddResourceDir(mpEditor->GetFolderFullPath(eEditorDir_Thumbnails),
                                                              false);
...
void iEditorObjectIndex::AddFilesInDir(tWStringList &asFileList, const tWString &asFolder, const tWString &asLocalPath,
                                       bool abAddFilesInSubDirs) {
    /////////////////////////////////////////////////////////////
...
iEditorObjectIndexDir::~iEditorObjectIndexDir() { Clear(); }
...
bool iEditorObjectIndexDir::CreateFromDir(iEditorObjectIndexDir *apParentDir, const tWString &asRelPath,
                                          bool abCreateSubDirs) {
    ...
    if (pEntries) {
        ...
        while (itEntries.HasNext()) {
            ...
            if (pEntry->CreateFromXmlElement(pXmlEntry)) {
                ...
                lstFiles.erase(find(lstFiles.begin(), lstFiles.end(),
                                    cString::To16Char(cString::GetFileName(pEntry->GetFileName()))));
            } else {
                hplDelete(pEntry);
            }
        }
...
bool iEditorObjectIndexEntry::CheckFileExists() { return cPlatform::FileExists(GetFileNameFullPath()); }
...
tWString iEditorObjectIndexEntry::GetFileNameFullPath() {
    // tWString sFilenameFullPath =
    // mpParentDir->GetIndex()->GetEditor()->GetEngine()->GetResources()->GetFileSearcher()->GetFilePath(msFileName);
    tWString sFilenameFullPath =
        cString::AddSlashAtEndW(mpParentDir->GetFullPath(), _W('/')) + cString::To16Char(msFileName);
