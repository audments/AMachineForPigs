...
void iEditorEditMode::SetCurrent(bool abX) {
    if (mpWindow)
        mpWindow->SetActive(abX);

    OnSetCurrent(abX);
}
//---------------------------------------------------------------------

void iEditorEditMode::DrawPreGrid(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
void iEditorEditMode::DrawPostGrid(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
                                   const cVector3f &avPos) {
    apFunctions->SetMatrix(NULL);
    apFunctions->SetBlendMode(eMaterialBlendMode_None);
    apFunctions->SetTextureRange(NULL, 0);
    ...
    for (; it != mapEntities.end(); ++it) {
        ...
        if (pEnt->IsVisible() && pEnt->IsCulledByFrustum(apViewport->GetCamera()) == false &&
...
iEditorEditModeObjectCreator::iEditorEditModeObjectCreator(iEditorBase *apEditor, const tString &asName,
                                                           iEditorWorld *apEditorWorld)
    : iEditorEditMode(apEditor, asName) {
...
void iEditorEditModeObjectCreator::OnAdd() { SetUpTypes(); }
...
void iEditorEditModeObjectCreator::DrawPostGrid(cEditorWindowViewport *apViewport,
                                                cRendererCallbackFunctions *apFunctions, const cVector3f &avPos) {
    ...
    cMatrixf mtxTransform =
        cMath::MatrixMul(cMath::MatrixTranslate(GetCreatorPosition()),
...
int iEditorEditModeObjectCreator::GetTypeNum() { return (int)mvTypes.size(); }
...
iEntityWrapperType *iEditorEditModeObjectCreator::GetType() { return GetType(mlSubType); }
