...
template <class T> class cEditorActionObjectSetProperty : public iEditorActionWorldModifier {
  public:
    cEditorActionObjectSetProperty(iEditorWorld *apEditorWorld, const tIntList &alstEntityIDs, iProp *apProp,
                                   const T &aNewValue)
        : iEditorActionWorldModifier("", apEditorWorld) {
        if (apProp == NULL) {
        ...
        if (mbValidProperty) {
            ...
            for (; it != mlstEntityIDs.end(); ++it) {
                // Fetch old value and check if the property exists in the object and if the current and next values are
                // different
  ...
  protected:
    virtual void DoModify() { Apply(mNewValue); }
    virtual void UndoModify() { Apply(mlstOldValues); }

    iEntityWrapper *GetObject(int alID) { return GetObjectHelper(mpEditorWorld, alID); }
...
class cEditorActionUserDefinedObjectSetVariable : public iEditorActionWorldModifier {
  public:
    cEditorActionUserDefinedObjectSetVariable(iEditorWorld *apWorld, int alID, const tWString &asName,
