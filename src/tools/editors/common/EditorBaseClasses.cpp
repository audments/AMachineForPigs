...
// Lovely magic from the "LuxBasePersonal.h" in game code.  We should maybe make this into a engine function or
// something
#if defined(WIN32)
...
#define PERSONAL_RELATIVEPIECES                                                                                        \
    _W(""), _W("Library"), _W("Library/Application Support"), _W("Library/Application Support/Frictional Games"),
...
iWidgetContainer::~iWidgetContainer() { DestroyWidgets(mpSet); }

void iWidgetContainer::SetSet(cGuiSet *apSet) { mpSet = apSet; }
...
cMeshEntity *cEditorEntityLoader::LoadEntFile(int alID, const tString &asName, const tString &asFilename,
                                              cWorld *apWorld, bool abLoadAnims, bool abLoadParticles,
    ...
    pEntity = LoadEntityFromElement(alID, asName, pDoc, apWorld, asFilename, sFullPath, abLoadAnims, abLoadParticles,
                                    abLoadBillboards, abLoadSounds, abLoadLights);
...
cMeshEntity *cEditorEntityLoader::LoadEntityFromElement(int alID, const tString &asName, cXmlElement *apElement,
                                                        cWorld *apWorld, const tString &asFilename,
                                                        const tWString &asFullPath, bool abLoadAnims,
                                                        bool abLoadParticles, bool abLoadBillboards, bool abLoadSounds,
                                                        bool abLoadLights) {
    ...
    cMeshEntity *pEntity = (cMeshEntity *)Load(asName, alID, true, apElement, cMatrixf::Identity, cVector3f(1), apWorld,
                                               asFilename, asFullPath, NULL);
...
iEditorBase::iEditorBase(const tWString &asFileCategoryName, const tWString &asFileCategoryString)
    : iUpdateable("EditorUpdate") {
...
void iEditorBase::Command_New() {
    ShowMessageBox(_W("Warning"), _W("All unsaved data will be lost, continue?"), _W("Yes"), _W("No"), this,
                   kGuiCallback(MessageBox_New_Callback));
}
...
void iEditorBase::Command_Load() {
    if (mpEditorWorld->IsModified()) {
        ShowMessageBox(_W("Warning"), _W("All unsaved data will be lost, continue?"), _W("Yes"), _W("No"), this,
                       kGuiCallback(MessageBox_Load_Callback));
...
void iEditorBase::Command_Save() {
    if (msSaveFilename == _W("") || cPlatform::FileExists(msSaveFilename) == false) {
        Command_SaveAs();
    } else
        Save();
}

void iEditorBase::Command_SaveAs() { ShowSaveFilePicker(this, kGuiCallback(SaveFileCallback)); }
...
void iEditorBase::Command_Exit() {
    ...
    if (mpEditorWorld && mpEditorWorld->IsModified()) {
        ShowMessageBox(_W("Warning"), _W("There are unsaved changes. Do you want to save them?"), _W("Yes"), _W("No"),
                       this, kGuiCallback(MessageBox_Exit_UnsavedChanges_Callback));
    } else {
        ShowMessageBox(_W("Exiting"), _W("Sure you want to exit?"), _W("Yes"), _W("No"), this,
                       kGuiCallback(QuitCallback));
    }
}
...
cEditorGrid *iEditorBase::GetGrid() { return mpFocusedViewport->GetGrid(); }
...
void iEditorBase::LoadEditorSession(iXmlDocument *apDoc, cXmlElement **apElement) {
    ...
    *apElement = apDoc->GetFirstElement("EditorSession");
    ...
    if (pPerfOptions) {
        ...
        mpEditorWorld->SetTypeActive(eEditorEntityType_ParticleSystem,
                                     (pPerfOptions->GetAttributeBool("PSActive", true)));
        SetWorldReflectionActive(pPerfOptions->GetAttributeBool("WorldReflection", true));
        cEditorWindowViewport::SetCamPlanes(
            pPerfOptions->GetAttributeVector2f("CamClipPlanes", cVector2f(0.05f, 1000)));
...
void iEditorBase::Exit() { mpEngine->Exit(); }
...
bool iEditorBase::GetFlags(int alFlags) { return ((mlFlags & alFlags) != 0); }
...
cEngine *iEditorBase::Init(cEngine *apEngine, bool abDestroyEngineOnExit) {
    ...
    //  Get settings from loaded config file
        ...
        mpEngine->GetResources()->GetMaterialManager()->SetTextureSizeDownScaleLevel(
            cString::ToInt(GetSetting("TexQuality").c_str(), 0));
    ...
    // mpEntityLoader = (cEditorEntityLoader*)cMemoryManager::AddPointer(cAllocatedPointer(new
    // cEditorEntityLoader(this), __FILE__, __LINE__, sizeof(cEditorEntityLoader)));
    ...
    tVertexElementFlag lFlags = eVertexElementFlag_Position | eVertexElementFlag_Normal | eVertexElementFlag_Texture0 |
...
void iEditorBase::SetFocusedViewport(cEditorWindowViewport *apViewport) {
    if (mpFocusedViewport == apViewport || mbViewportLocked)
        return;

    if (mpFocusedViewport)
        mpFocusedViewport->SetFocused(false);
...
void iEditorBase::SetViewportLocked(bool abX) { mbViewportLocked = abX; }
...
const cVector2f &iEditorBase::GetViewportCameraPlanes() { return cEditorWindowViewport::GetCamPlanes(); }
...
void iEditorBase::SetLayoutVec3f(int alIdx, const cVector3f &avX) { mmapLayoutVecs3f[alIdx] = avX; }
...
void iEditorBase::SetLayoutVec2f(int alIdx, const cVector2f &avX) { mmapLayoutVecs2f[alIdx] = avX; }
...
cEditorWindowTextureBrowser *iEditorBase::ShowTextureBrowser(eEditorTextureResourceType aType,
                                                             const tWString &asStartDir, tWString &asDestFilename,
                                                             void *apCallbackObject, tGuiCallbackFunc apCallback,
                                                             const tWStringList &alstFilters) {
    cEditorWindowTextureBrowser *pWindow =
        hplNew(cEditorWindowTextureBrowser,
               (this, aType, asStartDir, asDestFilename, apCallbackObject, apCallback, alstFilters));
    pWindow->Init();
...
cEditorWindowSoundBrowser *iEditorBase::ShowSoundBrowser(const tWString &asStartDir, tWString &asDestFilename,
                                                         void *apCallbackObject, tGuiCallbackFunc apCallback) {
    cEditorWindowSoundBrowser *pBrowser =
        hplNew(cEditorWindowSoundBrowser, (this, asStartDir, asDestFilename, apCallbackObject, apCallback));
...
void iEditorBase::SetUpWindowAreas() {
    ...
    SetLayoutVec2f(eLayoutVec2_ViewportAreaSize,
                   cVector2f(0.77f * mvScreenSize.x, mvScreenSize.y - 50 - vMenuSize.y - 4));
...
void iEditorBase::LoadConfig() {
    ...
    mvFolders[eEditorDir_StaticObjects] =
        mpMainConfig->GetStringW("Directories", "StaticObjectsDir", _W("static_objects"));
    ...
    for (int i = eEditorDir_Home; i < eEditorDir_LastEnum; ++i) {
        ...
        if (cPlatform::FolderExists(sDir) == false) {
            if (i < eEditorDir_Maps)
                ...
                cPlatform::CreateMessageBox(_W("Error"), _W("Could not find data folder: %ls"),
                                            cString::ReplaceCharToW(sDir, _W("\\"), _W("/")).c_str());
...
void iEditorBase::AppGotMouseFocus() { mpSet->SetDrawMouse(true); }

//----------------------------------------------------------------------------

void iEditorBase::AppLostMouseFocus() { mpSet->SetDrawMouse(false); }
...
bool iEditorBase::RecentFileCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mpEditorWorld->IsModified())
        ShowMessageBox(_W("Warning"), _W("All unsaved data will be lost, continue?"), _W("Yes"), _W("No"), this,
                       kGuiCallback(MessageBox_RecentFile_Callback));
    else
        Load();
...
void iEditorBase::ShowMessageBox(const tWString &asCaption, const tWString &asText, const tWString &asButton1,
                                 const tWString &asButton2, void *apCallbackObject, tGuiCallbackFunc apCallback) {
    SetFlags(eEditorFlag_PopUpActive, true);

    cGuiPopUpMessageBox *pBox =
        mpSet->CreatePopUpMessageBox(asCaption, asText, asButton1, asButton2, apCallbackObject, apCallback);
    pBox->AddOnDestroyCallback(this, kGuiCallback(PopUpCloseCallback));
}

//----------------------------------------------------------------------------

void iEditorBase::ShowLoadFilePicker(tWStringVec &avLoadFilenames, const tWString &asStartPath, void *apCallbackObject,
                                     tGuiCallbackFunc apCallback, const tWString &asCategoryName,
                                     const tWStringList &alstCategoryStrings) {
    SetFlags(eEditorFlag_PopUpActive, true);

    cGuiPopUpFilePicker *pPicker =
        mpSet->CreatePopUpLoadFilePicker(avLoadFilenames, false, asStartPath, false, apCallbackObject, apCallback);
...
void iEditorBase::ShowSaveFilePicker(void *apCallbackObject, tGuiCallbackFunc apCallback) {
    ...
    cGuiPopUpFilePicker *pPicker = mpSet->CreatePopUpSaveFilePicker(
        msSaveFilename, msFileCategoryName, msFileCategoryString, msLastSavePath, false, apCallbackObject, apCallback);
    pPicker->AddOnDestroyCallback(this, kGuiCallback(PopUpCloseCallback));
}
