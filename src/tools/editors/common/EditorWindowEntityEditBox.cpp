...
cEditorWindowEntityEditBox::cEditorWindowEntityEditBox(cEditorEditModeSelect *apEditMode, iEntityWrapper *apEntity)
    : iEditModeWindow(apEditMode) {
...
void cEditorWindowEntityEditBox::AddPropertyRotation(cWidgetTab *apParentTab) {
    ...
    mpInpRotation =
        CreateInputVec3(0, _W("Rotation"), "", apParentTab, 150, lstLabels, eEditorInputLayoutStyle_ColumnLabelOnTop,
...
void cEditorWindowEntityEditBox::AddPropertyScale(cWidgetTab *apParentTab) {
    ...
    mpInpScale = CreateInputVec3(0, _W("Scale"), "", apParentTab, 150, lstLabels,
                                 eEditorInputLayoutStyle_ColumnLabelOnTop, cEditorSelection::GetScaleSnap());
    ...
    if (apWidget == mpBEditAttachments) {
        ...
        if (mpEditMode->GetCurrentEntitySelector() == mpChildHighlighter) {
            ...
            if (mpChildHighlighter->IsAtLeastOneFilterActive()) {
                ...
                mpEditor->ShowMessageBox(_W("Warning"), _W("You must activate at least one filter"), _W("OK"), _W(""),
                                         NULL, NULL);
            }
        }
        ...
        for (int i = 0; i < (int)mvBAttachFilters.size(); ++i) {
            ...
            if (apWidget == pButton) {
                mpChildHighlighter->GetFilter()->SetTypeFilter((eEditorEntityType)pButton->GetUserValue(),
                                                               pButton->IsPressed());
                if (mpChildHighlighter->IsAtLeastOneFilterActive() == false) {
                    mpEditor->ShowMessageBox(_W("Warning"), _W("You must activate at least one filter"), _W("OK"),
                                             _W(""), NULL, NULL);
...
void cEditorWindowEntityEditBox::OnUpdate(float afTimeStep) {
    ...
    if (mpChildHighlighter) {
        for (int i = 0; i < (int)mvBAttachFilters.size(); ++i) {
            ...
            if (pButton)
                pButton->SetPressed(
                    mpChildHighlighter->GetFilter()->GetTypeFilter((eEditorEntityType)pButton->GetUserValue()), false);
        }
    }
}
...
void cEditorWindowEntityEditBox::OnSetActive(bool abX) {
    if (abX) {
        if (mCurrentEditedType != -1 && mLastEditedType == mCurrentEditedType)
...
bool cEditorWindowEntityEditBox::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpName) {
        ...
        if (mpEntity->GetName() != sName && pWorld->IsNameAvailable(sName) == false)
            mpEditor->ShowMessageBox(_W("Warning"), _W("There is already an object with same name"), _W("OK"), _W(""),
                                     NULL, NULL);
        else
...
cEditorWindowEntityEditBoxUserDefinedEntity::cEditorWindowEntityEditBoxUserDefinedEntity(
    cEditorEditModeSelect *apEditMode, iEntityWrapperUserDefinedEntity *apVarEntity)
    : cEditorWindowEntityEditBox(apEditMode, apVarEntity) {
...
cEditorWindowEntityEditBoxUserDefinedEntity::~cEditorWindowEntityEditBoxUserDefinedEntity() { hplDelete(mpInputPanel); }
...
bool cEditorWindowEntityEditBoxUserDefinedEntity::VarInputCallback(iEditorVarInput *apInput) {
    iEditorAction *pAction =
        mpEntity->CreateActionSetUserVariable(apInput->GetVar()->GetName(), apInput->GetInput()->GetValue());
    mpEditor->AddAction(pAction);
