...
iWidget *iEditorWindowLowerToolbar::AddGridControls() {
    ...
    mpGridPresetLabel =
        mpSet->CreateWidgetLabel(cVector3f(179, 4, 0.1f), cVector2f(32, 20), _W("Grid presets"), mpGridControlsGroup);

    mpSnapPreset1 =
        mpSet->CreateWidgetButton(cVector3f(175, 18, 0.1f), cVector2f(32, 20), _W("0.01"), mpGridControlsGroup);
    ...
    mpSnapPreset5 =
        mpSet->CreateWidgetButton(cVector3f(210, 18, 0.1f), cVector2f(32, 20), _W("0.05"), mpGridControlsGroup);
    ...
    mpSnapPreset25 =
        mpSet->CreateWidgetButton(cVector3f(245, 18, 0.1f), cVector2f(32, 20), _W("0.25"), mpGridControlsGroup);
...
bool iEditorWindowLowerToolbar::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpPlaneHeight) {
        ...
        if (pClipPlane) {
            pAction =
                hplNew(cEditorActionClipPlaneSetActive, (pWorld, lClipPlaneIdx, mpInpClipPlaneActive->GetValue()));
        }
    }
...
bool iEditorWindowLowerToolbar::InputCallback(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    // Camera lock to grid button
        ...
        if (pClipPlane) {
            pAction = hplNew(cEditorActionClipPlaneSetCullingOnPositiveSide,
                             (pWorld, lClipPlaneIdx, !pClipPlane->GetCullingOnPositiveSide()));
        }
