...
cEditorWindowLoaderStatus::cEditorWindowLoaderStatus(iEditorBase *apEditor)
    : iEditorWindowPopUp(apEditor, "Loader Status Window") {}
...
void cEditorWindowLoaderStatus::SetStatusString(const tWString &asStatus) {
    if (mpLabelCurrentStatus)
        mpLabelCurrentStatus->SetText(asStatus);
}
