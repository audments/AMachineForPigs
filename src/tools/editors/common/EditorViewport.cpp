...
float iViewportCameraZoomFunc::GetOrthoViewSizeFromZoom(float afX) { return mfMaxDistance * exp(-afX); }

float iViewportCameraZoomFunc::GetDistToTargetFromZoom(float afX) { return mfMaxDistance * exp(-afX); }

float iViewportCameraZoomFunc::GetZoomFromDistToTarget(float afX) { return log(mfMaxDistance / afX); }

float iViewportCameraZoomFunc::GetZoomFromOrthoViewSize(float afX) { return log(mfMaxDistance / afX); }
...
bool cEditorViewportCamera::IsOrtho() {
    return mCamType == eEditorCameraType_OrthoFront || mCamType == eEditorCameraType_OrthoRight ||
...
void cEditorViewportCamera::SetOrientation(const cVector3f &avForward, const cVector3f &avUp,
                                           const cVector3f &avRight) {
    mvForward = avForward;
    mvUp = avUp;
    mvRight = avRight;
}
...
void cEditorViewportCamera::SetCameraSphCoords(const cVector2f &avSphCoords) {
    if (mvCameraSphCoords == avSphCoords)
        return;
...
void cEditorViewportCamera::SetTargetPosition(const cVector3f &avTargetPos) {
    if (mvTargetPosition == avTargetPos)
        return;
...
void cEditorViewportCamera::SetTargetSphCoords(const cVector2f &avSphCoords) {
    if (mvTargetSphCoords == avSphCoords)
        return;
...
const cVector3f &cEditorViewportCamera::GetCameraPosition() {
    if (mbCameraPosNeedsUpdate) {
        if (mCamType == eEditorCameraType_Orbit)
            mvCameraPosition =
                cMath::GetPoint3DFromSphericalCoords(mvTargetPosition, mfDistToTarget, GetCameraSphCoords());
...
const cVector3f &cEditorViewportCamera::GetTargetPosition() {
    if (mbTargetPosNeedsUpdate) {
        if (mCamType == eEditorCameraType_Fly)
            mvTargetPosition =
                cMath::GetPoint3DFromSphericalCoords(mvCameraPosition, mfDistToTarget, GetTargetSphCoords());
...
void cEditorViewportCamera::Track(const cVector2f &avParams) {
    ...
    if (IsOrtho()) {
        mpEngineCamera->UnProjectHelper(
            &vRayStart, &vRayDir, mpViewport->GetMouseViewportPosition(), mpViewport->GetGuiViewportSize(), cMatrixf(),
    ...
    if (IsOrtho() || mbLockToGrid) {
        ...
        if (cMath::CheckPlaneLineIntersection(pGrid->GetPlane(), vRayStart, vRayEnd, &mvTrackNewMousePos, NULL) ==
            false) {
        ...
        vDisplacement =
            (mpEngineCamera->GetUp() * avParams.y + mpEngineCamera->GetRight() * (-avParams.x)) * mfTrackFactor;
...
const cVector3f &cEditorViewportCamera::GetPointFromClampedDistance(const cVector3f &avCenter, cVector3f &avPos,
                                                                    float afMaxDistance) {
...
void cEditorViewportCamera::UpdateEngineCamera() {
    ...
    if (mbKeepOrientation == false) {
        cVector3f vDisplacementPos =
            cMath::GetPoint3DFromSphericalCoords(vTarPos, 1, GetCameraSphCoords() - cVector2f(0.1f, 0));
...
float cEditorViewportCamera::BinaryDistanceSearch(float &afStart, float &afEnd, const cBoundingVolume &aBV) {
    ...
    cMath::GetNormalizedClipRectFromBV(vNormalMin, vNormalMax, bv, mpEngineCamera->GetFrustum(),
                                       mpEngineCamera->GetFrustum()->GetFOV() * 0.5f);
    vNormalClipRect = cRect2f(vNormalMin.x, vNormalMin.y, vNormalMax.x - vNormalMin.x, vNormalMax.y - vNormalMin.y);
    if (vNormalClipRect.x > -0.7f && vNormalClipRect.y > -0.7f && vNormalClipRect.w <= 1.4f &&
        vNormalClipRect.h <= 1.4f) {
...
iEditorViewport::~iEditorViewport() {
    ...
    if (mpImgViewport)
        mpGuiSet->DestroyWidget(mpImgViewport);
...
void iEditorViewport::LogDebug() {
    ...
    // pCam->GetAspect(), pCam->GetNearClipPlane(), pCam->GetFarClipPlane());
}
...
void iEditorViewport::CreateGuiViewport(iWidget *apParentWidget) {
    ...
    mpImgViewport =
        mpGuiSet->CreateWidgetImage("", mvViewportPos, mvViewportSize, eGuiMaterial_Alpha, false, apParentWidget);
...
void iEditorViewport::SetFrameBuffer(iFrameBuffer *apFB) {
    if (apFB == NULL || mpFB == apFB)
        return;

    mpFB = apFB;
    iFrameBufferAttachment *pColorBuffer = mpFB->GetColorBuffer(0);
    if (pColorBuffer)
        mpRenderTarget = pColorBuffer->ToTexture();
...
void iEditorViewport::SetEngineViewportPositionAndSize(const cVector2l &avPos, const cVector2l &avSize) {
    if (mvEngineViewportPos == avPos && mvEngineViewportSize == avSize)
        return;
    ...
    mvUVStart = (vPosFloat + cVector2f(0, vSizeFloat.y)) / vFBSizeFloat;
...
void iEditorViewport::SetEngineViewportSize(const cVector2l &avSize) {
    if (mvEngineViewportSize == avSize)
        return;
    mvEngineViewportSize = avSize;
    mpEngineViewport->SetSize(mvEngineViewportSize);
...
const cVector2f &iEditorViewport::GetMouseViewportPosition() {
    if (mbMousePositionUpdated) {
        cVector2f vViewportGlobalPos =
            cVector2f(mpImgViewport->GetGlobalPosition().x, mpImgViewport->GetGlobalPosition().y);
...
void iEditorViewport::UpdateCameraPlanes() { mCamera.SetCameraPlanes(mvCamPlanes); }
...
bool iEditorViewport::Viewport_OnDraw(iWidget *apWidget, const cGuiMessageData &aData) { return OnViewportDraw(aData); }
...
void iEditorViewport::UpdateUnprojection() {
    if (mbUnprojectionUpdated == false)
        return;
