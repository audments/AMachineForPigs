...
cSphereCreator::cSphereCreator(iEditorEditMode *apEditMode) : iShapeCreator(apEditMode) { mfDefaultRadius = 1; }
...
void cSphereCreator::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions) {
    ...
    */
    if (mvPoints.empty() == false && mbDragging == true) {
        apFunctions->GetLowLevelGfx()->DrawLine(mvPoints[0] - cVector3f(0.05f, 0, 0),
                                                mvPoints[0] + cVector3f(0.05f, 0, 0), cColor(1, 1));
        apFunctions->GetLowLevelGfx()->DrawLine(mvPoints[0] - cVector3f(0, 0.05f, 0),
                                                mvPoints[0] + cVector3f(0, 0.05f, 0), cColor(1, 1));
        apFunctions->GetLowLevelGfx()->DrawLine(mvPoints[0] - cVector3f(0, 0, 0.05f),
                                                mvPoints[0] + cVector3f(0, 0, 0.05f), cColor(1, 1));
...
void cSphereCreator::OnEditorUpdate() {
    ...
    if(cMath::CheckPlaneLineIntersection(mSphRadiusPlane, pViewport->GetUnprojectedStart(),
    pViewport->GetUnprojectedEnd(), &mvTempPoint, NULL))
    {
        vDebugPos = mvTempPoint;
    }
...
bool cSphereCreator::IsDoneCreating() { return mvPoints.size() == 2; }

void cSphereCreator::Reset() { mvPoints.clear(); }
