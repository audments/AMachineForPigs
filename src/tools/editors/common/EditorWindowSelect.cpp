...
void cEditorWindowSelect::AddFilter(eEditorEntityType aType) {
    ...
    cWidgetButton *pButton =
        mpSet->CreateWidgetButton(0, cVector2f(17), cString::SubW(sTypeName, 0, 2), mpObjectTypeGroup, true);
...
bool cEditorWindowSelect::TypeChange(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
...
void cEditorWindowSelect::OnInitLayout() {
    ...
    mpObjectTypeGroup =
        mpSet->CreateWidgetGroup(cVector3f(5, 8, 0.1f), cVector2f(160, 80), _W("Select Object Type"), mpBGFrame);
    ...
    mpCBSelectMultipleTypes =
        mpSet->CreateWidgetCheckBox(cVector3f(40, 10, 0.1f), 0, _W("MultiSelect"), mpObjectTypeGroup);
    ...
    for (int i = 0; i < 3; ++i) {
        cGuiGfxElement *pGfx = mpSet->GetGui()->CreateGfxImage(
            "editmode_select_" + cString::ToLowerCase(sButtonText[i]) + ".tga", eGuiMaterial_Alpha);
