...
iEditorInput::iEditorInput(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                           const tString &asName, iWidget *apParent, eEditorInputLayoutStyle aStyle) {
    mpWindow = apWindow;
...
void iEditorInput::SetPosition(const cVector3f &avPos) { mpHandle->SetPosition(avPos); }
...
iEditorInputLabeled::iEditorInputLabeled(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                                         const tString &asName, iWidget *apParent, eEditorInputLayoutStyle aStyle)
    : iEditorInput(apWindow, avPos, asLabel, asName, apParent, aStyle) {
...
cEditorInputBool::cEditorInputBool(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                                   const tString &asName, iWidget *apParent)
    : iEditorInput(apWindow, avPos, asLabel, asName, apParent) {
...
void cEditorInputBool::UpdateValue() { SetValue(mpChB->IsChecked(), false, false); }
...
cEditorInputText::cEditorInputText(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                                   const tString &asName, float afTextBoxWidth, int alAmount, bool abNumeric,
                                   float afNumericAdd, iWidget *apParent, const tWStringList &alstInputLabels)
    : iEditorInputLabeled(apWindow, avPos, asLabel, asName, apParent) {
    ...
    for (int i = 0; i < alAmount; ++i, ++itLabels) {
        ...
        cWidgetTextBox *pTB =
            mpSet->CreateWidgetTextBox(0, cVector2f(afTextBoxWidth, 25), _W(""), mpHandle,
                                       abNumeric ? eWidgetTextBoxInputType_Numeric : eWidgetTextBoxInputType_Normal,
                                       afNumericAdd, afNumericAdd != 0);
        AddWidget(pTB);
        mvTB.push_back(pTB);
...
cEditorInputFile::cEditorInputFile(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                                   const tString &asName, float afTextBoxWidth, iWidget *apParent)
    : cEditorInputText(apWindow, avPos, asLabel, asName, afTextBoxWidth, 1, false, 0, apParent) {
...
void cEditorInputFile::SetValue(const tWString &asX, bool abGenerateCallback, bool abCopyToInput) {
    msFilename = asX;
    msFullPath =
        mpWindow->GetEditor()->GetEngine()->GetResources()->GetFileSearcher()->GetFilePath(cString::To8Char(asX));
...
void cEditorInputFile::AddCustomFileType(const tWString &asType) { mlstCustomFileTypes.push_back(_W("*.") + asType); }
...
bool cEditorInputFile::BrowseButton_OnPressed(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mlstCustomFileTypes.empty() == false) {
        pEditor->ShowLoadFilePicker(mvTempLoadedFiles, cString::GetFilePathW(msTempLoadedFile), this,
                                    kGuiCallback(Browser_OnOkay), _W("Custom"), mlstCustomFileTypes);
    } else {
        //////////////////////////////////////////
        ...
        case eEditorResourceType_Material:
            ...
            pEditor->ShowTextureBrowser((eEditorTextureResourceType)mlBrowserSubtype, _W(""), msTempLoadedFile, this,
                                        kGuiCallback(Browser_OnOkay), lstCatString);
            break;
        ...
        case eEditorResourceType_ParticleSystem:
            ...
            pEditor->ShowLoadFilePicker(mvTempLoadedFiles, cString::GetFilePathW(msTempLoadedFile), this,
                                        kGuiCallback(Browser_OnOkay), sCatName, lstCatString);
            break;
        ...
        case eEditorResourceType_Sound:
            pEditor->ShowSoundBrowser(cString::GetFilePathW(msTempLoadedFile), msTempLoadedFile, this,
                                      kGuiCallback(Browser_OnOkay));
            break;
...
float cEditorInputNumber::GetValue() { return mvTB[0]->GetNumericValue(); }
...
cEditorInputEnum::cEditorInputEnum(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                                   const tString &asName, float afComboBoxWidth, iWidget *apParent)
    : iEditorInputLabeled(apWindow, avPos, asLabel, asName, apParent) {
...
void cEditorInputEnum::AddValue(const tWString &asX) { mpCB->AddItem(asX); }

//--------------------------------------------------------

void cEditorInputEnum::RemoveValue(int alIndex) { mpCB->RemoveItem(alIndex); }
...
void cEditorInputEnum::SetMaxShownItems(int alNum) { mpCB->SetMaxShownItems(alNum); }
...
void cEditorInputEnum::UpdateValue() { SetValue(mpCB->GetSelectedItem(), false, false); }
//--------------------------------------------------------
...
int cEditorInputEnum::GetNumValues() { return mpCB->GetItemNum(); }
...
int cEditorInputEnum::GetValue() { return mpCB->GetSelectedItem(); }

//--------------------------------------------------------

const tWString &cEditorInputEnum::GetEnumValue(int alIndex) { return mpCB->GetItemText(alIndex); }
...
cEditorInputColorFrame::cEditorInputColorFrame(iEditorWindow *apWindow, const cVector3f &avPos, const tWString &asLabel,
                                               const tString &asName, iWidget *apParent)
    : iEditorInputLabeled(apWindow, avPos, asLabel, asName, apParent, eEditorInputLayoutStyle_RowLabelOnLeft) {
    mCol = cColor(1, 1);
...
bool cEditorInputColorFrame::Frame_OnClick(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpSet->CreatePopUpColorPicker(&mCol, apWidget->GetGlobalPosition() + cVector3f(-50, 0, 20), this,
                                  kGuiCallback(ColorPicker_OnOK));
    return true;
}
...
void cEditorInputColorFrame::UpdateValue() { SetValue(mCol); }
...
const cColor &cEditorInputColorFrame::GetValue() { return mCol; }
