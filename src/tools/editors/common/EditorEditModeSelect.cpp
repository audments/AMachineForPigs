...
bool cUIPickFilter::Passes(iEntityWrapper *apEntity) { return (FirstPass(apEntity) && SecondPass(apEntity)); }

//----------------------------------------------------------------------

void cUIPickFilter::SetTypeFilter(int aType, bool abX) { mmapTypeFilters[aType] = abX; }
...
int cUIPickFilter::GetNumTypeFilters() { return (int)mmapTypeFilters.size(); }
...
cEditorEditModeSelect::cEditorEditModeSelect(iEditorBase *apEditor, iEditorWorld *apEditorWorld)
    : iEditorEditMode(apEditor, "Select") {
...
void cEditorEditModeSelect::OnViewportMouseDown(int alButtons) {
    ...
    if (mbAlwaysDeselect == false && mbToggleSelect == false && mbAllowTransform && mpCurrentTool &&
        mpCurrentTool->OnViewportMouseDown(alButtons)) {
        cEditorSelection *pSelection = mpEditor->GetSelection();
        ...
        if (mpCurrentSelector)
            mpCurrentSelector->OnViewportMouseDown();
    }
}
...
void cEditorEditModeSelect::DrawPostGrid(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
iEditorWindow *cEditorEditModeSelect::CreateSpecificWindow() { return hplNew(cEditorWindowSelect, (this)); }
...
iEditorAction *cEditorEditModeSelect::CreateDeleteEntitiesAction(const tIntList &alstEntityIDs) {
    ...
    iEditorAction *pAction =
        hplNew(cEditorActionEntityDelete, (mpEditorWorld, mpEditor->GetSelection(), alstEntityIDs));
    return pAction;
}
...
iEditorAction *cEditorEditModeSelect::CreateCompoundObjectAction() {
    ...
    if (bDetachObjectsFromCompound) {
        ...
        pCompoundAction->AddAction(
            hplNew(cEditorActionCompoundObjectAddEntities, (mpEditorWorld, lNewCompoundID, lstEntityIDs)));
...
cEditorActionCompoundAction *cEditorEditModeSelect::GetCurrentCompoundAction() { return mpCurrentCompoundAction; }
...
cEntitySelectorNormal::~cEntitySelectorNormal() { hplDelete(mpBox); }
...
void cEntitySelectorNormal::Draw(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions) {
    ...
    if (IsRayPickingActive() == false) {
        ...
        apFunctions->GetLowLevelGfx()->DrawLineQuad(
            cRect2f((float)mMouseRect.x, (float)mMouseRect.y, (float)mMouseRect.w, (float)mMouseRect.h), 0,
            cColor(1, 1));
...
void cEntitySelectorNormal::OnSetCurrent(bool abX) { mpEditMode->SetAllowTransform(abX); }
...
cEntitySelectorHighlighter::cEntitySelectorHighlighter(cEditorEditModeSelect *apEditMode)
    : iEntitySelector(apEditMode) {
...
cEntitySelectorHighlighter::~cEntitySelectorHighlighter() { hplDelete(mpFilter); }
...
bool cEntitySelectorHighlighter::IsAtLeastOneFilterActive() { return mpFilter->GetNumActiveTypeFilters() > 0; }

//----------------------------------------------------------------------------

void cEntitySelectorHighlighter::OnSetCurrent(bool abX) { mpEditMode->SetAllowTransform(false); }
...
void cEntitySelectorHighlighter::ClearHighlightedEntities() { mlstHighlightedEntities.clear(); }
