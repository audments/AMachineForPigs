...
cEditorActionEntitySetFilename::cEditorActionEntitySetFilename(iEditorWorld *apWorld, int alID, const tString &asX)
    : iEditorActionWorldModifier("Set entity File", apWorld) {
    mlID = alID;
    msNewFile = asX;
...
void cEditorActionEntitySetFilename::DoModify() { Apply(msNewFile); }

void cEditorActionEntitySetFilename::UndoModify() { Apply(msOldFile); }
