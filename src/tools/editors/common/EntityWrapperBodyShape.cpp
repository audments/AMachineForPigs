...
cEntityWrapperTypeBodyShape::cEntityWrapperTypeBodyShape()
    : iEntityWrapperType(eEditorEntityType_BodyShape, _W("Shape"), "BodyShape") {
...
cMaterial *cEntityWrapperTypeBodyShape::GetShapeMaterial(int alX, int alY) { return mvMaterials[alX][alY]; }

const tString &cEntityWrapperTypeBodyShape::GetMaterialFile(int alX, int alY) { return mvMaterialFiles[alX][alY]; }
...
iEntityWrapper *cEntityWrapperDataBodyShape::CreateSpecificEntity() { return hplNew(cEntityWrapperBodyShape, (this)); }
...
cEntityWrapperBodyShape::cEntityWrapperBodyShape(iEntityWrapperData *apData)
...
bool cEntityWrapperBodyShape::SetProperty(int alPropID, const tString &asX) {
    ...
    case eShapeStr_ShapeType:
        mShapeType = StringToEditorBodyShape(asX);
        break;
    default:
...
bool cEntityWrapperBodyShape::GetProperty(int alPropID, tString &asX) {
    switch (alPropID) {
    case eShapeStr_ShapeType:
        asX = EditorBodyShapeToString(GetShapeType());
        break;
    default:
...
cEditorWindowEntityEditBox *cEntityWrapperBodyShape::CreateEditBox(cEditorEditModeSelect *apEditMode) {
    cEditorWindowEntityEditBox *pEditBox =
        (cEditorWindowEntityEditBox *)hplNew(cEditorWindowEntityEditBoxBodyShape, (apEditMode, this));
...
cEditorWindowEntityEditBoxGroup *
cEntityWrapperBodyShape::CreateGroupAllSameTypeEditBox(cEditorEditModeSelect *apEditMode,
...
void cEntityWrapperBodyShape::SetAbsScale(const cVector3f &avScale, int alAxis) {
    ...
    // Log("%s %s input scale = %s\n", EditorBodyShapeToString(mShapeType).c_str(), msName.c_str(),
    // avScale.ToFileString().c_str());
...
cMesh *cEntityWrapperBodyShape::CreateShape() {
    ...
    switch (mShapeType) {
    case eEditorBodyShape_Box:
        pMesh = pCreator->CreateBox("", mvScale, sMat);
        break;
    case eEditorBodyShape_Sphere:
        pMesh = pCreator->CreateSphere("", mvScale.x, lMeshResolution, lMeshResolution,
                                       sMat); // mCustomBV.SetSize(cVector3f(2)); //diameter = radius*2 = 2
        break;
    case eEditorBodyShape_Cylinder:
        pMesh = pCreator->CreateCylinder("", cVector2f(mvScale.x, mvScale.y), lMeshResolution,
                                         sMat); // mCustomBV.SetSize(cVector3f(2,1,2));  //diameter = radius*2 = 2
        break;
    case eEditorBodyShape_Capsule:
        pMesh = pCreator->CreateCapsule("", cVector2f(mvScale.x, mvScale.y), lMeshResolution, 5,
                                        sMat); // mCustomBV.SetSize(cVector3f(2,1,2));  //diameter = radius*2 =
                                               // 2//mCustomBV.SetSize(cVector3f(2,1,2));  //diameter = radius*2 = 2
        break;
    }
...
eEditorBodyShape cEntityWrapperBodyShape::StringToEditorBodyShape(const tString &asName) {
    ...
    if (sLowName == "cylinder")
        return eEditorBodyShape_Cylinder;
    if (sLowName == "sphere")
        return eEditorBodyShape_Sphere;
    if (sLowName == "capsule")
        return eEditorBodyShape_Capsule;
...
iEditorAction *cEntityWrapperBodyShape::CreateActionCreateBodyFromShapes(iEditorWorld *apWorld,
                                                                         const tIntList &alstShapeIDs) {
...
void cEntityWrapperBodyShape::OnSetSelected(bool abX) {
    ...
    //((cEngineEntityGeneratedMesh*)mpEngineEntity)->SetCustomMaterial(pType->GetShapeMaterial(GetParentBody()!=NULL,
    //abX), false);
}
