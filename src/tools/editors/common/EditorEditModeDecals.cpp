...
cEditorEditModeDecals::cEditorEditModeDecals(iEditorBase *apEditor, iEditorWorld *apEditorWorld)
    : iEditorEditModeObjectCreator(apEditor, "Decals", apEditorWorld) {
...
void cEditorEditModeDecals::OnViewportMouseUp(int alButtons) {
    if (mpEditor->GetFlags(eEditorFlag_Placing) == false || (alButtons & eGuiMouseButton_Left) == 0)
        return;
...
void cEditorEditModeDecals::OnEditorUpdate(float afTimeStep) {
    ...
    cMatrixf mtxRotation = cMath::MatrixMul(cMath::MatrixInverse(mtxSurfaceRot),
                                            cMath::MatrixRotate(mvCreatorLocalRotation, eEulerRotationOrder_XYZ));
    cMatrixf mtxOrientation = mtxRotation.GetTranspose();

    cEntityWrapperDecal::BuildDecalVertexBuffer(
        mpEditorWorld->GetWorld(), pCreator, pPicker->GetPositionInSurface(), GetCreatorScale(), mfOffset,
        mtxOrientation.GetRight(), mtxOrientation.GetUp(), mtxOrientation.GetForward(), sMaterial, pWin->GetColor(),
        pWin->GetSubDivisions(), mlCurrentSubDiv, cEntityWrapperTypeDecal::GetGlobalMaxTriangles(),
        pPicker->IsAffected(eSurfaceType_StaticObject, false), pPicker->IsAffected(eSurfaceType_Primitive, false),
        pPicker->IsAffected(eSurfaceType_Entity, false));
}

//-----------------------------------------------------------------------------------

void cEditorEditModeDecals::DrawPostGrid(cEditorWindowViewport *apViewport, cRendererCallbackFunctions *apFunctions,
...
iEditorWindow *cEditorEditModeDecals::CreateSpecificWindow() { return hplNew(cEditorWindowDecals, (this)); }

//-----------------------------------------------------------------------------------

void cEditorEditModeDecals::CreateTypes() { mvTypes.push_back(hplNew(cEntityWrapperTypeDecal, ())); }
