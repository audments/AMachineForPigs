...
cEditorWindowEntityEditBoxLight::cEditorWindowEntityEditBoxLight(cEditorEditModeSelect *apEditMode,
                                                                 iEntityWrapperLight *apLight)
    : cEditorWindowEntityEditBox(apEditMode, apLight) {
    mpLight = apLight;
...
void cEditorWindowEntityEditBoxLight::AddPropertySetSpot(cWidgetTab *apParentTab) {
    ...
    mpInpSpotNearClipPlane = CreateInputNumber(vPos + cVector3f(mpGroupRadius->GetSize().x + 20, 0, 0),
                                               _W("Near Clip Plane"), "", apParentTab, 50, 0.1f);
    ...
    mpInpSpotAspect = CreateInputNumber(vPos + cVector3f(mpInpSpotFOV->GetSize().x + 20, 0, 0), _W("Aspect"), "",
                                        apParentTab, 50, 0.1f);
...
void cEditorWindowEntityEditBoxLight::OnUpdate(float afTimeStep) {
    ...
    if (mpInpRadius)
        mpInpRadius->SetValue(mpLight->GetRadius(), false);

    if (mpComboBoxBlendFunc)
        mpComboBoxBlendFunc->SetSelectedItem(((cEntityWrapperLightBox *)mpLight)->GetBlendFunc(), false, false);

    if (mpInpPriority)
        mpInpPriority->SetValue(((cEntityWrapperLightBox *)mpLight)->GetPriority(), false);
...
bool cEditorWindowEntityEditBoxLight::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    // Gobo Anim Mode
        ...
        pAction = mpEntity->CreateSetPropertyActionString(
            eLightStr_GoboAnimMode, cString::To8Char(mpInpGoboAnimMode->GetEnumValue(mpInpGoboAnimMode->GetValue())));
    }
    // Gobo Anim Frame Time
    else if (apInput == mpInpGoboAnimFrameTime) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eLightFloat_GoboAnimFrameTime, mpInpGoboAnimFrameTime->GetValue());
    }
    ///////////////////////////////////
    // Shadow related
    ...
    // Shadow Resolution
        ...
        pAction = mpEntity->CreateSetPropertyActionString(
            eLightStr_ShadowResolution, cString::To8Char(mpInpShadowRes->GetEnumValue(mpInpShadowRes->GetValue())));
    }
    // Shadow affect static
    else if (apInput == mpInpShadowAffectStatic) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(eLightBool_ShadowsAffectStatic, mpInpShadowAffectStatic->GetValue());
    }
    // Shadow affect dynamic
    else if (apInput == mpInpShadowAffectDynamic) {
        pAction = mpEntity->CreateSetPropertyActionBool(eLightBool_ShadowsAffectDynamic,
                                                        mpInpShadowAffectDynamic->GetValue());
    }
    ///////////////////////////////////
    // Diffuse
    ...
    // On Min Length
        ...
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOnMinLength, mpInpFlickerOnMinLength->GetValue());
    }

    // On Max Length
    else if (apInput == mpInpFlickerOnMaxLength) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOnMaxLength, mpInpFlickerOnMaxLength->GetValue());
    }

    // Off Min Length
    else if (apInput == mpInpFlickerOffMinLength) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOffMinLength,
                                                         mpInpFlickerOffMinLength->GetValue());
    }

    // Off Max Length
    else if (apInput == mpInpFlickerOffMaxLength) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOffMaxLength,
                                                         mpInpFlickerOffMaxLength->GetValue());
    }

    // On Sound
    else if (apInput == mpInpFlickerOnSound) {
        pAction = mpEntity->CreateSetPropertyActionString(eLightStr_FlickerOnSound,
                                                          cString::To8Char(mpInpFlickerOnSound->GetValue()));
    }

    // On PS
    else if (apInput == mpInpFlickerOnPS) {
        pAction = mpEntity->CreateSetPropertyActionString(eLightStr_FlickerOnPS,
                                                          cString::To8Char(mpInpFlickerOnPS->GetValue()));
    }

    // Off Sound
    else if (apInput == mpInpFlickerOffSound) {
        pAction = mpEntity->CreateSetPropertyActionString(eLightStr_FlickerOffSound,
                                                          cString::To8Char(mpInpFlickerOffSound->GetValue()));
    }

    // Off PS
    else if (apInput == mpInpFlickerOffPS) {
        pAction = mpEntity->CreateSetPropertyActionString(eLightStr_FlickerOffPS,
                                                          cString::To8Char(mpInpFlickerOffPS->GetValue()));
    }

    // Off Radius
    else if (apInput == mpInpFlickerOffRadius) {
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOffRadius, mpInpFlickerOffRadius->GetValue());
    }
    ...
    // Fade On Min Length
        ...
        pAction = mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOnFadeMinLength,
                                                         mpInpFlickerFadeOnMinLength->GetValue());
    }

    // Fade On Max Length
    else if (apInput == mpInpFlickerFadeOnMaxLength) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOnFadeMaxLength,
                                                         mpInpFlickerFadeOnMaxLength->GetValue());
    }

    // Fade Off Min Length
    else if (apInput == mpInpFlickerFadeOffMinLength) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOffFadeMinLength,
                                                         mpInpFlickerFadeOffMinLength->GetValue());
    }

    // Fade Off Max Length
    else if (apInput == mpInpFlickerFadeOffMaxLength) {
        pAction = mpEntity->CreateSetPropertyActionFloat(eLightFloat_FlickerOffFadeMaxLength,
                                                         mpInpFlickerFadeOffMaxLength->GetValue());
    }
    ...
    // Near Clip Plane
        ...
        pAction =
            mpEntity->CreateSetPropertyActionFloat(eLightSpotFloat_NearClipPlane, mpInpSpotNearClipPlane->GetValue());
    }
