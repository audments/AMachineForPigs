...
cEditorEditModeSelectToolScale::cEditorEditModeSelectToolScale(cEditorEditModeSelect *apParent,
                                                               cEditorSelection *apSelection)
    : cEditorEditModeSelectTool(eSelectToolMode_Scale, apParent, apSelection) {
    for (int i = 0; i < 6; ++i)
...
bool cEditorEditModeSelectToolScale::IsActive() { return mpSelection->CanScale(); }

//----------------------------------------------------------------

void cEditorEditModeSelectToolScale::DrawAxes(cEditorWindowViewport *apViewport,
                                              cRendererCallbackFunctions *apFunctions, float afAxisLength) {
    ...
    cMatrixf mtxTransform =
        cMath::MatrixMul(cMath::MatrixTranslate(mpSelection->GetCenterTranslation()),
    ...
    mtxTranslate[0] =
        cMath::MatrixMul(mtxTransform, cMath::MatrixTranslate(vAxes[0] - cVector3f(fCubeCenterOffset, 0, 0)));
    mtxTranslate[1] =
        cMath::MatrixMul(mtxTransform, cMath::MatrixTranslate(vAxes[1] - cVector3f(0, fCubeCenterOffset, 0)));
    mtxTranslate[2] =
        cMath::MatrixMul(mtxTransform, cMath::MatrixTranslate(vAxes[2] - cVector3f(0, 0, fCubeCenterOffset)));
...
cMatrixf &cEditorEditModeSelectToolScale::GetTransformMatrix() {
    ...
    //{
    mmtxTransformMatrix =
        cMath::MatrixMul(cMath::MatrixRotate(mpSelection->GetCenterRotation() * -1, eEulerRotationOrder_ZYX),
...
void cEditorEditModeSelectToolScale::CheckMouseOverAxis() {
    ...
    bool bAllAxesSelected = cMath::CheckAABBLineIntersection(mvAllAxesHandleMin, mvAllAxesHandleMax,
                                                             mvTransformedRayStart, mvTransformedRayEnd, NULL, NULL);

    for (int i = eSelectToolAxis_X; i < eSelectToolAxis_LastEnum; ++i) {
        mvAxisMouseOver[i] = bAllAxesSelected
                                 ? true
                                 : (cMath::CheckAABBLineIntersection(mvAxisMin[i], mvAxisMax[i], mvTransformedRayStart,
                                                                     mvTransformedRayEnd, NULL, NULL) ||
                                    cMath::CheckAABBLineIntersection(mvHeadMin[i], mvHeadMax[i], mvTransformedRayStart,
                                                                     mvTransformedRayEnd, NULL, NULL));
    ...
    mvScreenSpaceAxis = cMath::MatrixMul(cMath::MatrixRotate(mpSelection->GetCenterRotation(), eEulerRotationOrder_XYZ),
                                         mvScreenSpaceAxis);
    ...
    /*
        mvAxisMouseOver[eSelectToolAxis_X] = mvAxisMouseOver[eSelectToolAxis_X] || bRayIntersectedPlane[0] ||
       bRayIntersectedPlane[2]; mvAxisMouseOver[eSelectToolAxis_Y] = mvAxisMouseOver[eSelectToolAxis_Y] ||
       bRayIntersectedPlane[0] || bRayIntersectedPlane[1]; mvAxisMouseOver[eSelectToolAxis_Z] =
       mvAxisMouseOver[eSelectToolAxis_Z] || bRayIntersectedPlane[1] || bRayIntersectedPlane[2];
    */
}

//----------------------------------------------------------------------

bool cEditorEditModeSelectToolScale::CheckRayIntersectsAxis(eSelectToolAxis aeAxis, const cVector3f &avRayStart,
