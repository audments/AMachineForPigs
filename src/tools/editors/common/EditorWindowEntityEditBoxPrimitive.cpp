...
cEditorWindowEntityEditBoxPrimitive::cEditorWindowEntityEditBoxPrimitive(cEditorEditModeSelect *apEditMode,
                                                                         iEntityWrapperPrimitive *apObject)
...
void cEditorWindowEntityEditBoxPrimitive::AddPrimitivePropertySetPlane(cWidgetTab *apParentTab) {
    ...
    mpInpTileAmount = CreateInputVec3(vPos, _W("Tile amount"), "", apParentTab, 50, tWStringList(),
                                      eEditorInputLayoutStyle_RowLabelOnTop, 0.1f);
    vPos.y += mpInpTileAmount->GetSize().y + 10;
    mpInpTileOffset = CreateInputVec3(vPos, _W("Tile offset"), "", apParentTab, 50, tWStringList(),
                                      eEditorInputLayoutStyle_RowLabelOnTop, 0.1f);
    ...
    for(int i=0;i<3;++i)
    {
        mpTBTileAmount[i] = mpSet->CreateWidgetTextBox(vPos+cVector3f(i*60.0f,20,0), cVector2f(55,25), _W(""),
    apParentTab, eWidgetTextBoxInputType_Numeric, 0.1f); mpTBTileAmount[i]->AddCallback(eGuiMessage_TextBoxEnter, this,
    kGuiCallback(InputCallback));

        mpTBOffset[i] = mpSet->CreateWidgetTextBox(vPos+cVector3f(i*60.0f,70,0), cVector2f(55,25), _W(""), apParentTab,
    eWidgetTextBoxInputType_Numeric, 0.1f); mpTBOffset[i]->AddCallback(eGuiMessage_TextBoxEnter, this,
    kGuiCallback(InputCallback));
...
bool cEditorWindowEntityEditBoxPrimitive::InputCallback(iWidget* apWidget, const cGuiMessageData& aData)
{
    ...
    if(apWidget==mpTBMaterial)
    {
        ...
        if(bEmptyString ||
            ...
            //pAction = hplNew(cEditorActionPrimitiveSetStringProperty,(pWorld, lID,
ePrimitiveStringProperty_Material,sFilename)); else apWidget->SetText(_W(""));
    }
    else if(apWidget==mpBMaterialBrowse)
    {
        mpEditor->ShowLoadFilePicker(mvLoadedFiles, msLastPath,this,kGuiCallback(FileMaterialCallback), _W("Materials"),
tWStringList(1,_W("*.mat")));
    }
    else if(apWidget==mpTBTileAmount[0] || apWidget==mpTBTileAmount[1] || apWidget==mpTBTileAmount[2])
    {
        cVector3f vTemp = cVector3f(mpTBTileAmount[0]->GetNumericValue(), mpTBTileAmount[1]->GetNumericValue(),
mpTBTileAmount[2]->GetNumericValue()); pAction = hplNew(cEditorActionPrimitivePlaneSetVec3Property,(pWorld,lID,
ePrimitivePlaneVec3Property_TileAmount,vTemp));
    }
    else if(apWidget==mpTBOffset[0] || apWidget==mpTBOffset[1] || apWidget==mpTBOffset[2])
    {
        cVector3f vTemp = cVector3f(mpTBOffset[0]->GetNumericValue(), mpTBOffset[1]->GetNumericValue(),
mpTBOffset[2]->GetNumericValue()); pAction = hplNew(cEditorActionPrimitivePlaneSetVec3Property,(pWorld,lID,
ePrimitivePlaneVec3Property_TileOffset,vTemp));
    }
    else if(apWidget==mpCBAlignToWorld)
    {
        pAction = hplNew(cEditorActionPrimitivePlaneSetBoolProperty,(pWorld,lID,
ePrimitivePlaneBoolProperty_AlignToWorld, mpCBAlignToWorld->IsChecked()));
    }
...
bool cEditorWindowEntityEditBoxPrimitive::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpAlignToWorld) {
        pAction =
            mpEntity->CreateSetPropertyActionBool(ePrimitivePlaneBool_AlignToWorld, mpInpAlignToWorld->GetValue());
        // pAction = hplNew(cEditorActionPrimitivePlaneSetBoolProperty,(pWorld,lID,
        // ePrimitivePlaneBoolProperty_AlignToWorld, mpInpAlignToWorld->GetValue()));
        ...
        // pAction = hplNew(cEditorActionPrimitivePlaneSetBoolProperty,(pWorld, lID,
        // ePrimitivePlaneBoolProperty_CastShadows, mpInpCastShadows->GetValue()));
    } else if (apInput == mpInpCollides) {
        pAction = mpEntity->CreateSetPropertyActionBool(ePrimitiveBool_Collides, mpInpCollides->GetValue());
        // pAction = hplNew(cEditorActionPrimitivePlaneSetBoolProperty,(pWorld, lID,
        // ePrimitivePlaneBoolProperty_Collides, mpInpCollides->GetValue()));
        ...
        // pAction = hplNew(cEditorActionPrimitiveSetStringProperty,(pWorld, lID, ePrimitiveStringProperty_Material,
        // cString::To8Char(mpInpMaterial->GetValue())));
    } else if (apInput == mpInpTileAmount) {
        pAction =
            mpEntity->CreateSetPropertyActionVector3f(ePrimitivePlaneVec3f_TileAmount, mpInpTileAmount->GetValue());
    } else if (apInput == mpInpTileOffset) {
        pAction =
            mpEntity->CreateSetPropertyActionVector3f(ePrimitivePlaneVec3f_TileOffset, mpInpTileOffset->GetValue());
    } else if (apInput == mpInpTextureAngle) {
        pAction = mpEntity->CreateSetPropertyActionFloat(ePrimitivePlaneFloat_TextureAngle,
                                                         cMath::ToRad(mpInpTextureAngle->GetValue()));
    }
