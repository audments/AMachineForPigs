...
cEditorEditModeSelectToolRotate::cEditorEditModeSelectToolRotate(cEditorEditModeSelect *apParent,
                                                                 cEditorSelection *apSelection)
    : cEditorEditModeSelectTool(eSelectToolMode_Rotate, apParent, apSelection) {
...
bool cEditorEditModeSelectToolRotate::IsActive() { return mpSelection->CanRotate(); }
...
void cEditorEditModeSelectToolRotate::CheckMouseOverAxis() {
    ...
    if (cMath::CheckSphereLineIntersection(0, mfUsedAxisLength, mvTransformedRayStart, mvTransformedRayEnd, &fT[0],
                                           &vIntersection[0], &fT[1], &vIntersection[1])) {
        if (fT[0] != -1 && fT[1] != -1) {
            if (fT[0] < fT[1])
                lClosestIntIndex = 0;
            else
    ...
    vRotationAxis =
        cMath::MatrixMul(cMath::MatrixRotate(mpSelection->GetCenterRotation(), eEulerRotationOrder_XYZ), vRotationAxis);
...
void cEditorEditModeSelectToolRotate::DrawAxes(cEditorWindowViewport *apViewport,
    ...
    apFunctions->GetLowLevelGfx()->DrawSphere(0, afAxisLength, col[0], col[1], col[2]);
...
void cEditorEditModeSelectToolRotate::UpdateTransformation() {
    ...
    if (mvAxisSelected[0] == false && mvAxisSelected[1] == false && mvAxisSelected[2] == false)
        return;
...
void cEditorEditModeSelectToolRotate::Reset() { cEditorEditModeSelectTool::Reset(); }
