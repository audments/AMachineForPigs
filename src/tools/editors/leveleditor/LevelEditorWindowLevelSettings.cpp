...
cLevelEditorWindowLevelSettings::cLevelEditorWindowLevelSettings(iEditorBase *apEditor)
    : iEditorWindowPopUp(apEditor, "LevelSettings", true, false, true, cVector2f(300, 150)) {}
...
void cLevelEditorWindowLevelSettings::OnInitLayout() {
    ...
    cWidgetTabFrame *pTabFrame =
        mpSet->CreateWidgetTabFrame(cVector3f(10, 35, 0.1f), mvSize - cVector2f(20, 40), _W(""), mpWindow);
    ...
    mpInpSkyboxTexture =
        CreateInputFile(vPos + cVector3f(mpInpSkyboxColor->GetSize().x + 20, -4, 0), _W("Texture"), "", pTab);
    ...
    mpInpFogEnd =
        CreateInputNumber(vPos + cVector3f(mpInpFogStart->GetSize().x + 10, 0, 0), _W("End"), "", pTab, 50, 1);
...
bool cLevelEditorWindowLevelSettings::WindowSpecificInputCallback(iEditorInput *apInput) {
    ...
    if (apInput == mpInpSkyboxActive)
        ...
        pAction =
            hplNew(cLevelEditorActionSetSkyboxTexture, (pWorld, cString::To8Char(mpInpSkyboxTexture->GetValue())));
        ...
        pAction =
            hplNew(cLevelEditorActionSetFogBoolProperty, (pWorld, eFogBoolProp_Active, mpInpFogActive->GetValue()));
    else if (apInput == mpInpFogCulling)
        pAction =
            hplNew(cLevelEditorActionSetFogBoolProperty, (pWorld, eFogBoolProp_Culling, mpInpFogCulling->GetValue()));
    else if (apInput == mpInpFogStart)
        pAction =
            hplNew(cLevelEditorActionSetFogFloatProperty, (pWorld, eFogFloatProp_Start, mpInpFogStart->GetValue()));
        ...
        pAction = hplNew(cLevelEditorActionSetFogFloatProperty,
                         (pWorld, eFogFloatProp_FalloffExp, mpInpFogFallOffExp->GetValue()));
