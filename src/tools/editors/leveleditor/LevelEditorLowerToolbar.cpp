...
void cLevelEditorLowerToolbar::OnInitLayout() {
    ...
    // Grid Controls group
    mpGridControlsGroup =
        mpSet->CreateWidgetGroup(cVector3f(5, 5, 0.01f), cVector2f(440, 43), _W("Grid Controls"), mpBGFrame);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        mvTextBoxes[i] =
            mpSet->CreateWidgetTextBox(0, 75, _W(""), mpGridControlsGroup, eWidgetTextBoxInputType_Numeric, 0.5f);
    ...
    mpSnapToGridCheckBox =
        mpSet->CreateWidgetCheckBox(cVector3f(385, 10, 0.1f), cVector2f(40, 20), _W("Snap"), mpGridControlsGroup);
    ...
    mpCheckBoxGlobalAmbientLight =
        mpSet->CreateWidgetCheckBox(cVector3f(510, 10, 0.1f), 0, _W("Global Ambient Light"), mpBGFrame);
    ...
    mpCheckBoxGlobalPointLight =
        mpSet->CreateWidgetCheckBox(cVector3f(510, 30, 0.1f), 0, _W("Global Point Light"), mpBGFrame);
...
bool cLevelEditorLowerToolbar::TextBoxChange(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (apWidget == mvTextBoxes[0]) {
        if (pGrid) {
            ...
            cEditorActionGridChangeProperties *pAction =
                hplNew(cEditorActionGridChangeProperties, (pGrid, plane, fHeight, fSnapSeparation));
            mpEditor->AddAction(pAction);
        }
    }
