...
cLevelEditorWindowGroup::cLevelEditorWindowGroup(cLevelEditor *apEditor)
    : iEditorWindowPopUp(apEditor, "Group Window", false, false, true, cVector2f(640, 480)) {
...
bool cLevelEditorWindowGroup::MainMenu_ItemClick(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mpMainMenuNew == apWidget) {
        pAction =
            hplNew(cLevelEditorActionGroupAdd, (mpEditor, cLevelEditorGroup::GetGroupCount(),
                                                "Group_" + cString::ToString((int)cLevelEditorGroup::GetGroupCount())));
...
bool cLevelEditorWindowGroup::VisibilityCheckBox_OnChange(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (lSelectedItem != -1) {
        ...
        iEditorAction *pAction = hplNew(cLevelEditorActionGroupSetVisibility,
                                        (mpEditor, lSelectedGroupID, mpCheckBoxGroupVisibility->IsChecked()));
        mpEditor->AddAction(pAction);
    }
    return true;
}
...
bool cLevelEditorWindowGroup::Name_OnEnter(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (lSelectedItem != -1) {
        ...
        iEditorAction *pAction = hplNew(cLevelEditorActionGroupSetName,
                                        (mpEditor, lSelectedGroupID, cString::To8Char(mpInputName->GetText())));
        mpEditor->AddAction(pAction);
    }
    return true;
}
...
void cLevelEditorWindowGroup::OnInitLayout() {
    ...
    cWidgetLabel *pLabelParams =
        mpSet->CreateWidgetLabel(cVector3f(10, 400, 0.1f), 0, _W("Group parameters"), mpWindow);
