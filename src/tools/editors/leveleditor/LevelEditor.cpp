...
cLevelEditor::~cLevelEditor() { OnSaveConfig(); }
...
void cLevelEditor::Command_Import() {
    ShowLoadFilePicker(mvLoadFilenames, msLastImportPath, this, kGuiCallback(ImportFileCallback),
...
void cLevelEditor::Command_Export() {
    ...
    cGuiPopUpFilePicker *pPicker =
        mpSet->CreatePopUpSaveFilePicker(msLastImportFile, _W("Exported objects"), _W("*.expobj"), msLastSavePath,
...
cLevelEditorGroup &cLevelEditor::GetGroup(unsigned int alID) { return mmapGroups[alID]; }
...
void cLevelEditor::SetUpClassDefinitions(cEditorUserClassDefinitionManager *apManager) {
    apManager->RegisterDefFilename(eUserClassDefinition_Entity, "EntityTypes.cfg",
                                   eEditorVarCategory_Instance | eEditorVarCategory_EditorSetup);
    ...
    if (mpCurrentEditMode == GetEditMode("Select")) {
        ...
        if (apWidget == mpMainMenuDelete)
            ...
            if (mpSelection->GetNumEntities() == 1 &&
                mpSelection->GetEntities().front()->GetTypeID() == eEditorEntityType_Compound) {
...
iEditorWorld *cLevelEditor::CreateSpecificWorld() { return hplNew(cLevelEditorWorld, (this)); }
...
void cLevelEditor::OnUpdate(float afTimeStep) {
    ...
    pWidget?pWidget->GetTypeString().c_str():"NULL", pWidgetParent,
    pWidgetParent?pWidgetParent->GetTypeString().c_str():"NULL");
                                                                    */
}
...
void cLevelEditor::OnLoadConfig() {
    ...
    for (int i = 0; i < 10; ++i) {
        tWString sRecent =
            mpLocalConfig->GetStringW("RecentUsedFiles", "RecentFile" + cString::ToString(i + 1), _W(""));
    ...
    while (true) {
        tWString sExtraDir =
            mpLocalConfig->GetStringW("Directories", "ExtraStaticObjectDir" + cString::ToString(j), _W(""));
        if (sExtraDir == _W("")) {
            break;
        }
...
void cLevelEditor::OnSaveConfig() {
    ...
    for (tWStringListIt it = mlstRecentFiles.begin(); it != mlstRecentFiles.end(); ++it) {
        ...
        if (cPlatform::FileExists(sRecent)) {
            mpLocalConfig->SetString("RecentUsedFiles", "RecentFile" + cString::ToString(++i),
