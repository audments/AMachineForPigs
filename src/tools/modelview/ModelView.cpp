...
class cSimpleRenderCallback : public iRendererCallback {
  public:
    ...
    void DrawSkeletonRec(cRendererCallbackFunctions *apFunctions, cNode3D *apBoneState) {
        cVector3f vCameraSpacePos =
            cMath::MatrixMul(apFunctions->GetFrustum()->GetViewMatrix(), apBoneState->GetWorldPosition());
        ...
        while (it.HasNext()) {
            ...
            apFunctions->GetLowLevelGfx()->DrawLine(apBoneState->GetWorldPosition(), pChild->GetWorldPosition(),
                                                    cColor(1, 1));
    ...
    void OnPostSolidDraw(cRendererCallbackFunctions *apFunctions) {
        ...
        if (gbDrawGrid) {
            ...
            for (int i = -lNum / 2; i < lNum / 2 + 1; ++i) {
                ...
                apFunctions->GetLowLevelGfx()->DrawLine(cVector3f(fPos, fY, fStart), cVector3f(fPos, fY, fEnd),
                                                        cColor(0.5f, 1));
                apFunctions->GetLowLevelGfx()->DrawLine(cVector3f(fStart, fY, fPos), cVector3f(fEnd, fY, fPos),
                                                        cColor(0.5f, 1));
            }
        }
        ...
        if (gbPhysicsActive && gbDrawPhysicsDebug) {
            ...
            for (size_t i = 0; i < gvJoints.size(); ++i) {
                ...
                if (mpPhysicsWorld->JointExists(pJoint) == false)
                    continue;

                cVector3f vPivot = pJoint->GetPivotPoint();
                apFunctions->GetLowLevelGfx()->DrawSphere(vPivot, 0.2f, cColor(1, 0, 0, 1));
                apFunctions->GetLowLevelGfx()->DrawLine(vPivot, vPivot + pJoint->GetPinDir() * 0.25,
                                                        cColor(0, 1, 0, 1));
            }
            apFunctions->SetDepthTest(true);
        }
...
class cSimpleUpdate : public iUpdateable {
  public:
    ...
    void OnExit() { ResetEntitySubMeshes(); }
    ...
    cMaterialData *GetMaterialData(const tString &asName, bool abShowErrorMess) {
        ...
        for (size_t i = 0; i < mvMaterialData.size(); ++i) {
            ...
            if (mvMaterialData[i].msName == sProperName)
                return &mvMaterialData[i];
        }
        ...
        for (int i = 0; i < pMesh->GetSubMeshNum(); ++i) {
            ...
            if (pMat) {
                ...
                if (sMathPath != sMeshPath && DirectoryHasImageFile(sMeshPath, sMatName)) {
                    Log(" Material path: '%s' and mesh path '%s' does not match! Valid textures also found in mesh "
                        "path!\n",
            ...
            if (pMat) {
                ...
                if (pMat == NULL)
                    FatalError("Created material '%s' could not be loaded!\n", sMatPath.c_str());
    ...
    void LoadModel(const tString &asFileName) {
        ...
        // Entity files
            ...
            gpEntity =
                (cMeshEntity *)mpWorld->CreateEntity("Model", mtxTransform, asFileName, -1, true, cVector3f(1.0f));
            if (gpEntity == NULL)
                FatalError("Could not load '%s'\n", asFileName.c_str());
        ...
        for (int i = 0; i < pMesh->GetNodeNum(); ++i) {
            ...
            Log("   Name: '%s' Parent: '%s'\n", pNode->GetName().c_str(),
                pParent ? pParent->GetName().c_str() : "None");
            ...
            for (int i = 0; i < pMesh->GetAnimationNum(); ++i) {
                    ...
                    if (false) {
                        ...
                        for (int frame = 0; frame < pTrack->GetKeyFrameNum(); ++frame) {
                            ...
                            Log("    %f (%s) | (%f %f %f %f)\n", pFrame->time, pFrame->trans.ToString().c_str(),
                                pFrame->rotation.v.x, pFrame->rotation.v.y, pFrame->rotation.v.z, pFrame->rotation.w);
                        }
                        Log("-------\n\n");
                ...
                for(int frame =0; frame <pKnee1->GetKeyFrameNum(); ++frame)
                {
                    ...
                    Log("    %f (%f %f) vs (%f %f) ratio: (%f %f)\n", pKnee1Frame->time,
                        ...
                        pKnee1Frame->rotation.v.z / pKnee2Frame->rotation.v.z, pKnee1Frame->rotation.w /
                pKnee2Frame->rotation.w
                        );
                    qAvgRatio.v.x = pKnee2Frame->rotation.v.x==0 ? 0.0f : (qAvgRatio.v.x + (pKnee1Frame->rotation.v.x /
                pKnee2Frame->rotation.v.x) ); qAvgRatio.v.y = pKnee2Frame->rotation.v.y==0 ? 0.0f : (qAvgRatio.v.y +
                (pKnee1Frame->rotation.v.y / pKnee2Frame->rotation.v.y) ); qAvgRatio.v.z = pKnee2Frame->rotation.v.z==0
                ? 0.0f : (qAvgRatio.v.z + (pKnee1Frame->rotation.v.z / pKnee2Frame->rotation.v.z) ); qAvgRatio.w =
                qAvgRatio.w + (pKnee1Frame->rotation.w / pKnee2Frame->rotation.w); fCount++;
                }
                qAvgRatio = qAvgRatio * (1.0f / fCount);
                Log("Count: %f AvgRatio: (%f %f %f %f)\n", fCount, qAvgRatio.v.x, qAvgRatio.v.y, qAvgRatio.v.z,
                qAvgRatio.w); for(int frame =0; frame <pKnee1->GetKeyFrameNum(); ++frame)
                {
                    cKeyFrame *pKnee1Frame = pKnee1->GetKeyFrame(frame);
                    cKeyFrame *pKnee2Frame = pKnee2->GetKeyFrame(frame);
        ...
        if (pMesh->GetSkeleton()) {
            ...
            for (int bone = 0; bone != pSkeleton->GetBoneNum(); ++bone) {
                ...
                Log("   Name: '%s' Pos: (%s)\n", pBone->GetName().c_str(),
                    pBone->GetLocalTransform().GetTranslation().ToString().c_str());
    ...
    void SetRagdollActive(bool abX) {
        if (gpEntity == NULL)
            return;
        if (gpEntity->GetMesh()->GetSkeleton() == NULL)
            return;
    ...
    void SetPhysicsActive(bool abX) {
        ...
        if (gpEntity)
            gpEntity->SetMatrix(cMatrixf::Identity);
        mfModelRotation = 0;
    }
    ...
    void CreateGuiWindow() {
        ...
        ///////////////////////////
        // Frame
        cWidgetFrame *pRootFrame = pSet->CreateWidgetFrame(
            cVector3f(5, 30, 0.1f), cVector2f(vSize.x - 10, vSize.y - 40), true, mpOptionWindow, false, true);
        ...
        // Model
        {
            ...
            pCheckBox = pSet->CreateWidgetCheckBox(vGroupPos + cVector3f(vCheckSize.x, 0, 0), vCheckSize,
                                                   _W("Show AABB"), pGroup);
            ...
            // Material Textures
            tWString sTextureName[] = {_W("Diffuse"),
                                       _W("NMap"),
                                       _W("Specular"),
                                       _W("Alpha"),
                                       _W("Height"),
            ...
            for (int i = 0; i < eMaterialTexture_LastEnum; ++i) {
                ...
                pCheckBox = pSet->CreateWidgetCheckBox(vGroupPos + cVector3f(fAdd + 5, 0, 0), vMatSize, sTextureName[i],
                                                       pGroup);
                ...
                if (lAddCount % 2 == 1 || i == eMaterialTexture_LastEnum - 1)
                    vGroupPos.y += 20;
                lAddCount++;
            }
            vGroupPos.y += 2;
        ...
        // Animation
        {
            ...
            cVector2f vCheckSize = vSize;
            vCheckSize.x /= 2;
            pCheckBox = pSet->CreateWidgetCheckBox(vGroupPos + cVector3f(vCheckSize.x * 0, 0, 0), vCheckSize,
                                                   _W("Show Skeleton"), pGroup);
            pCheckBox->SetChecked(gbDrawSkeleton);
            pCheckBox->AddCallback(eGuiMessage_CheckChange, this, kGuiCallback(ChangeDrawSkeleton));

            pButton =
                pSet->CreateWidgetButton(vGroupPos + cVector3f(vCheckSize.x, 0, 0), vCheckSize, _W("Add anim"), pGroup);
            ...
            // Stop, pause and play
            cVector2f vButtonSize = vSize;
            vButtonSize.x /= 3;
            pButton = pSet->CreateWidgetButton(vGroupPos + cVector3f(vButtonSize.x * 0, 0, 0), vButtonSize, _W("Stop"),
                                               pGroup);
            pButton->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(PressStopButton));

            pButton = pSet->CreateWidgetButton(vGroupPos + cVector3f(vButtonSize.x * 1, 0, 0), vButtonSize, _W("Pause"),
                                               pGroup);
            pButton->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(PressPauseButton));

            pButton = pSet->CreateWidgetButton(vGroupPos + cVector3f(vButtonSize.x * 2, 0, 0), vButtonSize, _W("Play"),
                                               pGroup);
        ...
        // Lights
        {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                vSlidePos.x += 40;
                pSlider =
                    pSet->CreateWidgetSlider(eWidgetSliderOrientation_Horizontal, vSlidePos, vSlideSize, 255, pGroup);
    ...
    bool PressLoadModel(iWidget *apWidget, const cGuiMessageData &aData) {
        ...
        cGuiPopUpFilePicker *pPicker = pSet->CreatePopUpLoadFilePicker(mvPickedFiles, false, msCurrentFilePath, false,
                                                                       this, kGuiCallback(LoadModelFromFilePicker));
    ...
    kGuiCallbackFuncEnd(cSimpleUpdate, ChangeBloomActive)
        ...
        if (gbRotateModel)
            mpCBPhysicsActive->SetChecked(false);
    ...
    kGuiCallbackFuncEnd(cSimpleUpdate, ChangeShowAABB)
        ...
        for (int i = 0; i < pMesh->GetSubMeshNum(); ++i) {
            ...
            if (aData.mlVal == 1) {
                ...
                if (texType == eMaterialTexture_Diffuse)
                    pMat->SetTexture(texType, mpTexDiffuseNull);
                else if (texType == eMaterialTexture_NMap)
                    pMat->SetTexture(texType, mpTexNMapNull);
                else
                    pMat->SetTexture(texType, NULL);
            }
    ...
    kGuiCallbackFuncEnd(cSimpleUpdate, ChangeDrawDebug)
        ...
        if (gbPhysicsActive)
            mpCBRotateModel->SetChecked(false);
    ...
    kGuiCallbackFuncEnd(cSimpleUpdate, ChangeDrawSkeleton)
        ...
        cGuiSet *pSet = gpSimpleCamera->GetSet();
        cGuiPopUpFilePicker *pPicker = pSet->CreatePopUpLoadFilePicker(mvPickedFiles, false, msCurrentFilePath, false,
                                                                       this, kGuiCallback(LoadAnimationFromFilePicker));
            ...
            if (gpEntity) {
                if (gpEntity->GetAnimationStateNum() > 0) {
                    ...
                    pSet->DrawFont(pFont, cVector3f(5, 56, 0), 14, cColor(1, 1),
                                   _W("Animationstate: %d, time: %.2f / %.2f"), glCurrentAnimaiton,
                                   pAnimState->GetTimePosition(), pAnimState->GetLength());
