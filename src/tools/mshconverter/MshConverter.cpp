...
int GetCacheFileWantedVersion(int alFileType) {
    if (alFileType == 0)
        return MSH_FORMAT_VERSION;
    if (alFileType == 1)
        return MSH_FORMAT_VERSION;
    if (alFileType == 2)
        return MAP_CACHE_FORMAT_VERSION;
...
void GenerateAIPaths(cWorld *apWorld) {
    for (size_t i = 0; i < gvPathNodeData.size(); ++i) {
        ...
        apWorld->CreateAINodeContainer(data.msName, data.msNodeName, data.mvBodySize, data.mbNodeAtCenter,
                                       data.mlMinEdges, data.mlMaxEdges, data.mfMaxEdgeDistance, data.mfMaxHeight);
    }
}
...
bool ConvertFile(const tWString &asFile) {
    ...
    if (gbForce == false && cPlatform::FileExists(sMSHPath) &&
...
void Init() {
    gpMeshLoaderMSH =
        static_cast<cMeshLoaderMSH *>(gpEngine->GetResources()->GetMeshLoaderHandler()->GetLoaderForFile(".msh"));
    gpMeshLoaderCollada =
        static_cast<cMeshLoaderCollada *>(gpEngine->GetResources()->GetMeshLoaderHandler()->GetLoaderForFile(".dae"));
