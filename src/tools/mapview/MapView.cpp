...
bool BoxInsideBox(const cVector3f &avMinA, const cVector3f &avMaxA, const cVector3f &avMinB, const cVector3f &avMaxB) {
    if (avMinA.x + 0.001f < avMinB.x)
        return false;
    if (avMinA.y + 0.001f < avMinB.y)
        return false;
    if (avMinA.z + 0.001f < avMinB.z)
        return false;

    if (avMaxA.x - 0.001f > avMaxB.x)
        return false;
    if (avMaxA.y - 0.001f > avMaxB.y)
        return false;
    if (avMaxA.z - 0.001f > avMaxB.z)
        return false;
...
cColor gObjectDebugColor[glObjectDebugColorNum] = {
    cColor(1, 1, 1),         cColor(1, 0, 1),         cColor(1, 1, 0),         cColor(0, 1, 1),
    cColor(0, 0, 1),         cColor(0, 1, 0),         cColor(1, 0, 0),         cColor(0.5f, 0.5f, 1),
    cColor(1, 1, 0.5f),      cColor(1, 0.5f, 0.5f),   cColor(1, 0.75f, 0.25f), cColor(0.75f, 1, 0.25f),
    cColor(1, 0.75f, 1),     cColor(1, 0.25f, 1),     cColor(1, 0.75f, 0),     cColor(0, 1, 0.75f),
    cColor(0.25f, 0.25f, 1), cColor(0.25f, 1, 0.25f), cColor(1, 0.25f, 0.25f), cColor(0.75f, 0.75f, 1),
    cColor(0.75f, 1, 0.25f)};
class cSimpleRenderCallback : public iRendererCallback {
  public:
    ...
    void RenderDynamicContainerDebug(cRendererCallbackFunctions *apFunctions, iRenderableContainerNode *apNode,
                                     int alLevel) {
        ///////////////////////////////////////
    ...
    void OnPostSolidDraw(cRendererCallbackFunctions *apFunctions) {
        ...
        if (gbRenderSoundsPlaying) {
            ...
            for (tSoundEntryListIt it = pEntryList->begin(); it != pEntryList->end(); ++it) {
                ...
                if (pSound->Get3D() && pSound->GetPositionIsRelative() == false) {
                    ...
                    apFunctions->GetLowLevelGfx()->DrawSphere(pSound->GetPosition(), pSound->GetMinDistance(),
                                                              cColor(0.75, 1));
                    apFunctions->GetLowLevelGfx()->DrawSphere(pSound->GetPosition(), pSound->GetMaxDistance(),
                                                              cColor(0.5, 1));
                }
            }
        ...
        if (gbDrawDynContainerDebug) {
            RenderDynamicContainerDebug(apFunctions,
                                        mpWorld->GetRenderableContainer(eWorldContainerType_Dynamic)->GetRoot(), 0);
        }
        ...
        if (gbDrawOcclusionGfxInfo) {
            ...
            for (int i = lNum - 1; i >= 0; --i) {
                ...
                if (gbDrawStaticOcclusionGfxInfo == false && pObject->IsStatic())
                    continue;

                cColor col =
                    CalcDistColor(pObject); // cColor(1,1);//gObjectDebugColor[(size_t)pObject % glObjectDebugColorNum];
        ...
        if (gbDrawPhysicsDebug && mpWorld) {
            ...
            while (bodyIt.HasNext()) {
                ...
                if (pCam->GetFrustum()->CollideBoundingVolume(pBody->GetBoundingVolume()) == eCollision_Outside)
                    continue; // Frustum test for some extra speed!
        ...
        if (gbRenderPathNodes && gpNodeContainer) {
            ...
            for (int i = 0; i < gpNodeContainer->GetNodeNum(); ++i) {
                ...
                for (int j = 0; j < pNode->GetEdgeNum(); ++j) {
                    ...
                    apFunctions->GetLowLevelGfx()->DrawLine(pNode->GetPosition(), pEdge->mpNode->GetPosition(),
                                                            cColor(0.4f, 0.4f, 0.4f, 1));
                }
            }
        }
  ...
  public:
    cSimpleUpdate() : iUpdateable("Simple3D") {
        ...
        if (gsMapFile != "") {
            ...
            iTexture *pSkyBox =
                gpEngine->GetResources()->GetTextureManager()->CreateCubeMap("cubemap_evening.dds", false);
            if (pSkyBox) {
    ...
    void SetupView() {
        ...
        mpTestRenderTexture =
            gpEngine->GetGraphics()->CreateTexture("TestTarget", eTextureType_Rect, eTextureUsage_RenderTarget);
        mpTestRenderTexture->CreateFromRawData(cVector3l(vTestWindowSize.x, vTestWindowSize.y, 1), ePixelFormat_RGBA,
                                               NULL);

        mpTestRenderGfx = gpEngine->GetGui()->CreateGfxTexture(mpTestRenderTexture, false, eGuiMaterial_Diffuse);

        iDepthStencilBuffer *pDepthBuffer =
            gpEngine->GetGraphics()->CreateDepthStencilBuffer(vTestWindowSize, 24, 8, false);
    ...
    void LoadWorld(const tString &asFileName, bool abSetToViewport, bool abKeepPosition) {
        ...
        if (sExt == "map") {
            ...
            if (mpWorld == NULL)
                FatalError("Could not load world '%s'\n", asFileName.c_str());
        }
        ...
        //////////////////////////////////
        // Node container
        gpNodeContainer = mpWorld->CreateAINodeContainer(
            gsNodeCont_Name, "Default", gvNodeCont_BodySize, gbNodeCont_NodeAtCenter, glNodeCont_MinEdges,
            glNodeCont_MaxEdges, gfNodeCont_MaxEdgeDistance, gfNodeCont_MaxHeight);
        ...
        // iTexture *pSkyBox =
        // gpEngine->GetResources()->GetTextureManager()->CreateCubeMap("dds_cubemap_test.dds",false);
    ...
    void CheckDynamicContainerBugsRec(iRenderableContainerNode *apNode, int alLevel) {
        ...
        /*if(apNode->HasChildNodes()==false && BoxInsideBox(vBoxMin,vBoxMax,apNode->GetMin(), apNode->GetMax())==false)
        //(vBoxMin != apNode->GetMin() || vBoxMax != apNode->GetMax()))
        {
            Log("Bounding volume is not correct!!\n (%s)-(%s) vs\n (%s)-(%s)\n",	vBoxMin.ToString().c_str(),
        vBoxMax.ToString().c_str(), apNode->GetMin().ToString().c_str(), apNode->GetMax().ToString().c_str());
        }*/
    ...
    void CheckStaticContainerBugsRec(iRenderableContainerNode *apNode, int alLevel) {
        ...
        if (apNode->HasObjects()) {
            ...
            for (; it != apNode->GetObjectList()->end(); ++it) {
                iRenderable *pObject = *it;
                if (pObject->GetName() == "CombinedObjects131_SubMesh")
                    bPrintNodeInfo = true;
            }
    ...
    void CreateGuiWindow() {
        ...
        // Graphics
        {
            ...
            if (mpAmbientBox)
                pCheckBox->SetChecked(mpAmbientBox->IsVisible());
            pCheckBox->AddCallback(eGuiMessage_CheckChange, this, kGuiCallback(ChangeAmbientActive));
            mpCheckAmbientActive = pCheckBox;
            vGroupPos.y += 22;

            // Ambient intensity
            cVector3f vAmbSlidePos = vGroupPos;
            vAmbSlidePos.x += 55;
            cVector2f vAmbSlideSize = vSize;
            vAmbSlideSize.x -= 55;

            pSlider =
                pSet->CreateWidgetSlider(eWidgetSliderOrientation_Horizontal, vAmbSlidePos, vAmbSlideSize, 255, pGroup);
            pSlider->AddCallback(eGuiMessage_SliderMove, this, kGuiCallback(ChangeAmbientColor));
            if (mpAmbientBox)
                pSlider->SetValue((int)(mpAmbientBox->GetDiffuseColor().r * 255.0 + 0.5f));
    ...
    bool PressLoadWorld(iWidget *apWidget, const cGuiMessageData &aData) {
        mvPickedFiles.clear();
        cGuiSet *pSet = gpSimpleCamera->GetSet();
        cGuiPopUpFilePicker *pPicker = pSet->CreatePopUpLoadFilePicker(mvPickedFiles, false, msCurrentFilePath, false,
    ...
    void OnDraw(float afFrameTime) {
        if (gpSimpleCamera->GetFont() == NULL)
            return;
        ...
        if (gbDrawOcclusionTextInfo) {
            ...
            CheckObjectsInFrustum(mpWorld->GetRenderableContainer(eWorldContainerType_Dynamic), &lLightsInFrustum,
                                  &lObjectsInFrustum);
            CheckObjectsInFrustum(mpWorld->GetRenderableContainer(eWorldContainerType_Static), &lLightsInFrustum,
                                  &lObjectsInFrustum);
            ...
            gpSimpleCamera->GetSet()->DrawFont(gpSimpleCamera->GetFont(), cVector3f(5, fY, 0), 14, cColor(1, 1),
                                               ...
                                               pSettings->mlNumberOfLightsRendered, pRenderList->GetLightNum(),
                                               lLightsInFrustum, lShadowCasters);
            fY += 16;
            gpSimpleCamera->GetSet()->DrawFont(
                gpSimpleCamera->GetFont(), cVector3f(5, fY, 0), 14, cColor(1, 1), _W("Num of objects: %d / %d"),
                pRenderList->GetSolidObjectNum() + pRenderList->GetTransObjectNum(), lObjectsInFrustum);
            fY += 16;
            gpSimpleCamera->GetSet()->DrawFont(gpSimpleCamera->GetFont(), cVector3f(5, fY, 0), 14, cColor(1, 1),
                                               _W("Queries: %d"), pSettings->mlNumberOfOcclusionQueries);
            fY += 16;
        }
        ...
        if (gbDrawSoundDebug) {
            ...
            for (int i = 0; i < (int)vSoundNames.size(); i++) {
                ...
                gpSimpleCamera->GetSet()->DrawFont(
                    gpSimpleCamera->GetFont(), cVector3f(10 + (float)lCol * 250, fY + (float)lRow * 13, 0), 12,
                    cColor(1, 1), _W("%ls(%.2f)(%d) (%.2f/%.2f)"), cString::To16Char(vSoundNames[i]).c_str(),
                    pEntry->GetChannel()->GetVolume(), pEntry->GetChannel()->GetPriority(),
                    pEntry->GetChannel()->GetElapsedTime(), pEntry->GetChannel()->GetTotalTime());
            ...
            if (pMusic) {
                ...
                gpSimpleCamera->GetSet()->DrawFont(
                    gpSimpleCamera->GetFont(), cVector3f(5, fY, 0), 10, cColor(1, 1),
                    _W("Music: '%ls' vol: %.2f playing: %d prio: %d elapsed: %.2f total time: %.2f"),
                    cString::To16Char(pChannel->GetData()->GetName()).c_str(), pChannel->GetVolume(),
                    pChannel->IsPlaying(), pChannel->GetPriority(), pChannel->GetElapsedTime(),
    ...
    void Update(float afTimeStep) {
        ...
        if (mpWorld && gpEngine->GetInput()->BecameTriggerd("RebuildDynamic")) {
            ...
            cRenderableContainer_DynBoxTree *pBoxTree = static_cast<cRenderableContainer_DynBoxTree *>(
                mpWorld->GetRenderableContainer(eWorldContainerType_Dynamic));
        ...
        if (mpPhysicsWorld && gpEngine->GetInput()->IsTriggerd("LeftMouse")) {
            ...
            if (mBodyPicker.mpPickedBody) {
                // Get pos of start.
                mBodyPicker.mvPos =
                    cMath::MatrixMul(mBodyPicker.mpPickedBody->GetLocalMatrix(), mBodyPicker.mvLocalPos);
...
int hplMain(const tString &asCommandline) {
    ...
    tWString sPersonalDir =
        cString::ReplaceCharToW(cPlatform::GetSystemSpecialPath(eSystemPath_Personal), _W("\\"), _W("/"));
