...
#define kBeginSerialize(aClass, aParent)                                                                               \
    ...
    static cSerializeClass gSerializedClass_##aClass(                                                                  \
        #aClass, #aParent, SerializeNamespace_##aClass::mvTempMemberFields, sizeof(aClass), _Create_##aClass);         \
...
#define kBeginSerializeBase(aClass)                                                                                    \
    ...
    static iSerializable *_Create_##aClass() { return hplNew(aClass, ()); }                                            \
    static cSerializeClass gSerializedClass_##aClass(#aClass, "", SerializeNamespace_##aClass::mvTempMemberFields,     \
                                                     sizeof(aClass), _Create_##aClass);                                \
    const tString aClass::msSerialize_Name = #aClass;                                                                  \
...
#define kBeginSerializeVirtual(aClass, aParent)                                                                        \
    ...
    static cSerializeClass gSerializedClass_##aClass(                                                                  \
        #aClass, #aParent, SerializeNamespace_##aClass::mvTempMemberFields, sizeof(aClass), NULL);                     \
    const tString aClass::msSerialize_Name = #aClass;                                                                  \
    const tString aClass::msSerialize_ParentName = #aParent;                                                           \
    const cSerializeMemberField *aClass::mpSerialize_MemberFields = SerializeNamespace_##aClass::mvTempMemberFields;   \
    namespace SerializeNamespace_##aClass {                                                                            \
...
#define kSerializeVar(aVar, aType)                                                                                     \
    cSerializeMemberField(#aVar, ClassMemberOffset(tVarClass, aVar), ClassMemberSize(tVarClass, aVar), aType,          \
                          eSerializeMainType_Variable),
...
#define kSerializeVarArray(aVar, aType, aArraySize)                                                                    \
    cSerializeMemberField(#aVar, ClassMemberOffset(tVarClass, aVar), ClassMemberSize(tVarClass, aVar), aType,          \
                          eSerializeMainType_Array, aArraySize),
...
typedef struct cSerializeSavedClass {
  public:
    ...
    cSerializeSavedClass(const char *asName, const char *asParent, cSerializeMemberField *apMemberFields, size_t alSize,
...
class cSerializeClass {
  public:
    cSerializeClass(const char *asName, const char *asParent, cSerializeMemberField *apMemberFields, size_t alSize,
                    iSerializable *(*apCreateFunc)());
    ...
    static bool SaveToFile(iSerializable *apData, const tWString &asFile, const tString &asRoot,
                           bool abCompressAndCRC = false);
    static void SaveToElement(iSerializable *apData, const tString &asName, TiXmlElement *apParent,
                              bool abIsPointer = false);
