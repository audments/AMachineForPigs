...
class cString {
  public:
    ...
    /**
     * Converts unicode to local encoding. (This can be ascii, utf-8 or strange things. This depends on platform and
     * system setup. Keep in mind that that output might be utf-8 encoded. In any case, it is a format that things like
     * printf, Log, etc likes, so okay to output.
     */
    ...
    /**
     * Encode arbitrary data to special readable text string format (that can be used as text in XML files, etc). Size
     * must be multiple of 4
     */
    static void EncodeDataToTextString(const void *apData, size_t alSize, tString &asOutput);
    /**
     * Decode arbitrary data from special readable text string format (that can be used as text in XML files, etc). Size
     * must be multiple of 4. String must be 150% of alSize
     */
