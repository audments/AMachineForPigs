...
enum eSystemPath { eSystemPath_Personal, eSystemPath_LastEnum };
...
typedef enum { eMsgBoxType_Info, eMsgBoxType_Error, eMsgBoxType_Warning, eMsgBoxType_Default } eMsgBoxType;
...
class cDate {
  public:
    ...
    bool operator==(const cDate &aDate) const {
        if (seconds == aDate.seconds && minutes == aDate.minutes && hours == aDate.hours &&
            month_day == aDate.month_day && month == aDate.month && year == aDate.year) {
            return true;
        }
...
template <class CONT, class T> void STLFindAndRemove(CONT &aCont, T *pObject) {
...
template <class CONT, class T> void STLMapFindAndRemove(CONT &aCont, T *pObject) {
    typename CONT::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
...
template <class CONT, class T> void STLFindAndDelete(CONT &aCont, T *pObject) {
    typename CONT::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
        if (*it == pObject) {
            aCont.erase(it);
...
template <class CONT, class T> void STLMapFindAndDelete(CONT &aCont, T *pObject) {
    typename CONT::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
        if (it->second == pObject) {
            aCont.erase(it);
...
template <class CONT> void *STLFindByName(CONT &aCont, const tString &asName) {
    typename CONT::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
...
template <class CONT> bool STLContainsPointer(CONT &aCont, const void *apPointer) {
    typename CONT::iterator it = aCont.begin();
    for (; it != aCont.end(); ++it) {
        if ((*it) == apPointer)
            return true;
...
template <class T> void STLDeleteAll(T &aCont) {
...
template <class T> void STLDeleteAll_NoClear(T &aCont) {
    typename T::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
        hplDelete(*it);
    }
...
template <class T> void STLMapDeleteAll(T &aCont) {
    typename T::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
        hplDelete(it->second);
    }
...
template <class T> void STLMapDeleteAll_NoClear(T &aCont) {
    typename T::iterator it = aCont.begin();
    for (; it != aCont.end(); it++) {
        hplDelete(it->second);
    }
...
template <class CONT, class T> bool STLObjectExists(CONT &aCont, T *apObject) {
    return aCont.find(apObject) != aCont.end();
}

//--------------------------------------------------------

template <class T, class CONT, class IT> class cSTLIterator : public iContainerIterator {
  public:
    ///////////////////////////
...
template <class T, class CONT, class IT> class cSTLMapIterator : public iContainerIterator {
  public:
    ///////////////////////////
