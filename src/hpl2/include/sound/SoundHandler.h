...
class cSoundEntry {
  public:
    cSoundEntry(const tString &asName, iSoundChannel *apSound, float afVolume, eSoundEntryType aType, bool ab3D,
                bool abStream, int alId, cSoundHandler *apSoundHandler);
    ~cSoundEntry();
...
class cSoundHandler {
  ...
  public:
     ...
     * \param alPriorityModifier The priority of the sound, if 3D then this number is added to a calculated priority
     * based on distance from listener. \return
     */
    cSoundEntry *Play(const tString &asName, bool abLoop, float afVolume, const cVector3f &avPos, float afMinDist,
                      float afMaxDist, eSoundEntryType aEntryType, bool abRelative, bool ab3D, int alPriorityModifier,
                      bool abStream, bool *apNotEnoughChannels = NULL);

    cSoundEntry *Play3D(const tString &asName, bool abLoop, float afVolume, const cVector3f &avPos, float afMinDist,
                        float afMaxDist, eSoundEntryType aEntryType = eSoundEntryType_World, bool abRelative = false,
    ...
    cSoundEntry *PlayGui(const tString &asName, bool abLoop, float afVolume,
                         const cVector3f &avPos = cVector3f(0, 0, 1), eSoundEntryType aEntryType = eSoundEntryType_Gui,
    ...
    /**
     * Set the global volume for a specific type(s). If alId >= 0 then the settings will be applied to that id, else a
     * free id will be found. \param afVolume Global Volume to be set. \param mAffectedTypes Type(s) affected by this
     * setting. \param alId The Id to the setting applies to, if < 0, the the a free one is found. \return if alId >=0,
     * alId is returned, else the lowest free id.
     */
