...
enum eCharDir { eCharDir_Forward = 0, eCharDir_Right = 1, eCharDir_LastEnum = 2 };
...
class iCharacterBody {
  ...
  public:
    ...
    bool CheckCharacterFits(const cVector3f &avPosition, bool abFeetPosition = false, int alSizeIdx = -1,
                            cVector3f *apPushBackVec = NULL, float afEpsilon = 0.0f);
    bool CheckRayIntersection(const cVector3f &avStart, const cVector3f &avEnd, float *apDistance,
    ...
    }; // quite hacky, but necessary. Last position is used to determine velocity. If we want to teleport a pig and
       // maintain its velocity, we need to fix up the last position too.
  ...
  protected:
    ...
    bool CheckCollision(cVector3f *apPushBackVector, const cVector3f &avPos, iPhysicsWorldCollisionCallback *apCallback,
                        int alShapeIdx = -1);
