...
class cMath {
  public:
    ...
    static void ExpandAABB(cVector3f &avBaseMin, cVector3f &avBaseMax, const cVector3f &avAddMin,
    ...
    static inline bool CheckPointInSphereIntersection(const cVector3f &avPoint, const cVector3f &avSpherePos,
                                                      float afSphereRadius) {
        cVector3f vSepAxis = avPoint - avSpherePos;
        return vSepAxis.SqrLength() <= afSphereRadius * afSphereRadius;
    }

    static inline bool CheckSphereIntersection(const cVector3f &avPosA, float afRadiusA, const cVector3f &avPosB,
                                               float afRadiusB) {
     ...
     */
    static bool CheckSphereLineIntersection(const cVector3f &avSpherePos, float afSphereRadius,
     ...
     * \return true if intersection, else false.
     */
    static bool CheckAABBIntersection(const cVector3f &avMin1, const cVector3f &avMax1, const cVector3f &avMin2,
    ...
    /**
     * Creates a clip rect for a bounding volume that comes in -1 - 1 for points in screen. It can return points above
     * and below 1/-1 though. Returns false if all points are behind frustum,
     */
    static bool GetNormalizedClipRectFromBV(cVector3f &avDestMin, cVector3f &avDestMax, cBoundingVolume &aBV,
                                            cFrustum *apFrustum, float afTanHalfFov);

    /**
     * Creates a clip rect in pixel coords from normalized (-1 - 1) coords.
     */
    static cRect2l GetClipRectFromNormalizedMinMax(const cVector3f &avMin, const cVector3f &avMax,
                                                   const cVector2l &avScreenSize);

    /**
     * Creates a clip rect for a bounding volume (that contains world coords) in screen space.
     */
    static void GetClipRectFromBV(cRect2l &aDestRect, cBoundingVolume &aBV, cFrustum *apFrustum,
     ...
     */
    static cRect2l GetClipRectFromSphere(const cVector3f &avPosition, float afRadius, cFrustum *apFrustum,
                                         const cVector2l &avScreenSize, bool abPositionIsScreenSpace,
                                         float afTanHalfFov);

    static bool CheckSphereInPlanes(const cVector3f &avCenter, float afRadius, const cPlanef *apPlanes,
                                    int alPlaneCount);

    /**
     * Checks if AABB intersects with line. Returns false if there was no intersection, else true.
     */
    static bool CheckAABBLineIntersection(const cVector3f &avMin, const cVector3f &avMax, const cVector3f &avLineStart,
                                          const cVector3f &avLineEnd, cVector3f *apIntersectionPos, float *apT);

    /**
     * Checks if a point intersects with a on-screen projected mesh. Returns false if there was no intersection, else
     * true \param avPoint : Screen coordinates of the point \param avViewportSize : Screen size \param alNumIndices :
     * Num of triangles \param avIndices : Pointer to index array \param avVertexPos : Pointer to vertex position array
     * \param alStride
    ...
    static bool CheckPointInFlatPolygon(const cVector3f &avRayStart, const cVector3f &avRayEnd,
                                        const cVector2f &avPoint, const cVector2f &avViewportSize, int alNumIndices,
                                        unsigned int *avIndices, float *avVertexPos, int alStride,
                                        const cMatrixf &amtxObjWorldMatrix, const cMatrixf &amtxCamViewMatrix,
                                        const cMatrixf &amtxCamProjMatrix, unsigned int *apIntersectedTriIndex,
                                        cVector3f *apIntersectionWorldSpace, tVector3fVec *apTriangleWorldSpace);
     ...
     * \return
     */
    static bool CheckPointSameSideOfEdge2D(const cVector3f &avPoint1, const cVector3f &avPoint2,
                                           const cVector3f &avPoint3, const cVector3f &avPoint4);
    ...
    static void Vector3OrthonormalizeBasis(const cVector3f &avSrcRight, const cVector3f &avSrcUp,
                                           const cVector3f &avSrcForward, cVector3f &avDstRight, cVector3f &avDstUp,
                                           cVector3f &avDstForward);

    /**
     * Calculates a pseudo-random point based on x (which becomes the x coord) and y which is a seed for y,z on the
     * sphere \param avSeed both elements must be between 0 and 1.
     */
     ...
     */
    static cVector3f GetPoint3DFromSphericalCoords(const cVector3f &avSphCenter, float afSphRadius,
     ...
     */
    static bool CheckPlaneLineIntersection(const cPlanef &aPlane, const cVector3f &avLineStart,
                                           const cVector3f &avLineEnd, cVector3f *apIntersectionPos, float *apT);

    /**
     *  Checks if the points are all inside (inside), some inside (intersect) or all outside (outside) of the planes.
     */
    static eCollision CheckPointsPlanesCollision(cVector3f *apVertices, int alNumOfVertices, cPlanef *apPlanes,
                                                 int alNumOfPlanes);

    /**
     *  Checks if the points are all inside (inside), some inside (intersect) or all outside (outside) of the planes of
     * an AABB.
     */
    static eCollision CheckPointsAABBPlanesCollision(cVector3f *apVertices, int alNumOfVertices, const cVector3f &avMin,
     ...
     */
    static inline float MatrixMinor(const cMatrixf &a_mtxA, const size_t r0, const size_t r1, const size_t r2,
    ...
    /**
     * Changes the near clip plane of the projection matrix (non-ortho) to a oblique one. Plane must be normalized and
     * in camera space \param &a_mtxProjMatrix \param aCameraSpaceClipPlane \return
     */
     ...
     */
    static cMatrixf MatrixPerspectiveProjection(float afNearClipPlane, float afFarClipPlane, float afFOV,
                                                float afAspect, bool abInfFarPlane);
    /**
     * Creates an orthographics projection matrix.
     */
    static cMatrixf MatrixOrthographicProjection(float afNearClipPlane, float afFarClipPlane,
     ...
     */
    static bool CreateTriTangentVectors(float *apDestArray, const unsigned int *apIndexArray, int alIndexNum,
     ...
     * \return true if success, else false
     */
    static bool CreateTriangleData(tTriangleDataVec &avTriangles, const unsigned int *apIndexArray, int alIndexNum,
     ...
     * \return true if success, else false
     */
    static bool CreateEdges(tTriEdgeVec &avEdges, const unsigned int *apIndexArray, int alIndexNum,
