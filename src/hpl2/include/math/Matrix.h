...
template <class T> class cMatrix {
  public:
    ...
    inline cMatrix(T a00, T a01, T a02, T a03, T a10, T a11, T a12, T a13, T a20, T a21, T a22, T a23, T a30, T a31,
                   T a32, T a33) {
        m[0][0] = a00;
        m[0][1] = a01;
        m[0][2] = a02;
    ...
    inline bool operator==(const cMatrix<T> &aMtx) const {
        if (m[0][0] == aMtx.m[0][0] && m[0][1] == aMtx.m[0][1] && m[0][2] == aMtx.m[0][2] && m[0][3] == aMtx.m[0][3] &&
            m[1][0] == aMtx.m[1][0] && m[1][1] == aMtx.m[1][1] && m[1][2] == aMtx.m[1][2] && m[1][3] == aMtx.m[1][3] &&
            m[2][0] == aMtx.m[2][0] && m[2][1] == aMtx.m[2][1] && m[2][2] == aMtx.m[2][2] && m[2][3] == aMtx.m[2][3] &&
    ...
    inline cMatrix<T> GetRotation() const {
        return cMatrix<T>(m[0][0], m[0][1], m[0][2], 0, m[1][0], m[1][1], m[1][2], 0, m[2][0], m[2][1], m[2][2], 0, 0,
                          0, 0, 1);
    }
    ...
    inline cMatrix<T> GetTranspose() const {
        return cMatrix<T>(m[0][0], m[1][0], m[2][0], m[3][0], m[0][1], m[1][1], m[2][1], m[3][1], m[0][2], m[1][2],
    ...
    tString ToString() const {
        char buf[512];
        sprintf(buf, "[%f : %f : %f : %f] [%f : %f : %f : %f] [%f : %f : %f : %f] [%f : %f : %f : %f]", m[0][0],
    ...
    tString ToFileString() const {
        char buf[512];
        sprintf(buf, "%g %g %g %g %g %g %g %g %g %g %g %g %g %g %g %g", m[0][0], m[0][1], m[0][2], m[0][3], m[1][0],
