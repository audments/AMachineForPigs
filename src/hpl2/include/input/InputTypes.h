...
struct cKeyPress {
    cKeyPress() {}
    cKeyPress(eKey aKey, int alUnicode, int alModifier) : mKey(aKey), mlUnicode(alUnicode), mlModifier(alModifier) {}
...
struct cGamepadInputData {
    ...
    bool operator==(const cGamepadInputData &aData) const {
        return mlPadIndex == aData.mlPadIndex && mInputType == aData.mInputType && mlInputId == aData.mlInputId &&
               mfInputValue == aData.mfInputValue;
    }
