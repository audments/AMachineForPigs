...
class cBinaryBuffer {
  public:
    ...
    /**
     * Compresses the data from apSrcData to current pos. Compression level is 0 - 9, where 0 is no compression. -1 =
     * default (recommended basically) compression. If abWriteDataSize is true, then the first 4 bytes, will be a 32bit
     * int with the size of the compressed data.
     */
    ...
    /**
     * Decompresses the data from a source buffer, beginning at the current position. If abSizeDataWritten is false, it
     * stops at EOF and assumes size has not been writen. If true, it will assume that the first 4 bytes is the size of
     * the data. It also updates the current postion in the data for SrcBuffer
     */
    ...
    /**
     * Do this when the data position is where the CRC should be stored and then any subsequent data (until AddCRC_End
     * is called) will be part of the CRC calculation. This will add a 32 bit int with the CRC that is then set when
     * AddCRC_End is called.
     */
    void AddCRC_Begin();
    ...
    /**
     * Calculates the CRC from current position (if alDataPos<0) or alDataPos, to end (if alCount < 0) or according to
     * alCount
     */
    unsigned int GetCRC(unsigned int alKey, int alDataPos = -1, int alCount = -1);

    /**
     * This checks CRC from current position to end (if alCount<0) else according to alCount (which includes the the
     * first 32bit CRC) It assumes that the first the 32bit int is the CRC and will increase the data position to be
     * past these.
     */
    ...
    //      Must rethink how to store, as Mac/Linux is UTF-32 and Windows is UTF-16.  Better to store as UTF-8 on all
    //      platforms. Also, must take care of Endianess for PPC
  ...
  private:
    /**
     * Adds data to the buffer and will increase the pos to point at byte after written data (usually EOF). Will
     * increase reserved size when it runs out of space. Note that this function does _not_ handle endian issues!
     */
    void AddData(const void *apData, size_t alSize);
    /**
     * Gets data from buffer and puts in pointer, that needs to have memory allocated. Also moves forward the position
     * in data to after the chunk of data. Note that this function does _not_ handle endian issues! \param *apData
     * Pointer to data destination. Needs to be allocated in memory. \param alSize Size of the data to be transfered to
     * destination. \return bool if transfer was successful, false if EOF was found before entire size of data could be
     * read! If size is too large the position in data will move to EOF.
     */
