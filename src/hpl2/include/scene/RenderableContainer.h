...
class iRenderableContainer {
  public:
    ...
    /**
     * This compiles the container. Even if the container is static, it should be possible to change orientation (scale,
     * pos, rotation,radius etc) of added objects before this method is called. After compile is called, objects
     * orientation can not be changed!
     */
