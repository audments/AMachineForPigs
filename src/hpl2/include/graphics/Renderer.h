...
class cRenderSettings {
  public:
    ...
    void AssignOcclusionObject(iRenderer *apRenderer, void *apSource, int alCustomIndex, iVertexBuffer *apVtxBuffer,
                               cMatrixf *apMatrix, bool abDepthTest);
...
class iRenderer : public iRenderFunctions {
  ...
  public:
    ...
    void Render(float afFrameTime, cFrustum *apFrustum, cWorld *apWorld, cRenderSettings *apSettings,
    ...
    void AssignOcclusionObject(void *apSource, int alCustomIndex, iVertexBuffer *apVtxBuffer, cMatrixf *apMatrix,
                               bool abDepthTest);
    int RetrieveOcclusionObjectSamples(void *apSource, int alCustomIndex);
    /**
     * Retrieves number of samples for all active occlusion queries. It does not release the queries, but the values are
     * gotten and stored in query.
     */
  ...
  protected:
     ...
     * When sending to a frame buffer at the end, then this method is never called and the intermediate can be returned
     * with GetPostEffectFrameBuffer
     */
    ...
    void BeginRendering(float afFrameTime, cFrustum *apFrustum, cWorld *apWorld, cRenderSettings *apSettings,
                        cRenderTarget *apRenderTarget, bool abSendFrameBufferToPostEffects,
                        tRendererCallbackList *apCallbackList, bool abAtStartOfRendering = true);
    ...
    int RenderAndAddNodeObjects(iRenderableContainerNode *apNode, tRenderCHCObjectCallbackFunc apRenderCallback,
                                tRenderableFlag alNeededFlags);

    void PushNodeChildrenToStack(tRendererSortedNodeSet &a_setNodeStack, iRenderableContainerNode *apNode,
                                 int alNeededFlags);
    void AddAndRenderNodeOcclusionQuery(tNodeOcclusionPairList *apList, iRenderableContainerNode *apNode,
    ...
    /**
     * This retrieves all occlusion information for light pair queries and release occlusion queries. If specified, this
     * is a waiting operation.
     */
    ...
    /**
     * Checks if the renderable object is 1) submeshentity 2) is onesided plane 3)is away from camera. If all are true,
     * FALSE is returned.
     */
    ...
    /**
     * Checks the IsVisible and also clip planes (in setttings), if visible in reflection and other stuff . No frustum
     * check!
     */
    ...
    bool CheckFogAreaRayIntersection(cMatrixf &a_mtxInvBoxModelMatrix, const cVector3f &avBoxSpaceRayStart,
    ...
    iVertexBuffer *CreateQuadVertexBuffer(eVertexBufferType aType, const cVector3f &avPos, const cVector2f &avSize,
    ...
    void UpdateqQuadVertexPostion(iVertexBuffer *apVtxBuffer, const cVector3f &avPos, const cVector2f &avSize,
                                  bool abCallUpdate);
...
class cRendererCallbackFunctions {
  public:
    ...
    inline void DrawQuad(const cVector3f &aPos, const cVector2f &avSize, const cVector2f &avMinUV = 0,
                         const cVector2f &avMaxUV = 1, bool abInvertY = false, const cColor &aColor = cColor(1, 1)) {
