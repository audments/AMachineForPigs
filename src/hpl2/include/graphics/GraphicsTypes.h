...
enum eMatrix { eMatrix_ModelView, eMatrix_Projection, eMatrix_Texture, eMatrix_LastEnum };
//-----------------------------------------

enum ePrimitiveType { ePrimitiveType_Tri, ePrimitiveType_Quad, ePrimitiveType_LastEnum };
...
enum eCullMode { eCullMode_Clockwise, eCullMode_CounterClockwise, eCullMode_LastEnum };
...
enum eTextureUsage { eTextureUsage_Normal, eTextureUsage_RenderTarget, eTextureUsage_LastEnum };
...
enum eTextureCompareMode { eTextureCompareMode_None, eTextureCompareMode_RToTexture, eTextureCompareMode_LastEnum };
...
enum eGpuProgramFormat { eGpuProgramFormat_CG, eGpuProgramFormat_GLSL, eGpuProgramFormat_LastEnum };

//-----------------------------------------

enum eGpuShaderType { eGpuShaderType_Vertex, eGpuShaderType_Fragment, eGpuShaderType_LastEnum };
...
enum eFontAlign { eFontAlign_Left, eFontAlign_Right, eFontAlign_Center, eFontAlign_LastEnum };
...
enum eMaterialTexture {
    ...
    eMaterialTexture_Special, // This means that the texture is not data in the material, but retrieved else where. Such
...
enum eMaterialRenderMode {
    eMaterialRenderMode_Z, // Early Z test. Shall always return diffuse texture as unit 0, allthough it might not always
                           // be used.
...
struct cProgramComboFeature {
    ...
    cProgramComboFeature(const tString &asVar, tFlag alShaders)
        : msVariable(asVar), mlShaders(alShaders), mlNeededFlags(0) {}
    cProgramComboFeature(const tString &asVar, tFlag alShaders, tFlag alNeededFlags)
        : msVariable(asVar), mlShaders(alShaders), mlNeededFlags(alNeededFlags) {}
...
enum eAnimationEventType { eAnimationEventType_PlaySound, eAnimationEventType_Step, eAnimationEventType_LastEnum };
...
class cOcclusionQueryObject {
  public:
    cOcclusionQueryObject()
        : mpQuery(NULL), mpVtxBuffer(NULL), mpMatrix(NULL), mbDepthTest(false), mlSampleResults(0) {}
...
inline float UCharColorToFloat(unsigned char alColor) { return (float)(alColor) / 255.0f; }
