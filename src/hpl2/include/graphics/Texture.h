...
class iTexture : public iResourceBase, public iFrameBufferAttachment {
  public:
    iTexture(const tString &asName, const tWString &asFullPath, eTextureType aType, eTextureUsage aUsage,
             iLowLevelGraphics *apLowLevelGraphics)
        : iResourceBase(asName, asFullPath, 0), mUsage(aUsage), mType(aType), mpLowLevelGraphics(apLowLevelGraphics),
          mbUseMipMaps(false), mbIsCompressed(false), mlMemorySize(0), mPixelFormat(ePixelFormat_Unknown),
          mWrapS(eTextureWrap_Repeat), mWrapT(eTextureWrap_Repeat), mWrapR(eTextureWrap_Repeat), mfFrameTime(1),
          mAnimMode(eTextureAnimMode_Loop), mlSizeDownScaleLevel(0), mvMinDownScaleSize(16, 16, 16),
          mfAnisotropyDegree(1.0f), mFilter(eTextureFilter_Bilinear), mCompareMode(eTextureCompareMode_None),
    ...
    virtual void SetRawData(int alLevel, const cVector3l &avOffset, const cVector3l &avSize, ePixelFormat aPixelFormat,
                            void *apData) = 0;
