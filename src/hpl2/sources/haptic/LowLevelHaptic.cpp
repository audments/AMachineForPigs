...
cVector3f iLowLevelHaptic::GetHardwarePosDelta() { return mvCurrentHardwarePos - mvPreviousHardwarePos; }
...
cVector2f iLowLevelHaptic::GetVirtualMousePos() {
    ...
    cVector2f vPos =
        (cVector2f(mvCurrentHardwarePos.x, mvCurrentHardwarePos.y) - mvMinMousePos) / (mvMaxMousePos - mvMinMousePos);
    vPos.y = 1 - vPos.y;
    vPos = vPos * mvScreenSize;
    return vPos;
}
...
void iLowLevelHaptic::DestroyShape(iHapticShape *apShape) { STLFindAndDelete(mlstShapes, apShape); }
...
cHapticShapeIterator iLowLevelHaptic::GetShapeIterator() { return cHapticShapeIterator(&mlstShapes); }

//-----------------------------------------------------------------------

void iLowLevelHaptic::DestroyForce(iHapticForce *apForce) { STLFindAndDelete(mlstForces, apForce); }
