...
cRopeEntity::cRopeEntity(const tString &asName, cResources *apResources, cGraphics *apGraphics, iPhysicsRope *apRope,
                         int alMaxSegments)
    : iRenderable(asName) {
    ...
    mpVtxBuffer =
        mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri,
                                               eVertexBufferUsageType_Dynamic, 4 * mlMaxSegments, 6 * mlMaxSegments);
...
void cRopeEntity::SetMaterial(cMaterial *apMaterial) { mpMaterial = apMaterial; }
...
bool cRopeEntity::UpdateGraphicsForViewport(cFrustum *apFrustum, float afFrameTime) {
    ...
    mpVtxBuffer->UpdateData(eVertexElementFlag_Position | eVertexElementFlag_Texture0 | eVertexElementFlag_Texture1 |
                                eVertexElementFlag_Normal,
                            false);
...
int cRopeEntity::GetMatrixUpdateCount() { return GetTransformUpdateCount(); }
