...
cScene::cScene(cGraphics *apGraphics, cResources *apResources, cSound *apSound, cPhysics *apPhysics, cSystem *apSystem,
               cAI *apAI, cGui *apGui, cHaptic *apHaptic)
...
void cScene::DestroyViewport(cViewport *apViewPort) { STLFindAndDelete(mlstViewports, apViewPort); }
...
void cScene::DestroyCamera(cCamera *apCam) { STLFindAndDelete(mlstCameras, apCam); }
    ...
    for (; viewIt != mlstViewports.end(); ++viewIt) {
        ...
        if (alFlags & tSceneRenderFlag_World) {
            ...
            if (pRenderer && pViewPort->GetWorld() && pFrustum) {
                ...
                pRenderer->Render(afFrameTime, pFrustum, pViewPort->GetWorld(), pViewPort->GetRenderSettings(),
                                  pViewPort->GetRenderTarget(), bPostEffects, pViewPort->GetRendererCallbackList());
                ...
                mpGraphics->GetLowLevel()->SetCurrentFrameBuffer(pRenderTarget->mpFrameBuffer, pRenderTarget->mvPos,
        ...
        // Render Post effects
        if (bPostEffects) {
            // TODO: If renderer is null get texture from frame buffer and if frame buffer is NULL, then copy to a
            // texture. 		Or this is solved?
...
cWorld *cScene::CreateWorld(const tString &asName) {
    cWorld *pWorld =
        hplNew(cWorld, (asName, mpGraphics, mpResources, mpSound, mpPhysics, this, mpSystem, mpAI, mpHaptic));
...
void cScene::DestroyWorld(cWorld *apWorld) { STLFindAndDelete(mlstWorlds, apWorld); }
