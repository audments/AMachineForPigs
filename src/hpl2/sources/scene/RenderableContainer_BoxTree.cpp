...
cRCNode_BoxTree::~cRCNode_BoxTree() { STLDeleteAll(mlstChildNodes); }
...
cRenderableContainer_BoxTree::cRenderableContainer_BoxTree() {
    ...
    mfMaxIntersectionAmount = 0.03f; // The amount (1=100%) an object can be over the splitting line and still not be an
                                     // intersecting object Also is the amount by which a new AABB can be expanded.

    mlMaxVolumeCalcObjects =
        200; // The maximum number of objects present if a cpu intense volume calculations should be used.

    // THere are too few objects to create an intersection group, objects that have the relative length (percent of node
    // size) will still remian in intersection.
...
void cRenderableContainer_BoxTree::Add(iRenderable *apRenderable) { m_mlstTempObjects.push_back(apRenderable); }
...
iRenderableContainerNode *cRenderableContainer_BoxTree::GetRoot() { return mpRoot; }
 ...
 */
int cRenderableContainer_BoxTree::GetSplitGroup(iRenderable *apObject, float afCutPlane, int alAxis,
...
float cRenderableContainer_BoxTree::CalculateBestCutPlane(tRenderableList &alstObjects, int alAxis,
                                                          const cVector3f &avNodeSize) {
...
void cRenderableContainer_BoxTree::CompileTempNode(cBoxTreeTempNode *apNode, int alLevel, int alSplitAxis) {
    ...
    if (bLog) {
        Log("%sNode %d Parent: %d level %d. Axis: %d Size: (%s) Objects: %d\n", GetSpaces(alLevel), apNode,
            apNode->mpParent, alLevel, lAxis, vNodeSize.ToString().c_str(), lNumOfObjects);
        Log("%s==================================\n", GetSpaces(alLevel));
    }
    ...
    if (bLog) {
        ...
        if (pIntersectChild)
            Log("%s objects intersected: %d Longest side: %f\n", GetSpaces(alLevel),
                pIntersectChild->mlstObjects.size(), fIntersect_LongestSide);
    }
    ...
    if (pIntersectChild && (int)pIntersectChild->mlstObjects.size() < mlMinLeafObjects &&
        fIntersect_LongestSide < mfMaxSideLength) {
    ...
    // Above:
    if (pHighChild && (int)pHighChild->mlstObjects.size() < mlMinLeafObjects && pIntersectChild &&
        fHigh_LongestSide < mfMaxSideLength) {
        if (bLog)
    ...
    // Below
    if (pLowChild && (int)pLowChild->mlstObjects.size() < mlMinLeafObjects && pIntersectChild &&
        fLow_LongestSide < mfMaxSideLength) {
        if (bLog)
...
void cRenderableContainer_BoxTree::BuildNodeFromTemp(cBoxTreeTempNode *apTempNode, cRCNode_BoxTree *apNode,
                                                     int alLevel) {
    const bool bLog = false;
    ...
    if (bLog) {
        Log("%s-----------------------------------\n", GetSpaces(alLevel));
        Log("%sTempNode level %d Children: %d Objects: %d\n", GetSpaces(alLevel), alLevel,
            apTempNode->mlstChildren.size(), apTempNode->mlstObjects.size());
...
void cRenderableContainer_BoxTree::RenderDebugNode(cRendererCallbackFunctions *apFunctions, cRCNode_BoxTree *apNode,
                                                   int alLevel) {
