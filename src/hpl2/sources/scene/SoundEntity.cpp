...
cSoundEntity::cSoundEntity(const tString &asName, cSoundEntityData *apData, cSoundEntityManager *apSoundEntityManager,
                           cWorld *apWorld, cSoundHandler *apSoundHandler, bool abRemoveWhenOver, int alCreationID)
    : iEntity3D(asName) {
...
void cSoundEntity::Play(bool abPlayStart) {
    ...
    if (mvSoundEntries[eSoundEntityType_Main] == NULL && mvSoundEntries[eSoundEntityType_Start] == NULL &&
...
bool cSoundEntity::IsFadingOut() { return mbFadingOut; }

//-----------------------------------------------------------------------

bool cSoundEntity::GetRemoveWhenOver() { return mbRemoveWhenOver; }
...
void cSoundEntity::UpdateLogic(float afTimeStep) {
    ...
    if (mbStopped == false) {
        ...
        if (mpData->GetLoop() && mpData->GetInterval() > 0 && mvSoundEntries[eSoundEntityType_Start] == NULL &&
            mvSoundEntries[eSoundEntityType_Main] == NULL) {
        ...
        if (mvSoundEntries[eSoundEntityType_Start] == NULL && mpData->GetLoop() && mpData->GetUse3D()) {
            if (CheckIsOutOfRange() && !mpData->GetKeepPlayingOutOfRange()) {
                ...
                //////////////////////
                // If not fading, stop the sound and then set out of range as true. This will check if in range every
                // update.
                else {
                    if (mvSoundEntries[eSoundEntityType_Main]) {
                        if (mpSoundHandler->IsValid(mvSoundEntries[eSoundEntityType_Main],
                                                    mvSoundEntryID[eSoundEntityType_Main])) {
                            if (mbLog)
    ...
    if (mbForcePlayAsGUISound) {
        ...
        if (mpData->GetUse3D()) {
            mvSoundEntries[aType] = mpSoundHandler->Play3D(
                sSoundName, abLoop, mfVolume, GetWorldPosition(), mfMinDistance, mfMaxDistance, eSoundEntryType_World,
                false, mpData->GetPriority(), mpData->GetStream(), &bNotEnoughChannels);
        } else {
            mvSoundEntries[aType] = mpSoundHandler->Play3D(
                sSoundName, abLoop, mfVolume, cVector3f(0, 0, 1), 100.0f,
                200.0f, // Just set something so that sound is always inside min.
