...
cParticleEmitterData_UserData::cParticleEmitterData_UserData(const tString &asName, cResources *apResources,
...
iParticleEmitter *cParticleEmitterData_UserData::Create(tString asName, cVector3f avSize) {
    iParticleEmitter *pPE =
        hplNew(cParticleEmitter_UserData, (asName, &mvMaterials, avSize, mpGraphics, mpResources, this));
...
bool cParticleEmitterData_UserData::CheckCollision(const cVector3f &avStart, const cVector3f &avEnd,
                                                   iPhysicsWorld *apPhysicsWorld, cVector3f *apNormalVec,
                                                   cVector3f *apPosVec) {
    cVector3f vNormal;
...
cParticleEmitter_UserData::cParticleEmitter_UserData(tString asName, tMaterialVec *apMaterials, cVector3f avSize,
    ...
    /*		int i = 0;
            ...
            {
                case ePEType_Beam:
                    hplDelete(mpVtxBuffer);								// Destroy the current Vertex Buffer, as it's
       filled with quads we don't need now :)

                    if ( apData->mbUseBeamNoise )
                    {
                        // Calculate the Frequency Points Lookup Table, this is needed to create the Vertex Buffer.It is
       also needed by every beam particle created,
                ...
                //			tempBNP.lHighFreqNoiseIdx = j;
                    ...
                    mpVtxBuffer = apGraphics->GetLowLevel()->CreateVertexBuffer ( eVertexElementFlag_Position,		//
       Now that we know how many points we have, we create the Vertex Buffer eVertexBufferDrawType_Tri,
       eVertexBufferUsageType_Stream, mlMaxParticles*mpData->mvBeamNoisePoints.size()*2,
       mlMaxParticles*(mpData->mvBeamNoisePoints.size()-1)*6);
                    ...
                    for ( int i = 0; i < (int) mlMaxParticles; ++i )
                    {
                        ...
                        {
                            mpVtxBuffer->AddVertex(eVertexElementFlag_Texture0,
       cVector2f(mpData->mvBeamNoisePoints[j].fRelToBeamPos,1)); mpVtxBuffer->AddVertex(eVertexElementFlag_Texture0,
       cVector2f(mpData->mvBeamNoisePoints[j].fRelToBeamPos,0));
                        }
                    }
...
void cParticleEmitter_UserData::Kill() { mbRespawn = false; }
...
void cParticleEmitter_UserData::SetParticleDefaults(cParticle *apParticle) {
    ...
    if (mpData->mStartPosType == ePEStartPosType_Box) {
        apParticle->mvPos =
            mtxStart.GetTranslation() + cMath::RandRectVector3f(mpData->mvMinStartPos, mpData->mvMaxStartPos);
    ...
    /*
                ...
                */
    //			apParticle->mvPos = mtxStart.GetTranslation() + mpData->mVBMeshData->GetVector3(
    //eVertexElementFlag_Position, cMath::RandRectl(0,mpData->mVBMeshData->GetVertexNum()-1) );
    //			++posI;
    ...
    apParticle->mfLifeSize_MiddleEnd =
        apParticle->mfLife * (1 - (mpData->mfMiddleRelSizeTime + mpData->mfMiddleRelSizeLength));

    apParticle->mfLifeColor_MiddleStart = apParticle->mfLife * (1 - mpData->mfMiddleRelColorTime);
    apParticle->mfLifeColor_MiddleEnd =
        apParticle->mfLife * (1 - (mpData->mfMiddleRelColorTime + mpData->mfMiddleRelColorLength));

    /*Log("Created particle with Pos: (%s) Color: (%s) Size (%s) Vel: (%s) Acc: (%s) Life: %f SizeMiddleStart: %f
       SizeMiddleEnd: %f\n", apParticle->mvPos.ToString().c_str(), apParticle->mColor.ToString().c_str(),
    ...
    if (mPEType == ePEType_Beam) {
        ...
        int j = 1;
        cVector3f vLFPointPos = apParticle->mvPos + apParticle->mvVel *
        mpData->mvBeamNoisePoints.at(mpData->mvLFIndices[j]).fRelToBeamPos; cVector3f vLastLFPointPos =
        apParticle->mvPos; cVector3f vDir; for ( int i = 1; i < (int) mpData->mvBeamNoisePoints.size(); ++i )
        {
            ...
            case ePENoiseType_HighFreq:
                apParticle->mvBeamPoints[i] += cMath::RandRectVector3f(mpData->mvMinHighFreqNoise,
        mpData->mvMaxHighFreqNoise); break; case ePENoiseType_LowFreq:
                ++j;
                vLastLFPointPos = vLFPointPos;
                vLFPointPos = apParticle->mvPos + apParticle->mvVel *
        mpData->mvBeamNoisePoints.at(mpData->mvLFIndices[j]).fRelToBeamPos +
        cMath::RandRectVector3f(mpData->mvMinLowFreqNoise, mpData->mvMaxLowFreqNoise); vDir = vLFPointPos -
        vLastLFPointPos; break; case ePENoiseType_Both: apParticle->mvBeamPoints[i] +=
        cMath::RandRectVector3f(mpData->mvMinHighFreqNoise, mpData->mvMaxHighFreqNoise);
                ++j;
                vLastLFPointPos = vLFPointPos;
                vLFPointPos = apParticle->mvPos + apParticle->mvVel *
        mpData->mvBeamNoisePoints.at(mpData->mvLFIndices[j]).fRelToBeamPos +
        cMath::RandRectVector3f(mpData->mvMinLowFreqNoise, mpData->mvMaxLowFreqNoise); vDir = vLFPointPos -
        vLastLFPointPos; break; default: break;
            }
        }
        */
    }
...
void cParticleEmitter_UserData::UpdateMotion(float afTimeStep) {
    ...
    for (unsigned int i = 0; i < mlNumOfParticles; ++i) {
        ...
        if (mbUseRevolution) {
            ...
            cMatrixf mtxRotationMatrix =
                cMath::MatrixRotate(pParticle->mvRevolutionVel * afTimeStep, eEulerRotationOrder_XYZ);
        ...
        if (bColliding) {
            ...
            if (mpData->CheckCollision(pParticle->mvLastCollidePos, pParticle->mvPos, mpWorld->GetPhysicsWorld(),
                                       &vNormal, &vPos)) {
                // Log("Coll pos: %s\n",vPos.ToString().c_str());
