...
cMeshEntity::cMeshEntity(const tString asName, cMesh *apMesh, cMaterialManager *apMaterialManager,
    ...
    for (int i = 0; i < mpMesh->GetSubMeshNum(); i++) {
        ...
        cSubMeshEntity *pSub =
            hplNew(cSubMeshEntity, (asName + "_" + pSubMesh->GetName(), this, pSubMesh, mpMaterialManager));
        ...
        pSub->mBoundingVolume.AddArrayPoints(pVtxBuffer->GetFloatArray(eVertexBufferElement_Position),
            ...
            if (mpMesh->GetSkeleton()) {
                ...
                if (mbSkeletonPhysics && (!mbSkeletonPhysicsSleeping || mbUpdatedBones == false)) {
                    ...
                    if (mfSkeletonPhysicsWeight != 1.0f) {
                        for (size_t i = 0; i < mvBoneStates.size(); i++) {
                            cMatrixf mtxMixLocal =
                                cMath::MatrixSlerp(mfSkeletonPhysicsWeight, mvTempBoneStates[i]->GetLocalMatrix(),
                                                   mvBoneStates[i]->GetLocalMatrix(), true);
                            ...
                            for (int i = 0; i < pAnim->GetTrackNum(); i++) {
                                ...
                                if (pTrack->GetNodeIndex() < 0) {
                                    ...
                                    if (lBoneIdx < 0 && pTrack->GetNodeIndex() == -1) {
                                        // Error("Track '%s' in '%s' does not have a corresponding bone! Skeleton bone
                                ...
                                if (pState && pState->IsActive()) {
                                    pTrack->ApplyToNode(pState, pAnimState->GetTimePosition(),
                                                        pAnimState->GetWeight() * fAnimationWeightMul,
                                                        pAnimState->IsLooping());
                                }
                            }
                        }
                    }
                        ...
                        if (pAnimState->IsActive()) {
                            ...
                            for (int i = 0; i < pAnim->GetTrackNum(); i++) {
                                ...
                                if (pNodeState->IsActive())
                                    pTrack->ApplyToNode(pNodeState, pAnimState->GetTimePosition(),
                                                        pAnimState->GetWeight() * fAnimationWeightMul);
                            }
                        }
                    }
        ...
        for (size_t i = 0; i < mvAnimationStates.size(); ++i) {
            ...
            for (int j = 0; j < pState->GetEventNum(); ++j) {
                ...
                if ((pEvent->mfTime >= pState->GetPreviousTimePosition() &&
                    ...
                    (pState->GetPreviousTimePosition() > pState->GetTimePosition() &&
                     pState->GetTimePosition() > pEvent->mfTime))
...
void cMeshEntity::ClearAnimations() { STLDeleteAll(mvAnimationStates); }

//-----------------------------------------------------------------------

cAnimationState *cMeshEntity::GetAnimationState(int alIndex) { return mvAnimationStates[alIndex]; }
...
int cMeshEntity::GetAnimationStateNum() { return (int)mvAnimationStates.size(); }
...
bool cMeshEntity::IsMeshCulled() {
    for (size_t i = 0; i < mvSubMeshes.size(); i++) {
        if (mvSubMeshes[i]->GetRenderFrameCount() == iRenderer::GetRenderFrameCount() &&
            mvSubMeshes[i]->mfDistanceToFrustum < 20.0f * 20.0f)
            return false;
    }
...
int cMeshEntity::GetBoneStateNum() { return (int)mvBoneStates.size(); }
...
bool cMeshEntity::GetSkeletonPhysicsActive() { return mbSkeletonPhysics; }
...
float cMeshEntity::GetSkeletonPhysicsWeight() { return mfSkeletonPhysicsWeight; }
void cMeshEntity::SetSkeletonPhysicsWeight(float afX) { mfSkeletonPhysicsWeight = afX; }
...
bool cMeshEntity::GetSkeletonCollidersActive() { return mbSkeletonColliders; }
...
bool cMeshEntity::CheckColliderShapeCollision(iPhysicsWorld *apWorld, iCollideShape *apShape,
                                              const cMatrixf &a_mtxShape, tVector3fList *apPosList,
                                              tIntList *apNumList) {
    ...
    for (size_t i = 0; i < mvBoneStates.size(); ++i) {
        ...
        bool bRet = apWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), apShape, a_mtxShape,
                                                 collideData, 1, false);
...
cNode3D *cMeshEntity::GetNodeState(int alIndex) { return mvNodeStates[alIndex]; }
...
int cMeshEntity::GetNodeStateNum() { return (int)mvNodeStates.size(); }
...
int cMeshEntity::GetSubMeshEntityNum() { return (int)mvSubMeshes.size(); }
        ...
        if (pAnimState->IsActive()) {
            ...
            for (int i = 0; i < pAnim->GetTrackNum(); i++) {
                ...
                if (pTrack->GetNodeIndex() < 0) {
                    ...
                    pTrack->SetNodeIndex(lBoneIdx);
                    if (lBoneIdx < 0 && pTrack->GetNodeIndex() == -1) {
                        // Error("Track '%s' in '%s' does not have a corresponding bone! Skeleton bone name
                        // mismatch?\n", pTrack->GetName().c_str(), mpMesh->GetName().c_str());
                        pTrack->SetNodeIndex(-2);
                    }
                }
                ...
                if (pState && pState->IsActive()) {
                    pTrack->ApplyToNode(pState, pAnimState->GetTimePosition(),
                                        pAnimState->GetWeight() * fAnimationWeightMul, pAnimState->IsLooping());
                }
            }
        }
    }
...
void cMeshEntity::UpdateBVFromSkeleton() {
    ...
    if (mvBoneStates.empty()) {
        ...
        if (GetSubMeshEntityNum() > 0)
            mBoundingVolume.CreateFromPoints(
                GetSubMeshEntity(0)->GetVertexBuffer()->GetElementNum(eVertexBufferElement_Position));
    } else {
        ////////////////////////////////
