...
void iParticleEmitterData::AddMaterial(cMaterial *apMaterial) { mvMaterials.push_back(apMaterial); }
...
iParticleEmitter::iParticleEmitter(tString asName, tMaterialVec *avMaterials, unsigned int alMaxParticles,
    ...
    mpVtxBuffer = apGraphics->GetLowLevel()->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri,
                                                                eVertexBufferUsageType_Stream, alMaxParticles * 4,
                                                                alMaxParticles * 6);
    ...
    ////////////////////////////////////
    // Setup vars
    mlSleepCount = 60 * 5; // Start with high sleep count to make sure a looping particle system reaches equilibrium. 5
                           // secs should eb enough!
...
cMaterial *iParticleEmitter::GetMaterial() { return (*mvMaterials)[(int)mfFrame]; }
...
bool iParticleEmitter::UpdateGraphicsForViewport(cFrustum *apFrustum, float afFrameTime) {
    ...
    if (mpParentSystem->mbFadeAtDistance) {
        float fDistSqr =
            cMath::Vector3DistSqr(mpParentSystem->GetBoundingVolume()->GetWorldCenter(), apFrustum->GetOrigin());
    ...
    if (mPEType == ePEType_Beam) {
        ...
        if (mDrawType == eParticleEmitterType_FixedPoint) {
            cVector3f vAdd[4] = {cVector3f(mvDrawSize.x, -mvDrawSize.y, 0), cVector3f(-mvDrawSize.x, -mvDrawSize.y, 0),
                                 cVector3f(-mvDrawSize.x, mvDrawSize.y, 0), cVector3f(mvDrawSize.x, mvDrawSize.y, 0)};
        ...
        // DYNAMIC POINT
            ...
            for (int i = 0; i < (int)mlNumOfParticles; i++) {
                ...
                if (mbUsePartSpin) {
                    ...
                    SetPos(&pPosArray[i * lVtxQuadSize + 0 * lVtxStride],
                           vPos + cMath::MatrixMul(mtxRotationMatrix, vAdd[0] * vParticleSize));
                    SetCol(&pColArray[i * 16 + 0 * 4], finalColor);

                    SetPos(&pPosArray[i * lVtxQuadSize + 1 * lVtxStride],
                           vPos + cMath::MatrixMul(mtxRotationMatrix, vAdd[1] * vParticleSize));
                    SetCol(&pColArray[i * 16 + 1 * 4], finalColor);

                    SetPos(&pPosArray[i * lVtxQuadSize + 2 * lVtxStride],
                           vPos + cMath::MatrixMul(mtxRotationMatrix, vAdd[2] * vParticleSize));
                    SetCol(&pColArray[i * 16 + 2 * 4], finalColor);

                    SetPos(&pPosArray[i * lVtxQuadSize + 3 * lVtxStride],
                           vPos + cMath::MatrixMul(mtxRotationMatrix, vAdd[3] * vParticleSize));
                    SetCol(&pColArray[i * 16 + 3 * 4], finalColor);
        ...
        if (mvSubDivUV.size() > 1)
            mpVtxBuffer->UpdateData(
                eVertexElementFlag_Position | eVertexElementFlag_Color0 | eVertexElementFlag_Texture0, false);
        else
...
iVertexBuffer *iParticleEmitter::GetVertexBuffer() { return mpVtxBuffer; }
