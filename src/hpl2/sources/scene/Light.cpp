...
cColor iLight::GetColor() { return mDiffuseColor * cColor(mfBrightness, 1); }
        ...
        if (mbFlickerOn) {
            if (mfFlickerTime >= mfFlickerStateLength) {
                ...
                if (!mbFlickerFade) {
                    ...
                    FadeTo(mFlickerOffColor, mfFlickerOffRadius,
        ...
        // Off
            ...
            if (mfFlickerTime >= mfFlickerStateLength) {
                ...
                if (!mbFlickerFade) {
                    ...
                    FadeTo(mFlickerOnColor, mfFlickerOnRadius,
...
void iLight::StopFading() { mfFadeTime = 0; }

bool iLight::IsFading() { return mfFadeTime != 0; }
...
void iLight::SetFlicker(const cColor &aOffCol, float afOffRadius, float afOnMinLength, float afOnMaxLength,
                        const tString &asOnSound, const tString &asOnPS, float afOffMinLength, float afOffMaxLength,
                        const tString &asOffSound, const tString &asOffPS, bool abFade, float afOnFadeMinLength,
                        float afOnFadeMaxLength, float afOffFadeMinLength, float afOffFadeMaxLength) {
...
cMatrixf *iLight::GetModelMatrix(cFrustum *apFrustum) { return &GetWorldMatrix(); }
...
iTexture *iLight::GetGoboTexture() { return mpGoboTexture; }
...
void iLight::ClearShadowCasterCache() { m_mapShadowCasterCache.clear(); }
...
void iLight::OnSetDiffuse() {
    for (size_t i = 0; i < mvBillboards.size(); ++i) {
        mvBillboards[i].mpBillboard->SetColor(mvBillboards[i].mBaseColor *
                                              cColor(mDiffuseColor.r, mDiffuseColor.g, mDiffuseColor.b, 1));
    }
}
