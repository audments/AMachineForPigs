...
cFrustum *cLightSpot::GetFrustum() {
    if (mlFrustumMatrixCount != GetTransformUpdateCount() || mbFrustumUpdated || mbProjectionUpdated) {
        mpFrustum->SetupPerspectiveProj(GetProjectionMatrix(), GetViewMatrix(), mfRadius, mfNearClipPlane, mfFOV,
                                        mfAspect, GetWorldPosition(), false);
...
iTexture *cLightSpot::GetSpotFalloffMap() { return mpSpotFalloffMap; }
...
void cLightSpot::UpdateBoundingVolume() { mBoundingVolume = GetFrustum()->GetBoundingVolume(); }
