...
static bool CheckObjectIsAtEdgeOfAABB(iRenderable *apObject, const cVector3f &avNodeMin, const cVector3f &avNodeMax) {
    ...
    if (cMath::Abs(vMin.x - avNodeMin.x) < kEpsilonf || cMath::Abs(vMin.y - avNodeMin.y) < kEpsilonf ||
        cMath::Abs(vMin.z - avNodeMin.z) < kEpsilonf ||

        cMath::Abs(vMax.x - avNodeMax.x) < kEpsilonf || cMath::Abs(vMax.y - avNodeMax.y) < kEpsilonf ||
...
cRenderableContainer_DynBoxTree::cRenderableContainer_DynBoxTree() {
    ...
    mlMaxGarbageCollectCount =
        60; // How often (measured in accesses with change, meaning when the renderer checks the node and there has been
            // some movement) certain a "garbage check should be made. This means:
            //- If node has leaves as children, check so that the child objects contained is equal or greater then then
            //split threshold
            //  else move all the objects in the child nodes to the node.
            //- Recalculate the split axis.

    mlMaxIgnoreSplitCount =
        30; // How often new splits should be done, after the first split has been made.
    ...
    mfMaxIntersectionAmount = 0.2f; // The amount (1=100%) an object can be over the splitting line and still not be an
                                    // intersecting object Also is the amount by which a new AABB can be expanded.
...
cRenderableContainer_DynBoxTree::~cRenderableContainer_DynBoxTree() { hplDelete(mpObjectCalllback); }
...
void cRenderableContainer_DynBoxTree::Add(iRenderable *apRenderable) {
    ...
    if (gbLog || HasDebug(apRenderable)) {
        ...
        for (tRenderableListIt it = pObjList->begin(); it != pObjList->end(); ++it) {
            ...
            Log(" '%s'id: %d  parent: '%s' size: %s\n", pObj->GetName().c_str(), pObj->GetUniqueID(), sParent.c_str(),
                pObj->GetBoundingVolume()->GetSize().ToString().c_str());
        }
    }
...
void cRenderableContainer_DynBoxTree::Remove(iRenderable *apRenderable) {
    ...
    cRCNode_DynBoxTree *pNode =
        static_cast<cRCNode_DynBoxTree *>(apRenderable->GetRenderContainerNode()); // Assume one node only
    if (pNode == NULL)
        return;
...
iRenderableContainerNode *cRenderableContainer_DynBoxTree::GetRoot() { return &mRoot; }

//-----------------------------------------------------------------------

void cRenderableContainer_DynBoxTree::Compile() {
    // When doing a compile, wait a little while and then do a rebuild. So all particle system and usch have come into
    // place.
    mlRebuildCount = 10;
}
...
void cRenderableContainer_DynBoxTree::RebuildNodes() { mlRebuildCount = 0; }
...
static cColor LevelColor[10] = {cColor(1, 1, 1),    cColor(1, 0, 1),      cColor(1, 1, 0), cColor(0, 1, 1),
                                cColor(0, 0, 1),    cColor(0, 1, 0),      cColor(1, 0, 0), cColor(1, 0.5f, 1),
                                cColor(1, 1, 0.5f), cColor(1, 0.5f, 0.5f)};

void cRenderableContainer_DynBoxTree::RenderDebugNode(cRendererCallbackFunctions *apFunctions,
                                                      cRCNode_DynBoxTree *apNode, int alLevel) {
    // AABB
...
void cRenderableContainer_DynBoxTree::CheckNodeAABBNeedsUpdateIterative(cRCNode_DynBoxTree *apNode,
...
cRCNode_DynBoxTree *cRenderableContainer_DynBoxTree::GetAddNode(cRCNode_DynBoxTree *apStartNode,
                                                                iRenderable *apObject) {
...
void cRenderableContainer_DynBoxTree::AddObjectToNodeIterative(cRCNode_DynBoxTree *apNode, iRenderable *apObject) {
    if (gbLog || HasDebug(apObject)) {
        ...
        Log(" Testing node %d. Inside: %d Intersect: %d (%s)-(%s) vs (%s)-(%s)\n", apNode,
            ...
            pBV->GetMin().ToString().c_str(), pBV->GetMax().ToString().c_str(), apNode->mvMin.ToString().c_str(),
            apNode->mvMax.ToString().c_str());
    }
    /////////////////////////////
 ...
 */
int cRenderableContainer_DynBoxTree::GetSplitGroup(iRenderable *apObject, float afSplitPlane, int alAxis,
                                                   const cVector3f &avNodeSize) {
    ...
    // Intersection
        ...
        float fNodeSize =
            GetAxisFromVec(avNodeSize, alAxis); // Using the size of entire node since it one side might be very small.
...
void cRenderableContainer_DynBoxTree::UpdateObjectInContainer(iRenderable *apObject) {
    ...
    if (cMath::CheckAABBInside(pBV->GetMin(), pBV->GetMax(), pNode->GetMin(), pNode->GetMax())) {
        // Do not Check if AABB of node needs updating, since it will only take time and not gain much since the
        // garbaage collection will fix it after a while.
    ...
    if (pNewNode != pNode) {
        ...
        // if(CheckObjectIsAtEdgeOfAABB(apObject,pNewNode->mvMin,pNewNode->mvMax)) //<- Old bad code! Just have for
        // future reference!
