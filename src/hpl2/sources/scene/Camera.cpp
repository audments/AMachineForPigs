...
cFrustum *cCamera::GetFrustum() {
    if (mbFrustumUpdated) {
        ...
        if (mProjectionType == eProjectionType_Perspective) {
            mFrustum.SetupPerspectiveProj(GetProjectionMatrix(), GetViewMatrix(), GetFarClipPlane(), GetNearClipPlane(),
                                          GetFOV(), GetAspect(), GetPosition(), mbInfFarPlane);
        }
        //////////////////////////////////////////////
        // Orthographic projection
        else {
            mFrustum.SetupOrthoProj(GetProjectionMatrix(), GetViewMatrix(), GetFarClipPlane(), GetNearClipPlane(),
...
const cMatrixf &cCamera::GetProjectionMatrix() {
    if (mbProjectionUpdated) {
        ...
        if (mProjectionType == eProjectionType_Perspective) {
            m_mtxProjection =
                cMath::MatrixPerspectiveProjection(mfNearClipPlane, mfFarClipPlane, mfFOV, mfAspect, mbInfFarPlane);
        }
        //////////////////////////////////////////////
        // Orthographic projection
        else {
...
cVector3f cCamera::GetEyePosition() { return mvPosition; }
...
void cCamera::UnProject(cVector3f *apPosition, cVector3f *apDirection, const cVector2f &avScreenPos,
                        const cVector2f &avVirtualScreenSize) {
    UnProjectHelper(apPosition, apDirection, avScreenPos, avVirtualScreenSize, GetViewMatrix().GetRotation(),
                    GetPosition(), GetForward(), GetUp(), GetRight());
}

void cCamera::UnProjectHelper(cVector3f *apPosition, cVector3f *apDirection, const cVector2f &avScreenPos,
    ...
    if (mProjectionType == eProjectionType_Perspective) {
        ...
        cVector3f vDir(vNormalizedScreen.x * avViewportToWorld.x, vNormalizedScreen.y * avViewportToWorld.y,
                       -mfNearClipPlane);
...
void cCamera::AttachEntity(iEntity3D *apEntity) { mNode.AddEntity(apEntity); }

void cCamera::RemoveEntity(iEntity3D *apEntity) { mNode.RemoveEntity(apEntity); }

void cCamera::ClearAttachedEntities() { mNode.ClearEntities(); }
...
cVector3f cCamera::GetForward() { return GetViewMatrix().GetForward() * -1.0f; }
cVector3f cCamera::GetRight() { return GetViewMatrix().GetRight(); }
cVector3f cCamera::GetUp() { return GetViewMatrix().GetUp(); }
