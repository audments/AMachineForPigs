...
void cSoundRayCallback::Reset() { mbHasCollided = false; }
...
cSoundEntry::cSoundEntry(const tString &asName, iSoundChannel *apSound, float afVolume, eSoundEntryType aType,
                         bool ab3D, bool abStream, int alId, cSoundHandler *apSoundHandler) {
...
bool cSoundEntry::Update(float afTimeStep) {
    ...
    if (mpSound->GetPositionIsRelative()) {
        cVector3f vPos =
            cMath::MatrixMul(mpSoundHandler->mpLowLevelSound->GetListenerMatrix(), mpSound->GetRelPosition());
...
void cSoundEntry::SetPaused(bool abX) { mpSound->SetPaused(abX); }

//-----------------------------------------------------------------------

void cSoundEntry::SetDefaultVolume(float afVolume) { mfNormalVolume = afVolume; }

void cSoundEntry::SetDefaultSpeed(float afSpeed) { mfNormalSpeed = afSpeed; }
...
cSoundHandler::~cSoundHandler() { STLDeleteAll(m_lstSoundEntries); }
...
cSoundEntry *cSoundHandler::Play(const tString &asName, bool abLoop, float afVolume, const cVector3f &avPos,
                                 float afMinDist, float afMaxDist, eSoundEntryType aEntryType, bool abRelative,
...
cSoundEntry *cSoundHandler::Play3D(const tString &asName, bool abLoop, float afVolume, const cVector3f &avPos,
    ...
    return Play(asName, abLoop, afVolume, avPos, afMinDist, afMaxDist, aEntryType, abRelative, true, alPriorityModifier,
                abStream, apNotEnoughChannels);
}

//-----------------------------------------------------------------------

cSoundEntry *cSoundHandler::PlayGui(const tString &asName, bool abLoop, float afVolume, const cVector3f &avPos,
                                    eSoundEntryType aEntryType, bool *apNotEnoughChannels) {
    return Play(asName, abLoop, afVolume, avPos, 1.0f, 1000.0f, aEntryType, true, false, 0, false, apNotEnoughChannels);
}

//-----------------------------------------------------------------------

cSoundEntry *cSoundHandler::PlayGuiStream(const tString &asFileName, bool abLoop, float afVolume,
                                          const cVector3f &avPos, eSoundEntryType aEntryType,
                                          bool *apNotEnoughChannels) {
    return Play(asFileName, abLoop, afVolume, avPos, 1.0f, 1000.0f, aEntryType, true, false, 0, true,
                apNotEnoughChannels);
}
...
cSoundEntry *cSoundHandler::PlaySoundEntityGui(const tString &asName, bool abLoop, float afVolume,
                                               eSoundEntryType aEntryType, const cVector3f &avPos,
                                               bool *apNotEnoughChannels) {
    ...
    if (pData->GetStream()) {
        ...
        return PlayGui(sSoundName, abLoop, pData->GetVolume() * pData->GetVolume(), avPos, aEntryType,
                       apNotEnoughChannels);
    }
}
...
bool cSoundHandler::StopAllExcept(const tString &asName) { return false; }
...
int cSoundHandler::FadeGlobalVolume(float afDestVolume, float afSpeed, tFlag aAffectedTypes, int alId,
...
int cSoundHandler::FadeGlobalSpeed(float afDestSpeed, float afSpeed, tFlag aAffectedTypes, int alId,
                                   bool abDestroyIdAtDest) {
    if (alId < 0)
        alId = mGlobalSpeedHandler.CreateEntry();
...
void cSoundHandler::SetWorld(cWorld *apWorld) { mpWorld = apWorld; }

//-----------------------------------------------------------------------

tSoundEntryList *cSoundHandler::GetEntryList() { return &m_lstSoundEntries; }
...
bool cSoundHandler::CheckSoundIsBlocked(const cVector3f &avSoundPosition) {
    ...
    pPhysicsWorld->CastRay(&mSoundRayCallback, avSoundPosition, mpLowLevelSound->GetListenerPosition(), false, false,
                           false, true);
...
iSoundChannel *cSoundHandler::CreateChannel(const tString &asName, int alPriority, bool abStream,
                                            bool *apNotEnoughChannels) {
    if (apNotEnoughChannels)
        *apNotEnoughChannels = false;
