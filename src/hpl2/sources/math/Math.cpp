...
int cMath::RandRectl(int alMin, int alMax) { return (rand() % (alMax - alMin + 1)) + alMin; }
...
cColor cMath::RandRectColor(const cColor &aMin, const cColor &aMax) {
    return cColor(RandRectf(aMin.r, aMax.r), RandRectf(aMin.g, aMax.g), RandRectf(aMin.b, aMax.b),
...
void cMath::ExpandAABB(cVector3f &avBaseMin, cVector3f &avBaseMax, const cVector3f &avAddMin,
                       const cVector3f &avAddMax) {
...
bool cMath::CheckRectIntersection(cRect2f aRect1, cRect2f aRect2) {
    ...
    return (aRect1.x > aRect2.x + (aRect2.w) || aRect2.x > aRect1.x + (aRect1.w) || aRect1.y > aRect2.y + (aRect2.h) ||
            aRect2.y > aRect1.y + (aRect1.h)) == false;
}
...
bool cMath::CheckRectFit(const cRect2l &aRectSrc, const cRect2l &aRectDest) {
    ...
    if (aRectSrc.w > aRectDest.w || aRectSrc.h > aRectDest.h || aRectSrc.x + aRectSrc.w > aRectDest.x + aRectDest.w ||
        aRectSrc.y + aRectSrc.h > aRectDest.y + aRectDest.h)
        return false;

    // check if x,y is in borders
    if (aRectSrc.x < aRectDest.x || aRectSrc.y < aRectDest.y || aRectSrc.x > aRectDest.x + aRectDest.w ||
        aRectSrc.y > aRectDest.y + aRectDest.h)
        return false;
...
bool cMath::CheckSphereLineIntersection(const cVector3f &avSpherePos, float afSphereRadius,
                                        const cVector3f &avLineStart, const cVector3f &avLineEnd, float *apT1,
                                        cVector3f *apIntersection1, float *apT2, cVector3f *apIntersection2) {
...
bool cMath::CheckPointInFlatPolygon(const cVector3f &avRayStart, const cVector3f &avRayEnd, const cVector2f &avPoint,
                                    const cVector2f &avViewportSize, int alNumIndices, unsigned int *avIndices,
                                    float *avVertexPos, int alStride, const cMatrixf &amtxObjWorldMatrix,
    ...
    for (int i = 0; i < alNumIndices; i += 3) {
        for (int j = 0; j < 3; ++j) {
            ...
            vTriangleWorldSpace[j] =
                cMath::MatrixMul(amtxObjWorldMatrix, cVector3f(avVertexPos[lBaseIndex], avVertexPos[lBaseIndex + 1],
...
bool cMath::CheckPointSameSideOfEdge2D(const cVector3f &avPoint1, const cVector3f &avPoint2, const cVector3f &avPointA,
                                       const cVector3f &avPointB) {
...
bool cMath::CheckBVIntersection(cBoundingVolume &aBV1, cBoundingVolume &aBV2) {
    ...
    if (vMax1.x < vMin2.x || vMax1.y < vMin2.y || vMax1.z < vMin2.z || vMax2.x < vMin1.x || vMax2.y < vMin1.y ||
        vMax2.z < vMin1.z) {
        return false;
    }
...
bool cMath::CheckAABBIntersection(const cVector3f &avMin1, const cVector3f &avMax1, const cVector3f &avMin2,
                                  const cVector3f &avMax2) {
    if (avMax1.x < avMin2.x || avMax1.y < avMin2.y || avMax1.z < avMin2.z || avMax2.x < avMin1.x ||
        avMax2.y < avMin1.y || avMax2.z < avMin1.z) {
        return false;
    }
...
bool cMath::CheckAABBInside(const cVector3f &avInsideMin, const cVector3f &avInsideMax, const cVector3f &avOutsideMin,
                            const cVector3f &avOutsideMax) {
...
bool cMath::CheckPointInBVIntersection(const cVector3f &avPoint, cBoundingVolume &aBV) {
    ...
    if (avPoint.x > vMax.x || avPoint.y > vMax.y || avPoint.z > vMax.z || avPoint.x < vMin.x || avPoint.y < vMin.y ||
        avPoint.z < vMin.z) {
        return false;
    }
...
bool cMath::CheckPointInAABBIntersection(const cVector3f &avPoint, const cVector3f &avMin, const cVector3f &avMax) {
    if (avPoint.x > avMax.x || avPoint.y > avMax.y || avPoint.z > avMax.z || avPoint.x < avMin.x ||
        avPoint.y < avMin.y || avPoint.z < avMin.z) {
        return false;
    }
...
bool cMath::GetNormalizedClipRectFromBV(cVector3f &avDestMin, cVector3f &avDestMax, cBoundingVolume &aBV,
                                        cFrustum *apFrustum, float afTanHalfFov) {
    /////////////////////////////////
    // Setup variables.
    cVector3f vMax = aBV.GetMax();
    cVector3f vMin = aBV.GetMin();
...
cRect2l cMath::GetClipRectFromNormalizedMinMax(const cVector3f &avMin, const cVector3f &avMax,
...
void cMath::GetClipRectFromBV(cRect2l &aDestRect, cBoundingVolume &aBV, cFrustum *apFrustum,
                              const cVector2l &avScreenSize, float afTanHalfFov) {
    ////////////////////////
...
cRect2l cMath::GetClipRectFromSphere(const cVector3f &avPosition, float afRadius, cFrustum *apFrustum,
                                     const cVector2l &avScreenSize, bool abPositionIsScreenSpace, float afTanHalfFov) {
...
cVector3f cMath::GetPoint3DFromSphericalCoords(const cVector3f &avSphCenter, float afSphRadius,
                                               const cVector2f &avSphCoords) {
...
bool cMath::CheckAABBLineIntersection(const cVector3f &avMin, const cVector3f &avMax, const cVector3f &avLineStart,
                                      const cVector3f &avLineEnd, cVector3f *apIntersectionPos, float *apT) {
    cPlanef vPlanes[6] = {
...
float cMath::GetFraction(float afVal) { return afVal - floor(afVal); }
...
float cMath::GetAngleDistanceRad(float afAngle1, float afAngle2) { return GetAngleDistance(afAngle1, afAngle2, k2Pif); }
...
float cMath::ToRad(float afAngle) { return (afAngle / 360.0f) * k2Pif; }

//-----------------------------------------------------------------------

float cMath::ToDeg(float afAngle) { return (afAngle / k2Pif) * 360.0f; }
...
int cMath::Pow2(int alX) { return 1 << alX; }
...
float cMath::Interpolate(float afA, float afB, float afT) { return afA * (1 - afT) + afB * afT; }
...
float cMath::SmoothCurve(float afX) { return afX * afX * (3 - afX * 2); }
...
cVector2f cMath::ProjectVector2D(const cVector2f &avSrcVec, const cVector2f &avDestVec) {
    float fTemp =
        (avSrcVec.x * avDestVec.x + avSrcVec.y * avDestVec.y) / (avDestVec.x * avDestVec.x + avDestVec.y * avDestVec.y);
...
eCollision cMath::CheckPlaneAABBCollision(const cPlanef &aPlane, const cVector3f &avMin, const cVector3f &avMax) {
    cVector3f vCorners[8] = {
        cVector3f(avMax.x, avMax.y, avMax.z), cVector3f(avMax.x, avMax.y, avMin.z),
        cVector3f(avMax.x, avMin.y, avMax.z), cVector3f(avMax.x, avMin.y, avMin.z),

        cVector3f(avMin.x, avMax.y, avMax.z), cVector3f(avMin.x, avMax.y, avMin.z),
        cVector3f(avMin.x, avMin.y, avMax.z), cVector3f(avMin.x, avMin.y, avMin.z),
    };
...
eCollision cMath::CheckPlaneAABBCollision(const cPlanef &aPlane, const cVector3f &avMin, const cVector3f &avMax,
                                          const cVector3f &avSphereCenter, float afSphereRadius) {
    ///////////////////////////////
    // Sphere collision
...
eCollision cMath::CheckPointsPlanesCollision(cVector3f *apVertices, int alNumOfVertices, cPlanef *apPlanes,
                                             int alNumOfPlanes) {
    int lTotalIn = 0;
    ////////////////////////////
    // Iterate planes
...
eCollision cMath::CheckPointsAABBPlanesCollision(cVector3f *apVertices, int alNumOfVertices, const cVector3f &avMin,
...
bool cMath::CheckSeparatingAxisIntersection(cVector3f *apVerticesA, int alNumOfVerticesA, cPlanef *apPlanesA,
                                            int alNumOfPlanesA, cVector3f *apVerticesB, int alNumOfVerticesB,
                                            cPlanef *apPlanesB, int alNumOfPlanesB) {
...
bool cMath::CheckLineTriMeshIntersection(const cVector3f &avLineStart, const cVector3f &avLineEnd,
                                         const cMatrixf &a_mtxInvMeshMtx, const unsigned int *apIndexArray,
                                         int alIndexNum, const float *apVertexArray, int alVtxStride,
...
bool cMath::CheckLineTriVertexBufferIntersection(const cVector3f &avLineStart, const cVector3f &avLineEnd,
                                                 ...
                                                 cVector3f *apIntersectionPos, float *apT, int *apTriIndex,
                                                 bool abSkipBackfacing) {
    return CheckLineTriMeshIntersection(avLineStart, avLineEnd, a_mtxInvMeshMtx, apVtxBuffer->GetIndices(),
                                        apVtxBuffer->GetIndexNum(),
                                        apVtxBuffer->GetFloatArray(eVertexBufferElement_Position),
                                        apVtxBuffer->GetElementNum(eVertexBufferElement_Position), apIntersectionPos,
                                        apT, apTriIndex, abSkipBackfacing);
}
...
cMatrixf cMath::MatrixMul(const cMatrixf &a_mtxA, const cMatrixf &a_mtxB) {
    ...
    mtxC.m[0][0] = a_mtxA.m[0][0] * a_mtxB.m[0][0] + a_mtxA.m[0][1] * a_mtxB.m[1][0] + a_mtxA.m[0][2] * a_mtxB.m[2][0] +
                   a_mtxA.m[0][3] * a_mtxB.m[3][0];
    mtxC.m[0][1] = a_mtxA.m[0][0] * a_mtxB.m[0][1] + a_mtxA.m[0][1] * a_mtxB.m[1][1] + a_mtxA.m[0][2] * a_mtxB.m[2][1] +
                   a_mtxA.m[0][3] * a_mtxB.m[3][1];
    mtxC.m[0][2] = a_mtxA.m[0][0] * a_mtxB.m[0][2] + a_mtxA.m[0][1] * a_mtxB.m[1][2] + a_mtxA.m[0][2] * a_mtxB.m[2][2] +
                   a_mtxA.m[0][3] * a_mtxB.m[3][2];
    mtxC.m[0][3] = a_mtxA.m[0][0] * a_mtxB.m[0][3] + a_mtxA.m[0][1] * a_mtxB.m[1][3] + a_mtxA.m[0][2] * a_mtxB.m[2][3] +
                   a_mtxA.m[0][3] * a_mtxB.m[3][3];

    mtxC.m[1][0] = a_mtxA.m[1][0] * a_mtxB.m[0][0] + a_mtxA.m[1][1] * a_mtxB.m[1][0] + a_mtxA.m[1][2] * a_mtxB.m[2][0] +
                   a_mtxA.m[1][3] * a_mtxB.m[3][0];
    mtxC.m[1][1] = a_mtxA.m[1][0] * a_mtxB.m[0][1] + a_mtxA.m[1][1] * a_mtxB.m[1][1] + a_mtxA.m[1][2] * a_mtxB.m[2][1] +
                   a_mtxA.m[1][3] * a_mtxB.m[3][1];
    mtxC.m[1][2] = a_mtxA.m[1][0] * a_mtxB.m[0][2] + a_mtxA.m[1][1] * a_mtxB.m[1][2] + a_mtxA.m[1][2] * a_mtxB.m[2][2] +
                   a_mtxA.m[1][3] * a_mtxB.m[3][2];
    mtxC.m[1][3] = a_mtxA.m[1][0] * a_mtxB.m[0][3] + a_mtxA.m[1][1] * a_mtxB.m[1][3] + a_mtxA.m[1][2] * a_mtxB.m[2][3] +
                   a_mtxA.m[1][3] * a_mtxB.m[3][3];

    mtxC.m[2][0] = a_mtxA.m[2][0] * a_mtxB.m[0][0] + a_mtxA.m[2][1] * a_mtxB.m[1][0] + a_mtxA.m[2][2] * a_mtxB.m[2][0] +
                   a_mtxA.m[2][3] * a_mtxB.m[3][0];
    mtxC.m[2][1] = a_mtxA.m[2][0] * a_mtxB.m[0][1] + a_mtxA.m[2][1] * a_mtxB.m[1][1] + a_mtxA.m[2][2] * a_mtxB.m[2][1] +
                   a_mtxA.m[2][3] * a_mtxB.m[3][1];
    mtxC.m[2][2] = a_mtxA.m[2][0] * a_mtxB.m[0][2] + a_mtxA.m[2][1] * a_mtxB.m[1][2] + a_mtxA.m[2][2] * a_mtxB.m[2][2] +
                   a_mtxA.m[2][3] * a_mtxB.m[3][2];
    mtxC.m[2][3] = a_mtxA.m[2][0] * a_mtxB.m[0][3] + a_mtxA.m[2][1] * a_mtxB.m[1][3] + a_mtxA.m[2][2] * a_mtxB.m[2][3] +
                   a_mtxA.m[2][3] * a_mtxB.m[3][3];

    mtxC.m[3][0] = a_mtxA.m[3][0] * a_mtxB.m[0][0] + a_mtxA.m[3][1] * a_mtxB.m[1][0] + a_mtxA.m[3][2] * a_mtxB.m[2][0] +
                   a_mtxA.m[3][3] * a_mtxB.m[3][0];
    mtxC.m[3][1] = a_mtxA.m[3][0] * a_mtxB.m[0][1] + a_mtxA.m[3][1] * a_mtxB.m[1][1] + a_mtxA.m[3][2] * a_mtxB.m[2][1] +
                   a_mtxA.m[3][3] * a_mtxB.m[3][1];
    mtxC.m[3][2] = a_mtxA.m[3][0] * a_mtxB.m[0][2] + a_mtxA.m[3][1] * a_mtxB.m[1][2] + a_mtxA.m[3][2] * a_mtxB.m[2][2] +
                   a_mtxA.m[3][3] * a_mtxB.m[3][2];
    mtxC.m[3][3] = a_mtxA.m[3][0] * a_mtxB.m[0][3] + a_mtxA.m[3][1] * a_mtxB.m[1][3] + a_mtxA.m[3][2] * a_mtxB.m[2][3] +
                   a_mtxA.m[3][3] * a_mtxB.m[3][3];
...
float cMath::MatrixMinor(const cMatrixf &a_mtxA, const size_t r0, const size_t r1, const size_t r2, const size_t c0,
...
cMatrixf cMath::MatrixAdjoint(const cMatrixf &a_mtxA) {
    return cMatrixf(MatrixMinor(a_mtxA, 1, 2, 3, 1, 2, 3), -MatrixMinor(a_mtxA, 0, 2, 3, 1, 2, 3),
                    MatrixMinor(a_mtxA, 0, 1, 3, 1, 2, 3), -MatrixMinor(a_mtxA, 0, 1, 2, 1, 2, 3),

                    -MatrixMinor(a_mtxA, 1, 2, 3, 0, 2, 3), MatrixMinor(a_mtxA, 0, 2, 3, 0, 2, 3),
                    -MatrixMinor(a_mtxA, 0, 1, 3, 0, 2, 3), MatrixMinor(a_mtxA, 0, 1, 2, 0, 2, 3),

                    MatrixMinor(a_mtxA, 1, 2, 3, 0, 1, 3), -MatrixMinor(a_mtxA, 0, 2, 3, 0, 1, 3),
                    MatrixMinor(a_mtxA, 0, 1, 3, 0, 1, 3), -MatrixMinor(a_mtxA, 0, 1, 2, 0, 1, 3),

                    -MatrixMinor(a_mtxA, 1, 2, 3, 0, 1, 2), MatrixMinor(a_mtxA, 0, 2, 3, 0, 1, 2),
                    -MatrixMinor(a_mtxA, 0, 1, 3, 0, 1, 2), MatrixMinor(a_mtxA, 0, 1, 2, 0, 1, 2));
}
...
cMatrixf cMath::MatrixPlaneMirror(const cPlanef &aPlane) {
    return cMatrixf(1 - 2 * aPlane.a * aPlane.a, -2 * aPlane.b * aPlane.a, -2 * aPlane.c * aPlane.a,
                    -2 * aPlane.a * aPlane.d, -2 * aPlane.a * aPlane.b, 1 - 2 * aPlane.b * aPlane.b,
                    -2 * aPlane.c * aPlane.b, -2 * aPlane.b * aPlane.d, -2 * aPlane.a * aPlane.c,
                    -2 * aPlane.b * aPlane.c, 1 - 2 * aPlane.c * aPlane.c, -2 * aPlane.c * aPlane.d, 0, 0, 0, 1);
}
...
cVector3f cMath::MatrixEulerAngleDistance(const cMatrixf &a_mtxA, const cMatrixf &a_mtxB) {
    ...
    // Log("Global A: Fwd: (%s) Up: (%s)\n", a_mtxA.GetTranspose().GetForward().ToString().c_str(),
    // a_mtxA.GetTranspose().GetUp().ToString().c_str()); Log("Global B: Fwd: (%s) Up: (%s)\n",
...
cMatrixf cMath::MatrixUnitVectors(const cVector3f &avRight, const cVector3f &avUp, const cVector3f &avForward,
                                  const cVector3f &avTranslate) {
    return cMatrixf(avRight.x, avUp.x, avForward.x, avTranslate.x, avRight.y, avUp.y, avForward.y, avTranslate.y,
                    avRight.z, avUp.z, avForward.z, avTranslate.z, 0, 0, 0, 1);
}
...
cMatrixf cMath::MatrixPerspectiveProjection(float afNearClipPlane, float afFarClipPlane, float afFOV, float afAspect,
                                            bool abInfFarPlane) {
...
const char *cMath::MatrixToChar(const cMatrixf &a_mtxA) {
    sprintf(mpTempChar,
            ...
            a_mtxA.m[0][0], a_mtxA.m[0][1], a_mtxA.m[0][2], a_mtxA.m[0][3], a_mtxA.m[1][0], a_mtxA.m[1][1],
            a_mtxA.m[1][2], a_mtxA.m[1][3], a_mtxA.m[2][0], a_mtxA.m[2][1], a_mtxA.m[2][2], a_mtxA.m[2][3],
...
bool cMath::CreateTriTangentVectors(float *apDestArray, const unsigned int *apIndexArray, int alIndexNum,
                                    const float *apVertexArray, int alVtxStride, const float *apTexArray,
...
bool cMath::CreateTriangleData(tTriangleDataVec &avTriangles, const unsigned int *apIndexArray, int alIndexNum,
...
bool cMath::CreateEdges(tTriEdgeVec &avEdges, const unsigned int *apIndexArray, int alIndexNum,
                        const float *apVertexArray, int alVtxStride, int alVertexNum, bool *apIsDoubleSided) {
