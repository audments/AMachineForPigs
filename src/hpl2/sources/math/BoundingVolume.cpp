...
static const cTriEdge kvBVEdges[12] = {
    cTriEdge(1, 0, 0, 2), cTriEdge(3, 1, 0, 5), cTriEdge(2, 3, 0, 3), cTriEdge(0, 2, 0, 4),
...
bool cShadowVolumeBV::CollideBVAABB(cBoundingVolume *aBV) {
    ...
    cVector3f vCorners[9] = {cVector3f(vMax.x, vMax.y, vMax.z), cVector3f(vMax.x, vMax.y, vMin.z),
...
const cVector3f &cBoundingVolume::GetLocalMax() { return mvLocalMax; }

const cVector3f &cBoundingVolume::GetLocalMin() { return mvLocalMin; }
...
cVector3f cBoundingVolume::GetLocalCenter() { return (mvLocalMin + mvLocalMax) / 2.0f; }
...
cVector3f cBoundingVolume::GetPosition() { return m_mtxTransform.GetTranslation(); }
...
const cMatrixf &cBoundingVolume::GetTransform() { return m_mtxTransform; }
...
cShadowVolumeBV *cBoundingVolume::GetShadowVolume(const cVector3f &avLightPos, float afLightRange, bool abForceUpdate) {
    ...
    for (int face = 0; face < 6; face++) {
        gvFaces[face].facingLight =
            cMath::Vector3Dot(gvFaces[face].normal, vCorners[kvFacePoints[face]] - avLightPos) < 0;

        // Get a point for the near plane. (any edge point will do)
        if (gvFaces[face].facingLight) {
            mShadowVolume.mvPlanes[mShadowVolume.mlPlaneCount] =
                cPlanef(gvFaces[face].normal * -1.0f, vCorners[kvFacePoints[face]]);
    ...
    for (int i = 0; i < (int)mShadowVolume.mvPoints.size(); i += 4) {
        ...
        cVector3f vNormal = cMath::Vector3Cross(mShadowVolume.mvPoints[i + 1] - mShadowVolume.mvPoints[i + 0],
...
void cBoundingVolume::DrawEdges(const cVector3f &avLightPos, float afLightRange,
    ...
    for (int capplane = 0; capplane < mShadowVolume.mlCapPlanes; capplane++) {
        apLowLevelGraphics->DrawLine(GetWorldCenter(),
                                     GetWorldCenter() + mShadowVolume.mvPlanes[capplane].GetNormal() * -0.5f,
                                     cColor(1, 1, 1, 1));
    }
    ...
    for (int quad = 0; quad < (int)pVolume->mvPoints.size(); quad += 4) {
        ...
        apLowLevelGraphics->DrawLine(vCenter, vCenter + mShadowVolume.mvPlanes[lPlane].GetNormal() * -0.5f,
                                     cColor(1, 1, 1, 1));
        lPlane++;
    }
...
kBeginSerializeBase(cBoundingVolume) kSerializeVar(m_mtxTransform, eSerializeType_Matrixf)
    kSerializeVar(mvLocalMax, eSerializeType_Vector3f) kSerializeVar(mvLocalMin, eSerializeType_Vector3f)
        kSerializeVar(mvMax, eSerializeType_Vector3f) kSerializeVar(mvMin, eSerializeType_Vector3f)
            kSerializeVar(mvPivot, eSerializeType_Vector3f) kSerializeVar(mvWorldMax, eSerializeType_Vector3f)
                kSerializeVar(mvWorldMin, eSerializeType_Vector3f) kSerializeVar(mvPosition, eSerializeType_Vector3f)
                    kSerializeVar(mvSize, eSerializeType_Vector3f) kSerializeVar(mfRadius, eSerializeType_Float32)
                        kEndSerialize()
