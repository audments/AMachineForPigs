...
cFrustum::cFrustum() { mbInvertsCullMode = false; }
...
void cFrustum::SetupPerspectiveProj(const cMatrixf &a_mtxProj, const cMatrixf &a_mtxView, float afFarPlane,
                                    float afNearPlane, float afFOV, float afAspect, const cVector3f &avOrigin,
                                    bool abInfFarPlane, cMatrixf *apCustomFarProjMtx, bool abObliqueNearPlane) {
...
void cFrustum::SetupOrthoProj(const cMatrixf &a_mtxProj, const cMatrixf &a_mtxView, float afFarPlane, float afNearPlane,
                              const cVector2f &avViewSize, const cVector3f &avOrigin, bool abInfFarPlane) {
...
void cFrustum::Setup(const cMatrixf &a_mtxProj, const cMatrixf &a_mtxView, float afFarPlane, float afNearPlane,
                     const cVector3f &avOrigin, bool abInfFarPlane, cMatrixf *apCustomFarProjMtx) {
...
const cPlanef &cFrustum::GetPlane(eFrustumPlane aType) { return mPlane[aType]; }
...
eCollision cFrustum::CollideFrustum(cFrustum *apFrustum) {
    ...
    /*if(cMath::CheckSeparatingAxisIntersection(	mvVertices, 8, mPlane, mbInfFarPlane ? 5 : 6,
                                                apFrustum->mvVertices, 8, apFrustum->mPlane, apFrustum->mbInfFarPlane ?
    5 : 6)==false)
    {
        return eCollision_Outside;
    }*/
...
bool cFrustum::CheckFrustumNearPlaneIntersection(cFrustum *apFrustum) {
    ...
    if (cMath::CheckPlaneSphereCollision(nearPlane, apFrustum->mBoundingSphere.center, apFrustum->mBoundingSphere.r) !=
    ...
    if (cMath::CheckPointsPlanesCollision(mvVertices, 4, &apFrustum->mPlane[0], apFrustum->mbInfFarPlane ? 5 : 6) ==
        eCollision_Outside) {
        return false;
    }
...
void cFrustum::UpdateSphere() {
    ...
    // Orthographic projection
        ...
        // The view frustum is basically a rect, get half length of all sides in order to get distance from center to a
        // corner.
...
void cFrustum::UpdatePlanes(cMatrixf *apCustomFarProjMtx) {
    // Left
    mPlane[eFrustumPlane_Left] =
        cPlanef(m_mtxViewProj.m[3][0] + m_mtxViewProj.m[0][0], m_mtxViewProj.m[3][1] + m_mtxViewProj.m[0][1],
                m_mtxViewProj.m[3][2] + m_mtxViewProj.m[0][2], m_mtxViewProj.m[3][3] + m_mtxViewProj.m[0][3]);

    // Right
    mPlane[eFrustumPlane_Right] =
        cPlanef(m_mtxViewProj.m[3][0] - m_mtxViewProj.m[0][0], m_mtxViewProj.m[3][1] - m_mtxViewProj.m[0][1],
                m_mtxViewProj.m[3][2] - m_mtxViewProj.m[0][2], m_mtxViewProj.m[3][3] - m_mtxViewProj.m[0][3]);

    // Bottom
    mPlane[eFrustumPlane_Bottom] =
        cPlanef(m_mtxViewProj.m[3][0] + m_mtxViewProj.m[1][0], m_mtxViewProj.m[3][1] + m_mtxViewProj.m[1][1],
                m_mtxViewProj.m[3][2] + m_mtxViewProj.m[1][2], m_mtxViewProj.m[3][3] + m_mtxViewProj.m[1][3]);

    // Top
    mPlane[eFrustumPlane_Top] =
        cPlanef(m_mtxViewProj.m[3][0] - m_mtxViewProj.m[1][0], m_mtxViewProj.m[3][1] - m_mtxViewProj.m[1][1],
                m_mtxViewProj.m[3][2] - m_mtxViewProj.m[1][2], m_mtxViewProj.m[3][3] - m_mtxViewProj.m[1][3]);

    // Near
    mPlane[eFrustumPlane_Near] =
        cPlanef(m_mtxViewProj.m[3][0] + m_mtxViewProj.m[2][0], m_mtxViewProj.m[3][1] + m_mtxViewProj.m[2][1],
...
void cFrustum::UpdateVertices() {
    ...
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Near], mPlane[eFrustumPlane_Right], mPlane[eFrustumPlane_Top],
                                  mvVertices[0]);
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Near], mPlane[eFrustumPlane_Right], mPlane[eFrustumPlane_Bottom],
                                  mvVertices[1]);
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Near], mPlane[eFrustumPlane_Left], mPlane[eFrustumPlane_Bottom],
                                  mvVertices[2]);
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Near], mPlane[eFrustumPlane_Left], mPlane[eFrustumPlane_Top],
                                  mvVertices[3]);

    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Far], mPlane[eFrustumPlane_Right], mPlane[eFrustumPlane_Top],
                                  mvVertices[4]);
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Far], mPlane[eFrustumPlane_Right], mPlane[eFrustumPlane_Bottom],
                                  mvVertices[5]);
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Far], mPlane[eFrustumPlane_Left], mPlane[eFrustumPlane_Bottom],
                                  mvVertices[6]);
    cMath::PlaneIntersectionPoint(mPlane[eFrustumPlane_Far], mPlane[eFrustumPlane_Left], mPlane[eFrustumPlane_Top],
                                  mvVertices[7]);
...
const cVector3f &cFrustum::GetOrigin() { return mvOrigin; }

//-----------------------------------------------------------------------

cBoundingVolume *cFrustum::GetOriginBV() { return &mOriginBV; }

//-----------------------------------------------------------------------

cVector3f cFrustum::GetForward() { return m_mtxView.GetForward(); }
//-----------------------------------------------------------------------
