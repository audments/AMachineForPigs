...
static void __stdcall Print(std::string asText) { Log(asText.c_str()); }
...
static float __stdcall RandFloat(float afMin, float afMax) { return cMath::RandRectf(afMin, afMax); }

static int __stdcall RandInt(int alMin, int alMax) { return cMath::RandRectl(alMin, alMax); }
...
static void __stdcall ResetLogicTimer() { gpGame->ResetLogicTimer(); }
...
void cScriptFuncs::Init(cGraphics *apGraphics, cResources *apResources, cSystem *apSystem, cInput *apInput,
                        cScene *apScene, cSound *apSound, cEngine *apGame) {
