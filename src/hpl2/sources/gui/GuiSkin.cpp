    ...
    for (int i = 0; i < eGuiSkinAttribute_LastEnum; ++i) {
        if (vAttribLoaded[i] == false)
            Error("Could not find gui skin '%s' attribute %s!\n", msName.c_str(),
                  mpGui->GetSkinAttributeString((eGuiSkinAttribute)i).c_str());
    }
    ...
    for (int i = 0; i < eGuiSkinFont_LastEnum; ++i) {
        if (mvFonts[i] == NULL)
            Error("Could not find gui skin '%s' font %s!\n", msName.c_str(),
                  mpGui->GetSkinFontString((eGuiSkinFont)i).c_str());
    }
    ...
    for (int i = 0; i < eGuiSkinGfx_LastEnum; ++i) {
        if (mvGfxElements[i] == NULL)
            Error("Could not find gui skin '%s' gfx %s!\n", msName.c_str(),
                  mpGui->GetSkinGfxString((eGuiSkinGfx)i).c_str());
    }
...
cGuiGfxElement *cGuiSkin::GetGfx(eGuiSkinGfx aType) { return mvGfxElements[aType]; }

cGuiSkinFont *cGuiSkin::GetFont(eGuiSkinFont aType) { return mvFonts[aType]; }

const cVector3f &cGuiSkin::GetAttribute(eGuiSkinAttribute aType) { return mvAttributes[aType]; }
//-----------------------------------------------------------------------
