...
iGuiPopUp::iGuiPopUp(cGuiSet *apSet, bool abAddCloseButton, const cVector2f &avPopUpSize) {
    ...
    mpWindow =
        mpSet->CreateWidgetWindow(abAddCloseButton ? eWidgetWindowButtonFlag_ButtonClose : eWidgetWindowButtonFlag_None,
...
void iGuiPopUp::SetKillOnEscapeKey(bool abX) { mpWindow->SetEscapeKeyClosesWindow(abX); }

bool iGuiPopUp::GetKillOnEscapeKey() { return mpWindow->GetEscapeKeyClosesWindow(); }
...
void iGuiPopUp::SelfDestruct() { mpSet->DestroyPopUp(this); }

//-----------------------------------------------------------------------

bool iGuiPopUp::RunCallback(void *apObj, tGuiCallbackFunc apCallback, iWidget *apWidget, const cGuiMessageData &aData,
                            bool abRunFocusChangeChecks) {
    if (apObj == NULL || apCallback == NULL)
        return false;

    iWidget *pPreCallAtt, *pPreCallFocus, *pPreCallDefaultUIFocus, *pPostCallAtt, *pPostCallFocus,
        *pPostCallDefaultUIFocus;
