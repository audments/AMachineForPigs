...
void cWidgetMainMenu::OnChildMenuHide() { mpSet->SetAttentionWidget(mpPrevAttention); }
...
void cWidgetMainMenu::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    ...
    // mpSet->DrawGfx( mpGfxBackground, GetGlobalPosition()+cVector3f(0,mvSize.y-1,0.05f), cVector2f(mvSize.x,1),
    // cColor(0.5f,1));
}
