...
iGraphicPickerMode::iGraphicPickerMode(cGuiPopUpColorPicker *apPicker, const tWString &asName, int alSliderParamIndex,
                                       const cVector3f &avMaxValues)
    : mpPicker(apPicker), msName(asName), mlSliderParamIndex(alSliderParamIndex), mvMaxValues(avMaxValues) {
...
bool iGraphicPickerMode::IsCurrent() { return mpPicker->mpCurrentMode == this; }

//-------------------------------------------------------------------------------

void iGraphicPickerMode::OnSetBox(const cVector2f &avX) { mvColorMapPos = avX; }
...
void iGraphicPickerMode::SetRGB(float afR, float afG, float afB) { mpPicker->SetRGB(afR, afG, afB); }
...
void iGraphicPickerMode::OnDrawBox(iWidget *apWidget) {
    ...
    mpPicker->mpSet->DrawGfx(mpPicker->mpGfxHMarker, vPos + cVector3f(0, mvColorMapPos.y * (vSize.y - 1), 0),
                             cVector2f(vSize.x, 1), cColor(mvColorMapPos.y, 1), eGuiMaterial_Diffuse);
    mpPicker->mpSet->DrawGfx(mpPicker->mpGfxVMarker, vPos + cVector3f(mvColorMapPos.x * (vSize.x - 1), 0, 0),
                             cVector2f(1, vSize.y), cColor(1, 1), eGuiMaterial_Diffuse);
}
...
void iGraphicPickerMode::UpdateSliderMarkerColor() {
    ...
    mpSliderBmp->GetPixel(
        0, 0, cVector3l(cMath::FastPositiveFloatToInt(mfColorSliderPos * mpSliderBmp->GetSize().x), 0, 0), pixelData);
...
void cHSBMode::RebuildBox() {
    ...
    cVector3f vHSB;
    cVector3f vStepAmounts = cVector3f(mvMaxValues.x / float(vSize.x - 1), mvMaxValues.y / float(vSize.x - 1),
                                       mvMaxValues.z / float(vSize.x - 1));
    cVector2f vStep;
    cVector2l vIndices;
    cColor temp;
...
float cHSBMode::GetSliderParamValue() { return mpPicker->GetHSB().v[mlSliderParamIndex]; }
...
float cRGBMode::GetSliderParamValue() { return mpPicker->GetColor().v[mlSliderParamIndex]; }
...
cGuiPopUpColorPicker::cGuiPopUpColorPicker(cGraphics *apGraphics, cGuiSet *apSet, cColor *apDestColor,
    ...
    // Build vertical line marker
    {
        ...
        iTexture *pHoriMarkerTexture =
            mpGraphics->GetLowLevel()->CreateTexture("AlphaSlider", eTextureType_1D, eTextureUsage_Normal);
        ...
        mpGfxVMarker->AddTexture(pHoriMarkerTexture, cVector2f(1, 0), cVector2f(1, 1), cVector2f(0, 1),
                                 cVector2f(0, 0));
...
const cColor &cGuiPopUpColorPicker::GetOldColor() { return mpFPreviousColor->GetBackGroundColor(); }
...
void cGuiPopUpColorPicker::CreateHSBInputs(iWidget *apParent, cVector3f &avPos) {
    ...
    CreateInputs(apParent, avPos, kGuiCallback(Color_InputValueEnter), vMin, vMax, mvHSBInputs, mvHSBValueUpdated,
                 vModes);
}
...
void cGuiPopUpColorPicker::CreateRGBInputs(iWidget *apParent, cVector3f &avPos) {
    ...
    CreateInputs(apParent, avPos, kGuiCallback(Color_InputValueEnter), tFloatVec(3, 0.0f), tFloatVec(3, 255.0f),
                 mvRGBInputs, mvRGBValueUpdated, vModes);
}

//-------------------------------------------------------------------------------

void cGuiPopUpColorPicker::CreateInputs(iWidget *apParent, cVector3f &avPos, tGuiCallbackFunc apCallback,
                                        const tFloatVec &avMin, const tFloatVec &avMax,
                                        std::vector<cWidgetTextBox *> &avContainer, tBoolVec &avUpdatedContainer,
    ...
    for (size_t i = 0; i < avModes.size(); ++i) {
        ...
        cWidgetCheckBox *pCBModeSelector =
            mpSet->CreateWidgetCheckBox(vPos, -1, tWString(1, pMode->GetSubname()), apParent);
        ...
        cWidgetTextBox *pInput = mpSet->CreateWidgetTextBox(gvInputOffset, gvInputSize, _W(""), pCBModeSelector,
                                                            eWidgetTextBoxInputType_Numeric, 1);
...
void cGuiPopUpColorPicker::OnCloseSpecific() { RestorePreviewToOldColor(); }
...
void cGuiPopUpColorPicker::Init() {
    ...
    // Create alpha slider
    {
        // mpImgAlphaSliderBG = mpSet->CreateWidgetImage("gui_def_colorpicker_bgpattern.tga", vPos, cVector2f(20, 256),
        // eGuiMaterial_Diffuse, false, mpWindow);
    ...
    // Init Alpha input
    {
        ...
        mpInpAlpha =
            mpSet->CreateWidgetTextBox(gvInputOffset, gvInputSize, _W(""), mpCBAlpha, eWidgetTextBoxInputType_Numeric);
    ...
    // Init Hexcode input
    {
        ...
        */
        ...
        /*mpInpHexAlpha = mpSet->CreateWidgetTextBox(gvInputOffset+cVector3f(gvInputSize.x+5,0,0),
        gvInputSize*cVector2f(0.5f,1.0f), _W(""), pLabel); mpInpHexAlpha->SetDefaultFontSize(gvInputFontSize);
        ...
        */
        ...
        mpInpHexRGBA = mpSet->CreateWidgetTextBox(gvInputOffset, gvInputSize + cVector2f(gvInputSize.x * 0.333f, 0),
                                                  _W(""), pLabel);
        ...
        mpInpVecRGBA = mpSet->CreateWidgetTextBox(gvInputOffset, gvInputSize + cVector2f(gvInputSize.x * 0.333f, 0),
                                                  _W(""), pLabel);
...
void cGuiPopUpColorPicker::UpdateHSBInputs(bool abForce) {
    for (size_t i = 0; i < mvHSBInputs.size(); ++i) {
        ...
        // If fValue is Saturation or Brightness, do a lil conversion (from range 0-1 to 0-100) - Hue does not need
        // conversion
...
void cGuiPopUpColorPicker::UpdateColor(bool abForce) {
    ...
    if (abForce == false && previewColor.r == mColor.r && previewColor.g == mColor.g && previewColor.b == mColor.b &&
        mColor.a == mfOldAlpha)
        return;
