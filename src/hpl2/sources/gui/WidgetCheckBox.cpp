...
void cWidgetCheckBox::OnInit() {
    mpLabel->SetPosition(
        cVector3f(mvGfxBox[0][0]->GetActiveSize().x + 3, mvBoxSize.y / 2 - mvDefaultFontSize.y / 2, 0));
...
void cWidgetCheckBox::OnChangeText() { UpdateLabel(); }

//-----------------------------------------------------------------------

void cWidgetCheckBox::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    // Log("cWidgetCheckBox::OnDraw - text = %s - address:0x%x - Before DrawGfx ... ", cString::To8Char(msText).c_str(),
    // this);
    ////////////////////////////////
    // Box
    mpSet->DrawGfx(mvGfxBox[IsEnabled()][mbChecked],
                   GetGlobalPosition() + cVector3f(0, mvBoxSize.y / 2 - mvGfxBox[0][0]->GetActiveSize().y / 2, 0));
...
bool cWidgetCheckBox::OnMouseMove(const cGuiMessageData &aData) { return true; }
...
bool cWidgetCheckBox::OnMouseEnter(const cGuiMessageData &aData) { return false; }
