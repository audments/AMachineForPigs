...
void cWidgetComboBox::SetCanEdit(bool abX) { mpText->SetCanEdit(abX); }

bool cWidgetComboBox::GetCanEdit() { return mpText->GetCanEdit(); }
...
void cWidgetComboBox::OpenMenu() {
    ...
    if ((int)mvItems.size() > mlMaxItems) {
        ...
        mpSlider->SetSize(
            cVector2f(20, mfMenuHeight - mvGfxBorders[1]->GetActiveSize().y - mvGfxBorders[2]->GetActiveSize().y));
...
bool cWidgetComboBox::ButtonPress(iWidget *apWidget, const cGuiMessageData &aData) {
    if (aData.mlVal == eGuiMouseButton_Left) {
        if (mbMenuOpen) {
            ...
            if (vLocalPos.x >= mvMenuPos.x && vLocalPos.y <= mvMenuPos.x + mvSize.x && vLocalPos.y >= mvMenuPos.y &&
                vLocalPos.y <= mvMenuPos.y + mfMenuHeight) {
...
void cWidgetComboBox::OnChangeSize() {
    if (mpText && mpButton && mpSlider) {
        ...
        mpButton->SetPosition(
            cVector3f(mvSize.x - (mvSize.x - vBackSize.x) / 2 - mfButtonWidth, (mvSize.y - vBackSize.y) / 2, 0.3f));
...
void cWidgetComboBox::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    if (mbMenuOpen) {
        ...
        ////////////////////////////////
        // Draw borders
        DrawBordersAndCorners(NULL, mvGfxBorders, mvGfxCorners, GetGlobalPosition() + mvMenuPos + cVector3f(0, 0, 0.2f),
...
bool cWidgetComboBox::OnMouseEnter(const cGuiMessageData &aData) { return false; }

//-----------------------------------------------------------------------

bool cWidgetComboBox::OnMouseLeave(const cGuiMessageData &aData) { return false; }
...
bool cWidgetComboBox::OnLostFocus(const cGuiMessageData &aData) {
    if (mbMenuOpen && mbOpenByUIButton == false) {
        cVector3f vLocal = WorldToLocalPosition(aData.mvPos);
        if (vLocal.x < mvMenuPos.x || vLocal.x > mvMenuPos.x + mvSize.x || vLocal.y < mvMenuPos.y ||
            vLocal.y > mvMenuPos.y + mfMenuHeight)
            CloseMenu();
    }
...
bool cWidgetComboBox::PointIsInside(const cVector2f &avPoint, bool abOnlyClipped) {
    ...
    if (bInside == false) {
        if (mbMenuOpen) {
            if (vMousePos.x >= mvMenuPos.x && vMousePos.x <= mvMenuPos.x + mvSize.x && vMousePos.y >= mvMenuPos.y &&
                vMousePos.y <= mvMenuPos.y + mfMenuHeight)
                bInside = true;
        }
    }
...
bool cWidgetComboBox::SliderOnUIButtonRelease(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
