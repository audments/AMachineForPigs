...
cGuiGlobalShortcut *iWidget::AddShortcut(int alKeyModifiers, eKey aKey, eGuiMessage aMsg, bool abBypassVisibility,
                                         bool abBypassEnabled) {
    cGuiGlobalShortcut *pShortcut =
        mpSet->AddGlobalShortcut(alKeyModifiers, aKey, this, aMsg, abBypassVisibility, abBypassEnabled);
...
bool iWidget::ProcessMessage(eGuiMessage aMessage, const cGuiMessageData &aData, bool abSkipVisCheck,
...
bool iWidget::PointIsInside(const cVector2f &avPoint, bool abOnlyClipped) {
    ...
    if (avPoint.x < vGlobalPos.x || avPoint.x > vGlobalPos.x + mvSize.x || avPoint.y < vGlobalPos.y ||
        avPoint.y > vGlobalPos.y + mvSize.y) {
        return false;
    } else {
        return true;
    }
...
const cVector3f &iWidget::GetGlobalPosition() {
    if (mpParent) {
        if (mbPositionIsUpdated) {
            ...
            mvGlobalPosition = mpParent->GetGlobalPosition() + mpParent->GetChildrenOffset() -
                               (mbAffectedByScroll ? mpParent->GetScrollAmount() : cVector3f(0)) + mvPosition;
        }
...
iWidget *iWidget::GetFocusNavigation(eUIArrow aDir) { return mvFocusNavWidgets[UIArrowToArrayPos(aDir)]; }
...
cVector3f iWidget::WorldToLocalPosition(const cVector3f &avPos) { return avPos - GetGlobalPosition(); }
...
void iWidget::DrawBordersAndCorners(cGuiGfxElement *apBackground, cGuiGfxElement **apBorderVec,
                                    cGuiGfxElement **apCornerVec, const cVector3f &avPosition, const cVector2f &avSize,
    ...
    if (abBordersOutside) {
        ...
        // Left
        mpSet->DrawGfx(apBorderVec[1], cVector3f(-apCornerVec[0]->GetActiveSize().x, 0, 0),
                       cVector2f(apBorderVec[1]->GetImageSize().x, avSize.y));

        // Up
        mpSet->DrawGfx(apBorderVec[2], cVector3f(0, -apCornerVec[0]->GetActiveSize().y, 0),
        ...
        mpSet->DrawGfx(apCornerVec[0],
                       cVector3f(-apCornerVec[0]->GetActiveSize().x, -apCornerVec[0]->GetActiveSize().y, 0));
        // Right Up
        ...
        mpSet->DrawGfx(apBorderVec[0],
                       cVector3f(avSize.x - apBorderVec[0]->GetActiveSize().x, apCornerVec[1]->GetActiveSize().y, 0),
                       cVector2f(apBorderVec[0]->GetImageSize().x,
                                 avSize.y - (apCornerVec[2]->GetActiveSize().y + apCornerVec[1]->GetActiveSize().y)));
        // Left
        mpSet->DrawGfx(apBorderVec[1], cVector3f(0, apCornerVec[0]->GetActiveSize().y, 0),
                       cVector2f(apBorderVec[1]->GetImageSize().x,
                                 avSize.y - (apCornerVec[3]->GetActiveSize().y + apCornerVec[0]->GetActiveSize().y)));

        // Up
        mpSet->DrawGfx(apBorderVec[2], cVector3f(apCornerVec[0]->GetActiveSize().x, 0, 0),
                       cVector2f(avSize.x - (apCornerVec[0]->GetActiveSize().x + apCornerVec[1]->GetActiveSize().x),
                                 apBorderVec[2]->GetImageSize().y));

        // Down
        mpSet->DrawGfx(apBorderVec[3],
                       cVector3f(apCornerVec[3]->GetActiveSize().x, avSize.y - apBorderVec[3]->GetActiveSize().y, 0),
                       cVector2f(avSize.x - (apCornerVec[2]->GetActiveSize().x + apCornerVec[3]->GetActiveSize().x),
    ...
    /*
            if(alHighlightSize!=asText.size())
                mpSet->DrawFont(asText,mpDefaultFontType,avPosition,mvDefaultFontSize,
                            ...
                            */
    mpSet->DrawFont(asText, mpDefaultFontType, avPosition, mvDefaultFontSize, mDefaultFontHighlightColor * mColorMul,
                    aAlign);
}
