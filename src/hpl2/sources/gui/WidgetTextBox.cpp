...
cVector2f cWidgetTextBox::GetBackgroundSize() {
    return cVector2f(mvSize.x - mvGfxCorners[0]->GetActiveSize().x - mvGfxCorners[1]->GetActiveSize().x,
                     mvSize.y - mvGfxCorners[0]->GetActiveSize().y - mvGfxCorners[2]->GetActiveSize().y);
}
...
void cWidgetTextBox::SetIllegalChars(const tWString &asIllegalChars) { msIllegalChars = asIllegalChars; }

const tWString &cWidgetTextBox::GetIllegalChars() { return msIllegalChars; }

//-----------------------------------------------------------------------

void cWidgetTextBox::SetLegalCharCodeLimitEnabled(bool abX) { mbLegalCharCodeLimitEnabled = abX; }

void cWidgetTextBox::SetLegalCharCodeMinLimit(wchar_t alX) { mlLegalCharCodeMinLimit = alX; }

void cWidgetTextBox::SetLegalCharCodeMaxLimit(wchar_t alX) { mlLegalCharCodeMaxLimit = alX; }
...
float cWidgetTextBox::CharToLocalPos(int alChar) {
    ...
    if (alChar > 0 && alChar - mlFirstVisibleChar > 0) {
        fMarkerPos = mpDefaultFontType->GetLength(
            mvDefaultFontSize, cString::SubW(msText, mlFirstVisibleChar, alChar - mlFirstVisibleChar).c_str());
    }
    return fMarkerPos;
}
...
void cWidgetTextBox::OnChangeSize() {
    ...
    mfTextMaxSize = mvSize.x - mvGfxBorders[0]->GetActiveSize().x - mvGfxBorders[1]->GetActiveSize().x - 3 * 2 -
...
void cWidgetTextBox::OnInit() {
    ...
    for (int i = 0; i < 2; ++i) {
        mvButtons[i] =
            mpSet->CreateWidgetButton(cVector3f(mvSize.x - 10, 1 + i * 10.0f, 0.1f), cVector2f(10), _W(""), this);
...
void cWidgetTextBox::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    ...
    DrawDefaultText(cString::SubW(msText, mlFirstVisibleChar, mlVisibleCharSize), GetGlobalPosition() + vTextAdd,
                    eFontAlign_Left, col);
    ...
    if (mlMarkerCharPos >= 0) {
        ...
        if (mlSelectedTextEnd >= 0) {
            ...
            int lHighlightEnd =
                mlFirstVisibleChar + mlVisibleCharSize < lEnd ? mlFirstVisibleChar + mlVisibleCharSize : lEnd;
            ...
            mpSet->DrawGfx(mpGfxSelectedTextBack, GetGlobalPosition() + vTextAdd + cVector3f(fPos, 0, 0.01f),
                           cVector2f(fSize, mvDefaultFontSize.y));

            DrawDefaultTextHighlight(cString::SubW(msText, lHighlightStart, lHighlightSize),
                                     GetGlobalPosition() + vTextAdd + cVector3f(fPos, 0, 0.02f), eFontAlign_Left);
        }
    }
...
bool cWidgetTextBox::OnMouseEnter(const cGuiMessageData &aData) { return true; }
    ...
    // Copy / Pase / Cut
        ...
        // Paste
            ...
            if (mlSelectedTextEnd < 0) {
                ...
                if (mlMaxCharacters < 0 || (int)sExtra.size() <= lSelectSize ||
    ...
    // Character
        ...
        if (mInputType == eWidgetTextBoxInputType_Numeric) {
            if (unicode >= lFirstNum && unicode <= lLastNum ||
                (unicode == _W('.') || unicode == _W('-')) && mpDefaultFontType->GetGlyph(unicode - lFirstFontChar)) {
                if (mlSelectedTextEnd < 0) {
                    if (unicode == _W('-')) {
                        if (mlMarkerCharPos == 0) {
                            if (msText[0] != _W('-')) {
