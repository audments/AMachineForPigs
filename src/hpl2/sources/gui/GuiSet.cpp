...
cGuiClipRegion::~cGuiClipRegion() { Clear(); }
...
cGuiGlobalShortcut::cGuiGlobalShortcut(int alKeyModifiers, eKey aKey, iWidget *apWidget, eGuiMessage aMessage,
...
cGuiSet::cGuiSet(const tString &asName, cGui *apGui, cGuiSkin *apSkin, cResources *apResources, cGraphics *apGraphics,
...
cGuiGlobalShortcut *cGuiSet::AddGlobalShortcut(int alKeyModifiers, eKey aKey, iWidget *apWidget, eGuiMessage aMessage,
                                               bool abBypassVisibility, bool abBypassEnabled) {
...
void cGuiSet::ClearGlobalShortcuts() { STLDeleteAll(mlstShortcuts); }
...
void cGuiSet::Render(cFrustum *apFrustum) {
    ...
    if (mbIs3D) {
        ...
        // Also scale to size
        cVector3f vPreScale = cVector3f(mv3DSize.x / mvVirtualSize.x, -mv3DSize.y / mvVirtualSize.y,
        ...
        mtxPreMul.SetTranslation(
            cVector3f(mvVirtualSizeOffset.x * vPreScale.x, mvVirtualSizeOffset.y * vPreScale.y, 0));
    ...
    // Screen projection
        ...
        cVector3f vProjMax(mvVirtualSize.x - mvVirtualSizeOffset.x, mvVirtualSize.y - mvVirtualSizeOffset.y,
                           mfVirtualMaxZ);
...
void cGuiSet::ClearRenderObjects() { m_setRenderObjects.clear(); }

//-----------------------------------------------------------------------

void cGuiSet::DrawGfx(cGuiGfxElement *apGfx, const cVector3f &avPos, const cVector2f &avSize, const cColor &aColor,
                      eGuiMaterial aMaterial, float afRotationAngle, bool abUseCustomPivot,
...
void cGuiSet::DrawFont(const tWString &asText, iFontData *apFont, const cVector3f &avPos, const cVector2f &avSize,
                       const cColor &aColor, eFontAlign aAlign, eGuiMaterial aMaterial) {
...
void cGuiSet::DrawFont(iFontData *apFont, const cVector3f &avPos, const cVector2f &avSize, const cColor &aColor,
                       eFontAlign aAlign, eGuiMaterial aMaterial, const wchar_t *fmt, ...) {
    va_list ap;
    if (fmt == NULL)
        return;
    va_start(ap, fmt);
...
cWidgetWindow *cGuiSet::CreateWidgetWindow(tWidgetWindowButtonFlag alFlags, const cVector3f &avLocalPos,
                                           const cVector2f &avSize, const tWString &asText, iWidget *apParent,
...
cWidgetFrame *cGuiSet::CreateWidgetFrame(const cVector3f &avLocalPos, const cVector2f &avSize, bool abDrawFrame,
                                         iWidget *apParent, bool abHScrollBar, bool abVScrollBar,
                                         const tString &asName) {
...
cWidgetButton *cGuiSet::CreateWidgetButton(const cVector3f &avLocalPos, const cVector2f &avSize, const tWString &asText,
                                           iWidget *apParent, bool abToggleable, const tString &asName) {
...
cWidgetLabel *cGuiSet::CreateWidgetLabel(const cVector3f &avLocalPos, const cVector2f &avSize, const tWString &asText,
                                         iWidget *apParent, const tString &asName) {
...
cWidgetSlider *cGuiSet::CreateWidgetSlider(eWidgetSliderOrientation aOrientation, const cVector3f &avLocalPos,
                                           const cVector2f &avSize, int alMaxValue, iWidget *apParent,
                                           const tString &asName) {
...
cWidgetTextBox *cGuiSet::CreateWidgetTextBox(const cVector3f &avLocalPos, const cVector2f &avSize,
                                             const tWString &asText, iWidget *apParent, eWidgetTextBoxInputType aType,
                                             float afNumericAdd, bool abShowButtons, const tString &asName) {
...
cWidgetCheckBox *cGuiSet::CreateWidgetCheckBox(const cVector3f &avLocalPos, const cVector2f &avSize,
                                               const tWString &asText, iWidget *apParent, const tString &asName) {
...
cWidgetImage *cGuiSet::CreateWidgetImage(const tString &asFile, const cVector3f &avLocalPos, const cVector2f &avSize,
                                         eGuiMaterial aMaterial, bool abAnimate, iWidget *apParent,
                                         const tString &asName) {
...
cWidgetListBox *cGuiSet::CreateWidgetListBox(const cVector3f &avLocalPos, const cVector2f &avSize, iWidget *apParent,
                                             const tString &asName) {
...
cWidgetMultiPropertyListBox *cGuiSet::CreateWidgetMultiPropertyListBox(const cVector3f &avLocalPos,
                                                                       const cVector2f &avSize, iWidget *apParent,
                                                                       const tString &asName) {
...
cWidgetComboBox *cGuiSet::CreateWidgetComboBox(const cVector3f &avLocalPos, const cVector2f &avSize,
                                               const tWString &asText, iWidget *apParent, const tString &asName) {
...
cWidgetMenuItem *cGuiSet::CreateWidgetMenuItem(const cVector3f &avLocalPos, const cVector2f &avSize,
                                               const tWString &asText, iWidget *apParent, const tString &asName) {
...
cWidgetContextMenu *cGuiSet::CreateWidgetContextMenu(const cVector3f &avLocalPos, const cVector2f &avSize,
                                                     const tWString &asText, iWidget *apParent, const tString &asName) {
...
cWidgetTabLabel *cGuiSet::CreateWidgetTabLabel(const cVector3f &avLocalPos, const cVector2f &avSize,
                                               const tWString &asText, iWidget *apParent, const tString &asName) {
...
cWidgetTab *cGuiSet::CreateWidgetTab(const cVector3f &avLocalPos, const cVector2f &avSize, const tWString &asText,
                                     iWidget *apParent, const tString &asName) {
...
cWidgetTabFrame *cGuiSet::CreateWidgetTabFrame(const cVector3f &avLocalPos, const cVector2f &avSize,
                                               const tWString &asText, iWidget *apParent, bool abAllowHScroll,
                                               bool abAllowVScroll, const tString &asName) {
...
cWidgetGroup *cGuiSet::CreateWidgetGroup(const cVector3f &avLocalPos, const cVector2f &avSize, const tWString &asText,
                                         iWidget *apParent, const tString &asName) {
...
iWidget *cGuiSet::GetWidgetFromName(const tString &asName) { return (iWidget *)STLFindByName(mlstWidgets, asName); }
...
cGuiPopUpFilePicker *cGuiSet::CreatePopUpSaveFilePicker(tWString &asFileName, const tWString &asCategory,
                                                        ...
                                                        bool abShowHidden, void *apCallbackObject,
                                                        tGuiCallbackFunc apCallback, const tWString &asStartFilename) {
    cGuiPopUpFilePicker *pPicker =
        hplNew(cGuiPopUpFilePicker, (this, mpSkin, eFilePickerType_Save, asStartPath, abShowHidden, apCallbackObject,
                                     apCallback, asStartFilename));
    pPicker->Init();
...
/**	Returns a handle to the Load file popup, so new filters can be added outside. This handle doesn't need to be
 * destroyed.
 *
...
cGuiPopUpFilePicker *cGuiSet::CreatePopUpLoadFilePicker(tWStringVec &avFileList, bool abAddAllFilesFilter,
                                                        ...
                                                        void *apCallbackObject, tGuiCallbackFunc apCallback) {
    cGuiPopUpFilePicker *pPicker = hplNew(cGuiPopUpFilePicker, (this, mpSkin, eFilePickerType_Load, asStartPath,
                                                                abShowHidden, apCallbackObject, apCallback));
    pPicker->Init();
...
cGuiPopUpColorPicker *cGuiSet::CreatePopUpColorPicker(cColor *apDestColor, const cVector3f &avPos,
                                                      void *apCallbackObject, tGuiCallbackFunc apCallback) {
    cGuiPopUpColorPicker *pPicker =
        hplNew(cGuiPopUpColorPicker, (mpGraphics, this, apDestColor, apCallbackObject, apCallback, NULL, NULL));
...
bool cGuiSet::PopUpIsActive() { return mlPopupCount > 0; }
...
void cGuiSet::RemoveWindow(cWidgetWindow *apWin) { mlstWindows.remove(apWin); }

void cGuiSet::SetLastWindowZ(float afX) { mfWindowZ = afX; }
...
bool cGuiSet::GetRootWidgetClips() { return mpWidgetRoot->GetClipActive(); }
...
void cGuiSet::PushFocusedWidget() { mlstFocusedStack.push_back(mpFocusedWidget); }
...
void cGuiSet::PushAttentionWidget() { mlstAttentionStack.push_back(mpAttentionWidget); }
...
void cGuiSet::SetRendersBeforePostEffects(bool abX) { mbRendersBeforePostEffects = abX; }

//-----------------------------------------------------------------------

void cGuiSet::SetIs3D(bool abX) { mbIs3D = abX; }

void cGuiSet::Set3DSize(const cVector3f &avSize) { mv3DSize = avSize; }

void cGuiSet::Set3DTransform(const cMatrixf &a_mtxTransform) { m_mtx3DTransform = a_mtxTransform; }

//-----------------------------------------------------------------------

void cGuiSet::SetCurrentPointer(cGuiGfxElement *apGfx) { mpGfxCurrentPointer = apGfx; }

//-----------------------------------------------------------------------

bool cGuiSet::HasFocus() { return mpGui->GetFocusedSet() == this; }
...
void cGuiSet::DrawTextFromCharArry(const wchar_t *apString, iFontData *apFont, const cVector2f &avSize,
                                   const cVector3f &avPosition, const cColor &aColor, eGuiMaterial aMaterial,
                                   eFontAlign aAlign) {
    int lCount = 0;
    cVector3f vPos = avPosition;
...
static void SetClipArea(iLowLevelGraphics *pLowLevelGraphics, cGuiClipRegion *apRegion) {
    ...
    if (apRegion->mRect.w > 0) {
        ...
        if (kLogRender)
            Log("-- Clip region: %d Clipping: x %f y %f w %f h %f\n", apRegion, apRegion->mRect.x, apRegion->mRect.y,
...
void cGuiSet::RenderClipRegion() {
    ...
    while (it != setRenderObjects.end()) {
        ...
        do {
            ...
            {
                Log("Drawing: %d (%s):(%s) %d (%s)\n", object.mpGfx, object.mvPos.ToString().c_str(),
            object.mvSize.ToString().c_str(), object.mpClipRegion, object.mColor.ToString().c_str());
            }*/
            ...
            if (object.mbRotated) {
                for (int i = 0; i < 4; ++i) {
                    ...
                    pLowLevelGraphics->AddVertexToBatch_Raw(cVector3f(vVtxPos.x + vPos.x, vVtxPos.y + vPos.y, vPos.z),
                                                            vtx.col * object.mColor, vtx.tex);
                }
            } else {
                for (int i = 0; i < 4; ++i) {
                    cVertex &vtx = pGfx->mvVtx[i];
                    ...
                    pLowLevelGraphics->AddVertexToBatch_Raw(
                        cVector3f(vVtxPos.x * object.mvSize.x + vPos.x, vVtxPos.y * object.mvSize.y + vPos.y, vPos.z),
                        vtx.col * object.mColor, vtx.tex);
                }
            }
        ...
        //////////////////////////////
        // Render batch
        pLowLevelGraphics->FlushQuadBatch(eVtxBatchFlag_Position | eVtxBatchFlag_Texture0 | eVtxBatchFlag_Color0,
                                          false);
...
bool cGuiSet::OnMouseMove(const cGuiMessageData &aData) {
    ...
    for (; it != mlstWidgets.end(); ++it) {
        ...
        if (pWidget->PointIsInside(mvMousePos, false)) {
            if (pWidgetUnderMouse == NULL || pWidgetUnderMouse->IsConnectedTo(pWidget) ||
...
bool cGuiSet::OnUIArrowPress(const cGuiMessageData &aData) {
    ...
    if (bRet == false) {
        ...
        for (; it != mlstWidgets.end(); ++it) {
            ...
            if (pWidget->IsGlobalUIInputListener() == false || pWidget == mpFocusedWidget ||
                pWidget->IsVisible() == false)
                continue;
    ...
    if (mpCurrentToolTipWidget == NULL || mpCurrentToolTipWidget->IsVisible() == false) {
        ...
        if (mfToolTipTimer >= mfToolTipTimeToPopUp) {
            if (mpFrameToolTip->IsVisible() == false) {
                ...
                float fTextLength =
                    mpLabelToolTip->GetDefaultFontType()->GetLength(mvFontSize, sTipText.c_str()) + 3 + 3;
                ...
                cVector2f vToolTipSize =
                    cVector2f(cMath::Min(fTextLength, fMaxTextLength), lRows * (mvFontSize.y + 2) - 2 + 3 + 3);
...
void cGuiSet::PushDefaultFocusNavWidget() { mlstDefaultFocusStack.push_back(mpDefaultFocusNavWidget); }
