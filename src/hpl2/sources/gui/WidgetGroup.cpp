...
void cWidgetGroup::OnInit() {
    mpHeader =
        mpSet->CreateWidgetLabel(cVector3f(mfHeaderOffset, -mpDefaultFont->mvSize.y * 0.5f, 0.1f), 0, msText, this);
...
void cWidgetGroup::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    ...
    if (mbHeaderBreaksUpperBorder) {
...

        {
            ...
            mpSet->DrawGfx(
                mvGfxBorders[0],
                cVector3f(mvSize.x - mvGfxBorders[0]->GetActiveSize().x, mvGfxCorners[1]->GetActiveSize().y, 0),
                cVector2f(mvGfxBorders[0]->GetImageSize().x,
                          mvSize.y - (mvGfxCorners[2]->GetActiveSize().y + mvGfxCorners[1]->GetActiveSize().y)));
            // Left
            mpSet->DrawGfx(
                mvGfxBorders[1], cVector3f(0, mvGfxCorners[0]->GetActiveSize().y, 0),
                cVector2f(mvGfxBorders[1]->GetImageSize().x,
                          mvSize.y - (mvGfxCorners[3]->GetActiveSize().y + mvGfxCorners[0]->GetActiveSize().y)));

            // Up
            mpSet->DrawGfx(mvGfxBorders[2], cVector3f(mvGfxCorners[0]->GetActiveSize().x, 0, 0),
                           cVector2f(fPreHeaderWidth - 4, mvGfxBorders[2]->GetActiveSize().y));
            mpSet->DrawGfx(mvGfxBorders[2], vPostHeaderPos + cVector3f(4, 0, 0),
                           cVector2f(fPostHeaderWidth - mvGfxCorners[1]->GetActiveSize().x - 4,
                                     mvGfxBorders[2]->GetActiveSize().y));

            // Down
            mpSet->DrawGfx(
                mvGfxBorders[3],
                cVector3f(mvGfxCorners[3]->GetActiveSize().x, mvSize.y - mvGfxCorners[3]->GetActiveSize().y, 0),
                cVector2f(mvSize.x - (mvGfxCorners[2]->GetActiveSize().x + mvGfxCorners[3]->GetActiveSize().x),
