...
cGuiPopUpFilePicker::cGuiPopUpFilePicker(cGuiSet *apSet, cGuiSkin *apSkin, eFilePickerType aType,
                                         const tWString &asStartPath, bool abShowHiddenFiles, void *apCallbackObject,
                                         tGuiCallbackFunc apCallback, const tWString &asStartFilename)
    : iGuiPopUp(apSet, false, cVector2f(600, 500)), iFileBrowser(asStartPath, abShowHiddenFiles) {
    mpSkin = apSkin;
...
void cGuiPopUpFilePicker::SetLoadFileListDest(tWStringVec &asFileList) { mpLoadFileListDest = &asFileList; }
    ...
    // Save/Load button
        ...
        if (mpCurrentFileName->GetText() == _W(""))
            ...
            case eFilePickerType_Save:
                ...
                if (bFileExists) {
                    mpSet->CreatePopUpMessageBox(_W("Warning"), _W("File already exists. Overwrite?"), _W("Yes"),
                                                 _W("No"), this, kGuiCallback(MessageBoxReturnCallback));
                } else {
                    BuildSaveFileName();
                    ClosePopUp(true);
                }
            ...
            case eFilePickerType_Load:
                if (bFileExists == false) {
                    mpSet->CreatePopUpMessageBox(_W("Error"), _W("File does not exist. Please choose a valid file."),
                                                 _W("OK"), _W(""), NULL, NULL);
                } else {
                    BuildLoadFileList();
                    ClosePopUp(true);
...
void cGuiPopUpFilePicker::BuildSaveFileName() { *mpSaveFileDest = GetCurrentFullPath() + mpCurrentFileName->GetText(); }
...
void cGuiPopUpFilePicker::Init() {
    ...
    mpCurrentDirectory = mpSet->CreateWidgetComboBox(
        cVector3f(30, 60, 0.1f), cVector2f(400, mpWindow->GetDefaultFontSize().y + 2), _W(""), mpWindow);
    ...
    mpCurrentFileName = mpSet->CreateWidgetTextBox(
        cVector3f(30, 425, 0.5f), cVector2f(400, mpWindow->GetDefaultFontSize().y + 2), msStartFilename, mpWindow);
    ...
    mpFilterList = mpSet->CreateWidgetComboBox(cVector3f(30, 450, 0.5f),
                                               cVector2f(300, mpWindow->GetDefaultFontSize().y + 2), _W(""), mpWindow);
