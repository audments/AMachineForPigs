...
void cWidgetTabLabel::OnChangeText() {
    ...
    float fBordersWidth =
        pTabFrame->mvGfxTabLabelBorders[0]->GetImageSize().x + pTabFrame->mvGfxTabLabelBorders[1]->GetImageSize().x;
    float fNewWidth = mpDefaultFontType->GetLength(mpDefaultFont->mvSize, msText.c_str()) +
                      pTabFrame->mfTextLeftPadding + pTabFrame->mfTextRightPadding + fBordersWidth;
...
void cWidgetTabLabel::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    ...
    DrawBordersAndCorners(pBG, pTabFrame->mvGfxTabLabelBorders, pTabFrame->mvGfxTabLabelCorners, GetGlobalPosition(),
                          mvSize);
}
...
bool cWidgetTab::PointIsInside(const cVector2f &avPoint, bool abOnlyClipped) {
    ...
    if (avPoint.x < vFrameGlobalPos.x || avPoint.x > vFrameGlobalPos.x + vFrameSize.x ||
        ...
        if (bLabelVisible == false || avPoint.x < vLabelGlobalPos.x || avPoint.x > vLabelGlobalPos.x + vLabelSize.x ||
...
void cWidgetTab::OnInit() {
    ...
    mpFrame = mpSet->CreateWidgetFrame(
        0, mvSize - cVector2f(vCornerSize.x, fLabelHeight + fFrameOffset + vCornerSize.y), false, NULL,
    ...
    cVector3f vPos = cVector3f(mpParentTabFrame->mvGfxTabCorners[0]->GetActiveSize().x,
                               mpParentTabFrame->mvGfxTabCorners[0]->GetActiveSize().y, 0);
    mpFrame->SetPosition(cVector3f(
        vPos.x, vPos.y + fLabelHeight + fFrameOffset - mpParentTabFrame->mvGfxTabCorners[0]->GetActiveSize().y, 0));
...
void cWidgetTab::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    DrawBordersAndCorners(mpParentTabFrame->mpGfxTabBG, mpParentTabFrame->mvGfxTabBorders,
                          mpParentTabFrame->mvGfxTabCorners, mpFrame->GetGlobalPosition(), mpFrame->GetSize(), true);
}
...
int cWidgetTabFrame::GetTabNum() { return (int)mvTabs.size(); }

//-------------------------------------------------------------------

cWidgetTab *cWidgetTabFrame::GetTab(int alIdx) { return mvTabs[alIdx]; }
