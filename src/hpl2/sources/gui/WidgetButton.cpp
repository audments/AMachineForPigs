...
void cWidgetButton::SetRepeatFreq(float afFreq) { mfRepeatPeriod = 1 / afFreq; }
...
void cWidgetButton::OnDraw(float afTimeStep, cGuiClipRegion *apClipRegion) {
    ...
    if (IsEnabled()) {
        ...
        DrawDefaultText(msText,
                        GetGlobalPosition() + cVector3f(mvSize.x / 2, mvSize.y / 2 - mvDefaultFontSize.y / 2, 0.5f) +
                            vOffset,
...
bool cWidgetButton::OnMouseMove(const cGuiMessageData &aData) { return true; }
...
bool cWidgetButton::OnMouseEnter(const cGuiMessageData &aData) { return false; }
