...
void cGuiGfxAnimation::AddFrame(int alNum) { mvFrames.push_back(alNum); }
void cGuiGfxAnimation::SetType(eGuiGfxAnimationType aType) { mType = aType; }
void cGuiGfxAnimation::SetFrameLength(float afLength) { mfFrameLength = afLength; }
...
void cGuiGfxElement::Update(float afTimeStep) {
    ...
    if (pAnim->mType != eGuiGfxAnimationType_Random) {
        ...
        // Log("Frame %d %f actual_frame: %d size: %d\n",lFrame,mfCurrentFrame,((int)mvImageBufferVec.size()-1) -
        // lFrame,mvImageBufferVec.size());
...
void cGuiGfxElement::AddTexture(iTexture *apTexture, const cVector2f &avUVUpperLeft, const cVector2f &avUVUpperRight,
                                const cVector2f &avUVLowerRight, const cVector2f &avUVLowerLeft) {
    mvTextures[mlTextureNum] = apTexture;
...
void cGuiGfxElement::SetMaterial(iGuiMaterial *apMat) { mpMaterial = apMat; }
...
cVector2f cGuiGfxElement::GetImageSize() { return mvImageSize; }
