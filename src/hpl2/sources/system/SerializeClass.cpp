...
cSerializeClass::cSerializeClass(const char *asName, const char *asParent, cSerializeMemberField *apMemberFields,
                                 size_t alSize, iSerializable *(*apCreateFunc)()) {
    gvSerializeTempClasses[glSerializeTempClassesNum] =
        cSerializeSavedClass(asName, asParent, apMemberFields, alSize, apCreateFunc);
...
void cSerializeClass::SetLog(bool abX) { gbLog = abX; }

bool cSerializeClass::GetLog() { return gbLog; }
...
void cSerializeClass::PrintMembers(iSerializable *apData) {
    ...
    while (classIt.HasNext()) {
        ...
        Log(" '%s':'%s' type: %d offset: %d\n", pField->msName.c_str(),
            ValueToString(apData, pField->mlOffset, pField->mType), pField->mType, pField->mlOffset);
    }
}
...
const char *cSerializeClass::ValueToString(void *apData, size_t alOffset, eSerializeType aType) {
    ...
    case eSerializeType_Matrixf: {
        ...
        sprintf(msTempCharArray,
                ...
                "%f %f %f %f "
                "%f %f %f %f",
                Mtx.m[0][0], Mtx.m[0][1], Mtx.m[0][2], Mtx.m[0][3], Mtx.m[1][0], Mtx.m[1][1], Mtx.m[1][2], Mtx.m[1][3],
                Mtx.m[2][0], Mtx.m[2][1], Mtx.m[2][2], Mtx.m[2][3], Mtx.m[3][0], Mtx.m[3][1], Mtx.m[3][2], Mtx.m[3][3]);
        return msTempCharArray;
        break;
    }
...
void cSerializeClass::LoadClass(TiXmlElement *apElement, iSerializable *apData, cSerializeSavedClass *apClass) {
    ...
    if (gbLog)
        Log("%s Saving class name: '%s' top class: '%s'\n", GetTabs(), sName.c_str(),
            pClassData->Serialize_GetTopClass().c_str());
...
void cSerializeClass::SetUpData() {
    ...
    for (int i = 0; i < glSerializeTempClassesNum; i++) {
        m_mapSavedClasses.insert(
            tSerializeSavedClassMap::value_type(tString(gvSerializeTempClasses[i].msName), gvSerializeTempClasses[i]));
    }
}
