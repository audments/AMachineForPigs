...
cLanguageFile::cLanguageFile(cResources *apResources) { mpResources = apResources; }
...
bool cLanguageFile::AddFromFile(const tString &asFile, bool abAddResourceDirs, const tWString &asAltPath) {
    ...
    for (; pCatElem != NULL; pCatElem = pCatElem->NextSiblingElement("CATEGORY")) {
        ...
        for (; pEntryElem != NULL; pEntryElem = pEntryElem->NextSiblingElement("Entry")) {
            ...
            std::pair<tLanguageEntryMap::iterator, bool> ret =
                pCategory->m_mapEntries.insert(tLanguageEntryMap::value_type(sEntryName, pEntry));
            if (ret.second == false) {
