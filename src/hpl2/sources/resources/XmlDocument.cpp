...
iXmlNode::~iXmlNode() { DestroyChildren(); }
...
cXmlElement *iXmlNode::GetFirstElement() { return static_cast<cXmlElement *>(GetFirstOfType(eXmlNodeType_Element)); }
...
void iXmlNode::AddChild(iXmlNode *apNode) { mlstChildren.push_back(apNode); }

void iXmlNode::DestroyChild(iXmlNode *apNode) { STLFindAndDelete(mlstChildren, apNode); }
...
cXmlNodeListIterator iXmlNode::GetChildIterator() { return cXmlNodeListIterator(&mlstChildren); }

//-----------------------------------------------------------------------

void iXmlNode::DestroyChildren() { STLDeleteAll(mlstChildren); }
...
iXmlDocument::iXmlDocument(const tString &asName) : cXmlElement(asName, NULL) { msFile = _W(""); }
...
bool iXmlDocument::Save() { return SaveToFile(msFile); }

bool iXmlDocument::SaveToFile(const tWString &asPath) { return SaveDataToFile(asPath); }
