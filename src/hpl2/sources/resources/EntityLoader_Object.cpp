...
static iCollideShape *GetBodyShape(cXmlElement *apBodyElem, iPhysicsWorld *apPhysicsWorld,
...
static iCollideShape *CreateCollideShape(cXmlElement *apShapeElem, iPhysicsWorld *apPhysicsWorld,
...
static iPhysicsJoint *CreateJoint(const tString &asEntityName, cXmlElement *apJointElem, iPhysicsWorld *apPhysicsWorld,
                                  tLoaderPhysicsBodyMap &a_setBodies, const cMatrixf &a_mtxTransform,
                                  const cVector3f &avScale) {
    ...
    if (jointType == ePhysicsJointType_Hinge) {
        iPhysicsJointHinge *pJoint =
            apPhysicsWorld->CreateJointHinge(sJointName, vPivot, vPinDir, pParentBody, pChildBody);
    ...
    // Ball
        ...
        iPhysicsJointBall *pJoint =
            apPhysicsWorld->CreateJointBall(sJointName, vPivot, vPinDir, pParentBody, pChildBody);
    ...
    // Slider
        ...
        iPhysicsJointSlider *pJoint =
            apPhysicsWorld->CreateJointSlider(sJointName, vPivot, vPinDir, pParentBody, pChildBody);
    ...
    // Screw
        ...
        iPhysicsJointScrew *pJoint =
            apPhysicsWorld->CreateJointScrew(sJointName, vPivot, vPinDir, pParentBody, pChildBody);
...
iEntity3D *cEntityLoader_Object::Load(const tString &asName, int alID, bool abActive, cXmlElement *apRootElem,
                                      const cMatrixf &a_mtxTransform, const cVector3f &avScale, cWorld *apWorld,
                                      const tString &asFileName, const tWString &asFullPath,
        ...
        while (submeshIt.HasNext()) {
            ...
            if (bHasSkeleton == false) {
                cMatrixf mtxLocalTransform =
                    GetMatrixFromVectors(pSubMeshElem->GetAttributeVector3f("WorldPos") * mvScale,
            ...
            while (entityIt.HasNext()) {
                ...
                if (sEntityType == "ParticleSystem") {
                    if (mbLoadParticleSystems) {
                        cParticleSystem *pPS =
                            cEngineFileLoading::LoadParticleSystem(pEntityElem, asName + "_", apWorld);
                        if (pPS)
                ...
                // Billboard
                    ...
                    if (mbLoadBillboards) {
                        cBillboard *pBillboard = cEngineFileLoading::LoadBillboard(
                            pEntityElem, asName + "_", apWorld, apWorld->GetResources(), mbLoadAsStatic,
                ...
                // Light
                    ...
                    if (mbLoadLights) {
                        iLight *pLight = cEngineFileLoading::LoadLight(pEntityElem, asName + "_", apWorld,
                                                                       apWorld->GetResources(), mbLoadAsStatic);
                        if (pLight)
    ...
    if (pPhysicsWorld) {
        ...
        if (pBodiesElem) {
            ...
            while (bodyIt.HasNext()) {
                ...
                // Material
                iPhysicsMaterial *pPhysicsMat =
                    pPhysicsWorld->GetMaterialFromName(pBodyElem->GetAttributeString("Material"));
                if (pPhysicsMat)
    ...
    if (mvBodies.empty() == false) {
        ...
        for (tEntity3DListIt it = lstTempEntities.begin(); it != lstTempEntities.end(); ++it) {
            ...
            if (pEntity->GetEntityType() == "SubMesh" && mpMesh->GetSkeleton() == false &&
    ...
    // Load Joints
    {
        ...
        if (JointsElem) {
            ...
            while (jointIt.HasNext()) {
                ...
                iPhysicsJoint *pJoint =
                    CreateJoint(asName, pJointElem, pPhysicsWorld, setBodies, a_mtxTransform, mvScale);
                if (pJoint) {
...
void cEntityLoader_Object::AttachBoneChild(cBoneState *apBoneState, const cMatrixf &a_mtxInvParent,
                                           iEntity3D *apChild) {
    // Calculate local matrix
    cMatrixf mtxLocalTransform = cMath::MatrixMul(a_mtxInvParent, apChild->GetWorldMatrix());
...
void cEntityLoader_Object::AttachBoneToBody(iPhysicsBody *apParentBody, const cMatrixf &a_mtxInvParent,
...
void cEntityLoader_Object::LoadAndAttachChildren(cXmlElement *apMainElem, iEntity3D *apEntityParent,
                                                 cBoneState *apBoneStateParent, tEntity3DList &a_lstChildList,
                                                 tNodeStateMap &a_mapBoneStates, bool abRemoveAttachedChild,
                                                 bool abIsBody) {
        ...
        if (bFound == false) {
            if (apEntityParent)
                Warning("Could not find child with unique ID %d to attach to entity '%s' in '%s'\n", lID,
                        apEntityParent->GetName().c_str(), msFileName.c_str());
            else
                Warning("Could not find child with unique ID %d to attach to bone '%s' in '%s'\n", lID,
                        apBoneStateParent->GetName().c_str(), msFileName.c_str());
        }
    }
}
