...
cHplMapShapeBody::~cHplMapShapeBody() { STLDeleteAll(mvColliders); }
...
static void *GetVertexBufferWithFormat(iVertexBuffer *apVtxBuffer, eVertexBufferElement aElement,
...
static void AddBinaryBufferDataWithFormat(cBinaryBuffer *apBuffer, void *apSrcData, size_t alSize,
                                          eVertexBufferElementFormat aFormat) {
    switch (aFormat) {
    case eVertexBufferElementFormat_Int:
...
static void GetBinaryBufferDataWithFormat(cBinaryBuffer *apBuffer, void *apDestData, size_t alSize,
                                          eVertexBufferElementFormat aFormat) {
    switch (aFormat) {
    case eVertexBufferElementFormat_Int:
...
void cWorldLoaderHplMap::LoadCacheFile(const tWString &asFile) {
    ...
    for (int mesh = 0; mesh < lStaticMeshEntities; ++mesh) {
        ...
        cMesh *pMesh =
            hplNew(cMesh, (sName, asFile, mpResources->GetMaterialManager(), mpResources->GetAnimationManager()));
        ...
        ////////////////////
        // Vertex data
        iVertexBuffer *pVtxBuff = mpGraphics->GetLowLevel()->CreateVertexBuffer(
            eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static, 0, 0);
        {
...
static bool SortStaticSubMeshesForBodies(const cHplMapPhysicsObject &aObjectDataA,
                                         const cHplMapPhysicsObject &aObjectDataB) {
    // Is shadow caster check
...
void cWorldLoaderHplMap::CombineAndCreateMeshesAndPhysics(tRenderableList *apObjectList) {
    ...
    for (int i = 0; it != apObjectList->end(); ++it, ++i) {
        ...
        if (bCreateBodies) {
            ...
            // Log("'%s': charcollider: %d\n", vPhysicsObjects[i].mpObject->GetName().c_str(),
            // vPhysicsObjects[i].mbCharCollider);
        }
    }
    ...
    for (size_t i = 0; i < vMeshObjects.size(); ++i) {
        ...
        if (gbLog)
            Log("  %d Checking '%s', material: %d, cast shadows: %d\n", i, pMeshObject->GetName().c_str(),
                pMeshObject->GetMaterial(), pMeshObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster));
        ...
        if (i < vMeshObjects.size() - 1) {
            ...
            if (pNextObject->GetMaterial() != pMeshObject->GetMaterial() ||
                pNextObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster) !=
    ...
    for (size_t i = 0; i < vPhysicsObjects.size(); ++i) {
        ...
        if (i < vMeshObjects.size() - 1) {
            ...
            if (lIndexCount > lIndexCountLimit || nextObject.mbCharCollider != physicsObject.mbCharCollider ||
                nextObject.mpPhysicsMaterial != physicsObject.mpPhysicsMaterial ||
                nextObject.mpObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster) !=
                    physicsObject.mpObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster)) {
...
void cWorldLoaderHplMap::CombineObjectsAndCreateMeshEntity(tRenderableVec &avObjects, int alFirstIdx, int alLastIdx) {
    ...
    for (int i = alFirstIdx; i <= alLastIdx; ++i) {
        ...
        cHplMapStaticUserData *pUserData =
            (cHplMapStaticUserData *)static_cast<cSubMeshEntity *>(avObjects[i])->GetUserData();
        ...
        if (gbLog)
            Log("   '%s' has %d vtx and %d idx\n", avObjects[i]->GetName().c_str(), pVtxBuffer->GetVertexNum(),
    ...
    iVertexBuffer *pVtxBuffer =
        mpGraphics->GetLowLevel()->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri,
                                                      eVertexBufferUsageType_Static, lTotalVtxAmount, lTotalIdxAmount);

    // Set up what data arrays to use
    const int lDataArrayNum = 5;
    cVertexDataArray lDataArrayTypes[lDataArrayNum] = {{eVertexBufferElement_Position, 4},
    ...
    for (int i = 0; i < lDataArrayNum; ++i) {
        pVtxBuffer->CreateElementArray(lDataArrayTypes[i].mType, eVertexBufferElementFormat_Float,
                                       lDataArrayTypes[i].mlElementNum);
    ...
    for (int vtxbuffer = alFirstIdx; vtxbuffer <= alLastIdx; ++vtxbuffer) {
        // Check if the sub mesh is visible, else skip
        cHplMapStaticUserData *pUserData =
            (cHplMapStaticUserData *)static_cast<cSubMeshEntity *>(avObjects[vtxbuffer])->GetUserData();
        if (pUserData->mbVisible == false)
            continue;
        ...
        iVertexBuffer *pTransformedVtxBuffer = pSubVtxBuffer->CreateCopy(
            eVertexBufferType_Software, eVertexBufferUsageType_Static, pSubVtxBuffer->GetVertexElementFlags());
        ...
        for (int i = 0; i < lDataArrayNum; ++i) {
            ...
            memcpy(pDataArray[i], pTransformedVtxBuffer->GetFloatArray(lDataArrayTypes[i].mType),
                   lAmount * sizeof(float));
    ...
    cMesh *pMesh =
        hplNew(cMesh, (sName, _W(""), mpResources->GetMaterialManager(), mpResources->GetAnimationManager()));
    ...
    // Set up variables
    pMeshEntity->SetRenderFlagBit(eRenderableFlag_ShadowCaster,
                                  pFirstObject->GetRenderFlagBit(eRenderableFlag_ShadowCaster));
...
void cWorldLoaderHplMap::CombineObjectsAndCreatePhysics(std::vector<cHplMapPhysicsObject> &avObjects, int alFirstIdx,
                                                        int alLastIdx) {
    if (gbLog)
        Log("  Combining objects %d -> %d\n", alFirstIdx, alLastIdx);
    ...
    for (int i = alFirstIdx; i <= alLastIdx; ++i) {
        ...
        if (gbLog)
            Log("   '%s' has %d vtx and %d idx\n", avObjects[i].mpObject->GetName().c_str(), pVtxBuffer->GetVertexNum(),
                pVtxBuffer->GetIndexNum());
    }
    if (gbLog)
        Log("   Total amount %d vtx and %d idx\n", lTotalVtxAmount, lTotalIdxAmount);
    ...
    // Create the vertex buffer (skipping color!)
    iVertexBuffer *pVtxBuffer =
        mpGraphics->GetLowLevel()->CreateVertexBuffer(eVertexBufferType_Software, eVertexBufferDrawType_Tri,
    ...
    for (int vtxbuffer = alFirstIdx; vtxbuffer <= alLastIdx; ++vtxbuffer) {
        ...
        // Do a special debug test and skip highpoly entities, loading the map faster.
        if ((mlCurrentFlags & eWorldLoadFlag_FastPhysicsLoad) &&
            pSubMesh->GetVertexBuffer()->GetIndexNum() > lMaxIndices)
            continue;
        ...
        iVertexBuffer *pSubVtxBuffer = pObject->GetVertexBuffer();
        iVertexBuffer *pTransformedVtxBuffer = pSubVtxBuffer->CreateCopy(
            eVertexBufferType_Software, eVertexBufferUsageType_Static, eVertexElementFlag_Position);
        pTransformedVtxBuffer->Transform(pObject->GetWorldMatrix());
        ...
        memcpy(pDataArray, pTransformedVtxBuffer->GetFloatArray(eVertexBufferElement_Position),
               lAmount * sizeof(float));
        pDataArray += lAmount;
...
void cWorldLoaderHplMap::CreateStaticObjectEntity(cXmlElement *apElement, tMeshEntityList &alstMeshEntities,
    ...
    pDebugMeshEntity->SetWorldMatrix(cMath::MatrixMul(cMath::MatrixRotate(vRotation,
    eEulerRotationOrder_XYZ),cMath::MatrixScale(vScale))); pDebugMeshEntity->SetPosition(vPosition); return;*/

    cMeshEntity *pMeshEntity = hplNew(cMeshEntity, (sName, pMesh, mpResources->GetMaterialManager(),
    ...
    ////////////////////////////////
    // Setup mesh entity
    pMeshEntity->SetWorldMatrix(
        cMath::MatrixMul(cMath::MatrixRotate(vRotation, eEulerRotationOrder_XYZ), cMath::MatrixScale(vScale)));
...
void cWorldLoaderHplMap::CreateSubMeshShapeBodies(cSubMeshEntity *apSubEnt, const cMatrixf &a_mtxTransform,
...
void cWorldLoaderHplMap::CreatePrimitive(cXmlElement *apElement, tMeshEntityList &alstMeshEntities,
    ...
    if (sType == "Plane") {
        ...
        // Create the mesh
        cMesh *pMesh = mpGraphics->GetMeshCreator()->CreatePlane(
            sName, vStartCorner, vEndCorner, vUVCorners[0], vUVCorners[1], vUVCorners[2], vUVCorners[3], sMaterial);
        ...
        // Create mesh entity
        pMeshEntity = hplNew(cMeshEntity, (sName, pMesh, mpResources->GetMaterialManager(),
                                           mpResources->GetMeshManager(), mpResources->GetAnimationManager()));
        pMeshEntity->SetRenderFlagBit(eRenderableFlag_ShadowCaster, bCastsShadows);
...
void cWorldLoaderHplMap::CreateDecal(cXmlElement *apElement, tMeshEntityList &alstMeshEntities,
    ...
    cMesh *pMesh =
        cEngineFileLoading::LoadDecalMeshHelper(pDecalMeshElem, mpGraphics, mpResources, sName, sMaterial, decalColor);
    if (pMesh == NULL)
        return;
    ...
    //////////////////////////
    // Create mesh entity
    pMeshEntity = hplNew(cMeshEntity, (sName, pMesh, mpResources->GetMaterialManager(), mpResources->GetMeshManager(),
...
void cWorldLoaderHplMap::CreateLoadedEntity(cXmlElement *apElement,
...
void cWorldLoaderHplMap::LoadEntity(const tString &asName, int alID, bool abActive, const cVector3f &avPos,
                                    const cVector3f &avRot, const cVector3f &avScale, cXmlElement *apElement) {
...
void cWorldLoaderHplMap::LoadArea(const tString &asName, int alID, bool abActive, const cVector3f &avPos,
                                  const cVector3f &avRot, const cVector3f &avScale, cXmlElement *apElement) {
    cMatrixf mtxTransform = cMath::MatrixRotate(avRot, eEulerRotationOrder_XYZ);
    mtxTransform.SetTranslation(avPos);
...
bool cWorldLoaderHplMap::CheckTransformValidity(const tString &asName, const cVector3f &avPos, const cVector3f &avRot,
                                                const cVector3f &avScale) {
