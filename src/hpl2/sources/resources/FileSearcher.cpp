...
cFileSearcher::cFileSearcher() { msNull = _W(""); }
...
void cFileSearcher::AddDirectory(const tWString &asSearchPath, const tString &asMask, bool abAddSubDirectories) {
    ...
    for (tWStringListIt it = lstFileNames.begin(); it != lstFileNames.end(); it++) {
        ...
        tWString sFilePath =
            cString::ReplaceCharToW(cPlatform::GetFullFilePath(cString::SetFilePathW(sFile, sPath)), _W("\\"), _W("/"));
        ;
        ...
        // cString::To8Char(sFilePath).c_str(), 	cString::GetHash(cString::To8Char(sFilePath)),
        //cString::GetHashW(sFilePath));
