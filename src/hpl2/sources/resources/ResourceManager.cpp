...
iResourceManager::iResourceManager(cFileSearcher *apFileSearcher, iLowLevelResources *apLowLevelResources,
...
cResourceBaseIterator iResourceManager::GetResourceBaseIterator() { return cResourceBaseIterator(&m_mapResources); }
...
void iResourceManager::DestroyAll() {
    ...
    while (it != m_mapResources.end()) {
        ...
        // Log(" res: %d ...", pResource);
        // Log(" res: '%s' / '%s': %d ...",pResource->GetName().c_str(),
        // cString::To8Char(pResource->GetFullPath()).c_str(),pResource->GetUserCount());
...
void iResourceManager::EndLoad() { mlTabCount--; }
...
void iResourceManager::RemoveResource(iResourceBase *apResource) {
    // Log("Removing resource name: '%s' path: '%s' ", apResource->GetName().c_str(),
    // cString::To8Char(apResource->GetFullPath()).c_str());
    ...
    if (it == m_mapResources.end()) {
        ...
        // cString::To8Char(apResource->GetFullPath()).c_str(),lHash);
