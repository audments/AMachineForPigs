...
bool iBitmapLoaderDevil::SaveBitmap(cBitmap *apBitmap, const tWString &asFile, tBitmapSaveFlag aFlags) {
    ...
    ilTexImage(apBitmap->GetWidth(), apBitmap->GetHeight(), apBitmap->GetDepth(), lBytesPerPixel, lILFormat,
               IL_UNSIGNED_BYTE, apBitmap->GetData(0, 0)->mpData);
...
static void ILAPIENTRY DevilClose(ILHANDLE apHandle) { fclose((FILE *)apHandle); }
static ILHANDLE ILAPIENTRY DevilOpen(const ILstring apFile) { return fopen(apFile, "wb"); }
static ILint ILAPIENTRY DevilPutc(ILubyte alData, ILHANDLE apHandle) { return fputc(alData, (FILE *)apHandle); }
...
static ILint ILAPIENTRY DevilTell(ILHANDLE apHandle) { return ftell((FILE *)apHandle); }
...
ILint ILAPIENTRY DevilGetC(ILHANDLE Handle) { return fgetc((FILE *)Handle); }
