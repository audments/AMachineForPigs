...
bool cMeshLoaderCollada::FillStructures(const tWString &asFile, tColladaImageVec *apColladaImageVec,
                                        tColladaTextureVec *apColladaTextureVec,
                                        tColladaMaterialVec *apColladaMaterialVec, tColladaLightVec *apColladaLightVec,
                                        tColladaGeometryVec *apColladaGeometryVec,
                                        tColladaControllerVec *apColladaControllerVec,
                                        tColladaAnimationVec *apColladaAnimVec, cColladaScene *apColladaScene,
                                        bool abCache) {
    ...
    // LOAD CACHE
    if (bLoadCache) {
        return LoadStructures(sCacheFile, apColladaImageVec, apColladaTextureVec, apColladaMaterialVec,
                              apColladaLightVec, apColladaGeometryVec, apColladaControllerVec, apColladaAnimVec,
                              apColladaScene);
    }
    ...
    while (pLibraryElem) {
        ...
        // Controller
        else if ((sType == "CONTROLLER" || sValue == "library_controllers") && apColladaGeometryVec &&
...
bool cMeshLoaderCollada::SaveStructures(const tWString &asFile, tColladaImageVec *apColladaImageVec,
                                        tColladaTextureVec *apColladaTextureVec,
                                        tColladaMaterialVec *apColladaMaterialVec, tColladaLightVec *apColladaLightVec,
                                        tColladaGeometryVec *apColladaGeometryVec,
                                        tColladaControllerVec *apColladaControllerVec,
        ...
        // if(false)
        {
            ...
            for (size_t idx = 0; idx < pGeometry->mvExtraVtxVec.size(); ++idx) {
                ...
                for (; it != pList->end(); ++it) {
                    ...
                    sData += cString::ToString(extra.mlVtx) + " " + cString::ToString(extra.mlNorm) + " " +
                             cString::ToString(extra.mlTex) + " " + cString::ToString(extra.mlNewVtx) + " ";
                    ++lExtraCount;
                    ++vExtraNum[idx];
                }
            }
...
bool cMeshLoaderCollada::LoadStructures(const tWString &asFile, tColladaImageVec *apColladaImageVec,
                                        tColladaTextureVec *apColladaTextureVec,
                                        tColladaMaterialVec *apColladaMaterialVec, tColladaLightVec *apColladaLightVec,
                                        tColladaGeometryVec *apColladaGeometryVec,
                                        tColladaControllerVec *apColladaControllerVec,
...
static void LoadControllerVec(TiXmlElement *apRootElem, tColladaControllerVec *apColladaControllerVec) {
    ...
    for (; pControllerElem != NULL; pControllerElem = pControllerElem->NextSiblingElement(), ++lCount) {
        ...
        pController->m_mtxBindShapeMatrix =
            cString::ToMatrixf(pControllerElem->Attribute("BindShapeMatrix"), cMatrixf::Identity);
        ...
        // Matrices
        {
            ...
            for (int i = 0; i < lSize; ++i) {
                pController->mvMatrices.push_back(cMatrixf(pData[0], pData[1], pData[2], pData[3], pData[4], pData[5],
