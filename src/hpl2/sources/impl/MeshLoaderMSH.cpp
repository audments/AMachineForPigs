    ...
    for (int sub = 0; sub < apMesh->GetSubMeshNum(); sub++) {
        ...
        // Add Vertices
        {
            ...
            for (int i = 0; i < eVertexBufferElement_LastEnum; i++) {
                ...
                if (gbLogMSHLoad)
                    Log("  Vtx %d: %d %d %d\n", i, arrayType, pVtxBuff->GetElementProgramVarIndex(arrayType),
                        lElementNum);
...
void cMeshLoaderMSH::AddAnimation(cAnimation *apAnimation, cBinaryBuffer *apBuffer) {
    ...
    if (gbLogMSHLoad)
        Log(" Animation %s: %f %d\n", apAnimation->GetName().c_str(), apAnimation->GetLength(),
            apAnimation->GetTrackNum());
    ...
    for (int track = 0; track < apAnimation->GetTrackNum(); ++track) {
        ...
        if (gbLogMSHLoad)
            Log("  Track %d %s: %d %d\n", track, pTrack->GetName().c_str(), pTrack->GetTransformFlags(),
                pTrack->GetKeyFrameNum());
...
cAnimation *cMeshLoaderMSH::GetAnimation(cBinaryBuffer *apBuffer, const tWString &asFullPath) {
    ...
    cAnimation *pAnimation = hplNew(
        cAnimation, (cString::To8Char(asFullPath), asFullPath, cString::GetFileName(cString::To8Char(asFullPath))));
...
void *cMeshLoaderMSH::GetVertexBufferWithFormat(iVertexBuffer *apVtxBuffer, eVertexBufferElement aElement,
...
void cMeshLoaderMSH::AddBinaryBufferDataWithFormat(cBinaryBuffer *apBuffer, void *apSrcData, size_t alSize,
                                                   eVertexBufferElementFormat aFormat) {
    switch (aFormat) {
    case eVertexBufferElementFormat_Int:
...
void cMeshLoaderMSH::GetBinaryBufferDataWithFormat(cBinaryBuffer *apBuffer, void *apDestData, size_t alSize,
                                                   eVertexBufferElementFormat aFormat) {
    switch (aFormat) {
    case eVertexBufferElementFormat_Int:
