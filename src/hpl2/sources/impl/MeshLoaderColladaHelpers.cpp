...
cMatrixf cMeshLoaderCollada::GetMatrix(const cMatrixf &amtxA) { return amtxA; }

cVector3f cMeshLoaderCollada::GetVectorPosFromPtr(float *apVec) { return cVector3f(apVec[0], apVec[1], apVec[2]); }

cVector3f cMeshLoaderCollada::GetVectorRotationFromPtr(float *apVec) { return cVector3f(apVec[0], apVec[1], apVec[2]); }

cVector3f cMeshLoaderCollada::GetVectorScaleFromPtr(float *apVec) { return cVector3f(apVec[0], apVec[1], apVec[2]); }
...
cAnimationTrack *cMeshLoaderCollada::CreateAnimTrack(cAnimation *apAnimation, cSkeleton *apSkeleton,
    ...
    for (size_t i = 0; i < aAnim.mvSamplers.size(); i++) {
        ...
        if (pTrans->msType == "translate") {
            ...
            for (size_t j = 0; j < pTimeVec->size(); j++) {
                ...
                if (sExt == "") {
                    pTempData->mvTrans =
                        cVector3f((*pValueVec)[j * 3 + 0], (*pValueVec)[j * 3 + 1], (*pValueVec)[j * 3 + 2]);
                }
                // X
                else if (sExt == "x") {
        ...
        // Add the rotation in this sampler
            ...
            for (size_t j = 0; j < pTimeVec->size(); j++) {
                ...
                pTempData->mvRot +=
                    (cVector3f(pTrans->mvValues[0], pTrans->mvValues[1], pTrans->mvValues[2]) * (*pValueVec)[j]);
            }
            ...
            for (size_t j = 0; j < vTempData.size(); ++j) {
                ...
                // Time between two keys
                    ...
                    vTempData[j].mvRot.v[lVecNum] =
                        pBefore->mvRot.v[lVecNum] * (1.0f - fT) + pAfter->mvRot.v[lVecNum] * fT;
        ...
        // Add the rotation and translation from a matrix in this sampler
            ...
            for (size_t j = 0; j < pTimeVec->size(); j++) {
                ...
                pTempData->mvRot +=
                    (cMath::Vector3ToDeg(cMath::MatrixToEulerAngles(mtxFrame, eEulerRotationOrder_XYZ)));
            }
    ...
    cAnimationTrack *pTrack =
        apAnimation->CreateTrack(aAnim.msTargetNode, eAnimTransformFlag_Rotate | eAnimTransformFlag_Translate);
    ////////////////////////////////////
...
void cMeshLoaderCollada::CalcLocalMatrixAfterControllerBindMatrixRec(cBone *apBone, cMatrixf a_mtxParentGlobal,
                                                                     int alDepth) {
...
//,tColladaExtraVtxListVec &vExtraVtxVec)
{
    ...
    iVertexBuffer *pVtxBuff =
        mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, aUsageType,
    ...
    // Add tangents
    memcpy(pVtxBuff->GetFloatArray(eVertexBufferElement_Texture1Tangent), &aGeometry.mvTangents[0],
...
void cMeshLoaderCollada::LoadColladaScene(TiXmlElement *apRootElem, cColladaNode *apParentNode, cColladaScene *apScene,
...
void cMeshLoaderCollada::LoadControllers(TiXmlElement *apRootElem, tColladaControllerVec &avColladaControllerVec,
...
void cMeshLoaderCollada::LoadGeometry(TiXmlElement *apRootElem, tColladaGeometryVec &avColladaGeometryVec) {
    ...
    for (; pGeomElem != NULL; pGeomElem = pGeomElem->NextSiblingElement("geometry")) {
        ...
        if (cString::Sub(Geometry.msName, 0, 4) == "_ref" || cString::Sub(Geometry.msName, 0, 3) == "_bb" ||
            cString::Sub(Geometry.msName, 0, 4) == "_ps" || cString::Sub(Geometry.msName, 0, 6) == "_sound") {
            continue;
        }
        ...
        for (size_t i = 0; i < Geometry.mvArrayVec.size(); i++) {
            if (Geometry.mvArrayVec[i].mbIsInVertex) {
                if (Geometry.mvArrayVec[i].msType == "NORMAL") {
                    ...
                    Geometry.mlTexArrayIdx = i; // PUDDLES: starting point, here the dae index list referring to a
                                                // texture mapping channel starts
        ...
        while (pPElem) {
            ...
            for (int triangle = 0; triangle < lTriangleNum; triangle++) {
                ...
                // Skipping this test right now as it is only really interesting for shadow volumes and it will destroy
                // double sided textures! std::pair<tColladaTestTriMapIt,bool> TestPair = map_TestTris.insert(DataVec);
                // if(TestPair.second)
                {
        ...
        if (pProfileCommon) {
            ...
            if (pDiffuseElem) {
                ...
                if (pLocalTexture) {
                    ...
                    Texture.msImage = "";
                    // Warning("No diffuse texture effect element for effect '%s'! No file texture file will be
                    // loaded.\n", 			Texture.msId.c_str());
                }
            } else {
...
void cMeshLoaderCollada::FillVertexVec(const char *apChars, tVector3fVec &avVtxVec, int alElements, int alVtxCount) {
    ...
    // Also, the biggie is probably string -> float, which is no different really. Just some extra copying to arrays
    // happen. Plus side is that it is a lot safer!
...
tString cMeshLoaderCollada::GetMaterialTextureFile(const tString &asMaterial, tColladaMaterialVec &avColladaMaterialVec,
...
static bool CompareVertex(cColladaSortVertex *apA, cColladaSortVertex *apB) {
    return apA->mVertex.pos == apB->mVertex.pos && apA->mVertex.norm == apB->mVertex.norm &&
    ...
    for (size_t i = 0; i < avColladaGeometryVec.size(); ++i) {
        ...
        for (size_t j = 0; j < vSortVertices.size(); j += lPairs) {
            ...
            while (j + lPairs < vSortVertices.size() && CompareVertex(vSortVertices[j], vSortVertices[lPairs + j])) {
                vSortVertices[lPairs + j]->mlMergedIndex =
                    vSortVertices[j]->mlOriginalIndex; // Set the merge id to the original id of this vertex
                lPairs++;
            }
        }
        ...
        for (size_t j = 0; j < pGeometry.mvIndexVec.size(); ++j) {
            ...
            if (lMergeID >= 0) {
                // This vertex has been merged with another vertex, get the position in the new array from the id of the
                // merged vertex
...
void cMeshLoaderCollada::GenerateTangents(cColladaGeometry &aGeometry) {
    ...
    cMath::CreateTriTangentVectors(&aGeometry.mvTangents[0], &aGeometry.mvIndexVec[0], (int)aGeometry.mvIndexVec.size(),
                                   &vPosVec[0], 4, &vTexVec[0], &vNormVec[0], (int)aGeometry.mvVertexVec.size());
}
