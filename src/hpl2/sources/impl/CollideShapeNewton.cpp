...
cCollideShapeNewton::cCollideShapeNewton(eCollideShapeType aType, const cVector3f &avSize, cMatrixf *apOffsetMtx,
                                         NewtonWorld *apNewtonWorld, iPhysicsWorld *apWorld)
    ...
    case eCollideShapeType_Sphere:
        mpNewtonCollision =
            NewtonCreateSphere(apNewtonWorld,
...
cVector3f cCollideShapeNewton::GetInertia(float afMass) {
    ...
    if (mType == eCollideShapeType_Box) {
        ...
        vInertia = cVector3f(afMass * (mvSize.y * mvSize.y + mvSize.z * mvSize.z) * fInv,
    ...
    // This is only a very bad approximation.
        ...
        float fInv = 1.0f / 12.0f;
        vInertia = cVector3f(afMass * (vBoxSize.y * vBoxSize.y + vBoxSize.z * vBoxSize.z) * fInv,
...
void cCollideShapeNewton::CreateStaticSceneFromShapeVec(tCollideShapeVec &avShapes, tMatrixfVec *apMatrices) {
    ...
    for (size_t i = 0; i < avShapes.size(); ++i) {
        ...
        NewtonSceneProxy *pProxy =
            NewtonSceneCollisionCreateProxy(mpNewtonCollision, pNewtonShape->GetNewtonCollision());
...
void cCollideShapeNewton::CreateCompoundFromShapeVec(tCollideShapeVec &avShapes) {
    ...
    mpNewtonCollision =
        NewtonCreateCompoundCollision(mpNewtonWorld, (int)vNewtonColliders.size(), &vNewtonColliders[0], 0);
    ...
    for (int tri = 0; tri < alIndexNum; tri += 3) {
        ...
        if (false) // bOptimize==false)
        {
            ...
            // First plane created.
                ...
                if (std::abs(plane.a - tempPlane.a) > 0.001f || std::abs(plane.b - tempPlane.b) > 0.001f ||
                    std::abs(plane.c - tempPlane.c) > 0.001f || std::abs(plane.d - tempPlane.d) > 0.001f) {
                    bOptimize = true;
                }
            }
        }
...
void cCollideShapeNewton::CreateFromSerializedData(cBinaryBuffer *apBinBuffer) {
    ...
    mpNewtonCollision =
        NewtonCreateCollisionFromSerialization(mpNewtonWorld, NewtonReadFromBinaryBuffer, (void *)apBinBuffer);
