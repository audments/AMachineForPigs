...
iVertexBufferOpenGL::iVertexBufferOpenGL(iLowLevelGraphics *apLowLevelGraphics, eVertexBufferType aType,
...
iVertexBufferOpenGL::~iVertexBufferOpenGL() { STLDeleteAll(mvElementArrays); }
...
void iVertexBufferOpenGL::AddIndex(unsigned int alIndex) { mvIndexArray.push_back(alIndex); }
...
bool iVertexBufferOpenGL::Compile(tVertexCompileFlag aFlags) {
    ...
    if (aFlags & eVertexCompileFlag_CreateTangents) {
        ...
        cMath::CreateTriTangentVectors((float *)pTangentElement->GetArrayPtr(), &mvIndexArray[0], GetIndexNum(),
                                       ...
                                       (float *)pTextureElement->GetArrayPtr(), (float *)pNormalElement->GetArrayPtr(),
                                       GetVertexNum());
    }
...
unsigned int *iVertexBufferOpenGL::GetIndices() { return &mvIndexArray[0]; }
...
void iVertexBufferOpenGL::ResizeIndices(int alSize) { mvIndexArray.resize(alSize); }
...
iVertexBuffer *iVertexBufferOpenGL::CreateCopy(eVertexBufferType aType, eVertexBufferUsageType aUsageType,
    ...
    if (mpLowLevelGraphics->GetCaps(eGraphicCaps_VertexBufferObject) && aType == eVertexBufferType_Hardware) {
        pVtxBuff =
            hplNew(cVertexBufferOGL_VBO, (mpLowLevelGraphics, mDrawType, aUsageType, GetIndexNum(), GetVertexNum()));
    } else {
        pVtxBuff =
            hplNew(cVertexBufferOGL_Array, (mpLowLevelGraphics, mDrawType, aUsageType, GetIndexNum(), GetVertexNum()));
    }
    ...
    for (size_t i = 0; i < mvElementArrays.size(); ++i) {
        ...
        pVtxBuff->CreateElementArray(pSrcElement->mType, pSrcElement->mFormat, pSrcElement->mlElementNum,
                                     pSrcElement->mlProgramVarIndex);
...
int iVertexBufferOpenGL::GetIndexNum() { return (int)mvIndexArray.size(); }
