...
bool cSDLFontData::CreateFromBitmapFile(const tWString &asFileName) {
    ...
    mvSizeRatio.x = (float)(lBase + lOutline) /
                    (float)(lLineHeight + lOutline); // I think this is a not correct. Not sure what is done here :S
    mvSizeRatio.y = 1;
    ...
    for (; pPageElem != NULL; pPageElem = pPageElem->NextSiblingElement("page"), ++lCount) {
        ...
        ///////////////////////
        // Create a texture from bitmap (do not want to load it from texture manager since that would delete the texture
        // on its own).
