...
void cPlatform::RemoveFile(const tWString &asFilePath) { _wremove(asFilePath.c_str()); }
...
bool cPlatform::RemoveFolder(const tWString &asPath, bool abDeleteAllFiles, bool abDeleteAllSubFolders) {
    ...
    if (RemoveDirectory(sPath.c_str()) != TRUE) {
        ...
        Error("Could not remove folder: '%s': %s", cString::To8Char(sPath).c_str(),
              cString::To8Char(sTempString).c_str());
        return false;
    }
    return true;
}
...
void cPlatform::SetFileModifiedDate(const tWString &asFilePath, cDate aDate) {
    ...
    if (pGmtClock && pLocalClock) {
        ...
        HANDLE lFile = CreateFile(asFilePath.c_str(), GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING,
...
void cPlatform::FindFilesInDir(tWStringList &alstStrings, const tWString &asDir, const tWString &asMask,
                               bool abAddHidden) {
...
void cPlatform::FindFoldersInDir(tWStringList &alstStrings, const tWString &asDir, bool abAddHidden,
                                 bool abAddUpFolder) {
    // these windows functions only works with "\".. sucks ...
    tWString sDir = cString::ReplaceCharToW(asDir, _W("/"), _W("\\"));
...
ePlatform cPlatform::GetPlatform() { return ePlatform_Win32; }
...
unsigned long cPlatform::GetSystemAvailableDrives() { return _getdrives(); }
...
bool cPlatform::RunProgram(const tWString &asPath, const tWString &asParams) {
    ...
    return CreateProcess(asPath.c_str(), (LPWSTR)sCommandLine.c_str(), NULL, NULL, false, 0, NULL, NULL, &si, &pi) ==
           TRUE;
}
...
bool cPlatform::OpenFileOnShell(const tWString &asPath) {
    ...
    case SE_ERR_NOASSOC:
        sMessage +=
            _W("No associated app found to open files of type: ") + cString::ToUpperCaseW(cString::GetFileExtW(asPath));
        break;
