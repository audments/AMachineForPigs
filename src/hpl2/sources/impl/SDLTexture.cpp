...
cSDLTexture::cSDLTexture(const tString &asName, eTextureType aType, eTextureUsage aUsage,
...
bool cSDLTexture::CreateCubeFromBitmapVec(std::vector<cBitmap *> *avBitmaps) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        bool bRet = CreateTexture(mvTextureHandles[0], pBmp->GetData(0, 0), pBmp->GetNumOfMipMaps(), pBmp->GetSize(),
                                  pBmp->GetPixelFormat(), i, mbUseMipMaps, true);
        if (bRet == false)
            return false;
    }
...
void cSDLTexture::SetRawData(int alLevel, const cVector3l &avOffset, const cVector3l &avSize, ePixelFormat aPixelFormat,
                             void *apData) {
    ;
    ...
    if (mType == eTextureType_1D) {
        ...
        glTexSubImage2D(GLTarget, alLevel, avOffset.x, avOffset.y, avSize.x, avSize.y, GLFormat, GL_UNSIGNED_BYTE,
                        apData);
    } else if (mType == eTextureType_3D) {
        glTexSubImage3D(GLTarget, alLevel, avOffset.x, avOffset.y, avOffset.z, avSize.x, avSize.y, avSize.z, GLFormat,
...
bool cSDLTexture::HasAnimation() { return mvTextureHandles.size() > 1; }
...
float cSDLTexture::GetT() { return cMath::Modulus(mfTimeCount, 1.0f); }

float cSDLTexture::GetTimeCount() { return mfTimeCount; }
void cSDLTexture::SetTimeCount(float afX) { mfTimeCount = afX; }
int cSDLTexture::GetCurrentLowlevelHandle() { return GetTextureHandle(); }
...
bool cSDLTexture::CreateFromBitmapToIndex(cBitmap *apBmp, int alIdx) {
    ...
    if (mType == eTextureType_CubeMap) {
        ...
        if (apBmp->GetNumOfImages() < 6) {
            Error("Bitmap has to few images (%d) to create cubmap texture '%s'\n", apBmp->GetNumOfImages(),
                  msName.c_str());
            return false;
        }

        for (int i = 0; i < 6; ++i) {
            bool bRet = CreateTexture(mvTextureHandles[alIdx], apBmp->GetData(i, 0), apBmp->GetNumOfMipMaps(),
                                      apBmp->GetSize(), apBmp->GetPixelFormat(), i, mbUseMipMaps, true);
            if (bRet == false)
                return false;
        }
    ...
    // Create 1d, 2d or 3d texture
        ...
        bool bRet = CreateTexture(mvTextureHandles[alIdx], apBmp->GetData(0, 0), apBmp->GetNumOfMipMaps(),
                                  apBmp->GetSize(), apBmp->GetPixelFormat(), 0, mbUseMipMaps, true);
        if (bRet == false)
            return false;
    }
...
bool cSDLTexture::CreateTexture(int alTextureHandle, cBitmapData *apBitmapImage, int alNumOfMipMaps,
                                const cVector3l avSize, ePixelFormat aPixelFormat, int alFaceNum,
                                bool abGenerateMipMaps, bool abCheckForResize) {
    /////////////////////////////
    ...
    if (mlSizeDownScaleLevel > 0 && abCheckForResize && (mbIsCompressed == false || alNumOfMipMaps > 1) &&
        ...
        // Resize by changing data (do only for 2D textures)
        else if ((mType == eTextureType_2D || mType == eTextureType_CubeMap) && mvSize.x > mvMinDownScaleSize.x &&
                 mvSize.y > mvMinDownScaleSize.y) {
            ...
            gluScaleImage(GLFormat, mvSize.x, mvSize.y, GL_UNSIGNED_BYTE, apBitmapImage->mpData, vNewSize.x, vNewSize.y,
                          GL_UNSIGNED_BYTE, pResizeData);
    ...
    // lStartMipMapLevel,alNumOfMipMaps, abGenerateMipMaps);
...
void cSDLTexture::GenerateMipMaps(GLenum aGLTarget, ePixelFormat aPixelFormat, const cVector3l avSize,
    ...
    if (mUsage == eTextureUsage_RenderTarget) {
        ...
        if (mType == eTextureType_1D) {
            ...
            gluBuild2DMipmaps(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum
                                                            : aGLTarget,
                              GLInternalFormat, avSize.x, avSize.y, GLFormat, GL_UNSIGNED_BYTE, apData);
        }
...
bool cSDLTexture::CopyTextureDataToGL(int alTextureHandle, int alLevel, unsigned char *apData, int alDataSize,
    ...
    if (PixelFormatIsCompressed(aPixelFormat)) {
        ...
        if (mType == eTextureType_1D) {
            ...
            glCompressedTexImage2DARB(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum
                                                                    : GLTarget,
                                      alLevel, GLCompressionFormat, avSize.x, avSize.y, 0, alDataSize, apData);
        } else if (mType == eTextureType_3D) {
            glCompressedTexImage3DARB(GLTarget, alLevel, GLCompressionFormat, avSize.x, avSize.y, avSize.z, 0,
                                      alDataSize, apData);
        }
    }
    /////////////////////////////////////////
    // Load uncompressed data
        ...
        if (mType == eTextureType_1D) {
            ...
            glTexImage2D(mType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alFaceNum : GLTarget,
                         alLevel, GLInternalFormat, avSize.x, avSize.y, 0, GLFormat, glType, apData);
        } else if (mType == eTextureType_3D) {
            glTexImage3D(GLTarget, alLevel, GLInternalFormat, avSize.x, avSize.y, avSize.z, 0, GLFormat, glType,
                         apData);
        }
    }
