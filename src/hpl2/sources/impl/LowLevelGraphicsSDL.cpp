...
void CALLBACK OGLDebugOutputCallback(GLenum alSource, GLenum alType, GLuint alID, GLenum alSeverity, GLsizei alLength,
                                     const GLchar *apMessage, GLvoid *apUserParam) {
...
bool cLowLevelGraphicsSDL::Init(int alWidth, int alHeight, int alDisplay, int alBpp, int abFullscreen,
    ...
    mpScreen = SDL_CreateWindow(asWindowCaption.c_str(), SDL_WINDOWPOS_CENTERED_DISPLAY(mlDisplay),
                                SDL_WINDOWPOS_CENTERED_DISPLAY(mlDisplay), mvScreenSize.x, mvScreenSize.y, mlFlags);
    if (mpScreen == NULL) {
        // try disabling FSAA
...
void cLowLevelGraphicsSDL::SetupGL() {
    ...
    glScissor(mvScissorPos.x, (mvScissorSize.y - mvScissorPos.y - 1) - mvScissorSize.y, mvScissorSize.x,
              mvScissorSize.y);
    glDisable(GL_BLEND);
...
void cLowLevelGraphicsSDL::SetRelativeMouse(bool abX) { SDL_SetRelativeMouseMode(abX ? SDL_TRUE : SDL_FALSE); }

void cLowLevelGraphicsSDL::SetWindowCaption(const tString &asName) { SDL_SetWindowTitle(mpScreen, asName.c_str()); }
...
bool cLowLevelGraphicsSDL::GetWindowIsVisible() { return (SDL_GetWindowFlags(mpScreen) & SDL_WINDOW_SHOWN) != 0; }
...
iVertexBuffer *cLowLevelGraphicsSDL::CreateVertexBuffer(eVertexBufferType aType, eVertexBufferDrawType aDrawType,
...
iDepthStencilBuffer *cLowLevelGraphicsSDL::CreateDepthStencilBuffer(const cVector2l &avSize, int alDepthBits,
                                                                    int alStencilBits) {
    ;
...
void cLowLevelGraphicsSDL::CopyFrameBufferToTexure(iTexture *apTex, const cVector2l &avPos, const cVector2l &avSize,
    ...
    SetTexture(0, apTex);
    glCopyTexSubImage2D(GetGLTextureTargetEnum(apTex->GetType()), 0, vTexPos.x, vTexPos.y, vPos.x, vPos.y, vSize.x,
                        vSize.y);
}
...
cBitmap *cLowLevelGraphicsSDL::CopyFrameBufferToBitmap(const cVector2l &avScreenPos, const cVector2l &avScreenSize) {
    ...
    //////////////////////////////
    // Copy pixels
    glReadPixels(vPos.x, vPos.y, vSize.x, vSize.y, PixelFormatToGLFormat(pixelFormat), GL_UNSIGNED_BYTE,
...
void cLowLevelGraphicsSDL::SetCurrentFrameBuffer(iFrameBuffer *apFrameBuffer, const cVector2l &avPos,
...
void cLowLevelGraphicsSDL::SwapBuffers() { SDL_GL_SwapWindow(mpScreen); }
...
void cLowLevelGraphicsSDL::SetStencil(eStencilFunc aFunc, int alRef, unsigned int aMask, eStencilOp aFailOp,
                                      eStencilOp aZFailOp, eStencilOp aZPassOp) {
    ;
...
void cLowLevelGraphicsSDL::SetStencilTwoSide(eStencilFunc aFrontFunc, eStencilFunc aBackFunc, int alRef,
    ...
    // Ati implementation
        ...
        // Front
        glStencilOpSeparateATI(GL_FRONT, GetGLStencilOpEnum(aFrontFailOp), GetGLStencilOpEnum(aFrontZFailOp),
                               GetGLStencilOpEnum(aFrontZPassOp));
        // Back
        glStencilOpSeparateATI(GL_BACK, GetGLStencilOpEnum(aBackFailOp), GetGLStencilOpEnum(aBackZFailOp),
                               GetGLStencilOpEnum(aBackZPassOp));
...
void cLowLevelGraphicsSDL::SetBlendFuncSeparate(eBlendFunc aSrcFactorColor, eBlendFunc aDestFactorColor,
    ...
    if (GLEW_EXT_blend_func_separate) {
        glBlendFuncSeparateEXT(GetGLBlendEnum(aSrcFactorColor), GetGLBlendEnum(aDestFactorColor),
...
void cLowLevelGraphicsSDL::DrawQuad(const cVector3f &avPos, const cVector2f &avSize, const cVector2f &avMinTexCoord,
                                    const cVector2f &avMaxTexCoord, const cColor &aColor) {
    ;
...
void cLowLevelGraphicsSDL::DrawQuad(const cVector3f &avPos, const cVector2f &avSize, const cVector2f &avMinTexCoord0,
                                    const cVector2f &avMaxTexCoord0, const cVector2f &avMinTexCoord1,
                                    const cVector2f &avMaxTexCoord1, const cColor &aColor) {
    ;
    ...
    glBegin(GL_QUADS);
    {
        for (int i = 0; i < 4; i++) {
            ...
            for (int unit = 0; unit < lExtraUnits; ++unit) {
                glMultiTexCoord3fARB(GL_TEXTURE0_ARB + unit + 1, avExtraUvs[unit * 4 + i].x, avExtraUvs[unit * 4 + i].y,
                                     avExtraUvs[unit * 4 + i].z);
            }
...
void cLowLevelGraphicsSDL::DrawLine(const cVector3f &avBegin, const cColor &aBeginCol, const cVector3f &avEnd,
...
void cLowLevelGraphicsSDL::DrawSphere(const cVector3f &avPos, float afRadius, cColor aColX, cColor aColY,
                                      cColor aColZ) {
    ;
