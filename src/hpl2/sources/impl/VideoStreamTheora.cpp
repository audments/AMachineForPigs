...
cVideoStreamTheora_Loader::cVideoStreamTheora_Loader() {
    ...
    for (int i = 0; i < 256; ++i)
        for (int j = 0; j < 256; ++j) {
            ...
            mpYuv_G_UV[i * 256 + j] =
                (unsigned short)cMath::Clamp(((fUV - fMinVal) / (fMaxVal - fMinVal)) * 1023.0f, 0, 1023.0f);
        }
...
void cVideoStreamTheora::Play() { mbPlaying = true; }
...
void cVideoStreamTheora::Pause(bool abX) { mbPaused = abX; }
//-----------------------------------------------------------------------

void cVideoStreamTheora::SetLoop(bool abX) { mbLooping = abX; }
            ...
            if (lPackCount == 0) {
                ...
                if (ogg_page_bos(&page)) {
                    ...
                    if (testPacket.packet[0] & 0x80) {
                        memcpy(&mTheoraStreamState, &testStream,
                               sizeof(testStream)); // Copy contents of test stream to Theora stream
                        lPackCount = 1;
                    }
...
bool cVideoStreamTheora::GetHeaders() {
    ...
    while (!bFoundHeaders) {
        ...
        while (ogg_sync_pageout(&mOggSyncState, &page) > 0) {
            ...
            ogg_stream_state testStream;
            ogg_stream_init(&testStream,
                            ogg_page_serialno(&page)); // Init stream using logical serial to stream page belongs to.
            ogg_stream_pagein(&testStream, &page);     // Put current page into stream.
