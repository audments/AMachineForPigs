...
void cLowLevelSoundOpenAL::UpdateSound(float afTimeStep) { OAL_Update(); }
...
void cLowLevelSoundOpenAL::SetListenerAttenuation(bool abEnabled) { mbListenerAttenuation = abEnabled; }

//-----------------------------------------------------------------------

void cLowLevelSoundOpenAL::SetSetRolloffFactor(float afFactor) { OAL_SetRollOffFactor(afFactor); }
...
void cLowLevelSoundOpenAL::Init(int alSoundDeviceID, bool abUseEnvAudio, int alMaxChannels, int alStreamUpdateFreq,
                                bool abUseThreading, bool abUseVoiceManagement, int alMaxMonoSourceHint,
                                int alMaxStereoSourceHint, int alStreamingBufferSize, int alStreamingBufferCount,
    ...
    Log("  Streaming setup: %d Buffers x %d bytes each\n", OAL_Info_GetStreamBufferCount(),
        OAL_Info_GetStreamBufferSize());
...
void cLowLevelSoundOpenAL::FadeSoundEnvironment(iSoundEnvironment *apSourceSoundEnv, iSoundEnvironment *apDestSoundEnv,
                                                float afT) {
    if (!mbEnvAudioEnabled)
        return;
    ...
    if (pSrcEnv == NULL) {
        ...
        */
        ...
        pEnv.SetAirAbsorptionGainHF(pSrcEnv->GetAirAbsorptionGainHF() * fOneMinusT +
                                    pDstEnv->GetAirAbsorptionGainHF() * afT);
