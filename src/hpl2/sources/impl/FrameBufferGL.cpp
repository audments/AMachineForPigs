...
cDepthStencilBufferGL::cDepthStencilBufferGL(const cVector2l &avSize, int alDepthBits, int alStencilBits)
    ...
    if (alDepthBits > 0 && alStencilBits > 0) {
        ...
        if (alDepthBits != 24 || alStencilBits != 8) {
            Error("Depth-stencil has %d depth bits and %d stencil bits! Only a combination of 24 depth bits and 8 "
                  "stencil bits is supported!\n",
...
//-----------------------------------------------------------------------
void cFrameBufferGL::AttachTexture(int alAttachmentType, int alAttachmentIdx, iTexture *apTexture, int alMipmapLevel,
                                   int alExtra) {
    ;
    ...
    if (texType == eTextureType_2D || texType == eTextureType_Rect || texType == eTextureType_CubeMap) {
        glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, alAttachmentType + alAttachmentIdx,
                                  texType == eTextureType_CubeMap ? GL_TEXTURE_CUBE_MAP_POSITIVE_X_ARB + alExtra
                                                                  : GLTarget,
    ...
    // 3D Texture
        ...
        glFramebufferTexture3DEXT(GL_FRAMEBUFFER_EXT, alAttachmentType + alAttachmentIdx, GLTarget,
                                  pTextureSDL->GetTextureHandle(), alMipmapLevel, alExtra);
    }
    glBindFramebufferEXT(GL_FRAMEBUFFER_EXT, 0);
