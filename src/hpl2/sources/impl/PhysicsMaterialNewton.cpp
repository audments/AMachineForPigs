...
cPhysicsMaterialNewton::~cPhysicsMaterialNewton() { /*Might be just as well to let newton handle this*/ }
...
float cPhysicsMaterialNewton::GetElasticity() const { return mfElasticity; }
...
float cPhysicsMaterialNewton::GetStaticFriction() const { return mfStaticFriction; }
...
float cPhysicsMaterialNewton::GetKineticFriction() const { return mfKineticFriction; }
...
ePhysicsMaterialCombMode cPhysicsMaterialNewton::GetFrictionCombMode() const { return mFrictionMode; }
...
ePhysicsMaterialCombMode cPhysicsMaterialNewton::GetElasticityCombMode() const { return mElasticityMode; }
...
void cPhysicsMaterialNewton::UpdateMaterials() {
    ...
    while (MatIt.HasNext()) {
        ...
        ePhysicsMaterialCombMode elasticityMode =
            (ePhysicsMaterialCombMode)std::max(mElasticityMode, pMat->mElasticityMode);
        ...
        NewtonMaterialSetCollisionCallback(mpNewtonWorld, mlMaterialId, pMat->mlMaterialId, (void *)NULL,
...
int cPhysicsMaterialNewton::OnAABBOverlapCallback(const NewtonMaterial *apMaterial, const NewtonBody *apBody1,
                                                  const NewtonBody *apBody2, int alThreadIndex) {
...
void cPhysicsMaterialNewton::ContactsProcessCallback(const NewtonJoint *apContactJoint, dFloat afTimestep,
                                                     int alThreadIndex) {
    ////////////////////////////////
    // Get bodies
    ...
    if (pMaterial1->GetSurfaceData() && pMaterial2->GetSurfaceData() && pContactBody1->GetUseSurfaceEffects() &&
        pContactBody2->GetUseSurfaceEffects() && pContactBody1->GetBuoyancyActive() == false &&
        pContactBody2->GetBuoyancyActive() == false) {
        pMaterial1->GetSurfaceData()->CreateImpactEffect(contactData.mfMaxContactNormalSpeed,
                                                         contactData.mvContactPosition, lContactNum,
                                                         pMaterial2->GetSurfaceData(), pContactBody1->GetWorld());
        ...
        if (lPrio1 >= lPrio2) {
            if (std::abs(contactData.mfMaxContactNormalSpeed) > 0)
                pMaterial1->GetSurfaceData()->OnImpact(contactData.mfMaxContactNormalSpeed,
                                                       contactData.mvContactPosition, lContactNum, pContactBody1);
        ...
        if (lPrio2 >= lPrio1 && pMaterial2 != pMaterial1) {
            if (std::abs(contactData.mfMaxContactNormalSpeed) > 0)
                pMaterial2->GetSurfaceData()->OnImpact(contactData.mfMaxContactNormalSpeed,
                                                       contactData.mvContactPosition, lContactNum, pContactBody2);
            if (std::abs(contactData.mfMaxContactTangentSpeed) > 0)
