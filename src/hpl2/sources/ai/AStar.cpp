...
cAStarHandler::~cAStarHandler() { m_setOpenList.clear(); }
...
bool cAStarHandler::GetPath(const cVector3f &avStart, const cVector3f &avGoal, tAINodeList *apNodeList) {
    ...
    if (fHeight <= fMaxHeight &&
        mpContainer->FreePath(avStart, avGoal, -1, eAIFreePathFlag_SkipDynamic | eAIFreePathFlag_SkipVolatile)) {
        mpGoalNode = NULL;
        return true;
    }
...
void cAStarHandler::IterateAlgorithm() {
    ...
    while (m_setOpenList.empty() == false && m_setGoalNodes.empty() == false &&
           (mlMaxIterations < 0 || lIterationCount < mlMaxIterations)) {
        ///////////////
    ...
    if (pNode->mlAStarCount == mlAStarCount) {
        ...
        if (afDistance > pNode->mfDistance) {
            if ((pNode->mFlags & eAStarNodeFlag_CastRay) && apParent != NULL) {
                ...
                if (mpContainer->FreePath(mvStart, apAINode->GetPosition(), -1,
                                          eAIFreePathFlag_SkipDynamic | eAIFreePathFlag_SkipVolatile)) {
...
cAStarNode *cAStarHandler::GetBestNode() {
    ...
    while (it != m_setOpenList.end()) {
        ...
        if (pNode->mFlags & eAStarNodeFlag_CastRay) {
            ...
            if (mpContainer->FreePath(mvStart, pNode->mpAINode->GetPosition(), -1,
                                      eAIFreePathFlag_SkipDynamic | eAIFreePathFlag_SkipVolatile)) {
...
bool cAStarHandler::IsGoalNode(cAINode *apAINode) {
    ...
    if (mpContainer->FreePath(pAINode->GetPosition(), mvGoal, -1,
                              eAIFreePathFlag_SkipDynamic | eAIFreePathFlag_SkipVolatile)) {
        //////////
