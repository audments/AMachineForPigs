...
bool cAINodeRayCallback::Intersected() { return mbIntersected; }
...
cAINodeContainer::cAINodeContainer(const tString &asName, const tString &asNodeName, cWorld *apWorld,
                                   const cVector3f &avCollideSize) {
    mpWorld = apWorld;
...
void cAINodeContainer::ReserveSpace(size_t alReserveSpace) { mvNodes.reserve(alReserveSpace); }
...
int cAINodeContainer::GetNodeNum() const { return (int)mvNodes.size(); }
...
void cAINodeContainer::BuildNodeGridMap() {
    ...
    for (size_t i = 0; i < mvNodes.size(); ++i) {
        ...
        if (false)
            Log("Adding node %d, world: (%s) local (%s), at %d : %d\n", i, pNode->GetPosition().ToString().c_str(),
                vLocalPos.ToString().c_str(), vGridPos.x, vGridPos.y);
...
static const cVector2f gvPosAdds[] = {cVector2f(0, 0),      cVector2f(1, 0),      cVector2f(-1, 0),
                                      cVector2f(0, 1),      cVector2f(0, -1),     cVector2f(0.5, 0.5),
                                      cVector2f(0.5, -0.5), cVector2f(-0.5, 0.5), cVector2f(-0.5, -0.5)};

bool cAINodeContainer::FreePath(const cVector3f &avStart, const cVector3f &avEnd, int alRayNum, tAIFreePathFlag aFlags,
                                iAIFreePathCallback *apCallback) {
        ...
        for (; pEdgeElem != NULL; pEdgeElem = pEdgeElem->NextSiblingElement("Edge")) {
            ...
            if (pEdgeNode == NULL) {
                Error("Could not find edge node in node %d with name '%s' in node container cache '%s'\n", lID,
                      sNodeName.c_str(), cString::To8Char(asFile).c_str());
                continue;
            }
