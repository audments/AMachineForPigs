...
iPhysicsRope::iPhysicsRope(const tString &asName, iPhysicsWorld *apWorld, const cVector3f &avStartPos,
...
bool iPhysicsRope::GetAutoMoveActive() { return mbAutoMoveActive; }
...
void iPhysicsRope::SetAttachedEndBody(iPhysicsBody *apBody) { SetAttachedBody(1, GetEndParticle(), apBody); }
...
void iPhysicsRope::SetMinTotalLength(float afX) { mfMinTotalLength = afX; }

void iPhysicsRope::SetMaxTotalLength(float afX) { mfMaxTotalLength = afX; }
...
void iPhysicsRope::UpdateConstraints(float afTimeStep) {
    ...
    for (int lCount = 0; it != mlstParticles.end(); ++it, ++lCount) {
        ...
        if (lCount > 0) {
            ...
            // Log("Updating %d, pos1: (%s) pos2: (%s)\n", lCount, pPrevPart->mvPosition.ToString().c_str(),
            // pPart->mvPosition.ToString().c_str());
...
void iPhysicsRope::CalculateSmoothPositions(float afTimeStep) {
    ...
    for (int i = 0; i < 2; ++i) {
        if (mvAttachedBody[i].mpBody) {
            cVector3f vWantedPos =
                cMath::MatrixMul(mvAttachedBody[i].mpBody->GetLocalMatrix(), mvAttachedBody[i].mvBodyLocalPos);
