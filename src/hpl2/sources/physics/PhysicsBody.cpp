...
iPhysicsBody::iPhysicsBody(const tString &asName, iPhysicsWorld *apWorld, iCollideShape *apShape) : iEntity3D(asName) {
    mpWorld = apWorld;
    mpShape = apShape;
...
void iPhysicsBody::AddJoint(iPhysicsJoint *apJoint) { mvJoints.push_back(apJoint); }

iPhysicsJoint *iPhysicsBody::GetJoint(int alIndex) { return mvJoints[alIndex]; }

int iPhysicsBody::GetJointNum() { return (int)mvJoints.size(); }
...
void iPhysicsBody::UpdateAfterSimulate(float afTimeStep) {
    ...
    if (mbStaticMovement) {
        ...
        if (mvTotalStaticAngularVel != 0) {
            ...
            mtxRot = cMath::MatrixMul(
                cMath::MatrixRotate(mvTotalStaticAngularVel * afTimeStep, eEulerRotationOrder_XYZ), mtxRot);
...
void iPhysicsBody::AddBodyCallback(iPhysicsBodyCallback *apCallback) { mlstBodyCallbacks.push_back(apCallback); }
...
iPhysicsMaterial *iPhysicsBody::GetMaterial() { return mpMaterial; }

//-----------------------------------------------------------------------

iCollideShape *iPhysicsBody::GetShape() { return mpShape; }
