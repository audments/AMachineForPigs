...
void cPhysics::Init(cResources *apResources) { mpResources = apResources; }

//-----------------------------------------------------------------------

void cPhysics::Update(float afTimeStep) { UpdateImpactCounts(afTimeStep); }
...
void cPhysics::AddImpact() { mlstImpactCounts.push_back(cPhysicsImpactCount()); }
    ...
    for (; pChildElem != NULL; pChildElem = pChildElem->NextSiblingElement("Material")) {
        ...
        if (cHaptic::GetIsUsed() && apHaptic) {
            ...
            if (surfaceType == eHapticSurfaceType_Simple) {
                ...
                pSurface = apHaptic->GetLowLevel()->CreateRoughSurface(sName, fStickyStiffness, fMean, fDamping,
                                                                       fStiffness, fDynamicFriction, fStartingFriction,
                                                                       fTangentStiffness, fStoppingFriction);
            } else if (surfaceType == eHapticSurfaceType_Sticky) {
                pSurface = apHaptic->GetLowLevel()->CreateStickySurface(sName, fDeadHeight, fStickyStiffness, fDamping,
                                                                        fStiffness, fDynamicFriction, fStartingFriction,
                                                                        fTangentStiffness, fStoppingFriction);
            }
