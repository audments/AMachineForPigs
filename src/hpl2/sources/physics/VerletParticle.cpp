...
void cVerletParticle::UpdateMovement(float afTimeStep) {
    ...
    mvPosition += (mvPosition * mpContainer->mfDampingMul - mvPrevPosition * mpContainer->mfDampingMul) +
                  vAcc * afTimeStep * afTimeStep;
...
void iVerletParticleContainer::UpdateParticleCollisionConstraint(cVerletParticle *apPart, const cVector3f &avPrevPos,
                                                                 float afRadius) {
    if (mbCollide == false)
        return;
    ...
    if (mpRayParticleCallback->mbIntersected) {
        ...
        apPart->mvPrevPosition =
            apPart->mvPosition + mpRayParticleCallback->mvIntersectNormal * (fLength * mfSlideAmount);
    }
}
