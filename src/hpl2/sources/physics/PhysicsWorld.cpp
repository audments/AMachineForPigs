...
iPhysicsWorld::iPhysicsWorld() { mbLogDebug = false; }
...
cPhysicsBodyIterator iPhysicsWorld::GetBodyIterator() { return cPhysicsBodyIterator(&mlstBodies); }
...
cPhysicsJointIterator iPhysicsWorld::GetJointIterator() { return cPhysicsJointIterator(&mlstJoints); }
...
cPhysicsMaterialIterator iPhysicsWorld::GetMaterialIterator() { return cPhysicsMaterialIterator(&m_mapMaterials); }

//-----------------------------------------------------------------------

iPhysicsRope *iPhysicsWorld::GetRope(const tString &asName) { return (iPhysicsRope *)STLFindByName(mlstRopes, asName); }
...
void iPhysicsWorld::DestroyRope(iPhysicsRope *apRope) { STLFindAndDelete(mlstRopes, apRope); }
...
void iPhysicsWorld::RenderContactPoints(iLowLevelGraphics *apLowLevel, const cColor &aPointColor,
    ...
    for (size_t i = 0; i < mvContactPoints.size(); i++) {
        ...
        apLowLevel->DrawLine(mvContactPoints[i].mvPoint,
                             mvContactPoints[i].mvNormal * mvContactPoints[i].mfDepth * 0.2f, aLineColor);
        // Log("Rendering\n");
    }
}
...
bool iPhysicsWorld::CheckShapeWorldCollision(cVector3f *apPushVector, iCollideShape *apShape,
                                             const cMatrixf &a_mtxTransform, iPhysicsBody *apSkipBody,
                                             bool abSkipStatic, bool abIsCharacter,
                                             iPhysicsWorldCollisionCallback *apCallback, bool abCollideCharacter,
                                             int alMinPushStrength, tFlag alCollideFlags, bool abDebug,
