...
cCharacterBodyCollideGravity::cCharacterBodyCollideGravity(iCharacterBody *apCharBody) { mpCharBody = apCharBody; }
...
void cCharacterBodyCollideGravity::OnCollision(iPhysicsBody *apBody, cCollideData *apCollideData) {
    ...
    if (apBody->GetGravityCanAttachCharacter() && apBody->IsCharacter() == false &&
        apCollideData->mvContactPoints[0].mvNormal.y >
            0.001f) // Quick check to make sure that character is just not sliding against the body.
    {
    ...
    if (bPushDown) {
        ...
        float fForceMul =
            mpCharBody->mbCustomGravity ? mpCharBody->mvCustomGravity.y : mpCharBody->mpWorld->GetGravity().y;
        if (fForceMul < 0)
...
cCharacterBodyCollidePush::cCharacterBodyCollidePush(iCharacterBody *apCharBody) { mpCharBody = apCharBody; }
...
void cCharacterBodyCollidePush::OnCollision(iPhysicsBody *apBody, cCollideData *apCollideData) {
    ...
    if (apBody->IsCharacter()) {
        ...
        // fBottom,pHitCharBody->GetSize().y,mpCharBody->GetSize().y,
...
float iCharacterBody::GetMass() { return mfMass; }
void iCharacterBody::SetMass(float afMass) { mfMass = afMass; }
...
cVector3f iCharacterBody::GetSize() { return mvSize; }
...
void iCharacterBody::SetMaxPositiveMoveSpeed(eCharDir aDir, float afX) { mfMaxPosMoveSpeed[aDir] = afX; }
float iCharacterBody::GetMaxPositiveMoveSpeed(eCharDir aDir) { return mfMaxPosMoveSpeed[aDir]; }
void iCharacterBody::SetMaxNegativeMoveSpeed(eCharDir aDir, float afX) { mfMaxNegMoveSpeed[aDir] = afX; }
float iCharacterBody::GetMaxNegativeMoveSpeed(eCharDir aDir) { return mfMaxNegMoveSpeed[aDir]; }

//-----------------------------------------------------------------------

void iCharacterBody::SetMoveSpeed(eCharDir aDir, float afX) { mfMoveSpeed[aDir] = afX; }
float iCharacterBody::GetMoveSpeed(eCharDir aDir) { return mfMoveSpeed[aDir]; }

void iCharacterBody::SetMoveAcc(eCharDir aDir, float afX) { mfMoveAcc[aDir] = afX; }
float iCharacterBody::GetMoveAcc(eCharDir aDir) { return mfMoveAcc[aDir]; }

void iCharacterBody::SetMoveDeacc(eCharDir aDir, float afX) { mfMoveDeacc[aDir] = afX; }
float iCharacterBody::GetMoveDeacc(eCharDir aDir) { return mfMoveDeacc[aDir]; }
...
const cVector3f &iCharacterBody::GetPosition() { return mvPosition; }
const cVector3f &iCharacterBody::GetLastPosition() { return mvLastPosition; }
...
cVector3f iCharacterBody::GetFeetPosition() { return mvPosition - cVector3f(0, mvSize.y / 2, 0); }
...
float iCharacterBody::GetYaw() { return mfYaw; }

void iCharacterBody::AddYaw(float afX) { mfYaw += afX; }
...
void iCharacterBody::AddPitch(float afX) { mfPitch += afX; }

float iCharacterBody::GetPitch() { return mfPitch; }

//-----------------------------------------------------------------------

const cVector3f &iCharacterBody::GetForward() { return mvForward; }
const cVector3f &iCharacterBody::GetRight() { return mvRight; }
const cVector3f &iCharacterBody::GetUp() { return mvUp; }

//-----------------------------------------------------------------------

void iCharacterBody::SetGravityActive(bool abX) { mbGravityActive = abX; }

bool iCharacterBody::GravityIsActive() { return mbGravityActive; }

void iCharacterBody::SetMaxGravitySpeed(float afX) { mfMaxGravitySpeed = afX; }
float iCharacterBody::GetMaxGravitySpeed() { return mfMaxGravitySpeed; }

//-----------------------------------------------------------------------

bool iCharacterBody::GetCustomGravityActive() { return mbCustomGravity; }

void iCharacterBody::SetCustomGravityActive(bool abX) { mbCustomGravity = abX; }

void iCharacterBody::SetCustomGravity(const cVector3f &avCustomGravity) { mvCustomGravity = avCustomGravity; }

cVector3f iCharacterBody::GetCustomGravity() { return mvCustomGravity; }
...
void iCharacterBody::SetMaxNoSlideSlopeAngle(float afAngle) { mfMaxNoSlideSlopeAngleCos = cos(afAngle); }

float iCharacterBody::GetMaxNoSlideSlopeAngle() { return acos(mfMaxNoSlideSlopeAngleCos); }

//-----------------------------------------------------------------------

void iCharacterBody::ResetClimbing() { mbClimbing = false; }

//-----------------------------------------------------------------------

void iCharacterBody::SetForce(const cVector3f &avForce) { mvForce = avForce; }

void iCharacterBody::AddForce(const cVector3f &avForce) { mvForce += avForce; }

cVector3f iCharacterBody::GetForce() { return mvForce; }
...
void iCharacterBody::ConnectToBody(iPhysicsBody *apBody, const cVector3f &avConnectionPos,
    ...
    mvConnectionBodyStartRotation =
        cMath::MatrixToEulerAngles(mpConnectedBody->GetLocalMatrix().GetRotation(), eEulerRotationOrder_XYZ);
    ...
    if (mbConnectionDependOnCharRotation) {
        // If body is aligned according to character rotation, then we need to make sure vectors are local when rotation
        // is 0, hence we multiply with inverse rotation.
        UpdateMoveMatrix();
        cMatrixf mtxInvCharRot = cMath::MatrixInverse(m_mtxMove.GetRotation());
...
void iCharacterBody::SetCamera(cCamera *apCam) { mpCamera = apCam; }

cCamera *iCharacterBody::GetCamera() { return mpCamera; }

void iCharacterBody::SetCameraPosAdd(const cVector3f &avAdd) { mvCameraPosAdd = avAdd; }
cVector3f iCharacterBody::GetCameraPosAdd() { return mvCameraPosAdd; }

//-----------------------------------------------------------------------

iCollideShape *iCharacterBody::GetCurrentShape() { return mpCurrentShape; }

iPhysicsBody *iCharacterBody::GetCurrentBody() { return mpCurrentBody; }

iCollideShape *iCharacterBody::GetShape(int alIdx) { return mvShapes[alIdx]; }

bool iCharacterBody::IsOnGround() { return mlOnGroundCount > 0; }

//-----------------------------------------------------------------------

bool iCharacterBody::CheckCharacterFits(const cVector3f &avPosition, bool abFeetPosition, int alSizeIdx,
                                        cVector3f *apPushBackVec, float afEpsilon) {
    if (alSizeIdx < 0)
...
bool iCharacterBody::CheckRayIntersection(const cVector3f &avStart, const cVector3f &avEnd, float *apDistance,
...
void iCharacterBody::SetEntity(iEntity3D *apEntity) { mpEntity = apEntity; }
iEntity3D *iCharacterBody::GetEntity() { return mpEntity; }
void iCharacterBody::SetEntityOffset(const cMatrixf &a_mtxOffset) { m_mtxEntityOffset = a_mtxOffset; }
const cMatrixf &iCharacterBody::GetEntityOffset() { return m_mtxEntityOffset; }

void iCharacterBody::SetEntityPostOffset(const cMatrixf &a_mtxOffset) { m_mtxEntityPostOffset = a_mtxOffset; }
const cMatrixf &iCharacterBody::GetEntityPostOffset() { return m_mtxEntityPostOffset; }

//-----------------------------------------------------------------------

iPhysicsBody *iCharacterBody::GetConnectedBody() { return mpConnectedBody; }
...
void iCharacterBody::UpdateBodyConnection(float afTimeStep) {
    ...
    if (mbConnectionAlignRotation) {
        ...
        if (mbConnectionDependOnBodyRotation) {
            ...
            cVector3f vBodyUp =
                cMath::MatrixMul(mpConnectedBody->GetWorldMatrix().GetRotation(), mvConnectionLocalUpBody);
...
void iCharacterBody::UpdateCharacterConnection(float afTimeStep) {
    ...
    if (mbConnectionAlignCharacterRotation) {
        ...
        cVector3f vWantedForward =
            cMath::MatrixMul(mpConnectedBody->GetWorldMatrix().GetRotation(), mvConnectionCharLocalForward);
...
cVector3f iCharacterBody::UpdatePostionFromCharSpeed(float afTimeStep) {
    ...
    for (int i = 0; i < eCharDir_LastEnum; i++) {
        ...
        float fAccMul = 1.0f;
        if (IsOnGround() &&
            ((mfCurrentMoveAcc[i] > 0 && mfMoveSpeed[i] < 0) || (mfCurrentMoveAcc[i] < 0 && mfMoveSpeed[i] > 0))) {
    ...
    // TODO: This does not work when one lowers the speed (since it also caps max speed!). So leave in strafe bug /
    // trick for now! Make sure speed is not greater than max forward.
    /*float fMaxStep = mfMoveSpeed[eCharDir_Forward] >=0 ? mfMaxPosMoveSpeed[eCharDir_Forward] :
...
void iCharacterBody::CheckStepClimbing(const cVector3f &avPosAdd, float afTimeStep) {
    ...
    for (int i = 0; i < lNumRays; ++i) {
        ...
        if (fHeight <= fMaxHeight && fHeight > 0.015f) {
            ...
            cVector3f vStepPos =
                mvPosition + cVector3f(0, fHeight + mfClimbHeightAdd, 0) + (vMoveDir * fForwadAdd * mfClimbForwardMul);
...
void iCharacterBody::CheckForceCollision(float afTimeStep) {
    ...
    if (vPosAdd.y != 0) {
        ...
        if (bCollide && cMath::Vector3Abs(vPushBack) != cVector3f(0)) {
            ...
            // Check if the push back is mostly up and velocity points down, else decrement, ie signal that the player
            // is not on ground.
            ...
            // If the slope is less than a certain angle, then we treat it like a flat (according to gravity dir)
            // surface.
            // >= because cos gets large as angle gets closer to 0.
            cVector3f vInvGravityNormal =
                mbCustomGravity ? cMath::Vector3Normalize(mvCustomGravity * -1) : cVector3f(0, 1, 0);
            ...
            // vYVel.ToString().c_str(),fDot,fAngle,mfMaxNoSlideSlopeAngleCos);
...
void iCharacterBody::UpdateForcePushing(float afTimeStep) {
    ...
    for (size_t i = 0; i < mvTempBodies.size(); ++i) {
        ...
        collideData.SetMaxSize(32);
        bool bRet = mpWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), mpCurrentShape,
                                                 mtxTransform, collideData, 32, true);
...
void iCharacterBody::UpdateBody() { mpCurrentBody->SetPosition(mvPosition); }
...
bool iCharacterBody::CheckCollision(cVector3f *apPushBackVector, const cVector3f &avPos,
                                    iPhysicsWorldCollisionCallback *apCallback, int alShapeIdx) {
    if (alShapeIdx < 0)
        alShapeIdx = mlCurrentShapeIdx;
    iCollideShape *pShape = mvShapes[alShapeIdx];

    return mpWorld->CheckShapeWorldCollision(apPushBackVector, pShape, cMath::MatrixTranslate(avPos), mpCurrentBody,
                                             false, true, apCallback, true, mlMinBodyPushStrength, mlCollideFlags,
                                             false, !mbCollideStaticVolatile);
}
