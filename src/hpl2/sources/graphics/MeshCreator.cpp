...
iVertexBuffer *cMeshCreator::CreateWireframeVertexBuffer(iVertexBuffer *apSrc) {
    iVertexBuffer *pDest =
        apSrc->CreateCopy(eVertexBufferType_Hardware, eVertexBufferUsageType_Static, apSrc->GetVertexElementFlags());
...
cMesh *cMeshCreator::CreateBox(const tString &asName, cVector3f avSize, const tString &asMaterial) {
    cMesh *pMesh =
        hplNew(cMesh, (asName, _W(""), mpResources->GetMaterialManager(), mpResources->GetAnimationManager()));
...
cMesh *cMeshCreator::CreatePlane(const tString &asName, const cVector3f &avCorner1, const cVector3f &avCorner2,
                                 const cVector2f &avCorner1UV, const cVector2f &avCorner2UV,
                                 const cVector2f &avCorner3UV, const cVector2f &avCorner4UV,
    ...
    iVertexBuffer *pVtxBuffer = mpLowLevelGraphics->CreateVertexBuffer(
        eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Dynamic, 4, 6);
...
cMesh *cMeshCreator::CreateSphere(const tString &asName, float afRadius, int alSections, int alSlices,
                                  const tString &asMaterial) {
    //////////////////////////////////////////////////
...
cMesh *cMeshCreator::CreateCylinder(const tString &asName, const cVector2f &avSize, int alSections,
                                    const tString &asMaterial) {
    iVertexBuffer *pVtxBuffer = mpLowLevelGraphics->CreateVertexBuffer(
        eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static);
    pVtxBuffer->CreateElementArray(eVertexBufferElement_Position, eVertexBufferElementFormat_Float, 4);
...
cMesh *cMeshCreator::CreateCapsule(const tString &asName, const cVector2f &avSize, int alSections, int alSlices,
                                   const tString &asMaterial) {
    iVertexBuffer *pVtxBuffer = mpLowLevelGraphics->CreateVertexBuffer(
        eVertexBufferType_Hardware, eVertexBufferDrawType_Tri, eVertexBufferUsageType_Static);
    pVtxBuffer->CreateElementArray(eVertexBufferElement_Position, eVertexBufferElementFormat_Float, 4);
    ...
    for (int i = 0; i < alSlices - 1; ++i) {
        float fHeight =
            fCylinderHalfHeight + fRadius - fRadius * (1 + sin(fHalfPiOverSlices * (float)(i + 1) - fHalfPi));
...
cMesh *cMeshCreator::CreateCone(const tString &asName, const cVector2f &avSize, int alSections,
                                const tString &asMaterial) {
    //////////////////////////////////////////
    // Create Vertex Buffer
    iVertexBuffer *pVtxBuffer = mpLowLevelGraphics->CreateVertexBuffer(
...
iVertexBuffer *cMeshCreator::CreateBoxVertexBuffer(cVector3f avSize) {
    iVertexBuffer *pBox = mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri,
...
void cMeshCreator::CreateCircumference(float afRadius, float afAngleStep, float afHeight, tVector3fVec &avVertices) {
    ...
    for (float fAngle = 0; fAngle <= fEnd; fAngle += afAngleStep) {
        cVector3f vPoint =
            cVector3f(cMath::RoundFloatToDecimals(afRadius * cos(fAngle), 6), cMath::RoundFloatToDecimals(afHeight, 6),
...
void cMeshCreator::WrapUpperCap(iVertexBuffer *apVtxBuffer, int alCenterVertexIdx, int alStartVertexIdx,
                                int alSections) {
    for (int i = 0; i < alSections - 1; ++i) {
...
void cMeshCreator::WrapLowerCap(iVertexBuffer *apVtxBuffer, int alCenterVertexIdx, int alStartVertexIdx,
                                int alSections) {
    for (int i = 0; i < alSections - 1; ++i) {
        int lBase = alStartVertexIdx + i;
        apVtxBuffer->AddIndex(alCenterVertexIdx);
