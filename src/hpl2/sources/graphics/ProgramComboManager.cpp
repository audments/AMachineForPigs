...
cProgramComboManager::cProgramComboManager(const tString &asName, cGraphics *apGraphics, cResources *apResources,
                                           int alNumOfMainModes) {
...
cProgramComboManager::~cProgramComboManager() { DestroyShadersAndPrograms(); }
...
void cProgramComboManager::DestroyGeneratedShader(int alMainMode, iGpuShader *apShader, eGpuShaderType aType) {
    tProgramComboShaderMap *pShaderSet =
        aType == eGpuShaderType_Vertex ? &mvVtxShaderSets[alMainMode] : &mvFragShaderSets[alMainMode];
...
iGpuShader *cProgramComboManager::CreateShader(const tString &asName, eGpuShaderType aType, cParserVarContainer *apVars,
                                               bool abAddtoList) {
...
void cProgramComboManager::DestroyProgram(iGpuProgram *apProgram) { STLFindAndDelete(mlstExtraPrograms, apProgram); }
...
iGpuProgram *cProgramComboManager::CreateProgramFromShaders(const tString &asProgramName, iGpuShader *apVtxShader,
                                                            iGpuShader *apFragShader, bool abAddtoList) {
    iGpuProgram *pProgram;
...
iGpuShader *cProgramComboManager::GetShaderForCombo(int alMainMode, int alBitFlags, const tString &asShaderName,
                                                    tFlag aShaderType) {
    cProgramComboSettings &comboSettings = mvSettings[alMainMode];
    ...
    tProgramComboShaderMap *pShaderSet =
        aShaderType == kPC_VertexBit ? &mvVtxShaderSets[alMainMode] : &mvFragShaderSets[alMainMode];
    ...
    iGpuShader *pShader = CreateShaderFromFeatures(
        asShaderName, aShaderType, alBitFlags, &comboSettings.mvFeatures[0], (int)comboSettings.mvFeatures.size(),
...
iGpuShader *cProgramComboManager::CreateShaderFromFeatures(const tString &asShaderFile, tFlag aShaderType,
                                                           int alBitFlags, cProgramComboFeature *apFeatures,
                                                           int alFeatureNum, cProgramComboSettingsVar *apDefaultVars,
