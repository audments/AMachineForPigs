...
bool cDecalCreator::Compile() {
    if (mbUpdated) {
        ...
        mpDecalVB = mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Hardware, eVertexBufferDrawType_Tri,
        ...
        for (int i = 0; i < mpDecalVB->GetVertexNum(); ++i) {
            ...
            cVector3f vPointRelToCenter =
                cVector3f(pVertexPositions[lBaseIdx], pVertexPositions[lBaseIdx + 1], pVertexPositions[lBaseIdx + 2]) -
                mvDecalPosition;

            float u = vInvSubDivSize.x * (fStartU + cMath::Vector3Dot(vPointRelToCenter, mvDecalRight) * fInvW + 0.5f);
            float v =
                vInvSubDivSize.y * (fStartV + cMath::Vector3Dot(vPointRelToCenter, mvDecalForward) * fInvH + 0.5f);
...
cMesh *cDecalCreator::CreateDecalMesh() {
    ...
    pSubMesh->SetVertexBuffer(mpDecalVB->CreateCopy(eVertexBufferType_Hardware, eVertexBufferUsageType_Static,
                                                    mpDecalVB->GetVertexElementFlags()));
...
bool cDecalCreator::AddPolygon(int alVertexCount, const cVector3f *apVertices, const cVector3f *apNormals,
                               iVertexBuffer *apDecalVB, const cMatrixf &amtxWorldMatrix,
...
void cDecalCreator::ClipMesh(cSubMeshEntity *apSubMesh, iVertexBuffer *apDecalVB) {
    ...
    for (int i = 0; i < (int)mvClipPlanes.size(); ++i) {
        ...
        // vTransformedPlanes.back().GetNormal().ToFileString().c_str(), vTransformedPlanes.back().d);
    }
    ...
    for (int j = 0; j < pSubMeshVB->GetIndexNum(); j += 3) {
        ...
        if ((lCount != 0) && (AddPolygon(lCount, vNewVertices, vNewNormals, apDecalVB, mtxSubMeshWorldMatrix,
                                         mtxSubMeshWorldNormalRot) == false))
            break;
    }
}
...
int cDecalCreator::ClipPolygon(int alVertexCount, const cVector3f *apVertices, const cVector3f *apNormals,
    ...
    int lCount =
        ClipPolygonAgainstPlane(avPlanes[0], alVertexCount, apVertices, apNormals, vTempVertices, vTempNormals);
    if (lCount != 0) {
        lCount = ClipPolygonAgainstPlane(avPlanes[1], lCount, vTempVertices, vTempNormals, apNewVertices, apNewNormals);
        if (lCount != 0) {
            lCount =
                ClipPolygonAgainstPlane(avPlanes[2], lCount, apNewVertices, apNewNormals, vTempVertices, vTempNormals);
            if (lCount != 0) {
                lCount = ClipPolygonAgainstPlane(avPlanes[3], lCount, vTempVertices, vTempNormals, apNewVertices,
                                                 apNewNormals);
                if (lCount != 0) {
                    lCount = ClipPolygonAgainstPlane(avPlanes[4], lCount, apNewVertices, apNewNormals, vTempVertices,
                                                     vTempNormals);
                    if (lCount != 0) {
                        lCount = ClipPolygonAgainstPlane(avPlanes[5], lCount, vTempVertices, vTempNormals,
                                                         apNewVertices, apNewNormals);
                    }
                }
            }
        }
...
int cDecalCreator::ClipPolygonAgainstPlane(const cPlanef &aPlane, int alVertexCount, const cVector3f *apVertices,
                                           const cVector3f *apNormals, cVector3f *apNewVertices,
