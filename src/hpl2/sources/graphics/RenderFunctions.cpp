...
void iRenderFunctions::InitAndResetRenderFunctions(cFrustum *apFrustum, cRenderTarget *apRenderTarget, bool abLog,
                                                   bool abUseGlobalScissorRect, const cVector2l &avGlobalScissorRectPos,
...
void iRenderFunctions::SetNormalFrustumProjection() { SetFrustumProjection(mpCurrentFrustum); }
...
void iRenderFunctions::SetStencilWriteMask(unsigned int alMask) { mpLowLevelGraphics->SetStencilWriteMask(alMask); }

void iRenderFunctions::SetStencil(eStencilFunc aFunc, int alRef, unsigned int aMask, eStencilOp aFailOp,
                                  eStencilOp aZFailOp, eStencilOp aZPassOp) {
    mpLowLevelGraphics->SetStencil(aFunc, alRef, aMask, aFailOp, aZFailOp, aZPassOp);
}

void iRenderFunctions::SetStencilTwoSide(eStencilFunc aFrontFunc, eStencilFunc aBackFunc, int alRef, unsigned int aMask,
                                         eStencilOp aFrontFailOp, eStencilOp aFrontZFailOp, eStencilOp aFrontZPassOp,
                                         eStencilOp aBackFailOp, eStencilOp aBackZFailOp, eStencilOp aBackZPassOp) {
    mpLowLevelGraphics->SetStencilTwoSide(aFrontFunc, aBackFunc, alRef, aMask, aFrontFailOp, aFrontZFailOp,
                                          aFrontZPassOp, aBackFailOp, aBackZFailOp, aBackZPassOp);
}
...
bool iRenderFunctions::SetScissorRect(const cVector2l &avPos, const cVector2l &avSize, bool abAutoEnabling) {
    ...
    /////////////////////////////
    // Set the clip rect
    if (mbUseGlobalScissorRect == false && vFinalPos.x == 0 && vFinalPos.y == 0 &&
    ...
    if (abUsePosAndSize) {
        if (mbLog) {
            Log(" Setting FrameBuffer %d '%s', pos: %d,%d size: %dx%d\n", apFrameBuffer,
                apFrameBuffer ? apFrameBuffer->GetName().c_str() : "", mpCurrentRenderTarget->mvPos.x,
...
void iRenderFunctions::ClearFrameBuffer(tClearFrameBufferFlag aFlags, bool abUsePosAndSize) {
    ...
    if (abUsePosAndSize && (mpCurrentRenderTarget->mvPos != cVector2l(0, 0) ||
                            mvRenderTargetSize != mvCurrentFrameBufferSize || mbUseGlobalScissorRect)) {
        /////////////////////////////////
        // Global scissor rect (do no setup and do not turn off scissor at the end!)
        if (mbUseGlobalScissorRect) {
            if (mbLog) {
                Log(" Clearing portion of frame buffer. Pos: %d,%d Size: %dx%d\n", mvGlobalScissorRectPos.x,
                    mvGlobalScissorRectPos.y, mvGlobalScissorRectSize.x, mvGlobalScissorRectSize.y);
            }
        }
        /////////////////////////////////
        // Render target size
...
void iRenderFunctions::DrawQuad(const cVector3f &aPos, const cVector2f &avSize, const cVector2f &avMinUV,
                                const cVector2f &avMaxUV, bool abInvertY, const cColor &aColor) {
    if (mbLog) {
        Log("   Drawing quad to: pos (%s) size (%s) uv: (%s)-(%s)\n", aPos.ToString().c_str(),
            avSize.ToString().c_str(), avMinUV.ToString().c_str(), avMaxUV.ToString().c_str());
    }
    if (abInvertY) {
        mpLowLevelGraphics->DrawQuad(aPos, avSize, cVector2f(avMinUV.x, avMaxUV.y), cVector2f(avMaxUV.x, avMinUV.y),
                                     aColor);
    } else {
...
void iRenderFunctions::DrawQuad(const cVector3f &aPos, const cVector2f &avSize, const cVector2f &avMinUV0,
                                const cVector2f &avMaxUV0, const cVector2f &avMinUV1, const cVector2f &avMaxUV1,
                                bool abInvertY0, bool abInvertY1, const cColor &aColor) {
    if (mbLog) {
        Log("   Drawing quad to: pos (%s) size (%s) uv0: (%s)-(%s) uv1: (%s)-(%s)\n", aPos.ToString().c_str(),
            avSize.ToString().c_str(), avMinUV0.ToString().c_str(), avMaxUV0.ToString().c_str(),
            avMinUV1.ToString().c_str(), avMaxUV0.ToString().c_str());
    }
...
iTexture *iRenderFunctions::CreateRenderTexture(const tString &asName, const cVector2l &avSize,
                                                ePixelFormat aPixelFormat, eTextureFilter aFilter, eTextureType aType) {
...
void iRenderFunctions::CopyFrameBufferToTexure(iTexture *apTexture, const cVector2l &avPos, const cVector2l &avSize,
    ...
    if (mbLog) {
        if (abTextureOffsetUsesRenderTargetPos)
            Log(" Copying current to texture '%s' (%d). Pos: %d:%d Size: %dx%d TextureOffset: %d:%d\n",
                apTexture->GetName().c_str(), apTexture, avPos.x, avPos.y, avSize.x, avSize.y,
                avTextureOffset.x + mpCurrentRenderTarget->mvPos.x, avTextureOffset.y + mpCurrentRenderTarget->mvPos.y);
        else
            Log(" Copying current to texture '%s' (%d). Pos: %d:%d Size: %dx%d TextureOffset: %d:%d\n",
                apTexture->GetName().c_str(), apTexture, avPos.x, avPos.y, avSize.x, avSize.y, avTextureOffset.x,
                avTextureOffset.y);
    }
    if (abTextureOffsetUsesRenderTargetPos)
        mpLowLevelGraphics->CopyFrameBufferToTexure(apTexture, avPos, avSize,
                                                    avTextureOffset + mpCurrentRenderTarget->mvPos);
    else
