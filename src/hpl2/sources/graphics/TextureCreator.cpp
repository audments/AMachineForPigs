...
void cTextureCreator::GenerateScatterDiskMap3D(iTexture *apOffsetTexture, int alSize, int alSamples,
                                               bool abSortSamples) {
    ////////////////////
...
void cTextureCreator::GenerateScatterDiskMap2D(iTexture *apOffsetTexture, int alSize, int alSamples,
                                               bool abSortSamples) {
    ////////////////////
    // Test so sizes are correct
    if (cMath::IsPow2(alSize) == false)
    ...
    ////////////////////////////
    // Create texture data
    apOffsetTexture->CreateFromRawData(cVector3l(vTextureSize.x, vTextureSize.y * vTextureSize.z, 1), ePixelFormat_RGBA,
                                       &vTextureData[0]);
    apOffsetTexture->SetWrapSTR(eTextureWrap_Repeat);
    apOffsetTexture->SetFilter(eTextureFilter_Nearest); // Do not use when using lightspace as lookup
}
