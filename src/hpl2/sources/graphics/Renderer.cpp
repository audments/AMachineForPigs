...
cRenderSettings::cRenderSettings(bool abIsReflection) {
    ...
    mlMinimumObjectsBeforeOcclusionTesting =
        0; // 8;//8 should be good default, giving a good amount of colliders, or? Clarifiction: Minium num of object
           // rendered until node visibility tests start!
...
void cRenderSettings::AddOcclusionPlane(const cPlanef &aPlane) { mvOcclusionPlanes.push_back(aPlane); }

//-----------------------------------------------------------------------

void cRenderSettings::ResetOcclusionPlanes() { mvOcclusionPlanes.clear(); }

//-----------------------------------------------------------------------

void cRenderSettings::AssignOcclusionObject(iRenderer *apRenderer, void *apSource, int alCustomIndex,
                                            iVertexBuffer *apVtxBuffer, cMatrixf *apMatrix, bool abDepthTest) {
...
iRenderer::iRenderer(const tString &asName, cGraphics *apGraphics, cResources *apResources,
    ...
    mpDepthOnlyProgram = mpProgramManager->CreateProgramFromShaders("DepthOnly", "deferred_base_vtx.glsl",
                                                                    "deferred_depthonly_frag.glsl", &vars, false);

    ////////////
    // Create shapes
    //  Color and Texture because Geforce cards fail without it, no idea why...
    mpShapeBox = LoadVertexBufferFromMesh("core_box.dae", eVertexElementFlag_Position | eVertexElementFlag_Texture0 |
                                                              eVertexElementFlag_Color0);
}
...
void iRenderer::Render(float afFrameTime, cFrustum *apFrustum, cWorld *apWorld, cRenderSettings *apSettings,
                       cRenderTarget *apRenderTarget, bool abSendFrameBufferToPostEffects,
                       tRendererCallbackList *apCallbackList) {
    BeginRendering(afFrameTime, apFrustum, apWorld, apSettings, apRenderTarget, abSendFrameBufferToPostEffects,
                   apCallbackList);
...
void iRenderer::Update(float afTimeStep) { mfTimeCount += afTimeStep; }
...
void iRenderer::AssignOcclusionObject(void *apSource, int alCustomIndex, iVertexBuffer *apVtxBuffer, cMatrixf *apMatrix,
                                      bool abDepthTest) {
    if (mbLog)
...
void iRenderer::BeginRendering(float afFrameTime, cFrustum *apFrustum, cWorld *apWorld, cRenderSettings *apSettings,
                               cRenderTarget *apRenderTarget, bool abSendFrameBufferToPostEffects,
                               tRendererCallbackList *apCallbackList, bool abAtStartOfRendering) {
    if (apSettings->mbLog) {
        Log("Start Rendering Frustum: %d, World: '%s' TargetBuffer %d\n", apFrustum, apWorld->GetName().c_str(),
    ...
    InitAndResetRenderFunctions(apFrustum, apRenderTarget, apSettings->mbLog, apSettings->mbUseScissorRect,
                                apSettings->mvScissorRectPos, apSettings->mvScissorRectSize);
    ...
    // Fog
    if (mbSetupOcclusionPlaneForFog && apWorld->GetFogActive() && apWorld->GetFogColor().a >= 1.0f &&
        apWorld->GetFogCulling()) {
        cPlanef fogPlane;
...
void iRenderer::CreateAndAddShadowMap(eShadowMapResolution aResolution, const cVector3l &avSize, ePixelFormat aFormat) {
    ...
    if (mpLowLevelGraphics->GetCaps(eGraphicCaps_OGL_ATIFragmentShader)) {
        pData->mpTempDiffTexture =
            mpGraphics->CreateTexture(sName + "_TempDiff", eTextureType_2D, eTextureUsage_RenderTarget);
...
void iRenderer::RenderZObject(iRenderable *apObject, cFrustum *apCustomFrustum) {
    ...
    eMaterialRenderMode renderMode =
        apObject->GetCoverageAmount() >= 1 ? eMaterialRenderMode_Z : eMaterialRenderMode_Z_Dissolve;
 ...
 */
void iRenderer::PushNodeChildrenToStack(tRendererSortedNodeSet &a_setNodeStack, iRenderableContainerNode *apNode,
                                        int alNeededFlags) {
    ...
    for (; childIt != apNode->GetChildNodeList()->end(); ++childIt) {
        ...
        if (pChildNode->UsesFlagsAndVisibility() && (pChildNode->HasVisibleObjects() == false ||
                                                     (pChildNode->GetRenderFlags() & alNeededFlags) != alNeededFlags)) {
            continue;
        }
        ...
        eCollision frustumCollision = apNode->GetPrevFrustumCollision() == eCollision_Inside
                                          ? eCollision_Inside
                                          : mpCurrentFrustum->CollideNode(pChildNode);
        ...
        // Frustum origin is outside of node. Do intersection test.
            ...
            cMath::CheckAABBLineIntersection(pChildNode->GetMin(), pChildNode->GetMax(), mpCurrentFrustum->GetOrigin(),
                                             pChildNode->GetCenter(), &vIntersection, NULL);
...
void iRenderer::AddAndRenderNodeOcclusionQuery(tNodeOcclusionPairList *apList, iRenderableContainerNode *apNode,
                                               bool abObjectsRendered) {
    // DEBUG
...
int iRenderer::RenderAndAddNodeObjects(iRenderableContainerNode *apNode, tRenderCHCObjectCallbackFunc apRenderCallback,
...
static void PrintNodeDebugContents(tRendererSortedNodeSet &a_setNodes) {
    ...
    for (; it != a_setNodes.end(); ++it) {
        ...
        Log("   Node: %d, inside: %d, children %d, objects: %d\n", pNode, pNode->IsInsideView(),
            pNode->GetChildNodeList()->size(), pNode->GetObjectNum());
    }
}
...
void iRenderer::CheckForVisibleObjectsAddToListAndRenderZ(cVisibleRCNodeTracker *apVisibleNodeTracker,
                                                          ...
                                                          tRenderableFlag alNeededFlags, bool abSetupRenderStates,
...
bool iRenderer::CheckShadowCasterContributesToView(iRenderable *apObject) {
    // This should be always true since the shadow map might be saved and will then end up faulty if some objects have
    // been dismissed!
    return true;
...
void iRenderer::GetShadowCastersIterative(iRenderableContainerNode *apNode, eCollision aPrevCollision) {
    ...
    eCollision frustumCollision =
        aPrevCollision == eCollision_Inside ? aPrevCollision : gpLightFrustum->CollideNode(apNode);
    ...
    // Iterate children
    if (apNode->HasChildNodes()) {
        for (tRenderableContainerNodeListIt childIt = apNode->GetChildNodeList()->begin();
             childIt != apNode->GetChildNodeList()->end(); ++childIt) {
            iRenderableContainerNode *pChildNode = *childIt;
    ...
    if (apNode->HasObjects()) {
        for (tRenderableListIt it = apNode->GetObjectList()->begin(); it != apNode->GetObjectList()->end(); ++it) {
            ...
            if (CheckObjectIsVisible(pObject, eRenderableFlag_ShadowCaster) == false ||
                pObject->GetMaterial() == NULL || pObject->GetMaterial()->GetType()->IsTranslucent()) {
                continue;
            }
            ...
            pObject->SetViewSpaceZ(
                cMath::Vector3DistSqr(pObject->GetBoundingVolume()->GetWorldCenter(), gpLightFrustum->GetOrigin()));
...
void iRenderer::GetShadowCasters(iRenderableContainer *apContainer, tRenderableVec &avObjectVec,
                                 cFrustum *apLightFrustum) {
    apContainer->UpdateBeforeRendering();
...
bool iRenderer::SetupShadowMapRendering(iLight *apLight) {
    ...
    for (int i = 0; i < eFrustumPlane_LastEnum; ++i) {
        ...
        // Note that this is not optimal, but good enough for now. Should have some other way of choosing to make it
        // better.
    ...
    if (apLight->GetShadowCastersAffected() & eObjectVariabilityFlag_Dynamic)
        GetShadowCasters(mpCurrentWorld->GetRenderableContainer(eWorldContainerType_Dynamic), mvShadowCasters,
                         pSpotLight->GetFrustum());

    if (apLight->GetShadowCastersAffected() & eObjectVariabilityFlag_Static)
        GetShadowCasters(mpCurrentWorld->GetRenderableContainer(eWorldContainerType_Static), mvShadowCasters,
                         pSpotLight->GetFrustum());
...
void iRenderer::RenderShadowMap(iLight *apLight, iFrameBuffer *apShadowBuffer) {
    ...
    mpLowLevelGraphics->SetPolygonOffset(mpCurrentSettings->mfShadowMapBias * apLight->GetShadowMapBiasMul(),
                                         mpCurrentSettings->mfShadowMapSlopeScaleBias *
                                             apLight->GetShadowMapSlopeScaleBiasMul());
    ...
    if (apLight->GetOcclusionCullShadowCasters()) {
        ...
        CheckForVisibleObjectsAddToListAndRenderZ(apLight->GetVisibleNodeTracker(), apLight->GetShadowCastersAffected(),
                                                  eRenderableFlag_ShadowCaster, false,
...
void iRenderer::AssignAndRenderOcclusionQueryObjects(bool abSetFrameBuffer, iFrameBuffer *apFrameBuffer,
...
bool iRenderer::SetupLightScissorRect(iLight *apLight, cMatrixf *apViewSpaceMatrix) {
    ...
    mTempClipRect = cMath::GetClipRectFromSphere(apViewSpaceMatrix->GetTranslation(), apLight->GetRadius(),
...
cRect2l iRenderer::GetClipRectFromObject(iRenderable *apObject, float afPaddingPercent, cFrustum *apFrustum,
...
bool iRenderer::CheckObjectIsVisible(iRenderable *apObject, tRenderableFlag alNeededFlags) {
    ...
    /////////////////////////////
    // Clip plane check.
    // NOTE: This shall always be the user clip planes! Since we wanna cull stuff like nodes where clip planes are not
    // active, etc.
...
bool iRenderer::CheckNodeIsVisible(iRenderableContainerNode *apNode) {
    ...
    // NOTE: This shall always be the user clip planes! The render function ones might not be active when culling is
    // needed and so on.
    for (size_t i = 0; i < mvCurrentOcclusionPlanes.size(); ++i) {
        cPlanef &plane = mvCurrentOcclusionPlanes[i];

        if (cMath::CheckPlaneAABBCollision(plane, apNode->GetMin(), apNode->GetMax(), apNode->GetCenter(),
                                           apNode->GetRadius()) == eCollision_Outside) {
            return false;
        }
    }
...
bool iRenderer::CheckFogAreaInsideNearPlane(cMatrixf &a_mtxInvBoxModelMatrix) {
    cPlanef boxspaceNearPlane =
        cMath::TransformPlane(a_mtxInvBoxModelMatrix, mpCurrentFrustum->GetPlane(eFrustumPlane_Near));
...
bool iRenderer::CheckFogAreaRayIntersection(cMatrixf &a_mtxInvBoxModelMatrix, const cVector3f &avBoxSpaceRayStart,
                                            const cVector3f &avRayDir, float &afEntryDist, float &afExitDist) {
...
void iRenderer::SetupFogRenderDataArray(bool abSort) {
    ...
    if (abSort && mpCurrentSettings->mvFogRenderData.empty() == false) {
        std::sort(mpCurrentSettings->mvFogRenderData.begin(), mpCurrentSettings->mvFogRenderData.end(),
                  SortFunc_FogAreaData);
    }
}
...
float iRenderer::GetFogAreaVisibilityForObject(cFogAreaRenderData *apFogData, iRenderable *apObject) {
    ...
    if (CheckFogAreaRayIntersection(apFogData->m_mtxInvBoxSpace, apFogData->mvBoxSpaceFrustumOrigin, vRayDir,
                                    fEntryDist, fExitDist) == false) {
        return 1.0f;
    }
...
void iRenderer::SetOcclusionPlanesActive(bool abX) { mbOcclusionPlanesActive = abX; }

//-----------------------------------------------------------------------

iVertexBuffer *iRenderer::CreateQuadVertexBuffer(eVertexBufferType aType, const cVector3f &avPos,
                                                 const cVector2f &avSize, const cVector2f &avMinUV,
                                                 const cVector2f &avMaxUV, bool abInvertY) {
    iVertexBuffer *pVtxBuffer =
        mpLowLevelGraphics->CreateVertexBuffer(aType, eVertexBufferDrawType_Quad, eVertexBufferUsageType_Dynamic);
    ...
    pVtxBuffer->AddVertexVec3f(eVertexBufferElement_Position,
                               cVector3f(avPos.x + avSize.x, avPos.y + avSize.y, avPos.z));
...
void iRenderer::UpdateqQuadVertexPostion(iVertexBuffer *apVtxBuffer, const cVector3f &avPos, const cVector2f &avSize,
                                         bool abCallUpdate) {
