...
void cRenderList::AddObject(iRenderable *apObject) {
    ...
    if (pMaterialType && pMaterialType->IsTranslucent() && pMaterialType->IsDecal() == false) {
        ...
        if (cMath::CheckAABBLineIntersection(pBV->GetMin(), pBV->GetMax(), mpFrustum->GetOrigin(),
                                             pBV->GetWorldCenter(), &vIntersectionPos, NULL) == false) {
        ...
        cVector3f vCameraPos =
            cMath::MatrixMul(mpFrustum->GetViewMatrix(), apObject->GetBoundingVolume()->GetWorldCenter());
...
void cRenderList::PrintAllObjects() {
    ...
    for (size_t i = 0; i < mvTransObjects.size(); ++i)
        Log(" '%s' ViewspaceZ: %f LargeSurfacePlacement: %d Mat: '%s' RenderCount: %d\n",
            ...
            mvTransObjects[i]->GetLargePlaneSurfacePlacement(), mvTransObjects[i]->GetMaterial()->GetName().c_str(),
            mvTransObjects[i]->GetRenderFrameCount());

    Log("Solid Objects:\n");
    for (size_t i = 0; i < mvSolidObjects.size(); ++i)
        Log(" '%s' Mat: '%s' RenderCount: %d\n", mvSolidObjects[i]->GetName().c_str(),
            mvSolidObjects[i]->GetMaterial()->GetName().c_str(), mvSolidObjects[i]->GetRenderFrameCount());

    Log("Decal Objects:\n");
    for (size_t i = 0; i < mvDecalObjects.size(); ++i)
        Log(" '%s' Mat: '%s' RenderCount: %d\n", mvDecalObjects[i]->GetName().c_str(),
            mvDecalObjects[i]->GetMaterial()->GetName().c_str(), mvSolidObjects[i]->GetRenderFrameCount());

    Log("Illum Objects:\n");
    for (size_t i = 0; i < mvIllumObjects.size(); ++i)
        Log(" '%s' Mat: '%s' RenderCount: %d\n", mvIllumObjects[i]->GetName().c_str(),
            mvIllumObjects[i]->GetMaterial()->GetName().c_str(), mvSolidObjects[i]->GetRenderFrameCount());
...
bool cRenderList::ArrayHasObjects(eRenderListType aType) { return mvSortedArrays[aType].empty() == false; }
...
static tSortRenderableFunc vSortFunctions[eRenderListType_LastEnum] = {
    SortFunc_Z, SortFunc_Diffuse, SortFunc_Translucent, SortFunc_Decal, SortFunc_Illumination, SortFunc_Z};
...
void cRenderList::FindNearestLargeSurfacePlane() {
    ...
    if (bHasLargeSurfacePlane) {
        // Log(" LargePlaneObject: '%s' Mat:'%s'\n", pLargeSurfaceObject->GetName().c_str(),
        ...
        for (size_t i = 0; i < mvTransObjects.size(); ++i) {
            ...
            // Log(" %d Object: '%s' %d  Mat:'%s'\n",i, pObject->GetName().c_str(),
            // pObject->GetLargePlaneSurfacePlacement(),  pLargeSurfaceObject->GetMaterial()->GetName().c_str());
        }
    }
    ////////////////////////////////////
