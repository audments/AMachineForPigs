...
cMaterialType_Water::cMaterialType_Water(cGraphics *apGraphics, cResources *apResources)
    ...
    AddVarBool("LargeSurface", true,
               "If the water will cover a large surface and will need special sorting when rendering other transperant "
               "objects.");
...
void cMaterialType_Water::LoadData() {
    ...
    mpProgramManager->SetupGenerateProgramData(eMaterialRenderMode_Diffuse, "Diffuse", "deferred_base_vtx.glsl",
                                               "water_surface_frag.glsl", vDiffuseFeatureVec, kDiffuseFeatureNum,
                                               defaultVars);

    ////////////////////////////////
    // Set up variable ids
    mpProgramManager->AddGenerateProgramVariableId("afT", kVar_afT, eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("afRefractionScale", kVar_afRefractionScale,
                                                   eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("a_mtxInvViewRotation", kVar_a_mtxInvViewRotation,
                                                   eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("avReflectionMapSizeMul", kVar_avReflectionMapSizeMul,
                                                   eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("avFrenselBiasPow", kVar_avFrenselBiasPow,
                                                   eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("avReflectionFadeStartAndLength",
                                                   kVar_avReflectionFadeStartAndLength, eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("afWaveAmplitude", kVar_afWaveAmplitude,
                                                   eMaterialRenderMode_Diffuse);
    mpProgramManager->AddGenerateProgramVariableId("afWaveFreq", kVar_afWaveFreq, eMaterialRenderMode_Diffuse);

    mpProgramManager->AddGenerateProgramVariableId("avFogStartAndLength", kVar_avFogStartAndLength,
                                                   eMaterialRenderMode_Diffuse);
...
void cMaterialType_Water::DestroyData() { mpProgramManager->DestroyShadersAndPrograms(); }
...
iTexture *cMaterialType_Water::GetSpecialTexture(cMaterial *apMaterial, eMaterialRenderMode aRenderMode,
...
void cMaterialType_Water::SetupTypeSpecificData(eMaterialRenderMode aRenderMode, iGpuProgram *apProgram,
...
void cMaterialType_Water::SetupMaterialSpecificData(eMaterialRenderMode aRenderMode, iGpuProgram *apProgram,
    ...
    if (aRenderMode == eMaterialRenderMode_Diffuse || aRenderMode == eMaterialRenderMode_DiffuseFog) {
        ...
        if (pVars->mbHasReflection && iRenderer::GetRefractionEnabled()) {
            ...
            // World reflection
                ...
                apProgram->SetVec2f(kVar_avReflectionMapSizeMul,
                                    cVector2f(1.0f / (float)iRenderer::GetReflectionSizeDiv()));
            }
            ...
            if (pVars->mfReflectionFadeEnd > 0) {
                ...
                apProgram->SetVec2f(kVar_avReflectionFadeStartAndLength,
                                    cVector2f(-pVars->mfReflectionFadeStart, -fLength));
            }
        }
    }
}
...
void cMaterialType_Water::SetupObjectSpecificData(eMaterialRenderMode aRenderMode, iGpuProgram *apProgram,
