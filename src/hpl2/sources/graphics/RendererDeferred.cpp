...
cRendererDeferred::cRendererDeferred(cGraphics *apGraphics, cResources *apResources)
    ...
    mbSetFrameBufferAtBeginRendering =
        false; // Not using the input frame buffer for any rendering. Only doing copy at the end!
...
cRendererDeferred::~cRendererDeferred() { STLDeleteAll(mvTempDeferredLights); }
...
bool cRendererDeferred::LoadData() {
    ...
    for (int i = 0; i < mlNumOfGBufferTextures; ++i) {
        ...
        mpGBufferTexture[1][i] =
            CreateRenderTexture(sName + "_Reflection", vRelfectionSize, pixelFormat, eTextureFilter_Nearest);
    }
    ...
    for (int type = 0; type < 2; ++type) {
        ...
        mpGBuffer[type][eGBufferComponents_ColorAndDepth] =
            mpGraphics->CreateFrameBuffer("Deferred_GBuffer_ColorDepth");
    ...
    mpRefractionTexture =
        mpGraphics->GetTempFrameBuffer(mvScreenSize, ePixelFormat_RGBA, 0)->GetColorBuffer(0)->ToTexture();
    ...
    cVector3l vShadowSize[] = {cVector3l(128, 128, 1), cVector3l(256, 256, 1), cVector3l(256, 256, 1),
                               cVector3l(512, 512, 1), cVector3l(1024, 1024, 1)};
    int lStartSize = 2;
    ...
    for (int i = 0; i < 1; ++i)
        CreateAndAddShadowMap(eShadowMapResolution_High, vShadowSize[lStartSize + eShadowMapResolution_High],
                              ePixelFormat_Depth16);
    for (int i = 0; i < 4; ++i)
        CreateAndAddShadowMap(eShadowMapResolution_Medium, vShadowSize[lStartSize + eShadowMapResolution_Medium],
                              ePixelFormat_Depth16);
    for (int i = 0; i < 6; ++i)
        CreateAndAddShadowMap(eShadowMapResolution_Low, vShadowSize[lStartSize + eShadowMapResolution_Low],
                              ePixelFormat_Depth16);
    ...
    if (mpLowLevelGraphics->GetCaps(eGraphicCaps_ShaderModel_4)) {
        ...
        if (mShadowMapQuality == eShadowMapQuality_High) {
            ...
            mlShadowJitterSamples =
                32; // 64 here instead? I mean, ATI has to deal with medium has max? or different max for ATI?
        }
        // Medium
    ...
    if (mShadowMapQuality != eShadowMapQuality_Low) {
        ...
        mpGraphics->GetTextureCreator()->GenerateScatterDiskMap2D(mpShadowJitterTexture, mlShadowJitterSize,
                                                                  mlShadowJitterSamples, true);
    } else {
    ...
    // Create Sky box program
    {
        ...
        iGpuShader *pFragShader =
            mpShaderManager->CreateShader("deferred_gbuffer_skybox_frag.glsl", eGpuShaderType_Fragment, &vars);
    ...
    // Create Fog program
    {
        ...
        mpFogProgramManager->SetupGenerateProgramData(0, "Fog", "deferred_fog_vtx.glsl", "deferred_fog_frag.glsl",
        ...
        mpFogProgramManager->AddGenerateProgramVariableId("a_mtxBoxInvViewModelRotation",
                                                          kVar_a_mtxBoxInvViewModelRotation, 0);
        ...
        // mpModulatedFogProgram->AddGenerateProgramVariableId("a_mtxBoxInvViewModelRotation",
        // kVar_a_mtxBoxInvViewModelRotation,0);
    ...
    // Create Light programs
    {
        ...
        // Misc
        {
            ...
            //////////////
            // Light Stencil
            mpLightStencilProgram = mpProgramManager->CreateProgramFromShaders("LightStencil", "deferred_base_vtx.glsl",
                                                                               "deferred_base_frag.glsl", &vars, true);
            ...
            for (int i = 0; i < 2; ++i) {
                if (i == 1)
                    vars.Add("UseSSAO");
                mpLightBoxProgram[i] = mpProgramManager->CreateProgramFromShaders(
                    "LightBoxNormal", "deferred_base_vtx.glsl", "deferred_light_box_frag.glsl", &vars, true);
            ...
            mpLightBoxProgram[2] = mpProgramManager->CreateProgramFromShaders(
                "LightBoxNormal", "deferred_base_vtx.glsl", "deferred_light_box_frag.glsl", &vars, true);
            ...
            mpOverdrawProgram = mpProgramManager->CreateProgramFromShaders(
                "Debug Light Complexity", "deferred_base_vtx.glsl", "debug_overdraw_frag.glsl", &vars, true);
            mpHeatMapProgram = mpProgramManager->CreateProgramFromShaders("Debug Heat map", "deferred_base_vtx.glsl",
                                                                          "debug_heat_map_frag.glsl", &vars, true);
        }
        ...
        // Lights
        {
            ...
            mpProgramManager->SetupGenerateProgramData(eDefferredProgramMode_Lights, "Lights", "deferred_base_vtx.glsl",
                                                       "deferred_light_frag.glsl", gvLightFeatureVec, kLightFeatureNum,
                                                       defaultVars);
            // 1, defaultVars);

            mpProgramManager->AddGenerateProgramVariableId("avLightPos", kVar_avLightPos, eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("avLightColor", kVar_avLightColor,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("afInvLightRadius", kVar_afInvLightRadius,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("afNegFarPlane", kVar_afNegFarPlane,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("afOneMinusCosHalfSpotFOV", kVar_afOneMinusCosHalfSpotFOV,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("avLightForward", kVar_avLightForward,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("a_mtxSpotViewProj", kVar_a_mtxSpotViewProj,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("a_mtxInvViewRotation", kVar_a_mtxInvViewRotation,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("avShadowMapOffsetMul", kVar_avShadowMapOffsetMul,
                                                           eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("afFalloff", kVar_afFalloff, eDefferredProgramMode_Lights);
            mpProgramManager->AddGenerateProgramVariableId("afComplexity", kVar_afComplexity,
                                                           eDefferredProgramMode_Lights);
        }
    ...
    if (mbSSAOLoaded) {
        ...
        mpUnpackDepthProgram = mpGraphics->CreateGpuProgramFromShaders("UnpackDepth", "deferred_base_vtx.glsl",
                                                                       "deferred_unpack_depth_frag.glsl", &programVars);
        ...
        mpSSAOBlurProgram[0] = mpGraphics->CreateGpuProgramFromShaders("SSAOBlurHori", "deferred_ssao_blur_vtx.glsl",
                                                                       "deferred_ssao_blur_frag.glsl", &programVars);
        programVars.Clear();
        mpSSAOBlurProgram[1] = mpGraphics->CreateGpuProgramFromShaders("SSAOBlurVert", "deferred_ssao_blur_vtx.glsl",
                                                                       "deferred_ssao_blur_frag.glsl", &programVars);
    ...
    if (mbEdgeSmoothLoaded) {
        ...
        // Textures
        mpEdgeSmooth_LinearDepthTexture =
            CreateRenderTexture("EdgeSmoothLinearDepth", mvScreenSize, ePixelFormat_RGB16);
        mpEdgeSmooth_TempAccum =
            mpGraphics->GetTempFrameBuffer(mvScreenSize, ePixelFormat_RGBA, 0)->GetColorBuffer(0)->ToTexture();
        ...
        programVars.Add("UseUv");
        mpEdgeSmooth_RenderProgram = mpGraphics->CreateGpuProgramFromShaders(
            "EdgeSmoothRender", "deferred_base_vtx.glsl", "deferred_edge_smooth_frag.glsl", &programVars);
    ...
    mpBatchBuffer =
        mpLowLevelGraphics->CreateVertexBuffer(eVertexBufferType_Software, eVertexBufferDrawType_Quad,
                                               eVertexBufferUsageType_Stream, mlMaxBatchVertices, mlMaxBatchIndices);
...
int cRendererDeferred::GetDeferredLightNum() { return mvTempDeferredLights.size(); }
...
void cRendererDeferred::SetupRenderList() { mpCurrentRenderList->Setup(mfCurrentFrameTime, mpCurrentFrustum); }
...
void cRendererDeferred::RenderObjects() {
    ...
    if (mpCurrentSettings->mbUseOcclusionCulling) {
        CheckForVisibleObjectsAddToListAndRenderZ(mpCurrentSettings->mpVisibleNodeTracker, eObjectVariabilityFlag_All,
                                                  lVisibleFlags, true, NULL);

        mpCurrentRenderList->Compile(eRenderListCompileFlag_Z_Dissolve | eRenderListCompileFlag_Diffuse |
    ...
    // Brute force
        ...
        mpCurrentRenderList->Compile(eRenderListCompileFlag_Z_Dissolve | eRenderListCompileFlag_Z |
...
void cRendererDeferred::SetupGBuffer() {
    ...
    /////////////////////////////
    // Set G-Buffer as frame buffer (need to set something with color else textures will not be used and alpha will not
    // work!)
...
void cRendererDeferred::RenderSSAO() {
    ...
    cVector2f vGBufferDepthSize =
        cVector2f((float)pGBufferDepthTexture->GetWidth(), (float)pGBufferDepthTexture->GetHeight());
 ...
 * \param a_mtxDestRender This has a scale based on radius and radius mul. The mul is to make sure that the shape covers
 * the while light. \param a_mtxDestTransform A simple view space transform for the light. \param afRadiusMul
 */
static inline void SetupLightMatrix(cMatrixf &a_mtxDestRender, cMatrixf &a_mtxDestTransform, iLight *apLight,
                                    cFrustum *apFrustum, float afRadiusMul) {
    ////////////////////////////
    // Point Light
    if (apLight->GetLightType() == eLightType_Point) {
        a_mtxDestRender = cMath::MatrixScale(apLight->GetRadius() *
                                             afRadiusMul); // kLightRadiusMul = make sure it encapsulates the light.
    ...
    // Spot Light
        ...
        a_mtxDestRender = cMath::MatrixScale(
            cVector3f(fFarWidth, fFarHeight, apLight->GetRadius())); // x and y = "far plane", z = radius
        a_mtxDestTransform = cMath::MatrixMul(apFrustum->GetViewMatrix(), apLight->GetWorldMatrix());
        a_mtxDestRender = cMath::MatrixMul(a_mtxDestTransform, a_mtxDestRender);
    }
}
...
static tSortDeferredLightFunc vLightSortFunctions[eDeferredLightList_LastEnum] = {
    SortFunc_Default, SortFunc_Default, SortFunc_Default,
...
void cRendererDeferred::SetupLightsAndRenderQueries() {
    ...
    for (int i = 0; i < mpCurrentRenderList->GetLightNum(); ++i) {
        ...
        // Point
        if (lightType == eLightType_Point) {
            pLightData->mbInsideNearPlane = mpCurrentFrustum->CheckSphereNearPlaneIntersection(
                pLight->GetWorldPosition(), pLight->GetRadius() * kLightRadiusMul_Low);
        }
        // Spot
        else if (lightType == eLightType_Spot) {
            cLightSpot *pLightSpot = static_cast<cLightSpot *>(pLight);
            pLightData->mbInsideNearPlane =
                mpCurrentFrustum->CheckFrustumNearPlaneIntersection(pLightSpot->GetFrustum());
        }

        ////////////////////////////////////
        // Setup Light matrix
        SetupLightMatrix(pLightData->m_mtxViewSpaceRender, pLightData->m_mtxViewSpaceTransform, pLight,
                         mpCurrentFrustum, kLightRadiusMul_Medium);
        ...
        // Point
        if (lightType == eLightType_Point) {
            pLightData->mClipRect =
                cMath::GetClipRectFromSphere(pLightData->m_mtxViewSpaceRender.GetTranslation(), pLight->GetRadius(),
                                             mpCurrentFrustum, mvRenderTargetSize, true, mfScissorLastTanHalfFov);
        }
        // Spot
        else if (lightType == eLightType_Spot) {
        ...
        if (lightType == eLightType_Spot && pLight->GetCastShadows() && mpCurrentSettings->mbRenderShadows) {
            ...
            if (pLightData->mbInsideNearPlane) {
                ...
                pLightData->mShadowResolution = GetShadowMapResolution(pLight->GetShadowMapResolution(),
                                                                       mpCurrentSettings->mMaxShadowMapResolution);
            }
            ////////////////////////
            // Outside near plane, calculate distance and chose resolution
                ...
                pLightSpot->GetFrustum()->CheckLineIntersection(
                    mpCurrentFrustum->GetOrigin(), pLight->GetBoundingVolume()->GetWorldCenter(), vIntersection);
...
void cRendererDeferred::InitLightRendering() {
    ...
    if (mfLastFrustumFOV != mpCurrentFrustum->GetFOV() || mfLastFrustumFarPlane != mfFarPlane) {
        UpdateqQuadVertexPostion(mpFullscreenLightQuad, cVector3f(mfFarLeft, mfFarBottom, -mfFarPlane),
                                 cVector2f(mfFarRight * 2, mfFarTop * 2), true);
    }
    ...
    for (size_t i = 0; i < mvTempDeferredLights.size(); ++i) {
        ...
        if (lightType == eLightType_Box) {
            ...
            pLightData->m_mtxViewSpaceRender =
                cMath::MatrixMul(mpCurrentFrustum->GetViewMatrix(), pLightData->m_mtxViewSpaceRender);
            ...
            if (pQuery->FetchResults()) {
                ...
                if (mbLog)
                    Log(" Fetching query for light '%s'/%d. Have %d samples. Visible: %d\n", pLight->GetName().c_str(),
                        pLight, lSampleCount, !bLightInvisible);
            }
...
void cRendererDeferred::RenderLights_StencilBack_ScreenQuad() {
    ...
    while (lNumOfNearPlaneLights > 0) {
        ...
        for (int i = 0; i < lIterations; ++i) {
            ...
            // Stencil
            SetStencil(eStencilFunc_Equal, 0xFF, cMath::GetFlagBit(kStartStencilBit + i), eStencilOp_Keep,
                       eStencilOp_Keep, eStencilOp_Keep);
            ...
            if(vMaxClip.x < lightData.mClipRect.x+lightData.mClipRect.w) vMaxClip.x =
            lightData.mClipRect.x+lightData.mClipRect.w; if(vMaxClip.y < lightData.mClipRect.y+lightData.mClipRect.h)
            vMaxClip.y = lightData.mClipRect.y+lightData.mClipRect.h;*/
        }
    ...
    while (lNumOfNearPlaneLights > 0) {
        ...
        for (int i = 0; i < lIterations; ++i) {
            ...
            iGpuProgram *pProgram =
                SetupProgramAndTextures(pLightData, eFeature_Light_LightShapes | eFeature_Light_DivideInFrag);
            ...
            // Draw the light
            ////////////////////////
            // If lights are occlusion tested, render a query with the front and push back into Light-Occlusion pair
            // vector
            if (mbOcclusionTestLargeLights) {
...
void cRendererDeferred::RenderBoxLight(cDeferredLight *apLightData) {
    ...
    int lProgramNum =
        (mbSSAOLoaded && mpCurrentSettings->mbSSAOActive && mSSAOType == eDeferredSSAO_InBoxLight) ? 1 : 0;
...
void cRendererDeferred::RenderLights_Box_StencilFront_RenderBack() {
    ...
    while (lNumOfNearPlaneLights > 0) {
        ...
        SetStencil(eStencilFunc_Always, 0xFF, 0xFF, eStencilOp_Keep, eStencilOp_Replace, eStencilOp_Keep);
        for (int i = 0; i < lIterations; ++i) {
            cDeferredLight *pLightData =
                mvSortedLights[eDeferredLightList_Box_StencilFront_RenderBack][lStartLight + i];
            iLight *pLight = pLightData->mpLight;
            if (mbLog)
                Log(" Stencil light: '%s' / %d\n", pLight->GetName().c_str(), pLight);
...
void cRendererDeferred::RenderLights() {
    ...
    // Set up frame buffer
    SetAccumulationBuffer();
    ClearFrameBuffer(eClearFrameBufferFlag_Color | eClearFrameBufferFlag_Stencil,
                     true); // Perhaps there is some ambient light that can be added instead?
...
void cRendererDeferred::RenderIllumination() {
    ...
    while (illumIt.HasNext()) {
        ...
        eMaterialRenderMode renderMode = pMaterial->GetTexture(eMaterialTexture_IlluminationModulate)
                                             ? eMaterialRenderMode_IlluminationModulate
...
void cRendererDeferred::RenderFullScreenFog() {
    ...
    if (pProgram) {
        if (GetGBufferType() == eDeferredGBuffer_32Bit)
            pProgram->SetFloat(kVar_afNegFarPlane, -mpCurrentFrustum->GetFarPlane());
        pProgram->SetVec2f(
            kVar_avFogStartAndLength,
            cVector2f(mpCurrentWorld->GetFogStart(), mpCurrentWorld->GetFogEnd() - mpCurrentWorld->GetFogStart()));
...
void cRendererDeferred::RenderFog() {
    ...
    for (size_t i = 0; i < mpCurrentSettings->mvFogRenderData.size(); ++i) {
        ...
        if (GetGBufferType() == eDeferredGBuffer_32Bit)
            pProgram->SetFloat(kVar_afNegFarPlane, -mpCurrentFrustum->GetFarPlane());
        pProgram->SetVec2f(kVar_avFogStartAndLength,
                           cVector2f(pFogArea->GetStart(), pFogArea->GetEnd() - pFogArea->GetStart()));
        ...
        if (fogData.mbInsideNearFrustum == false) {
            cMatrixf mtxInvModelView = cMath::MatrixInverse(
                cMath::MatrixMul(mpCurrentFrustum->GetViewMatrix(), *pFogArea->GetModelMatrixPtr()));
            ...
            cVector3f vNegPlaneDistNeg(cMath::PlaneToPointDist(cPlanef(-1, 0, 0, 0.5f), vRayCastStart),
                                       cMath::PlaneToPointDist(cPlanef(0, -1, 0, 0.5f), vRayCastStart),
                                       cMath::PlaneToPointDist(cPlanef(0, 0, -1, 0.5f), vRayCastStart));
            cVector3f vNegPlaneDistPos(cMath::PlaneToPointDist(cPlanef(1, 0, 0, 0.5f), vRayCastStart),
                                       cMath::PlaneToPointDist(cPlanef(0, 1, 0, 0.5f), vRayCastStart),
                                       cMath::PlaneToPointDist(cPlanef(0, 0, 1, 0.5f), vRayCastStart));
...
void cRendererDeferred::RenderTranslucent() {
    ...
    while (transIt.HasNext()) {
        ...
        if (pMaterial->HasRefraction()) {
            ...
            CopyFrameBufferToTexure(mpRefractionTexture, cVector2l(clipRect.x, clipRect.y),
                                    cVector2l(clipRect.w, clipRect.h), cVector2l(clipRect.x, clipRect.y), true);
        }
        ...
        if (pMaterial->HasTranslucentIllumination()) {
            renderMode = renderMode == eMaterialRenderMode_Diffuse ? eMaterialRenderMode_Illumination
...
void cRendererDeferred::RenderReflection(iRenderable *apObject) {
    ...
    if (pRelfMaterial->GetMaxReflectionDistance() > 0) {
        cVector3f vPoint = mpCurrentFrustum->GetOrigin() +
                           mpCurrentFrustum->GetForward() * -1 * pRelfMaterial->GetMaxReflectionDistance();
        ...
        if (cMath::CheckPlaneBVCollision(maxRelfctionDistPlane, *pReflectionObject->GetBoundingVolume()) ==
            eCollision_Outside) {
...
void cRendererDeferred::RenderSubMeshEntityReflection(cSubMeshEntity *pReflectionObject) {
    ...
    // Make render target
    cRenderTarget renderTarget;
    cVector2l vCurrentFrameBufferSize =
        mpCurrentRenderTarget->mpFrameBuffer ? mpCurrentRenderTarget->mpFrameBuffer->GetSize() : mvScreenSize;
    renderTarget.mpFrameBuffer = mpReflectionBuffer;
    renderTarget.mvPos = mpCurrentRenderTarget->mvPos / mlReflectionSizeDiv;
    renderTarget.mvSize.x = mpCurrentRenderTarget->mvSize.x == -1
                                ? vCurrentFrameBufferSize.x / mlReflectionSizeDiv
                                : mpCurrentRenderTarget->mvSize.x / mlReflectionSizeDiv;
    renderTarget.mvSize.y = mpCurrentRenderTarget->mvSize.y == -1
                                ? vCurrentFrameBufferSize.y / mlReflectionSizeDiv
                                : mpCurrentRenderTarget->mvSize.y / mlReflectionSizeDiv;
    ...
    reflectFrustum.SetupPerspectiveProj(mtxReflProj, mtxReflView, mpCurrentFrustum->GetFarPlane(),
                                        mpCurrentFrustum->GetNearPlane(), mpCurrentFrustum->GetFOV(),
    ...
    if (mpCurrentSettings->mbClipReflectionScreenRect) {
        ...
        bool bVisible = cMath::GetNormalizedClipRectFromBV(vMin, vMax, *pReflectionObject->GetBoundingVolume(),
                                                           &reflectFrustum, fHalfFovTan);
        if (bVisible) {
            if (mbLog)
            ...
            if (bNeedsClipRect) {
                ...
                cRect2l clipRect;
                cMath::GetClipRectFromBV(clipRect, *pReflectionObject->GetBoundingVolume(), mpCurrentFrustum,
                                         vRenderTargetSize, fHalfFovTan);

                if (mbLog)
                    Log("  Setting up scissor rect. pos: (%d, %d)  %d x %d\n", clipRect.x, clipRect.y, clipRect.w,
                        clipRect.h);
    ...
    Render(mfCurrentFrameTime, &reflectFrustum, mpCurrentWorld, mpCurrentSettings->mpReflectionSettings, &renderTarget,
           false, mpCallbackList);
