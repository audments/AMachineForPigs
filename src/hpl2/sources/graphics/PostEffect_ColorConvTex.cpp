...
cPostEffectType_ColorConvTex::cPostEffectType_ColorConvTex(cGraphics *apGraphics, cResources *apResources)
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        mpProgram[i] = mpGraphics->CreateGpuProgramFromShaders(
            "BloomBlur" + cString::ToString(i), "deferred_base_vtx.glsl", "posteffect_color_conv_tex_frag.glsl", &vars);
        if (i == 1) {
            if (mpProgram[i]) {
...
cPostEffect_ColorConvTex::cPostEffect_ColorConvTex(cGraphics *apGraphics, cResources *apResources,
