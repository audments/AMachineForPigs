...
iTexture *cFrameTexture::GetTexture() { return mpTexture; }
...
cFrameSubImage *cFrameTexture::CreateCustomImage(const cVector2l &avPixelPos, const cVector2l &avPixelSize) {
    ...
    cVector2f vDestPos =
        cVector2f((float)avPixelPos.x / (float)vSourceSize.x, (float)avPixelPos.y / (float)vSourceSize.y);
    cVector2f vDestSize =
        cVector2f((float)avPixelSize.x / (float)vSourceSize.x, (float)avPixelSize.y / (float)vSourceSize.y);

    cFrameSubImage *pImage = hplNew(cFrameSubImage, ("", _W(""), this, NULL, cRect2l(avPixelPos, avPixelSize),
