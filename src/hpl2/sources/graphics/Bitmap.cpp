...
void cBitmap::Clear(const cColor &aColor, int alImage, int alMipMap) {
    ...
    unsigned char vClearColor[] = {FloatColorToUChar(aColor.r), FloatColorToUChar(aColor.g),
                                   FloatColorToUChar(aColor.b), FloatColorToUChar(aColor.a)};
...
void cBitmap::Blit(cBitmap *apSrcBmp, const cVector3l &avDestPosition, const cVector3l &avSrcSize,
                   const cVector3l &avSrcPosition, int alDestImage, int alDestMipMap, int alSrcImage, int alSrcMipMap) {
    if (mbDataIsCompressed)
    ...
    int lSrcAdress =
        vSrcPos.x + (vSrcPos.y * apSrcBmp->GetSize().x) + (vSrcPos.z * apSrcBmp->GetSize().x * apSrcBmp->GetSize().y);
    ...
    /*if(bOutOfBounds)
    {
            Log(" (%d, %d, %d) (%d, %d, %d) (%dx%dx%d) (%dx%dx%d) (%dx%dx%d)\n",	avDestPosition.x,avDestPosition.y,
    avDestPosition.z, avSrcPosition.x,avSrcPosition.y, avSrcPosition.z, avSrcSize.x, avSrcSize.y, avSrcSize.z,
                                                                                apSrcBmp->GetSize().x,
    apSrcBmp->GetSize().y, apSrcBmp->GetSize().z, GetSize().x, GetSize().y, GetSize().z
                                                );
    }*/
}
...
void cBitmap::SetPixel(int alImage, int alMipMap, const cVector3l &avPixelPos, unsigned char *apPixelData) {
    ...
    unsigned char *pPixelData =
        &pData->mpData[avPixelPos.z * mvSize.x * mvSize.y * mlBytesPerPixel +
                       avPixelPos.y * mvSize.x * mlBytesPerPixel + avPixelPos.x * mlBytesPerPixel];
...
void cBitmap::CopyPixel(unsigned char *apDest, ePixelFormat aDestFormat, unsigned char *apSrc,
                        ePixelFormat aSrcFormat) {
...
unsigned char *cBitmap::ConvertDataToFormat(unsigned char *apPixelData, ePixelFormat aSrcFormat,
                                            ePixelFormat aDestFormat) {
