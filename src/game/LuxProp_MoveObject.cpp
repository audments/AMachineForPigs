...
bool cLuxProp_MoveObject::CanInteract(iPhysicsBody *apBody) { return msInteractCallback != ""; }

//-----------------------------------------------------------------------

bool cLuxProp_MoveObject::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos) { return msInteractCallback != ""; }
...
void cLuxProp_MoveObject::UpdatePropSpecific(float afTimeStep) {
    // Log("Body pos: (%s) Size: (%s)\n", mvBodies[0]->GetWorldPosition().ToString().c_str(),
    // mvBodies[0]->GetShape()->GetSize().ToString().c_str());
...
void cLuxProp_MoveObject::MoveToState(float afState, float afAcc, float afMaxSpeed, float afSlowdownDist,
                                      bool abResetSpeed) {
    ///////////////////
    // Linear
    if (mMoveObjectType == eLuxMoveObjectType_Linear) {
        cVector3f vWanted =
            m_mtxClosedTransform.GetTranslation() * (1 - afState) + m_mtxOpenTransform.GetTranslation() * afState;
    ...
    // Angular
        ...
        if (mbUseAngularLocalOffset)
            MoveAngularTo(mtxWanted, afAcc, afMaxSpeed, afSlowdownDist, abResetSpeed, true, mvAngularOffsetPos,
                          vLocalOffset);
        else
...
void cLuxProp_MoveObject::OnStartMove() { mbAutoMoveReachedGoal = false; }
...
void cLuxProp_MoveObject::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
