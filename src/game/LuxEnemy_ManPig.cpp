...
bool cLuxEnemy_ManPig::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage *apMessage) {
    ...
    kLuxOnMessage(eLuxEnemyMessage_ChangePose)
        ...
        kLuxState(eLuxEnemyState_Idle) kLuxOnEnter mbPlayerInRange =
            false; // Reset so new checks are made in case player is near.
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut)
        ...
        if (mPreviousState == eLuxEnemyState_Search || mPreviousState == eLuxEnemyState_Stalk ||
    ...
    if (mbThreatenOnAlert && PlayerIsDetected()) {
        float fAngleDist =
            cMath::GetAngleDistanceRad(mpCharBody->GetYaw() + kPif, gpBase->mpPlayer->GetCharacterBody()->GetYaw());
    ...
    // Player is seen, see if close enough for hunt
        ...
        if (mbThreatenOnAlert) {
            ...
            if (mbFleeFromPlayer) {
                ...
                if (mfAlertRunTowardsCount > mfAlertRunTowardsToHuntLimit ||
                    fDistToPlayer < mfAlertToInstantHuntDistance * 2) {
            ...
            // Attack
                ...
                if ((gpBase->mpPlayer->GetTerror() >= 1 && (mfAlertRunTowardsCount > mfAlertRunTowardsToHuntLimit ||
                                                            fDistToPlayer < mfAlertToInstantHuntDistance * 2)) ||
            ...
            if (mbIsTelsa == true)
                fTerror *= 3;
            if ((fTerror >= 1 &&
                 (fDistToPlayer > mfAlertToHuntDistance || mfAlertRunTowardsCount > mfAlertRunTowardsToHuntLimit)) ||
                fDistToPlayer < mfAlertToInstantHuntDistance) {
                gpBase->mpPlayer->SetTerror(1.0f);
    ...
    if (fPlayerDistance < 4.5f || bSeenByPlayer || IsVisibleToPlayerAtFeetPos(mpPathfinder->GetNextGoalPos()) ||
    ...
    if (CanSeePlayer() && fDist > mfNormalAttackDistance && fDist < mfNormalAttackDistance * 4.0f &&
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut)
        ...
        mpMap->BroadcastEnemyMessage(eLuxEnemyMessage_HelpMe, true, mpCharBody->GetPosition(),
                                     mfActivationDistance * 0.5f, 0, false, mpCharBody->GetFeetPosition());
    ...
    SendMessage(eLuxEnemyMessage_TimeOut_2,
                cMath::RandRectf(mfHuntPauseMinTime, mfHuntPauseMaxTime) * mfHuntPauseTimeMul, true);
    mpPathfinder->Stop();
    ...
    kLuxOnMessage(eLuxEnemyMessage_TimeOut) if (cMath::RandRectl(0, 1) == 0 &&
                                                DistToPlayer() < mfNormalAttackDistance * 1.3f) {
        ChangeState(eLuxEnemyState_AttackMeleeShort);
    }
...
bool cLuxEnemy_ManPig::PlayerIsDetected() {
    if (CanSeePlayer()) {
        ...
        return (DistToPlayer() < mpCharBody->GetSize().x && PlayerInFOV()) ||
               (mfInLanternLightCount >= 1 && mbBlind == false);
    }
}
...
float cLuxEnemy_ManPig::GetDamageMul(float afAmount, int alStrength) {
    if (mCurrentState == eLuxEnemyState_Idle || mCurrentState == eLuxEnemyState_Wait ||
        mCurrentState == eLuxEnemyState_Patrol || mCurrentState == eLuxEnemyState_Investigate) {
        return 1.0f;
    }
...
bool cLuxEnemy_ManPig::FleeTryToFindSafeNode() {
    ...
    for (int i = 0; i < 10; ++i) {
        ...
        cAINode *pNode = mpPathfinder->GetNodeAtPos(mpCharBody->GetPosition() + vDirToPlayer * fLength * 0.2f, 0,
                                                    fLength, false, false, true, NULL);
...
bool cLuxEnemy_ManPig::TrackFindNode() {
    ...
    for (int i = 0; i < lNumOfTries; ++i) {
        ...
        // Find a node to run to
        cAINode *pNode =
            mpPathfinder->GetNodeAtPos(GetPlayerFeetPos(), fMinDistance, fMaxDistance, false, false, true, NULL);
        if (pNode == NULL) {
            return false;
        }
...
bool cLuxEnemy_ManPig::TrackTeleportBehindPlayer() {
    ...
    for (int i = 0; i < lNumOfTries; ++i) {
        ...
        // Find a node to run to
        cAINode *pNode =
            mpPathfinder->GetNodeAtPos(GetPlayerFeetPos(), fDistance * 0.5f, fDistance, false, true, false, NULL);
        if (pNode == NULL) {
            return false;
        }
...
void cLuxEnemy_ManPig::SetTeslaFadeDisabled(bool abX) { mbTeslaFadeDisabled = abX; }
...
void cLuxEnemy_ManPig::SetTeslaEasyEscapeDisabled(bool abX) { mbTeslaEasyEscapeDisabled = abX; }
...
void cLuxEnemy_ManPig::UpdateTesla(float afTimeStep) {
    ...
    // Tesla Flickering
    {
        ...
        if (mfTeslaFlickerTimer <= 0) {
            if (mlTeslaFlickerState == 0) {
                ...
                mlTeslaFlickerState = 0;
                mfTeslaFlickerTimer =
                    cMath::RandRectl(0, 8) == 0 ? cMath::RandRectf(0.1f, 0.2f) : cMath::RandRectf(0.05f, 0.15f);
            }
        }
    ...
    // Special Notice Checks
    {
        ...
        if (mbTeslaEasyEscapeDisabled == false && mfTeslaSpecialNoticeCount < 0.35f && mbCanSeePlayer == false &&
            gpBase->mpPlayer->GetHealth() < 75) {
            if (mCurrentState == eLuxEnemyState_Hunt || mCurrentState == eLuxEnemyState_HuntPause ||
            ...
            // Blink
            {
                ...
                if (mfMindFuckBlinkCount < 0) {
                    if (mlMindFuckBlinkState == 0) {
                        ...
                        mlMindFuckBlinkState = 0;
                        mfMindFuckBlinkCount =
                            cMath::RandRectl(0, 4) == 0 ? cMath::RandRectf(0.75f, 2.2f) : cMath::RandRectf(0.1f, 0.5f);
                    }
                }
...
kBeginSerialize(cLuxEnemy_ManPig_SaveData, iLuxEnemy_SaveData)
    ...
    kSerializeVar(mbThreatenOnAlert, eSerializeType_Bool) kSerializeVar(mbFleeFromPlayer, eSerializeType_Bool)
        kSerializeVar(mbAutoDisableAfterFlee, eSerializeType_Bool)
            kSerializeVar(mfLanternSensitivity, eSerializeType_Float32)
                kSerializeVar(mIdleBehavior, eSerializeType_Int32) kSerializeVar(mPatrolMoveSpeed, eSerializeType_Int32)
...
void cLuxEnemy_ManPig::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
