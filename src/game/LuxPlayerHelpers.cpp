...
void cLuxPlayerVoiceFlashback::Start(const tString &asVoiceFile, const tString &asTextEntry1, float afDelay1,
                                     const tString &asTextEntry2, float afDelay2, const tString &asTextEntry3,
                                     float afDelay3, const tString &asTextEntry4, float afDelay4,
                                     const tString &asTextEntry5, float afDelay5, const tString &asTextEntry6,
                                     float afDelay6, const tString &asTextEntry7, float afDelay7, bool abUseEffects,
    ...
    gpBase->mpEffectHandler->GetPlayVoice()->AddMultiSubbedVoice(
        asVoiceFile, "", "Voice", asTextEntry1, afDelay1, asTextEntry2, afDelay2, asTextEntry3, afDelay3, asTextEntry4,
...
cLuxPlayerInfectionCollapse::cLuxPlayerInfectionCollapse(cLuxPlayer *apPlayer)
    ...
    mfHeightAddCollapseSpeed =
        gpBase->mpGameCfg->GetFloat("Player_General", "InfectionCollapse_HeightAddCollapseSpeed", 0);
...
void cLuxPlayerInfectionCollapse::Start() {
    ...
    // pSoundHandler->FadeGlobalSpeed(0.5f, 0.125f, eSoundEntryType_World,eLuxGlobalVolumeType_InfectionCollapse,
    // false);
...
void cLuxPlayerInfectionCollapse::Update(float afTimeStep) {
    ...
    if (mlState == 1) {
        ...
        if (mfTimer > mfSleepTime) {
            cSoundHandler *pSoundHandler = gpBase->mpEngine->GetSound()->GetSoundHandler();
            pSoundHandler->FadeGlobalVolume(1, 0.2f, eSoundEntryType_World, eLuxGlobalVolumeType_InfectionCollapse,
                                            false);
            // pSoundHandler->FadeGlobalSpeed(1, 0.25f, eSoundEntryType_World,eLuxGlobalVolumeType_InfectionCollapse,
            // false);
...
void cLuxPlayerSpawnPS::Stop() { mbActive = false; }
...
cLuxPlayerHurt::cLuxPlayerHurt(cLuxPlayer *apPlayer) : iLuxPlayerHelper(apPlayer, "LuxPlayerHurt") {
    ...
    for (size_t i = 0; i < mvNoiseGfx.size(); ++i) {
        mvNoiseGfx[i] = pGui->CreateGfxTexture("hud_hurt_noise0" + cString::ToString((int)i) + ".dds",
                                               eGuiMaterial_Modulative, eTextureType_2D);
    }
    ...
    if (mfPantCount > 0 && fSpeed < 0.05f) {
        ...
        if (mfPantPosAddDir > 0) {
            if (mfPantPosAdd > mfPantSize) {
                ...
                gpBase->mpHelpFuncs->PlayGuiSoundData("hurt_pant", eSoundEntryType_Gui,
                                                      cMath::Min(mfAlpha + 0.3f, 1.0f));
            }
        } else {
...
void cLuxPlayerHurt::OnDraw(float afFrameTime) {
    ...
    cVector2l vCount = cVector2l((int)(gpBase->mvHudVirtualSize.x / vNoiseSize.x) + 1,
                                 (int)(gpBase->mvHudVirtualSize.y / vNoiseSize.y) + 1);
    ...
    for (int y = 0; y < vCount.y; ++y) {
        for (int x = 0; x < vCount.x; ++x) {
            gpBase->mpGameHudSet->DrawGfx(mvNoiseGfx[mlCurrentNoise], vPos, vNoiseSize,
                                          cColor(1, 1 - mfAlpha, 1 - mfAlpha));
            vPos.x += vNoiseSize.x;
        }
...
void cLuxPlayerFlashback::Start(const tString &asFlashbackFile, const tString &asCallback) {
    ...
    gpBase->mpEngine->GetSound()->GetSoundHandler()->FadeGlobalVolume(
        mfWorldSoundVolume, (1 - mfWorldSoundVolume) / 1.5f, eSoundEntryType_World, eLuxGlobalVolumeType_Flashback,
        false);
...
void cLuxPlayerFlashback::Update(float afTimeStep) {
    ...
    // Update effects and check if over
        ...
        if (gpBase->mpEffectHandler->GetPlayVoice()->VoiceDonePlaying()) {
            ...
            // World sounds
            gpBase->mpEngine->GetSound()->GetSoundHandler()->FadeGlobalVolume(1, 1 / fFadeTime, eSoundEntryType_World,
                                                                              eLuxGlobalVolumeType_Flashback, true);
...
void cLuxPlayerFlashback::LoadAndPlayFlashbackFile(const tString &asFlashbackFile) {
    ...
    while (it.HasNext()) {
        ...
        gpBase->mpEffectHandler->GetPlayVoice()->AddVoice(sVoiceFile, sEffectFile, sTextCat, sTextEntry, false, 0, 0,
                                                          0);
    }
...
cLuxPlayerLookAt::cLuxPlayerLookAt(cLuxPlayer *apPlayer) : iLuxPlayerHelper(apPlayer, "LuxPlayerLookAt") { Reset(); }
...
void cLuxPlayerLookAt::SetTarget(const cVector3f &avTargetPos, float afSpeedMul, float afMaxSpeed,
                                 const tString &asAtTargetCallback) {
...
cLuxPlayerInfection::cLuxPlayerInfection(cLuxPlayer *apPlayer) : iLuxPlayerHelper(apPlayer, "LuxPlayerInfection") {
    ...
    mfHighInfectionLimitForDeathTimer =
        gpBase->mpGameCfg->GetFloat("Player_Infection", "HighInfectionLimitForDeathTimer", 0);
    mfTimeUntilDeathAtHighInfection =
        gpBase->mpGameCfg->GetFloat("Player_Infection", "TimeUntilDeathAtHighInfection", 0);
...
void cLuxPlayerInfection::StartInfectionIncreaseEffects() { mfInfectionIncreaseSoundEffectsTimer = 1.0f; }
...
void cLuxPlayerInfection::UpdateInfectionVisuals(float afTimeStep) {
    ...
    gpBase->mpPostEffectHandler->GetInfection()->SetWaveAlpha(mfInfectionWaveAlpha *
    mfInfectionWaveAlphaMul);//mfInfectionWaveAlpha);
    gpBase->mpPostEffectHandler->GetInfection()->SetWaveSpeed(mfInfectionWaveAlpha *
    mfInfectionWaveSpeedMul);//*mfInfectionWaveAlpha);
...
void cLuxPlayerInfection::UpdateCheckEnemySeen(float afTimeStep) {
    ...
    while (it.HasNext()) {
        ...
        if (pCam->GetFrustum()->CollideBoundingVolume(pCharBody->GetCurrentBody()->GetBoundingVolume()) ==
            eCollision_Outside) {
            continue;
        }
...
void cLuxPlayerInfection::UpdateStingersAndLoops(float afTimeStep) {
    if (mbIsFauxMode) {
        ...
        if (mpCurrentInfectionLoopSound &&
            pSoundHandler->IsValid(mpCurrentInfectionLoopSound, mlCurrentInfectionLoopSoundID)) {
    ...
    if (nInfectionLevel != mnPreviousSoundInfectionLevel) {
        ...
        // pSoundHandler->FadeGlobalVolume(1, 0.3f, eSoundEntryType_World,eLuxGlobalVolumeType_InfectionCollapse,
        // false); pSoundHandler->FadeGlobalSpeed(1, 0.5f, eSoundEntryType_World,eLuxGlobalVolumeType_InfectionCollapse,
        // false);
...
void cLuxPlayerDeath::LoadFonts() { mpFont = LoadFont("game_default.fnt"); }

//-----------------------------------------------------------------------

void cLuxPlayerDeath::LoadUserConfig() { mbShowHint = gpBase->mpUserConfig->GetBool("Game", "ShowDeathHints", true); }

void cLuxPlayerDeath::SaveUserConfig() { gpBase->mpUserConfig->SetBool("Game", "ShowDeathHints", mbShowHint); }
    ...
    if (mlState == 1) {
        ...
        if (mfFadeAlpha > 1) {
            ...
            if (mpPlayer->UsePermaDeath()) {
                ...
                if (gpBase->mpHelpFuncs->PlayGuiSoundData(mpPlayer->GetCurrentPermaDeathSound(), eSoundEntryType_Gui, 1,
                                                          eSoundEntityType_Main, true, &extraData)) {
...
void cLuxPlayerDeath::OnDraw(float afFrameTime) {
    ...
    gpBase->mpGameHudSet->DrawGfx(mpWhiteModGfx, gpBase->mvHudVirtualStartPos + cVector3f(0, 0, 3),
                                  gpBase->mvHudVirtualSize, cColor(1 - mfFadeAlpha, 1));
    ...
    for (size_t row = 0; row < vRows.size(); ++row) {
        ...
        for (size_t i = 0; i < sStr.length(); ++i) {
            ...
            gpBase->mpGameHudSet->DrawFont(sChar, mpFont, vPos + cVector3f(vSize.x * 0.05, vSize.y * 0.05, -1),
                                           vSize * 1.1, cColor(1, fBlurAlpha * 0.3f), eFontAlign_Right);
            gpBase->mpGameHudSet->DrawFont(sChar, mpFont, vPos + cVector3f(vSize.x * 0.15, vSize.y * 0.15, -2),
                                           vSize * 1.3, cColor(1, fBlurAlpha * 0.2f), eFontAlign_Right);
    ...
    // gpBase->mpGameHudSet->DrawFont(kTranslate("Game", "DeathPress"),mpFont, cVector3f(400, 550, 6), 17,
    // cColor(1,mFlashOscill.val,mFlashOscill.val, mfTextAlpha2),eFontAlign_Center);
}
...
void cLuxPlayerLean::Update(float afTimeStep) {
    ...
    if (mbPressed) {
        ...
        int lCount = 0;
        while (pPhysicsWorld->CheckShapeWorldCollision(NULL, mpHeadShape, cMath::MatrixTranslate(vPos), NULL, false,
                                                       true, NULL, false)) {
...
void cLuxPlayerHudEffect::DrawSplashes(float afFrameTime) {
    ...
    for (; it != mlstSplashes.end(); ++it) {
        ...
        gpBase->mpGameHudSet->DrawGfx(pSplash->mpImage, pSplash->mvPos, pSplash->mvSize,
                                      cColor(1, pSplash->mfAlpha * pSplash->mfAlphaMul));
    }
}
...
void cLuxPlayerHudEffect::DrawFlash(float afFrameTime) {
    ...
    gpBase->mpGameHudSet->DrawGfx(mpFlashGfx, gpBase->mvHudVirtualStartPos + cVector3f(0, 0, 0),
                                  gpBase->mvHudVirtualSize, col, mFlashMaterial);
}
...
void cLuxPlayerLightLevel::OnMapEnter(cLuxMap *apMap) { mfUpdateCount = 0; }
...
bool cLuxPlayerIsMoving::PlayerIsMovingFast() { return mfAveragePlayerSpeed > mfFastMovementThreshold; }

//-----------------------------------------------------------------------

bool cLuxPlayerIsMoving::PlayerIsMovingSlowly() { return mfImmediatePlayerSpeed > mfSlowMovementThreshold; }
...
void cLuxPlayerIsMoving::Reset() { mfImmediatePlayerSpeed = 0.0f; }
