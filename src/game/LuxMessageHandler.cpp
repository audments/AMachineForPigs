...
void cLuxMessageHandler::SaveUserConfig() { gpBase->mpUserConfig->SetBool("Game", "ShowSubtitles", mbShowSubtitles); }

//-----------------------------------------------------------------------

void cLuxMessageHandler::OnMapEnter(cLuxMap *apMap) { Reset(); }
...
void cLuxMessageHandler::Update(float afTimeStep) {
    ...
    if (mbQuestMessageActive) {
        if (mfQuestMessageAlpha < 0.1) {
            ...
            if (mfQuestMessageAlpha >= 0.1)
                gpBase->mpHelpFuncs->PlayGuiSoundData(
                    msQuestAddedSound, eSoundEntryType_Gui); // Have it here so that is not played directly!
...
void cLuxMessageHandler::DrawQuestAdded() {
    ...
    for (int i = 0; i < 2; ++i)
        gpBase->mpGameHudSet->DrawGfx(mpQuestAddedIcon, vPos + cVector3f(0, 0, 1), -1,
                                      cColor(mfQuestMessageAlpha * mQuestOscill.val, 1), eGuiMaterial_Additive);
}
...
void cLuxMessageHandler::DrawMessage() {
    ...
    for (size_t i = 0; i < mvMessageRows.size(); ++i) {
        gpBase->mpGameHudSet->DrawFont(mvMessageRows[i], mpFont, vTextPos, mvFontSize, cColor(1, fAlpha),
                                       eFontAlign_Center);
...
void cLuxMessageHandler::DrawPauseMessage() {
    ...
    ////////////////////////
    // Black background
    gpBase->mpGameHudSet->DrawGfx(mpBlackGfx, cVector3f(0, 0, 3), cVector2f(800, 600),
                                  cColor(1, mfPauseMessageAlpha * 0.5f));
    ...
    if (mbMessageYesNo) {
        ...
        vTextPos.x += 80;
        gpBase->mpGameHudSet->DrawFont(mpFont, vTextPos, mvFontSize - cVector2f(1), cColor(1, mfPauseMessageAlpha),
                                       _W("(%ls)"), gpBase->mpInputHandler->GetInputName("Interact").c_str());
        ...
        vTextPos.x += 80;
        gpBase->mpGameHudSet->DrawFont(mpFont, vTextPos, mvFontSize - cVector2f(1), cColor(1, mfPauseMessageAlpha),
                                       _W("(%ls)"), gpBase->mpInputHandler->GetInputName("Attack").c_str());
    }
}
