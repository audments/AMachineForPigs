...
void cLuxLineOfSightCallback::Reset() { mbIntersected = false; }
...
bool cLuxMapHelper::ShapeDamage(iCollideShape *apShape, const cMatrixf &a_mtxTransform, const cVector3f &avOrigin,
                                float afMinDamage, float afMaxDamage, float afForce, float afMaxImpulse, int alStrength,
                                float afHitSpeed, eLuxDamageType aDamageType, eLuxWeaponHitType aWeaponHitType,
    ...
    for (size_t i = 0; i < vBodies.size(); ++i) {
        ...
        bool bCollide = pPhysicsWorld->CheckShapeCollision(apShape, a_mtxTransform, pBody->GetShape(),
                                                           pBody->GetLocalMatrix(), collideData, 4, false);
        if (bCollide == false) {
            continue;
        }
...
bool cLuxMapHelper::GetClosestCharCollider(const cVector3f &avStart, const cVector3f &avDir, float afRayLength,
                                           bool abCheckDynamic, float *afDistance, cVector3f *avNormal,
                                           iPhysicsBody **apBody) {
    cLuxMap *pCurrentMap = gpBase->mpMapHandler->GetCurrentMap();
    if (pCurrentMap == NULL)
        return false;
...
float cLuxMapHelper::GetLightLevelAtPos(const cVector3f &avPos, std::vector<iLight *> *apSkipLightsVec,
                                        float afRadiusAdd) {
    ////////////////////////////
    // Check so there really is a world
    cLuxMap *pCurrentMap = gpBase->mpMapHandler->GetCurrentMap();
...
void cLuxMapHelper::GetLightsAtNode(iRenderableContainerNode *apNode, tLightList &alstLights, const cVector3f &avPos,
                                    float afRadius, bool includeInvisible, bool includeBoxLights) {
    ///////////////////////////////////////
    // Make sure node is updated and check point in bv
    apNode->UpdateBeforeUse();
    ...
    if (apNode->HasObjects()) {
        ...
        for (; it != apNode->GetObjectList()->end(); ++it) {
            iRenderable *pObject = *it;
            if ((pObject->IsVisible() == false && !includeInvisible) ||
                pObject->GetRenderType() != eRenderableType_Light)
                continue;
            ...
            case eLightType_Box:
                bAdd = includeBoxLights && (cMath::CheckPointInBVIntersection(avPos, *pLight->GetBoundingVolume()) ||
                                            cMath::CheckPointInBVIntersection(avPos + cVector3f(afRadius, 0, 0),
                                                                              *pLight->GetBoundingVolume()) ||
                                            cMath::CheckPointInBVIntersection(avPos + cVector3f(-afRadius, 0, 0),
                                                                              *pLight->GetBoundingVolume()) ||
                                            cMath::CheckPointInBVIntersection(avPos + cVector3f(0, 0, afRadius),
                                                                              *pLight->GetBoundingVolume()) ||
                                            cMath::CheckPointInBVIntersection(avPos + cVector3f(0, 0, -afRadius),
                                                                              *pLight->GetBoundingVolume()) ||
                                            cMath::CheckPointInBVIntersection(avPos + cVector3f(0, afRadius, 0),
                                                                              *pLight->GetBoundingVolume()) ||
                                            cMath::CheckPointInBVIntersection(avPos + cVector3f(0, -afRadius, 0),
                                                                              *pLight->GetBoundingVolume()));
                break;
            case eLightType_Point:
                bAdd = cMath::Vector3DistSqr(avPos, pLight->GetWorldPosition()) <=
                       pLight->GetRadius() * pLight->GetRadius() * afRadius * afRadius;
                break;
            case eLightType_Spot:
                cLightSpot *pSpotLight = static_cast<cLightSpot *>(pLight);
