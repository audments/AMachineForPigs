...
iLuxPlayerState_DefaultBase::iLuxPlayerState_DefaultBase(cLuxPlayer *apPlayer, eLuxPlayerState aType)
    ...
    mvFocusCrosshairGfx[eLuxFocusCrosshair_Grab] =
        pGui->CreateGfxImage("hud_crosshair_over_grab.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_Push] =
        pGui->CreateGfxImage("hud_crosshair_over_push.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_Ignite] =
        pGui->CreateGfxImage("hud_crosshair_over_ignite.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_Pick] =
        pGui->CreateGfxImage("hud_crosshair_over_pick.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_LevelDoor] =
        pGui->CreateGfxImage("hud_crosshair_over_leveldoor.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_Ladder] =
        pGui->CreateGfxImage("hud_crosshair_over_ladder.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_VoiceOver] =
        pGui->CreateGfxImage("hud_crosshair_over_voice.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_PhoneBox] =
        pGui->CreateGfxImage("hud_crosshair_over_phone.tga", eGuiMaterial_Alpha);
    mvFocusCrosshairGfx[eLuxFocusCrosshair_Note] =
        pGui->CreateGfxImage("hud_crosshair_over_note.tga", eGuiMaterial_Alpha);
}
...
void iLuxPlayerState_DefaultBase::Update(float afTimeStep) {
    ...
    gpBase->mpMapHelper->GetClosestEntity(pCam->GetPosition(), pCam->GetForward(), 20.0f, &mfFocusDistance,
                                          &mpBodyInFocus, &mpEntityInFocus);
...
float iLuxPlayerState_DefaultBase::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 14, cColor(1, 1), _W("Focus Body: %ls Dist: %f"),
                    sBodyName.c_str(), mfFocusDistance);
...
bool iLuxPlayerState_DefaultBase::ShowOutlineOnEntity(iLuxEntity *apEntity, iPhysicsBody *apBody,
    ...
    return pProp->GetPropType() == eLuxPropType_Item && apEntity->CanInteract(apBody) &&
           apEntity->GetInteractionDisabled() == false;
}
...
void iLuxPlayerState_DefaultBase::AddOutlineObjects(iPhysicsBody *apBody, iLuxEntity *apEntity,
                                                    const cVector3f &avFocusPos) {
    gpBase->mpEffectRenderer->ClearOutlineObjects();
    if (apEntity == NULL || apEntity->GetEntityType() != eLuxEntityType_Prop ||
