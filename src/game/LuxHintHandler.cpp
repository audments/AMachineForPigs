...
cLuxHintHandler::cLuxHintHandler() : iLuxUpdateable("LuxHintHandler") {
    ...
    mpGamepadButtonIcon[eGamepadButton_A] =
        pGui->CreateGfxTexture("gamepad_a.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_B] =
        pGui->CreateGfxTexture("gamepad_b.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_X] =
        pGui->CreateGfxTexture("gamepad_x.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_Y] =
        pGui->CreateGfxTexture("gamepad_y.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_LeftShoulder] =
        pGui->CreateGfxTexture("gamepad_lb.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_RightShoulder] =
        pGui->CreateGfxTexture("gamepad_rb.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_Back] =
        pGui->CreateGfxTexture("gamepad_select.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_Start] =
        pGui->CreateGfxTexture("gamepad_start.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_LeftStick] =
        pGui->CreateGfxTexture("gamepad_ls.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_RightStick] =
        pGui->CreateGfxTexture("gamepad_rs.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_DpadUp] =
        pGui->CreateGfxTexture("gamepad_dpad_u.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_DpadRight] =
        pGui->CreateGfxTexture("gamepad_dpad_r.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_DpadLeft] =
        pGui->CreateGfxTexture("gamepad_dpad_l.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGamepadButtonIcon[eGamepadButton_DpadDown] =
        pGui->CreateGfxTexture("gamepad_dpad_d.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    ...
    mpGampadAxisIcon[eGamepadAxis_LeftTrigger * 2 + eGamepadAxisRange_Positive - 1] =
        pGui->CreateGfxTexture("gamepad_lt.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
    mpGampadAxisIcon[eGamepadAxis_RightTrigger * 2 + eGamepadAxisRange_Positive - 1] =
        pGui->CreateGfxTexture("gamepad_rt.tga", eGuiMaterial_Alpha, eTextureType_2D, cColor(1, 1), false);
...
void cLuxHintHandler::LoadUserConfig() { mbActive = gpBase->mpUserConfig->GetBool("Game", "ShowHints", true); }

//-----------------------------------------------------------------------

void cLuxHintHandler::SaveUserConfig() { gpBase->mpUserConfig->SetBool("Game", "ShowHints", mbActive); }

//-----------------------------------------------------------------------

void cLuxHintHandler::Update(float afTimeStep) { UpdateHintText(afTimeStep); }

//-----------------------------------------------------------------------

void cLuxHintHandler::OnDraw(float afFrameTime) { DrawHintText(afFrameTime, gpBase->mpGameHudSet); }
...
void cLuxHintHandler::DrawHintText(float afFrameTime, cGuiSet *apGuiSet) {
    ...
    apGuiSet->DrawFont(mpFont, cVector3f(400, mfYPos, 20), mvFontSize, hintCol, eFontAlign_Center,
                       eGuiMaterial_FontNormal, _W(" %ls"), kTranslate("Hints", "HINT:").c_str());
    ...
    if (vRows.size() == 1) {
        apGuiSet->DrawFont(mpFont, cVector3f(400, fStartY, 20), mvFontSize, cColor(1, mfAlpha), eFontAlign_Center,
                           eGuiMaterial_FontNormal, _W(" %ls"), msCurrentText.c_str());
    } else {
        for (size_t i = 0; i < vRows.size(); ++i) {
            apGuiSet->DrawFont(
                mpFont, cVector3f(400 - fMaxWidth * 0.5f, fStartY + (float)i * (mvFontSize.y + 2.0f), 20), mvFontSize,
                cColor(1, mfAlpha), eFontAlign_Left, eGuiMaterial_FontNormal, _W(" %ls"), vRows[i].c_str());
        }
    }
    ...
    for (size_t i = 0; i < mvHintIcons.size(); ++i) {
        apGuiSet->DrawGfx(mvHintIcons[i].mpGuiIcon, mvHintIcons[i].mvPosition, mvHintIcons[i].mvSize,
                          cColor(1, mfAlpha), eGuiMaterial_Alpha);
    }
#endif
}
...
void cLuxHintHandler::Block(const tString &asName) { m_setHintsBlocked.insert(asName); }

void cLuxHintHandler::UnBlock(const tString &asName) { m_setHintsBlocked.erase(asName); }
