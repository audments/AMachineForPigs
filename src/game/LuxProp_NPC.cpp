...
void cLuxPropLoader_NPC::LoadVariables(iLuxProp *apProp, cXmlElement *apRootElem) {
    ...
    // Add arrays!
        ...
        for (size_t i = 0; i < vBoneNameVec.size(); ++i) {
            ...
            boneData.mvRotVec = cMath::Vector3Normalize(cMath::MatrixMul(
                mtxInvWorldBone, cVector3f(0, 1, 0))); // Make sure to normalize since matrix might have scale!
        }
    }
...
bool cLuxProp_NPC::CanInteract(iPhysicsBody *apBody) { return false; }

//-----------------------------------------------------------------------

bool cLuxProp_NPC::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos) { return false; }
...
kBeginSerialize(cLuxProp_NPC_SaveData, iLuxProp_SaveData)
    ...
    kSerializeVar(msFollowPlayerArea, eSerializeType_String) kSerializeVar(mfHeadAngle, eSerializeType_Float32)
        kSerializeVar(mbPlayerIsInArea, eSerializeType_Bool) kSerializeVar(mbAwake, eSerializeType_Bool)
            kSerializeVar(mbPlayingWakeAnim, eSerializeType_Bool) kSerializeVar(mbFollowPlayer, eSerializeType_Bool)
...
void cLuxProp_NPC::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
