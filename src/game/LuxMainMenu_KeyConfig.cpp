...
cLuxInputMenuEntry::cLuxInputMenuEntry(cLuxMainMenu_KeyConfig *apWindow, cLuxAction *apAction, iWidget *apParent,
    ...
    for (int i = 0; i < eInputMenuEntryPos_LastEnum; ++i) {
        cWidgetLabel *pLabel = mpWindow->mpGuiSet->CreateWidgetLabel(cVector3f(mpWindow->mvHeaderPositions.v[i], 0, 0),
                                                                     mpWindow->mvEntrySize, _W(""), mpLAction);
...
eInputMenuEntryPos cLuxInputMenuEntry::GetGamepadHatStatePos(eGamepadHat aHat, eGamepadHatState aState) {
    for (size_t i = 0; i < mvSubActions.size(); ++i) {
        const cSubActionWrapper &subAction = mvSubActions[i];
        if (subAction.mGamepadInputType == eGamepadInputType_Hat && subAction.mlInputId == aHat &&
            subAction.mfInputValue == aState)
            return (eInputMenuEntryPos)i;
    }
...
eInputMenuEntryPos cLuxInputMenuEntry::GetGamepadAxisPos(eGamepadAxis aAxis, eGamepadAxisRange aRange) {
    for (size_t i = 0; i < mvSubActions.size(); ++i) {
        const cSubActionWrapper &subAction = mvSubActions[i];
        if (subAction.mGamepadInputType == eGamepadInputType_Axis && subAction.mlInputId == aAxis &&
            subAction.mfInputValue == aRange)
            return (eInputMenuEntryPos)i;
    }
...
bool cLuxInputMenuEntry::HasKey(eKey aKey) { return GetKeyPos(aKey) != eInputMenuEntryPos_LastEnum; }
    ...
    for (size_t i = 0; i < mvSubActions.size(); ++i) {
        ...
        case eInputDeviceType_Gamepad:
            ...
            case eGamepadInputType_Button:
                ...
                break;
            case eGamepadInputType_Hat:
                if (apEntry->HasGamepadHatState((eGamepadHat)subAction.mlInputId,
                                                eGamepadHatState((int)subAction.mfInputValue)))
                    vEntryPositions.push_back((eInputMenuEntryPos)i);
                break;
            case eGamepadInputType_Axis:
                if (apEntry->HasGamepadAxis((eGamepadAxis)subAction.mlInputId,
                                            eGamepadAxisRange((int)subAction.mfInputValue)))
                    vEntryPositions.push_back((eInputMenuEntryPos)i);
                break;
            }
            break;
...
tString cLuxInputMenuEntry::GetStringFromGamepadSubAction(const cSubActionWrapper &aSubAction) {
    ...
    case eGamepadInputType_Hat: {
        int lState = (int)aSubAction.mfInputValue;
        return iGamepad::HatToString((eGamepadHat)aSubAction.mlInputId) + "." +
               iGamepad::HatStateToString((eGamepadHatState)lState);
    }
    case eGamepadInputType_Axis: {
        int lRange = (int)aSubAction.mfInputValue;
        return iGamepad::AxisToString((eGamepadAxis)aSubAction.mlInputId) + "." +
               iGamepad::AxisRangeToString((eGamepadAxisRange)lRange);
    }
    }
...
cSubActionWrapper cLuxInputMenuEntry::GetSubActionWrapperFromStringVecGamepad(const tStringVec &avInputParts,
                                                                              int alValue) {
    /*
...
void cLuxInputMenuEntry::SaveGamepadSubAction(eInputMenuEntryPos aPos, eGamepadInputType aType, int alInputId,
                                              float afValue) {
    if (aPos == eInputMenuEntryPos_LastEnum)
        return;
...
bool cLuxInputMenuEntry::InputEntryClick(iWidget *apWidget, const cGuiMessageData &aData) {
    if (mpWindow->GetWaitingInput() == NULL) {
        ...
        SaveSubAction(GetPosFromInput(apWidget), eInputDeviceType_Mouse,
...
bool cLuxInputMenuEntry::InputEntryGamepadButtonPress(iWidget *apWidget, const cGuiMessageData &aData) {
    if (IsWaiting()) {
        if (aData.mGamepadInputData.mfInputValue == 0.0f)
            return false;
        if (aData.mGamepadInputData.mInputType == eGamepadInputType_Button &&
            aData.mGamepadInputData.mlInputId == eGamepadButton_B) {
            //////////
        ...
        SaveGamepadSubAction(GetPosFromInput(apWidget), aData.mGamepadInputData.mInputType,
                             aData.mGamepadInputData.mlInputId, aData.mGamepadInputData.mfInputValue);
    }
...
void cLuxMainMenu_KeyConfig::CreateGui() {
    ...
    mpWindow = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_None, cVector3f(0, 0, 5), mvWindowSize,
                                            kTranslate("KeyConfig", "KeyConfigTitle"));
    ...
    mpCBCategory =
        mpGuiSet->CreateWidgetComboBox(cVector3f(pLabel->GetSize().x + 10, 0, 0), cVector2f(175, 25), _W(""), pGroup);
    ...
    pLabel = mpGuiSet->CreateWidgetLabel(vPos + cVector3f(mvHeaderPositions.x, 0, 0), mvEntrySize,
                                         kTranslate("KeyConfig", "HeaderPrimary"), mpWindow);
    pLabel->SetDefaultFontSize(pLabel->GetDefaultFontSize() * fHeaderFontMul);
    pLabel->SetTextAlign(eFontAlign_Center);

    pLabel = mpGuiSet->CreateWidgetLabel(vPos + cVector3f(mvHeaderPositions.y, 0, 0), mvEntrySize,
                                         kTranslate("KeyConfig", "HeaderSecondary"), mpWindow);
    pLabel->SetDefaultFontSize(pLabel->GetDefaultFontSize() * fHeaderFontMul);
    pLabel->SetTextAlign(eFontAlign_Center);
    ...
    // Set default keys
    cWidgetButton *pButton = mpGuiSet->CreateWidgetButton(vPos + cVector3f(10, -35, 0), cVector2f(fButtonWidth, 25),
                                                          kTranslate("KeyConfig", "SetDefault"), mpWindow);
    pButton->AddCallback(eGuiMessage_ButtonPressed, this, kGuiCallback(PressSetDefault));
    float fSetDefaultsWidth =
        pButton->GetDefaultFontType()->GetLength(pButton->GetDefaultFontSize(), pButton->GetText().c_str());
    ...
    mpBCancel =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(fButtonWidth, 30), kTranslate("Global", "Cancel"), mpWindow);
...
cLuxInputMenuEntry *cLuxMainMenu_KeyConfig::CreateInputEntry(cLuxAction *apAction, iWidget *apParent,
                                                             const cVector3f &avPos) {
...
bool cLuxMainMenu_KeyConfig::PressSetDefault(iWidget *apWidget, const cGuiMessageData &aData) {
    mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("KeyConfig", "LoadDefaultsMessage"),
                                    kTranslate("Global", "OK"), kTranslate("Global", "Cancel"), this,
                                    kGuiCallback(LoadDefaultsCallback));
    return true;
}
...
bool cLuxMainMenu_KeyConfig::PressOK(iWidget *apWidget, const cGuiMessageData &aData) {
    if (mbInvalidInputs) {
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("KeyConfig", "InvalidInputMessage"),
                                        kTranslate("Global", "OK"), kTranslate("Global", "Cancel"), this,
                                        kGuiCallback(ExitCallback));
    } else {
