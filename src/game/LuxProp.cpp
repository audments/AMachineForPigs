...
void iLuxPropLoader::AfterLoad(cXmlElement *apRootElem, const cMatrixf &a_mtxTransform, cWorld *apWorld,
    ...
    if (apInstanceVars) {
        if (pProp->mpMeshEntity) {
            ...
            pProp->mpMeshEntity->SetUpdateBonesWhenCulled(
                apInstanceVars->GetVarBool("UpdateAnimationWhenCulled", false));
        }
...
void iLuxProp::SetParentBone(cBoneState *apParentBone, const cMatrixf &apParentBoneOffsetMatrix, tString asParentName,
...
void iLuxProp::UpdateParentBone(float afTimeStep) {
    if (mpParentBone && mvBodies.size() > 0) {
        ...
        if (gpBase->mpDebugHandler->GetPositionAttachedProps()) {
            pBody->SetMatrix(
                cMath::MatrixMul(mpParentBone->GetWorldMatrix(), gpBase->mpDebugHandler->GetParentBoneOffsetMatrix()));
        } else {
            pBody->SetMatrix(cMath::MatrixMul(mpParentBone->GetWorldMatrix(), mpParentBoneOffsetMatrix));
        }
    }
...
void iLuxProp::DestroyAttachedProp(cLuxProp_AttachedProp *apAttachedProp) {
    apAttachedProp->mpProp->mpAttachmentParent =
        NULL; // Must set this null, else it will try and erase itself when delted!
...
void iLuxProp::UpdateAttachedProps(float afTimeStep, bool abForceUpdate) {
    ...
    for (; it != mlstAttachedProps.end(); ++it) {
        ...
        if (pProp->GetBodyNum() > 0) {
            ...
            cMatrixf mtxProp = cMath::MatrixMul(
                GetMainBody()->GetLocalMatrix(),
...
void iLuxProp::UpdateAngularMoving(float afTimeStep) {
    ...
    // Not used! Old stuff. The angles returned can be quite different at times when no rotaiton is needed (0,0,0)
    // (360,0,360) cVector3f vCurrentAngles =	cMath::MatrixToEulerAngles(pBody->GetLocalMatrix(),
    // eEulerRotationOrder_XYZ); cVector3f vWantedAngles =	cMath::MatrixToEulerAngles(m_mtxMoveAngularGoal,
    // eEulerRotationOrder_XYZ); cVector3f vDelta = cMath::Vector3AngleDistanceRad(vCurrentAngles, vWantedAngles);
    // Log("B---------------\n");
    // Log("Angles %s -> %s\n", cMath::Vector3ToDeg(vCurrentAngles).ToString().c_str(),
    // cMath::Vector3ToDeg(vWantedAngles).ToString().c_str());
...
void iLuxProp::UpdateCheckIfOutsidePlayer(float afTimeStep) {
    ...
    for (size_t i = 0; i < mvBodies.size(); ++i) {
        ...
        cVector3f vPush(0);
        bool bCollide =
            pPhysicsWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), pPlayerBody->GetShape(),
...
kBeginSerializeBase(cLuxPropConnectedProp) kSerializeVar(msName, eSerializeType_String) kEndSerialize()
        ...
        kSerializeVar(msFileName, eSerializeType_String) kSerializeVar(
            ...
            kSerializeVar(mfHealth, eSerializeType_Float32)
                ...
                kSerializeVar(mbCheckOutsidePlayer, eSerializeType_Bool)
                    ...
                    kSerializeVar(mbEffectsActive, eSerializeType_Bool) kSerializeVar(mfEffectsAlpha,
                        ...
                        kSerializeVar(mbStaticPhysics, eSerializeType_Bool)
                            ...
                            kSerializeVar(mbMoving, eSerializeType_Bool)
                                ...
                                kSerializeVar(mbMovingLinear, eSerializeType_Bool) kSerializeVar(
                                    mfMoveLinearMaxSpeed, eSerializeType_Float32) kSerializeVar(mfMoveLinearAcc,
                                                                                                eSerializeType_Float32)
                                    kSerializeVar(mfMoveLinearSpeed, eSerializeType_Float32) kSerializeVar(
                                        mfMoveLinearSlowdownDist,
                                        eSerializeType_Float32) kSerializeVar(mvMoveLinearGoal, eSerializeType_Vector3f)

                                        kSerializeVar(mbMovingAngular, eSerializeType_Bool) kSerializeVar(
                                            mfMoveAngularMaxSpeed,
                                            eSerializeType_Float32) kSerializeVar(mfMoveAngularAcc,
                                                                                  eSerializeType_Float32)
                                            kSerializeVar(mfMoveAngularSpeed, eSerializeType_Float32) kSerializeVar(
                                                mfMoveAngularSlowdownDist,
                                                eSerializeType_Float32) kSerializeVar(m_mtxMoveAngularGoal,
                                                                                      eSerializeType_Matrixf)
                                                kSerializeVar(mbMoveAngularUseOffset, eSerializeType_Bool) kSerializeVar(
                                                    mvMoveAngularWorldOffset,
                                                    eSerializeType_Vector3f) kSerializeVar(mvMoveAngularLocalOffset,
                                                                                           eSerializeType_Vector3f)

                                                    kSerializeVar(
                                                        mbMoveAngularNoGoal,
                                                        eSerializeType_Bool) kSerializeVar(mvMoveAngularNoGoalDir,
                                                                                           eSerializeType_Vector3f)

                                                        kSerializeVar(mbGlowEnabled, eSerializeType_Bool)

                                                            kSerializeVar(
                                                                mlCurrentNonLoopAnimIndex,
                                                                eSerializeType_Int32) kSerializeVar(msAnimCallback,
                                                                                                    eSerializeType_String)

                                                                kSerializeVar(msParentEntityName, eSerializeType_String) kSerializeVar(
                                                                    msParentBoneName,
                                                                    eSerializeType_String) kSerializeVar(mpParentBoneOffsetMatrix,
                                                                                                         eSerializeType_Matrixf)

                                                                    kSerializeVar(mfBodyMassBackup, eSerializeType_Float32) kSerializeVar(
                                                                        mbBodyCollideBackup,
                                                                        eSerializeType_Bool) kSerializeVar(mbBodyCollideCharacterBackup,
                                                                                                           eSerializeType_Bool)
                                                                        kSerializeVar(
                                                                            mbBodyActiveBackup,
                                                                            eSerializeType_Bool) kSerializeVar(mBodyMatrixBackup,
                                                                                                               eSerializeType_Matrixf)

                                                                            kSerializeClassContainer(
                                                                                mvAttachedProps, cLuxProp_AttachedProp,
                                                                                eSerializeType_Class)

                                                                                kSerializeClassContainer(
                                                                                    mvConnectedProps,
                                                                                    cLuxPropConnectedProp,
                                                                                    eSerializeType_Class)

                                                                                    kSerializeVar(mMeshEntity, eSerializeType_Class) kSerializeClassContainer(
                                                                                        mvBodies, cEngineBody_SaveData,
                                                                                        eSerializeType_Class)
                                                                                        kSerializeClassContainer(
                                                                                            mvJoints,
                                                                                                    ...
                                                                                                    kSerializeClassContainer(
                                                                                                        ...
                                                                                                        kSerializeClassContainer(
                                                                                                            ...
                                                                                                            kSerializeClassContainer(
                                                                                                                ...
                                                                                                                kSerializeClassContainer(
                                                                                                                    mvInteractConnections,
                                                                                                                    iLuxInteractConnection_SaveData,
...
// CreateEntity seems broken. It does not add the entity to the map, but returns map->GetLatestEntity(). I do not want
// to interfere with the rest at this point, so creating a second method for my means.
