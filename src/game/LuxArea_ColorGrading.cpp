...
void cLuxAreaLoader_ColorGrading::LoadVariables(iLuxArea *apArea, cWorld *apWorld) {
    ...
    pColorGradingArea->mfCollisionCheckInterval =
        gpBase->mpGameCfg->GetFloat("Effects", "GradingAreaCollisionCheckInterval", 1.0f);
...
kBeginSerialize(cLuxArea_ColorGrading_SaveData, iLuxArea_SaveData)
    ...
    kSerializeVar(msLookupTable, eSerializeType_String) kSerializeVar(mfFadeTime, eSerializeType_Float32)
...
iLuxEntity_SaveData *cLuxArea_ColorGrading::CreateSaveData() { return hplNew(cLuxArea_ColorGrading_SaveData, ()); }
...
void cLuxArea_ColorGrading::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
