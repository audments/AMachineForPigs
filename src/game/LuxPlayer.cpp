...
cLuxPlayer::cLuxPlayer() : iLuxUpdateable("LuxPlayer"), iLuxCollideCallbackContainer() {
    ...
    mfInfectionLevelOneSpeedMul =
        gpBase->mpGameCfg->GetFloat("Player_Infection", "InfectionLevelOneSpeedMultiplier", 0);
    mfInfectionLevelTwoSpeedMul =
        gpBase->mpGameCfg->GetFloat("Player_Infection", "InfectionLevelTwoSpeedMultiplier", 0);
    mfInfectionLevelThreeSpeedMul =
        gpBase->mpGameCfg->GetFloat("Player_Infection", "InfectionLevelThreeSpeedMultiplier", 0);
    mfInfectionLevelFourSpeedMul =
        gpBase->mpGameCfg->GetFloat("Player_Infection", "InfectionLevelFourSpeedMultiplier", 0);

    mnMaxInfectionLevelAtWhichPlayerCanRun =
        gpBase->mpGameCfg->GetInt("Player_Infection", "MaxInfectionLevelAtWhichPlayerCanRun", 0);
    //////////////////////////////////
...
void cLuxPlayer::OnLeaveContainer(const tString &asNewContainer) {
    ...
    if (mState == eLuxPlayerState_InteractGrab || mState == eLuxPlayerState_InteractSlide ||
        mState == eLuxPlayerState_InteractPush || mState == eLuxPlayerState_InteractWheel ||
...
void cLuxPlayer::RenderTrans(cRendererCallbackFunctions *apFunctions) { mvStates[mState]->RenderTrans(apFunctions); }
...
void cLuxPlayer::Lean(float afMul) { mpLean->Lean(afMul); }

//-----------------------------------------------------------------------

void cLuxPlayer::Scroll(float afAmount) { mvStates[mState]->OnScroll(afAmount); }
...
const cVector3f &cLuxPlayer::GetHeadPosAdd(eLuxHeadPosAdd aType) { return mvHeadPosAdds[aType].mvAdd; }

//-----------------------------------------------------------------------

void cLuxPlayer::SetActive(bool abX) { mbActive = abX; }
...
int cLuxPlayer::GetInfectionLevel() { return GetInfectionLevelForInfection(mfInfection); }

bool cLuxPlayer::IsAtMaxInfectionLevel() { return GetInfectionLevel() == miNumberOfInfectionLevels; }
...
void cLuxPlayer::AddTerrorEnemy(iLuxEnemy *apEnemy) { m_setTerrorEnemies.insert(apEnemy); }

void cLuxPlayer::RemoveTerrorEnemy(iLuxEnemy *apEnemy) { m_setTerrorEnemies.erase(apEnemy); }

void cLuxPlayer::ClearTerrorEnemies() { m_setTerrorEnemies.clear(); }
...
void cLuxPlayer::AddCoins(int alX) { mlCoins += alX; }
...
void cLuxPlayer::DrawHud(float afFrameTime) {
    ...
    if (msFocusText != _W("")) {
        ...
        for (size_t i = 0; i < vRows.size(); ++i)
            gpBase->mpGameHudSet->DrawFont(vRows[i], mpFocusFont, cVector3f(400, fFocusTextY + i * 24, 1), 28,
...
void cLuxPlayer::UpdateFocusText(float afTimeStep) {
    if (gpBase->mpMessageHandler->IsPauseMessageActive() ||
        // gpBase->mpMessageHandler->IsMessageActive() || <- this interfeere with signs and stuff so not a good thing to
        // have!
        msFocusText == _W("")) {
...
bool cLuxPlayer::CanRun() { return GetInfectionLevel() <= mnMaxInfectionLevelAtWhichPlayerCanRun; }

//-----------------------------------------------------------------------

float cLuxPlayer::GetExhaustionFactor() { return mpPlayerStamina ? mpPlayerStamina->GetExhaustionFactor() : 0.0f; }

//-----------------------------------------------------------------------

void cLuxPlayer::ReleasePlayerFromLimbo() { GetHelperDeath()->ReleasePlayerFromLimbo(); }