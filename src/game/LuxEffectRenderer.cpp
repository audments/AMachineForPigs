...
cLuxEffectRenderer::cLuxEffectRenderer() : iLuxUpdateable("LuxEffectRenderer") {
    ...
    mpOutlineColorTexture =
        pGraphics->GetTempFrameBuffer(vScreenSize, ePixelFormat_RGBA, 0)->GetColorBuffer(0)->ToTexture();
    ...
    mpFlashProgram = pGraphics->CreateGpuProgramFromShaders("GameOutline", "deferred_base_vtx.glsl",
                                                            "game_object_flash_frag.glsl", &programVars);
    ...
    programVars.Add("UseUv");
    programVars.Add("UseNormals");
    mpEnemyGlowProgram = pGraphics->CreateGpuProgramFromShaders("EnemyGlow", "deferred_base_vtx.glsl",
                                                                "game_enemy_darkness_glow_frag.glsl", &programVars);
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        mpOutlineColorProgram[i] = pGraphics->CreateGpuProgramFromShaders("GameOutline", "game_outline_vtx.glsl",
                                                                          "game_outline_frag.glsl", &programVars);
        programVars.Clear();
    }

    mpOutlineStencilProgram = pGraphics->CreateGpuProgramFromShaders("GameOutline", "deferred_base_vtx.glsl",
                                                                     "deferred_base_frag.glsl", &programVars);

    programVars.Add("UseUv");
    programVars.Add("UseDiffuse");
    mpOutlineStencilAlphaProgram = pGraphics->CreateGpuProgramFromShaders("GameOutline", "deferred_base_vtx.glsl",
                                                                          "deferred_base_frag.glsl", &programVars);
    programVars.Clear();
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        mpBlurProgram[i] = pGraphics->CreateGpuProgramFromShaders("BloomBlur", "posteffect_bloom_blur_vtx.glsl",
...
void cLuxEffectRenderer::Update(float afTimeStep) { mFlashOscill.Update(afTimeStep); }
...
void cLuxEffectRenderer::ClearOutlineObjects() { mvOutlineObjects.clear(); }
...
void cLuxEffectRenderer::RenderFlashObjects(cRendererCallbackFunctions *apFunctions) {
    ...
    for (size_t i = 0; i < mvFlashObjects.size(); ++i) {
        ...
        if (mpFlashProgram) {
            mpFlashProgram->SetColor4f(kVar_afColorMul,
                                       mvFlashObjects[i].mcColor * mvFlashObjects[i].mfAlpha * fGlobalAlpha);
        }
...
void cLuxEffectRenderer::RenderOutline(cRendererCallbackFunctions *apFunctions) {
    ...
    apFunctions->GetLowLevelGfx()->SetStencil(eStencilFunc_Always, 0xFF, 0xFF, eStencilOp_Keep, eStencilOp_Keep,
                                              eStencilOp_Replace);
    ...
    apFunctions->GetLowLevelGfx()->SetStencil(eStencilFunc_NotEqual, 0xFF, 0xFF, eStencilOp_Keep, eStencilOp_Keep,
                                              eStencilOp_Keep);
    ...
    for (tRenderableListIt it = lstObjects.begin(); it != lstObjects.end() && cIt != lstColors.end(); ++it, ++cIt) {
        ...
        cMatrixf mtxScale =
            cMath::MatrixMul(cMath::MatrixScale(vScale), cMath::MatrixTranslate(pBV->GetLocalCenter() * -1));
