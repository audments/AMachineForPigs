...
cGuiGfxElement *iLuxPlayerState_InteractRotateBase::GetCrosshair() { return NULL; }

//-----------------------------------------------------------------------

float iLuxPlayerState_InteractRotateBase::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("Speed: %f"),
                    mfRotSpeed); // mSmoothSpeed.GetOutput());
    afStartY += 13.0f;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("Angle: %f"),
                    cMath::ToDeg(mpCurrentJoint->GetAngle()));
    afStartY += 13.0f;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("AngularVel: (%ls)"),
                    cString::To16Char(mpCurrentBody->GetAngularVelocity().ToString()).c_str());
    afStartY += 13.0f;
...
void iLuxPlayerState_InteractRotateBase::RenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    //apFunctions->GetLowLevelGfx()->DrawLine(vJointPos, vJointPos+cMath::Vector3Normalize(vPushAmount),
    cColor(1,0,0,1));
    //apFunctions->GetLowLevelGfx()->DrawLine(vJointPos, vJointPos+cMath::Vector3Normalize(vPushRotateDir),
    cColor(0,1,0,1));
    ...
    //apFunctions->GetLowLevelGfx()->DrawLine(vJointPos, vJointPos + mpCurrentJoint->GetPinDir()*fAdd*10.0f,
    cColor(0,0,1,1));*/
}
...
kBeginSerializeVirtual(iLuxPlayerState_InteractRotateBase_SaveData, iLuxPlayerState_Interact_SaveData) kEndSerialize()
    ...
    iLuxPlayerState_InteractRotateBase_SaveData *pData =
        static_cast<iLuxPlayerState_InteractRotateBase_SaveData *>(apSaveData);
...
void iLuxPlayerState_InteractRotateBase::LoadFromSaveDataBeforeEnter(cLuxMap *apMap,
                                                                     iLuxPlayerState_SaveData *apSaveData) {
    ///////////////////////
    // Init
...
void iLuxPlayerState_InteractRotateBase::LoadFromSaveDataAfterEnter(cLuxMap *apMap,
                                                                    iLuxPlayerState_SaveData *apSaveData) {
    ///////////////////////
    // Init
