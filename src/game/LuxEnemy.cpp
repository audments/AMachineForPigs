...
tWString gsLuxEnemyStates[eLuxEnemyState_LastEnum + 1] = {_W("Idle"),
                                                          _W("GoHome"),
...
void iLuxEnemyLoader::AfterLoad(cXmlElement *apRootElem, const cMatrixf &a_mtxTransform, cWorld *apWorld,
    ...
    pEnemy->m_mtxCharMeshOffset =
        cMath::MatrixRotate(cMath::Vector3ToRad(GetVarVector3f("Body_OffsetRot", 0)), eEulerRotationOrder_XYZ);
    pEnemy->m_mtxCharMeshOffset.SetTranslation(GetVarVector3f("Body_OffsetTrans", 0) -
                                               cVector3f(0, pCharBody->GetSize().y / 2, 0));
...
void iLuxEnemy::OnMapEnter() { mvStartPosition = mpCharBody->GetFeetPosition() + cVector3f(0, 0.1f, 0); }
...
void iLuxEnemy::OnUpdate(float afTimeStep) {
    ...
    // In case the update is called in save or anyhting else that just want a quick update, we do not want to be
    // updated. Enemies can be disabled when player is not properly placed and the like.
...
void iLuxEnemy::OnRenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    // pPhysicsWorld->RenderShapeDebugGeometry(mpCharBody->GetCurrentShape(), mpCharBody->GetMoveMatrix(),
    // apFunctions->GetLowLevelGfx(),cColor(1,1));
...
bool iLuxEnemy::CanInteract(iPhysicsBody *apBody) { return false; }

bool iLuxEnemy::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos) { return false; }
...
iEntity3D *iLuxEnemy::GetAttachEntity() { return mpMeshEntity; }
...
void iLuxEnemy::ShowPlayerPosition() { mvLastKnownPlayerPos = gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition(); }
...
void iLuxEnemy::ChangeState(eLuxEnemyState aState) {
    ...
    // Log("State %s' -> '%s'\n", cString::To8Char(gsLuxEnemyStates[mCurrentState]).c_str(),
    // cString::To8Char(gsLuxEnemyStates[aState]).c_str());
...
void iLuxEnemy::SendMessage(eLuxEnemyMessage aType, float afTime, bool abLocalScope, const cVector3f &avX, float afX,
    ...
    if (mbDisabled)
        return;
    if (TriggersDisabled() && aType > eLuxEnemyMessage_EndOfPath && aType != eLuxEnemyMessage_PlayerInRange &&
        aType != eLuxEnemyMessage_PlayerOutOfRange && aType != eLuxEnemyMessage_ChangePose) {
        return;
    }
...
void iLuxEnemy::PlayScriptedAnimation(const tString &asName, bool abLoop) { PlayAnim(asName, abLoop, 0.5f); }

//-----------------------------------------------------------------------

void iLuxEnemy::PlayAnim(const tString &asName, bool abLoop, float afFadeTime, bool abDependsOnSpeed, float afSpeedMul,
                         bool abSyncWithPrevFrame, bool abOverideMoveState, bool abUseMoveAnimWhenCurrentIsOver,
                         bool abCanBlend, bool abPlayTransition) {
    ...
    if (mpCurrentAnimation != NULL && mpCurrentAnimation->GetName() == asName && mpCurrentAnimation->IsActive() &&
...
void iLuxEnemy::SetPositionAtStartPos() { mpCharBody->SetFeetPosition(mvStartPosition); }
...
bool iLuxEnemy::IsAtLastPatrolNode() { return mlCurrentPatrolNode >= (int)mvPatrolNodes.size() - 1; }

bool iLuxEnemy::IsAtFirstPatrolNode() { return mlCurrentPatrolNode <= 0; }
...
float iLuxEnemy::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1),
                    _W("  Anim: '%ls' pos: %0.2f / %0.2f Speed: %0.2f Loop: %d"), sAnim.c_str(), fPos, fMaxPos, fSpeed,
                    bLoop);
    afStartY += 14;
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 10), 13, cColor(1, 1),
                    ...
                    DistToPlayer(), GetPlayerMovementTowardEnemyAmount(), mlCurrentPatrolNode,
                    mpMover->CalculateSpeedMul(1.0f / 60.0f));
    afStartY += 14;
...
void iLuxEnemy::AddTransitionAnimation(const tString &asMainAnim, const tString &asTransAnim, const tString &asPrevAnim,
                                       float afMinTime, float afMaxTime) {
...
void iLuxEnemy::UpdateAnimation(float afTimeStep) {
    ...
    if (mpCurrentAnimation->IsOver()) {
        ...
        if (mlNextAnimationIndex >= 0) {
            ...
            if (pAnim) {
                PlayAnim(pAnim->GetName(), mbNextAnimationLoop, 0.3f, mbNextAnimationDependsOnSpeed, mfNextAnimSpeedMul,
                         false, mbNextAnimOverideMoveState, mbNextAnimUseMoveAnimWhenCurrentIsOver, pAnim->CanBlend(),
                         false);
            } else {
                if (mpMover->GetOverideMoveState()) {
                    if (mbUseMoveAnimWhenCurrentIsOver)
...
void iLuxEnemy::UpdateCanSeePlayer(float afTimeStep) {
    ...
    if (bLanternOn == false && pPlayer->GetHelperLightLevel()->GetNormalLightLevel() < mfPlayerInDarknessLightLevel &&
        mbSkipVisibilityRangeHandicaps == false) {
        fMaxRange = mfDarknessSightRange;
...
void iLuxEnemy::UpdateDarknessGlow(float afTimeStep) {
        ...
        if (pCam->GetFrustum()->CollideBoundingVolume(mpMeshEntity->GetBoundingVolume()) != eCollision_Outside) {
            ...
            if (fDistSqrt < mfEnemyDarknessGlowMaxDistance * mfEnemyDarknessGlowMaxDistance) {
                ...
                if (fDist > mfEnemyDarknessGlowMaxDistance * 0.5f) {
                    fDistMul =
                        1 - ((fDist - mfEnemyDarknessGlowMaxDistance * 0.5f) / (mfEnemyDarknessGlowMaxDistance * 0.5f));
                    if (fDistMul < 0)
                        fDistMul = 0;
                }
    ...
    mfDarknessGlowAlphaGoal +=
        gpBase->mpMapHelper->GetLightLevelAtPos(mpCharBody->GetFeetPosition() + cVector3f(0, 0.1f, 0));
...
void iLuxEnemy::UpdateHallucination(float afTimeStep) {
    ...
    float fDistSqr =
        cMath::Vector3DistSqr(mpCharBody->GetPosition(), gpBase->mpPlayer->GetCharacterBody()->GetPosition());
...
void iLuxEnemy::UpdateAlignEntityWithGroundRay(float afTimeStep) {
    ...
    bool bIntersect =
        gpBase->mpMapHelper->GetClosestCharCollider(vStartPos, cVector3f(0, -1, 0), 0.5f, true, &fDist, &vNormal, NULL);
    if (bIntersect == false)
        return;
    fDist -= fStartAdd;
...
bool iLuxEnemy::TriggersDisabled() { return mbDisableTriggers || gpBase->mpPlayer->GetHelperFlashback()->IsActive(); }

//-----------------------------------------------------------------------

bool iLuxEnemy::Attack(const cEnemyAttackSizeData &aSizeData, const cEnemyAttackDamageData &aDamageData,
                       float afDamageMul) {
    bool bHitPlayer = false;
    bool bHit = gpBase->mpMapHelper->ShapeDamage(
        GetAttackShape(aSizeData.mlShapeIdx), GetDamageShapeMatrix(aSizeData.mvOffset), mpCharBody->GetPosition(),
        aDamageData.mfMinDamage * afDamageMul, aDamageData.mfMaxDamage * afDamageMul, aDamageData.mfForce,
        aDamageData.mfMaxImpulse, aDamageData.mlStrength, aDamageData.mfHitSpeed, aDamageData.mDamageType,
        aDamageData.mWeaponHitType, false, aDamageData.mbCheckPlayer, aDamageData.mbCheckProps, true, &bHitPlayer);
    if (bHit) {
        mlAttackHitCounter++;
...
cAINode *iLuxEnemy::GetSearchForPlayerNode(int alMaxIterations, float afMaxAngleMul, float afMinDistMul,
                                           float afMaxDistMul) {
...
cAINode *iLuxEnemy::GetPatrolAroundPlayerNode(int alMaxIterations, float afMaxAngleMul, float afMinDistMul,
                                              float afMaxDistMul) {
...
static const cVector2f gvPosAdds[] = {cVector2f(0, 0),      cVector2f(0.75, 0), cVector2f(-0.75, 0),
                                      cVector2f(0, 1),      cVector2f(0, -1),   cVector2f(0.5, 0.5),
                                      cVector2f(-0.5, 0.5), cVector2f(0.25, 0), cVector2f(-0.25, 0)};
...
bool iLuxEnemy::LineOfSight(const cVector3f &avPos, const cVector3f &avSize, bool abCheckFOV) {
    return LineOfSight(avPos, avSize, abCheckFOV,
                       mpCharBody->GetPosition() + cVector3f(0, mpCharBody->GetSize().y / 2 - 0.2f, 0));
}

bool iLuxEnemy::LineOfSight(const cVector3f &avPos, const cVector3f &avSize, bool abCheckFOV,
                            const cVector3f &avSourcePos) {
    /////////////////////////////////////////
...
cMatrixf iLuxEnemy::GetDamageShapeMatrix(const cVector3f &avOffset) {
    cVector3f vPos = mpCharBody->GetPosition() + mpCharBody->GetRight() * avOffset.x +
                     mpCharBody->GetUp() * avOffset.y + mpCharBody->GetForward() * avOffset.z;
...
cVector3f iLuxEnemy::GetPlayerFeetPos() { return gpBase->mpPlayer->GetCharacterBody()->GetFeetPosition(); }
...
bool iLuxEnemy::PlayerInFOV(float fFOV) { return InFOV(gpBase->mpPlayer->GetCharacterBody()->GetPosition(), fFOV); }

//-----------------------------------------------------------------------

bool iLuxEnemy::PlayerInFOV() { return InFOV(gpBase->mpPlayer->GetCharacterBody()->GetPosition()); }
...
kBeginSerializeBase(cLuxStateMessage_SaveData)
    ...
    kSerializeVar(mlType, eSerializeType_Int32) kSerializeVar(mfTime, eSerializeType_Float32)
        kSerializeVar(mfCount, eSerializeType_Float32)

            kSerializeVar(mvCustomValue, eSerializeType_Vector3f) kSerializeVar(mfCustomValue, eSerializeType_Float32)
    ...
    kBeginSerializeBase(cLuxEnemyPatrolNode_SaveData)
        ...
        kSerializeVar(mlNodeId, eSerializeType_Int32) kSerializeVar(mfWaitTime, eSerializeType_Float32)
            kSerializeVar(msAnimation, eSerializeType_String) kSerializeVar(mbLoopAnimation, eSerializeType_Bool)
    ...
    kBeginSerializeVirtual(iLuxEnemy_SaveData, iLuxEntity_SaveData)
        ...
        kSerializeVar(msFileName, eSerializeType_String) kSerializeVar(
            m_mtxOnLoadTransform, eSerializeType_Matrixf) kSerializeVar(mvOnLoadScale, eSerializeType_Vector3f)

            kSerializeVar(mbDisabled, eSerializeType_Bool) kSerializeVar(mbDisableTriggers, eSerializeType_Bool)

                kSerializeVar(mfHealth, eSerializeType_Float32) kSerializeVar(mbCausesInfectionIncrease,
                                                                              eSerializeType_Bool)

                    kSerializeVar(mbBlind, eSerializeType_Bool) kSerializeVar(mbDeaf, eSerializeType_Bool)

                        kSerializeVar(mbHallucination, eSerializeType_Bool)

                            kSerializeVar(mlCurrentState, eSerializeType_Int32) kSerializeVar(
                                mlNextState, eSerializeType_Int32) kSerializeVar(mlPreviousState, eSerializeType_Int32)

                                kSerializeVar(mCurrentPose, eSerializeType_Int32) kSerializeVar(mCurrentMoveType,
                                                                                                eSerializeType_Int32)

                                    kSerializeVar(mlSoundState, eSerializeType_Int32)

                                        kSerializeVar(mfLookForPlayerCount, eSerializeType_Float32) kSerializeVar(mlPlayerInLOSCount, eSerializeType_Int32) kSerializeVar(
                                            mbCanSeePlayer,
                                            eSerializeType_Bool) kSerializeVar(mbPlayerDetected,
                                                                               eSerializeType_Bool) kSerializeVar(mbPlayerInRange,
                                                                                                                  eSerializeType_Bool)

                                            kSerializeVar(mfCheckAtDoorCount, eSerializeType_Float32) kSerializeVar(
                                                mbStuckAtDoor, eSerializeType_Bool) kSerializeVar(mlStuckDoorID,
                                                                                                  eSerializeType_Int32)

                                                kSerializeVar(mfForwardSpeed, eSerializeType_Float32) kSerializeVar(
                                                    mfBackwardSpeed,
                                                    eSerializeType_Float32) kSerializeVar(mfForwardAcc,
                                                                                          eSerializeType_Float32) kSerializeVar(mfForwardDeacc,
                                                                                                                                eSerializeType_Float32)

                                                    kSerializeVar(
                                                        mfDarknessGlowAlpha,
                                                        eSerializeType_Float32) kSerializeVar(mfDarknessGlowAlphaGoal,
                                                                                              eSerializeType_Float32)
                                                        ...
                                                        kSerializeVar(mlAttackHitCounter, eSerializeType_Int32)
                                                            ...
                                                            kSerializeVar(mfFOVMul, eSerializeType_Float32)
                                                                ...
                                                                kSerializeVar(mbSkipVisibilityRangeHandicaps,
                                                                              eSerializeType_Bool)

                                                                    kSerializeVar(msCurrentAnimName, eSerializeType_String) kSerializeVar(
                                                                        mbAnimationIsSpeedDependant,
                                                                        eSerializeType_Bool) kSerializeVar(mfAnimationSpeedMul,
                                                                                                           eSerializeType_Float32) kSerializeVar(mbUseMoveAnimWhenCurrentIsOver,
                                                                                                                                                 eSerializeType_Bool)

                                                                        kSerializeVar(mlNextAnimationIndex, eSerializeType_Int32) kSerializeVar(
                                                                            mbNextAnimationLoop,
                                                                            eSerializeType_Bool) kSerializeVar(mbNextAnimationDependsOnSpeed,
                                                                                                               eSerializeType_Bool)
                                                                            kSerializeVar(mfNextAnimSpeedMul, eSerializeType_Float32) kSerializeVar(
                                                                                mbNextAnimUseMoveAnimWhenCurrentIsOver,
                                                                                eSerializeType_Bool) kSerializeVar(mbNextAnimOverideMoveState,
                                                                                                                   eSerializeType_Bool)

                                                                                kSerializeVar(mvStartPosition,
                                                                                              eSerializeType_Vector3f)

                                                                                    kSerializeVar(mvLastKnownPlayerPos, eSerializeType_Vector3f) kSerializeVar(
                                                                                        mfLastPlayerPosCount,
                                                                                        eSerializeType_Float32)

                                                                                        kSerializeVar(mvTempPos, eSerializeType_Vector3f) kSerializeVar(
                                                                                            mfTempVal,
                                                                                            eSerializeType_Float32)
                                                                                            kSerializeVar(mlTempVal, eSerializeType_Int32) kSerializeVar(
                                                                                                mfDamageCount,
                                                                                                eSerializeType_Float32)

                                                                                                kSerializeClassContainer(
                                                                                                    mvPatrolNodes,
                                                                                                    cLuxEnemyPatrolNode_SaveData,
                                                                                                    eSerializeType_Class) kSerializeVar(mlCurrentPatrolNode,
                                                                                                                                        eSerializeType_Int32)

                                                                                                    kSerializeVar(mMeshEntity, eSerializeType_Class) kSerializeClassContainer(
                                                                                                        mvPS,
                                                                                                        cEnginePS_SaveData,
                                                                                                        ...
                                                                                                        kSerializeClassContainer(
                                                                                                            ...
                                                                                                            kSerializeClassContainer(
                                                                                                                ...
                                                                                                                kSerializeClassContainer(
                                                                                                                    mvBillboards,
                                                                                                                    cEngineBillboard_SaveData,
                                                                                                                    eSerializeType_Class)

                                                                                                                    kSerializeClassContainer(
                                                                                                                        mvMessages,
                                                                                                                        cLuxStateMessage_SaveData,
                                                                                                                        eSerializeType_Class)
