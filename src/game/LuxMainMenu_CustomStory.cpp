...
void cLuxMainMenu_CustomStory::CreateGui() {
    ...
    mpIPicture = mpGuiSet->CreateWidgetImage("", cVector3f(6, 75, 0.05f), cVector2f(588, 360), eGuiMaterial_Alpha,
                                             false, mpWindow);
    mpIPicture->SetColorMul(cColor(1, 0.65f));

    mpLAuthor = mpGuiSet->CreateWidgetLabel(vPos + cVector3f(50, -50, 0), 0, _W(" "), mpWindow);
    cWidgetFrame *pFDescription =
        mpGuiSet->CreateWidgetFrame(vPos + cVector3f(0, 10, 0), cVector2f(420, 345), false, mpWindow, false, true);
...
void cLuxMainMenu_CustomStory::SetCurrentStory(cLuxCustomStorySettings *apStory) {
    ...
    if (mpStory->msImgFile != "") {
        ...
        iTexture *pTex = gpBase->mpEngine->GetResources()->GetTextureManager()->Create2D(mpStory->msImgFile, false,
                                                                                         eTextureType_Rect);
        if (pTex)
...
cLuxMainMenu_CustomStoryList::cLuxMainMenu_CustomStoryList(cGuiSet *apGuiSet, cGuiSkin *apGuiSkin,
                                                           cLuxMainMenu_CustomStory *apWindow)
    : iLuxMainMenuWindow(apGuiSet, apGuiSkin) {
    mvWindowSize = cVector2f(600, 440);
...
void cLuxMainMenu_CustomStoryList::CreateGui() {
    ...
    mpWindow = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_None, cVector3f(0, 0, 5), mvWindowSize,
                                            kTranslate("MainMenu", "Custom Map"));
    mpWindow->AddCallback(eGuiMessage_OnUpdate, this, kGuiCallback(WindowOnUpdate));
    ...
    //////////////////////////
    // Custom story list
    mpLBStories = mpGuiSet->CreateWidgetListBox(vPos + cVector3f(0, 0, 1),
                                                cVector2f((mvWindowSize.x - fBorderSize * 2), 300), mpWindow);
    ...
    cWidgetButton *pButton =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(fButtonWidth, 30), kTranslate("Global", "OK"), mpWindow);
...
bool cLuxMainMenu_CustomStoryList::WindowOnUpdate(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
kGuiCallbackDeclaredFuncEnd(cLuxMainMenu_CustomStoryList, WindowOnUpdate);

//-----------------------------------------------------------------------

bool cLuxMainMenu_CustomStoryList::SelectStory(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
...
bool cLuxMainMenu_CustomStoryList::PressOK(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mpLBStories->GetSelectedItem() < 0)
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("CustomStory", "NoValidStory"),
                                        kTranslate("Global", "OK"), _W(""), NULL, NULL);
    else
