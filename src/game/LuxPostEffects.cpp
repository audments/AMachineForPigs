...
cLuxPostEffect_Infection::cLuxPostEffect_Infection(cGraphics *apGraphics, cResources *apResources)
    ...
    vars.Add("UseUv");
    mpProgram = mpGraphics->CreateGpuProgramFromShaders("LuxInfection", "deferred_base_vtx.glsl",
                                                        "posteffect_infection_frag.glsl", &vars);
    if (mpProgram) {
    ...
    for (size_t i = 0; i < mvAmpMaps.size(); ++i)
        mvAmpMaps[i] = mpResources->GetTextureManager()->Create2D(
            "posteffect_infection_ampmap" + cString::ToString((int)i), false);

    mpZoomMap = mpResources->GetTextureManager()->Create2D("posteffect_infection_zoom.jpg", false);
    mpInfectionNormalBlendMap =
        mpResources->GetTextureManager()->Create2D("posteffect_infection_normal_blend.tga", false);
    mpInfectionOverlayBlendMap =
        mpResources->GetTextureManager()->Create2D("posteffect_infection_overlay_blend.tga", false);
    mpInfectionColorDodgeBlendMap =
        mpResources->GetTextureManager()->Create2D("posteffect_infection_colordodge_blend.tga", false);
...
cLuxPostEffectHandler::~cLuxPostEffectHandler() { STLDeleteAll(mvPostEffects); }
