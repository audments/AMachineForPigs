...
void cLuxMainMenu_LoadGame::CreateGui() {
    ...
    mpWindow = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_None, cVector3f(0, 0, 5), mvWindowSize,
                                            kTranslate("LoadGame", "LoadGameTitle"));
    ...
    //////////////////////////
    // Saved game list
    mpLBSavedGames = mpGuiSet->CreateWidgetListBox(vPos + cVector3f(0, 0, 1),
    ...
    mpISavedGameSnapShot = mpGuiSet->CreateWidgetImage("", mpLBSavedGames->GetSize().x + 10, cVector2f(200),
                                                       eGuiMaterial_Alpha, false, mpLBSavedGames);
...
bool cLuxMainMenu_LoadGame::WindowOnUpdate(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
...
bool cLuxMainMenu_LoadGame::PressOK(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mpLBSavedGames->GetSelectedItem() < 0)
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("LoadGame", "NoGame"),
                                        kTranslate("Global", "OK"), _W(""), NULL, NULL);
    else
        mpGuiSet->CreatePopUpMessageBox(kTranslate("Global", "Warning"), kTranslate("LoadGame", "LoadGameMessage"),
                                        kTranslate("Global", "OK"), kTranslate("Global", "Cancel"), this,
                                        kGuiCallback(LoadGameCallback));
