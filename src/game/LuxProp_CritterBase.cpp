...
void iLuxPropLoader_Critter::LoadVariables(iLuxProp *apProp, cXmlElement *apRootElem) {
    ...
    pCritter->m_mtxMeshOffset = cMath::MatrixMul(cMath::MatrixRotate(vMeshOffsetRot, eEulerRotationOrder_XYZ),
                                                 cMath::MatrixScale(vMeshOffsetScale));
...
iLuxProp_CritterBase::~iLuxProp_CritterBase() { hplDelete(mpRayCallback); }
...
bool iLuxProp_CritterBase::CanInteract(iPhysicsBody *apBody) { return false; }
...
void iLuxProp_CritterBase::OnRenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    pPhysicsWorld->RenderShapeDebugGeometry(mpBody->GetShape(), mpBody->GetLocalMatrix(), apFunctions->GetLowLevelGfx(),
                                            cColor(1, 1));

    apFunctions->GetLowLevelGfx()->DrawLine(mpBody->GetLocalPosition(),
                                            mpBody->GetLocalPosition() + mvGroundNormal * 0.5f, cColor(1, 0, 0, 1));
    cVector3f vFwdDir = cMath::Vector3Normalize(mvVel == 0 ? mlstFwdDirs.back() : mvVel);
    apFunctions->GetLowLevelGfx()->DrawLine(mpBody->GetLocalPosition(), mpBody->GetLocalPosition() + vFwdDir * 0.5f,
                                            cColor(0, 1, 0, 1));

    if (mpDamageShape) {
        // cVector3f vDir = cMath::Vector3Normalize(gpBase->mpPlayer->GetCharacterBody()->GetPosition() -
        // mpBody->GetLocalPosition()); pPhysicsWorld->RenderShapeDebugGeometry(mpDamageShape,
        // GetAttackMatrix(vDir),apFunctions->GetLowLevelGfx(), cColor(1,1));
    }
...
void iLuxProp_CritterBase::OnDamage(float afAmount, int alStrength) { OnDamageCritter(afAmount); }
...
bool iLuxProp_CritterBase::Attack(const cVector3f &avDir) {
    ...
    bool bHit = gpBase->mpMapHelper->ShapeDamage(mpDamageShape, GetAttackMatrix(avDir), mpBody->GetLocalPosition(),
                                                 mvAttackDamageMinMax.x, mvAttackDamageMinMax.y, mvAttackForce.x,
                                                 mvAttackForce.y, mlAttackStrength, 3, eLuxDamageType_BloodSplat,
                                                 eLuxWeaponHitType_Sword, false, true, false, false, NULL);
    if (bHit) {
...
cVector3f iLuxProp_CritterBase::GetWanderAdd(float afLength, float afRadius, float afTimeStep) {
    ...
    vDir = vDir - mvGroundNormal *
                      cMath::Vector3Dot(mvGroundNormal, vDir); // Only want the direction in the current movement plane.
...
cVector3f iLuxProp_CritterBase::GetTowardPlayerAdd(bool abDependOnDistance, float afTimeStep) {
    ...
    vToPlayer.Normalize();
    vToPlayer = vToPlayer -
                mvGroundNormal * cMath::Vector3Dot(mvGroundNormal,
                                                   vToPlayer); // Only want the direction in the current movement plane.
...
void iLuxProp_CritterBase::UpdateMesh(float afTimeStep) {
    ...
    if (mpMeshEntity->GetMesh()->GetSkeleton()) {
        cMatrixf mtxRot =
            cMath::MatrixMul(cMath::MatrixMul(mtxRotate.GetTranspose(), cMath::MatrixRotateY(kPif)), m_mtxMeshOffset);

        mpMeshEntity->SetMatrix(mtxRot);
    } else {
        for (int i = 0; i < mpMeshEntity->GetSubMeshEntityNum(); ++i) {
            cMatrixf mtxLocal =
                cMath::MatrixMul(cMath::MatrixMul(mtxRotate.GetTranspose(), m_mtxMeshOffset), mvBaseMatrices[i]);
...
void iLuxProp_CritterBase::CheckRayCollision(const cVector3f &avVelAdd, const cVector3f &avGravityAdd,
                                             float afTimeStep) {
    iPhysicsWorld *pPhysicsWorld = mpWorld->GetPhysicsWorld();
    ...
    if (avGravityAdd != 0) {
        ...
        pPhysicsWorld->CastRay(mpRayCallback, vPos,
                               vPos + mpBody->GetBoundingVolume()->GetSize().y * -0.5f + avGravityAdd, false, true,
                               true, true);
        if (mpRayCallback->GetIntersected()) {
            mvGravityVel = 0;
        } else {
...
void iLuxProp_CritterBase::CheckShapeCollision(const cVector3f &avVelAdd, const cVector3f &avGravityAdd,
                                               float afTimeStep) {
    iPhysicsWorld *pPhysicsWorld = mpWorld->GetPhysicsWorld();
    cVector3f vPos = mpBody->GetWorldPosition();
    ...
    if (avVelAdd != 0) {
        ...
        bool bCollide = pPhysicsWorld->CheckShapeWorldCollision(&vPushVec, mpBody->GetShape(),
                                                                cMath::MatrixTranslate(vPos), mpBody);
...
kBeginSerialize(iLuxProp_CritterBase_SaveData, iLuxProp_SaveData) kSerializeVar(mvVel, eSerializeType_Vector3f)
    kSerializeVar(mvGravityVel, eSerializeType_Vector3f) kSerializeVar(mvSwarmPoint, eSerializeType_Vector3f)
        kSerializeVar(mlAnimState, eSerializeType_Int32) kSerializeVar(mvGroundNormal, eSerializeType_Vector3f)
            kSerializeVar(mbColliding, eSerializeType_Bool) kSerializeVar(mbUpdateAnimation, eSerializeType_Bool)
                kEndSerialize()
...
void iLuxProp_CritterBase::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
