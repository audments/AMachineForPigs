...
cLuxInventory::cLuxInventory() : iLuxUpdateable("LuxInventory") {
    ...
    // Sanity frame
    // Corners
    mpFrameSanityCorners[0] = NULL; // mpGui->CreateGfxImage("inventory_frame_sanity_corner_lu.tga",
                                    // eGuiMaterial_Alpha);
    mpFrameSanityCorners[1] = NULL; // mpGui->CreateGfxImage("inventory_frame_sanity_corner_ru.tga",
                                    // eGuiMaterial_Alpha);
    mpFrameSanityCorners[2] = NULL; // mpGui->CreateGfxImage("inventory_frame_sanity_corner_rd.tga",
                                    // eGuiMaterial_Alpha);
    mpFrameSanityCorners[3] = NULL; // mpGui->CreateGfxImage("inventory_frame_sanity_corner_ld.tga",
                                    // eGuiMaterial_Alpha);
    ...
    // Health frame
    // Corners
    mpFrameHealthCorners[0] = NULL; // mpGui->CreateGfxImage("inventory_frame_health_corner_lu.tga",
                                    // eGuiMaterial_Alpha);
    mpFrameHealthCorners[1] = NULL; // mpGui->CreateGfxImage("inventory_frame_health_corner_ru.tga",
                                    // eGuiMaterial_Alpha);
    mpFrameHealthCorners[2] = NULL; // mpGui->CreateGfxImage("inventory_frame_health_corner_rd.tga",
                                    // eGuiMaterial_Alpha);
    mpFrameHealthCorners[3] = NULL; // mpGui->CreateGfxImage("inventory_frame_health_corner_ld.tga",
                                    // eGuiMaterial_Alpha);
    ...
    // Generic frame
    // Corners
    mpFrameGenericCorners[0] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_corner_lu.tga", eGuiMaterial_Alpha);
    mpFrameGenericCorners[1] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_corner_ru.tga", eGuiMaterial_Alpha);
    mpFrameGenericCorners[2] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_corner_rd.tga", eGuiMaterial_Alpha);
    mpFrameGenericCorners[3] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_corner_ld.tga", eGuiMaterial_Alpha);

    // Borders
    mpFrameGenericBorders[0] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_border_u.tga", eGuiMaterial_Alpha);
    mpFrameGenericBorders[1] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_border_r.tga", eGuiMaterial_Alpha);
    mpFrameGenericBorders[2] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_border_d.tga", eGuiMaterial_Alpha);
    mpFrameGenericBorders[3] =
        NULL; // mpGui->CreateGfxImage("inventory_frame_generic_border_l.tga", eGuiMaterial_Alpha);
...
bool cLuxInventory_Slot::OnUIButtonRelease(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (pPickedItem && pItem && pItem == pPickedItem) {
        if (mpInventory->GetItemPickedAt() + 0.5f > gpBase->mpEngine->GetGameTime() &&
            mpInventory->GetPickedItemMoved() == false) {
            return OnMouseDoubleClick(apWidget, aData);
        }
    }
...
bool cLuxInventory_Slot::OnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    /*if(mfGlowAlpha >0)
    {
        mpInventory->mpGuiSet->DrawGfx(	mpGlowGfx,
    apWidget->GetGlobalPosition()+mpInventory->mvLayout_SlotMouseOverOffset, mpGlowGfx->GetActiveSize(),
    cColor(mfGlowAlpha * mpInventory->mfAlpha,1) );
    }*/
...
void cLuxInventory::OnDraw(float afFrameTime) {
    ...
    mpGuiSet->DrawFont(	mpFontDefault, vPlayerDataStart,20,cColor(1,mfAlpha),_W("%ls"), kTranslate("Inventory",
    "Thalers:").c_str()); mpGuiSet->DrawFont(	mpFontDefault,
    vPlayerDataStart+cVector3f(fPlayerDataXAdd,0,0),20,cColor(1,mfAlpha),_W("%d"), gpBase->mpPlayer->GetCoins());
    vPlayerDataStart.y+= 22;

    mpGuiSet->DrawFont(	mpFontDefault, vPlayerDataStart,20,cColor(1,mfAlpha),_W("%ls"), kTranslate("Inventory",
    "Tinderboxes:").c_str()); mpGuiSet->DrawFont(	mpFontDefault,
    vPlayerDataStart+cVector3f(fPlayerDataXAdd,0,0),20,cColor(1,mfAlpha),_W("%d"), gpBase->mpPlayer->GetTinderboxes());
    vPlayerDataStart.y+= 22;
    */
    ...
    if (mfMessageTextAlpha > 0) {
        if (msMessageText != _W("")) {
            ...
            mpGuiSet->DrawGfx(
                mpWhiteGfx,
                cVector3f(mvGuiSetStartPos.x, mvLayout_PopUpMessageCenter.y - 20, mvLayout_PopUpMessageCenter.z - 0.1f),
                cVector2f(mvGuiSetSize.x, mvLayout_FontSize_Message.y + 20 * 2), cColor(1.0f - fAlpha * 0.8f),
    ...
    // Draw frame around text
    {
        ...
        cVector3f vTextFramePos = vHeaderCenter - cVector3f(mvLayout_ItemTextFrameSize.x, 0, 0) * 0.5f -
...
cLuxInventory_Item *cLuxInventory::AddItem(const tString &asName, eLuxItemType aType, const tString &asSubTypeName,
                                           const tString &asImageName, float afAmount, const tString &asVal,
                                           const tString &asExtraVal, bool *apRemoveItemProp) {
    ////////////
    // First Init
...
void cLuxInventory::AddCombineCallback(const tString &asName, const tString &asItemA, const tString &asItemB,
...
//-----------------------------------------------------------------------
#ifdef USE_GAMEPAD
void cLuxInventory::AppDeviceWasPlugged() { mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent()); }

//-----------------------------------------------------------------------

void cLuxInventory::AppDeviceWasRemoved() { AppDeviceWasPlugged(); }
#endif
//-----------------------------------------------------------------------
...
void cLuxInventory::DrawDescTextRow(const tWString &asText, const cColor &aCol) {
    ...
    for (size_t i = 0; i < vRows.size(); ++i) {
        ...
        // mpGuiSet->DrawFont(vRows[i], mpFontDefault, cVector3f(vMessageBodyCenter.x,fY
        // ,1),vFontSize.y,finalCol,eFontAlign_Center);
...
static bool CheckItemTypeCombo(cLuxInventory_Item *apItemA, cLuxInventory_Item *apItemB, eLuxItemType aTypeA,
                               eLuxItemType aTypeB) {
    return (apItemA->GetType() == aTypeA && apItemB->GetType() == aTypeB) ||
           (apItemA->GetType() == aTypeB && apItemB->GetType() == aTypeA);
}

bool cLuxInventory::CheckSpecialCombineAction(cLuxInventory_Item *apItemA, cLuxInventory_Item *apItemB,
                                              int alSlotIndex) {
    /////////////////////////////
...
void cLuxInventory::CreateGui() {
    ...
    for (int i = 0; i < 4; ++i) {
        mpSanityStatus[i] =
            NULL; // pGui->CreateGfxImage("inventory/inventory_sanity_"+sStatusSuffix[i]+".tga", eGuiMaterial_Alpha);
        mpSanityGlow[i] = NULL; // pGui->CreateGfxImage("inventory/inventory_sanity_glow_"+sStatusSuffix[i]+".tga",
                                // eGuiMaterial_Alpha);

        mpHealthStatus[i] =
            NULL; // pGui->CreateGfxImage("inventory/inventory_health_"+sStatusSuffix[i]+".tga", eGuiMaterial_Alpha);
        mpHealthGlow[i] = NULL; // pGui->CreateGfxImage("inventory/inventory_health_glow_"+sStatusSuffix[i]+".tga",
                                // eGuiMaterial_Alpha);
    ...
    */
    /////////////////////////////////
    // Equip slot
    /*pImage = mpGuiSet->CreateWidgetImage("inventory/inventory_equip_slot.tga",
    0,-1,eGuiMaterial_Alpha,false,pSlotGroup); pImage->AddCallback(eGuiMessage_OnDraw,this,
    kGuiCallback(EquipSlotOnDraw)); pImage->AddCallback(eGuiMessage_MouseUp,this, kGuiCallback(EquipSlotMouseUp));
    mvImageWidgets.push_back(pImage);*/

    /////////////////////////////////
    // Journal
    mpJournalMouseOverGfx =
        NULL; // pGui->CreateGfxImage("inventory/inventory_journal_mouse_over.tga",eGuiMaterial_Alpha);
    /*
    ...
    for (int y = 0; y < mlLayout_SlotsRows; ++y)
        for (int x = 0; x < mlLayout_SlotsColumns; ++x) {
            ...
            cVector3f vPos = cVector3f((float)x * (pImage->GetSize().x + vSlotSeparation.x),
                                       (float)y * (pImage->GetSize().y + vSlotSeparation.y), 0);
...
bool cLuxInventory::SanityOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    /////////////////////////////////////////
    // Draw frame
    DrawFrameAroundWidget(mpFrameSanityCorners, mpFrameSanityBorders, apWidget, mvLayout_SanityFrameHPadding,
                          mvLayout_SanityFrameVPadding, mvLayout_SanityFrameOffset, mvLayout_SanityFrameSize);
...
bool cLuxInventory::HealthOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    /////////////////////////////////////////
    // Draw frame
    DrawFrameAroundWidget(mpFrameHealthCorners, mpFrameHealthBorders, apWidget, mvLayout_HealthFrameHPadding,
                          mvLayout_HealthFrameVPadding, mvLayout_HealthFrameOffset, mvLayout_HealthFrameSize);
...
bool cLuxInventory::TinderboxOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpGuiSet->DrawFont(mpFontDefault, vTinderboxCenter + cVector3f(0, -vCountFontSize.y * 0.5f, 1), vCountFontSize,
                       cColor(1, mfAlpha), eFontAlign_Center, eGuiMaterial_FontNormal, _W("x %d"),
                       gpBase->mpPlayer->GetTinderboxes());

    // mpGuiSet->DrawFont(	mpFontDefault, mvLayout_TinderboxesLabelCenter,
    //					mvLayout_FontSize_Label, cColor(1,mfAlpha), eFontAlign_Center, eGuiMaterial_FontNormal,
    //					_W("%ls"),kTranslate("Inventory", "Tinderboxes").c_str());

    DrawFrameAroundWidget(mpFrameGenericCorners, mpFrameGenericBorders, apWidget, mvLayout_TinderboxesFrameHPadding,
                          mvLayout_TinderboxesFrameVPadding, mvLayout_TinderboxesFrameOffset,
                          mvLayout_TinderboxesFrameSize);
...
bool cLuxInventory::JournalOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    /////////////////////////////////////////
    // Draw frame
    DrawFrameAroundWidget(mpFrameGenericCorners, mpFrameGenericBorders, apWidget, mvLayout_JournalFrameHPadding,
                          mvLayout_JournalFrameVPadding, mvLayout_JournalFrameOffset, mvLayout_JournalFrameSize);
...
bool cLuxInventory::ItemSlotsOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    if (mbLayout_DrawSlotsFrame) {
        DrawFrameAroundWidget(mpFrameGenericCorners, mpFrameGenericBorders, apWidget, mvLayout_SlotFrameHPadding,
                              mvLayout_SlotFrameVPadding, mvLayout_SlotFrameOffset, mvLayout_SlotFrameSize);
    }

    if (mbLayout_DrawSlotsGrid) {
        // Get grid lines size
        cVector2f vHoriSize =
            cVector2f(apWidget->GetSize().x + mvLayout_SlotGridHExtension.x + mvLayout_SlotGridHExtension.y,
                      mvLayout_SlotsSeparation.y);
        cVector2f vVertSize =
            cVector2f(mvLayout_SlotsSeparation.x,
                      apWidget->GetSize().y + mvLayout_SlotGridVExtension.x + mvLayout_SlotGridVExtension.y);

        // Get grid lines separation
        cVector2f vLineSeparation =
            mvLayout_SlotSize + cVector2f(mvLayout_SlotsSeparation.x, mvLayout_SlotsSeparation.y);
...
void cLuxInventory::DrawFrameAroundWidget(cGuiGfxElement **apGfxCorners, cGuiGfxElement **apGfxBorders,
                                          iWidget *apWidget, const cVector2f &avHPadding, const cVector2f &avVPadding,
    ...
    DrawFrame(apGfxCorners, apGfxBorders,
              vPosition -
                  cVector3f(apGfxCorners[0]->GetActiveSize().x + avHPadding.x,
                            apGfxCorners[0]->GetActiveSize().y + avVPadding.x, 0) +
                  avOffset,
...
void cLuxInventory::DrawFrame(cGuiGfxElement **apGfxCorners, cGuiGfxElement **apGfxBorders, const cVector3f &avPosition,
    ...
    // Left part
    {
        ...
        // Draw Left border
        mpGuiSet->DrawGfx(apGfxBorders[3], vPos + cVector3f(0, apGfxCorners[0]->GetActiveSize().y, 0),
                          cVector2f(apGfxCorners[0]->GetActiveSize().x, avSize.y), cColor(1, mfAlpha));

        // Draw Lower left corner
        mpGuiSet->DrawGfx(apGfxCorners[3], vPos + cVector3f(0, apGfxCorners[0]->GetActiveSize().y + avSize.y, 0), -1,
                          cColor(1, mfAlpha));
    }
    ...
    {
        // Draw Upper border
        mpGuiSet->DrawGfx(apGfxBorders[0], vPos, cVector2f(avSize.x, apGfxCorners[0]->GetActiveSize().y),
                          cColor(1, mfAlpha));

        // Draw Lower border
        mpGuiSet->DrawGfx(apGfxBorders[2], vPos + cVector3f(0, apGfxCorners[0]->GetActiveSize().y + avSize.y, 0),
                          cVector2f(avSize.x, apGfxCorners[3]->GetActiveSize().y), cColor(1, mfAlpha));
    }
    ...
    // Right part
    {
        ...
        // Draw Right border
        mpGuiSet->DrawGfx(apGfxBorders[1], vPos + cVector3f(0, apGfxCorners[1]->GetActiveSize().y, 0),
                          cVector2f(apGfxCorners[1]->GetActiveSize().x, avSize.y), cColor(1, mfAlpha));

        // Draw Lower right corner
        mpGuiSet->DrawGfx(apGfxCorners[2], vPos + cVector3f(0, apGfxCorners[1]->GetActiveSize().y + avSize.y, 0), -1,
                          cColor(1, mfAlpha));
    }
}
