...
bool cLuxProp_Photocell::CanInteract(iPhysicsBody *apBody) { return false; }
...
void cLuxProp_Photocell::UpdatePropSpecific(float afTimeStep) {
    ...
    // Set mesh alpha
    float fGoalAmount =
        cMath::Clamp((mfLightLevel - mfLightLevelOffLimit) / (mfLightLevelOnLimit - mfLightLevelOffLimit), 0.0f, 1.0f);
...
void cLuxProp_Photocell::UpdateLightLevel(float afTimeStep) {
    ...
    cVector3f vSamplePos[6] = {
        pBV->GetWorldCenter() + cVector3f(vAxisAdd.x, 0, 0), pBV->GetWorldCenter() - cVector3f(vAxisAdd.x, 0, 0),
        pBV->GetWorldCenter() + cVector3f(0, vAxisAdd.y, 0), pBV->GetWorldCenter() - cVector3f(0, vAxisAdd.y, 0),
        pBV->GetWorldCenter() + cVector3f(0, 0, vAxisAdd.z), pBV->GetWorldCenter() - cVector3f(0, 0, vAxisAdd.z)};
...
kBeginSerialize(cLuxProp_Photocell_SaveData, iLuxProp_SaveData) kSerializeVar(mbLit, eSerializeType_Float32)
    kSerializeVar(mfLightLevel, eSerializeType_Float32) kSerializeVar(mfLightLevelCheckCount, eSerializeType_Float32)
        kSerializeVar(mfLightLevelOnLimit, eSerializeType_Float32)
            kSerializeVar(mfLightLevelOffLimit, eSerializeType_Float32) kEndSerialize()
...
void cLuxProp_Photocell::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
