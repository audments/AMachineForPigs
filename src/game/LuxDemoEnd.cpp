...
void cLuxDemoEnd::OnEnterContainer(const tString &asOldContainer) {
    ...
    // Load background
    iTexture *pTex =
        gpBase->mpEngine->GetResources()->GetTextureManager()->Create2D(msBackgroundFile, false, eTextureType_Rect);
    if (pTex)
...
void cLuxDemoEnd::OnDraw(float afFrameTime) {
    ...
    if (mpFontMessage) {
        ...
        if (mbIsSteamVersion)
            mpGuiSet->DrawFont(_W("Available now on Steam!"), mpFontMessage, cVector3f(400.0f, mfAvailableAtY, 10),
...
bool cLuxDemoEnd::ButtonLabelOnDraw(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
