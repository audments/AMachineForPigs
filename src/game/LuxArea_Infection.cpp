    ...
    if (mbCollidesWithPlayer) {
        ...
        if (mfIncrementSpeed < 0 && (currentInfection + fIncrementThisFrame) <= mfInfectionLevelLowerBound) {
            ...
            if (fIncrementThisFrame > 0.0f)
                fIncrementThisFrame = 0.0f; // If the area is meant to heal up to a certain level, it can't damage if
                                            // the player isn't at that level yet
            ...
            if (fIncrementThisFrame < 0.0f)
                fIncrementThisFrame = 0.0f; // If the area is meant to damage up to a certain level, it can't heal if
                                            // the player is at a higher level
...
iLuxEntity_SaveData *cLuxArea_Infection::CreateSaveData() { return hplNew(cLuxArea_Infection_SaveData, ()); }
...
void cLuxArea_Infection::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
