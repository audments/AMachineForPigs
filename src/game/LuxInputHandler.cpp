...
// This is a list of all inputs, note that there can be several inputs per action!! (and this is the reason for having
// two arrays!!) Also note though that the current save/load of actions assume that there are no duplicates, and hence
// DO NOT add duplicates here
// until the key config load is updated!! (Remove this message then).
static cLuxInput gvLuxInputs[] = {
    cLuxInput("Keyboard", eKey_Escape, eLuxAction_Exit), cLuxInput("Keyboard", eKey_F12, eLuxAction_ExitDirect),
    ...
    cLuxInput("Keyboard", eKey_F1, eLuxAction_OpenDebug), cLuxInput("Keyboard", eKey_F4, eLuxAction_QuickSave),
    ...
    cLuxInput("Keyboard", eKey_W, eLuxAction_Forward), cLuxInput("Keyboard", eKey_S, eLuxAction_Backward),
    cLuxInput("Keyboard", eKey_D, eLuxAction_Right), cLuxInput("Keyboard", eKey_A, eLuxAction_Left),
    ...
    cLuxInput("MouseButton", eMouseButton_Left, eLuxAction_Interact), cLuxInput("Keyboard", eKey_C, eLuxAction_Ignite),
    cLuxInput("Keyboard", eKey_R, eLuxAction_Rotate), cLuxInput("Keyboard", eKey_H, eLuxAction_Holster),
    cLuxInput("Keyboard", eKey_F, eLuxAction_Lantern),

    cLuxInput("Keyboard", eKey_LeftShift, eLuxAction_Run), cLuxInput("Keyboard", eKey_LeftCtrl, eLuxAction_Crouch),
...
void cLuxInputHandler::OnStart() { mpPlayer = gpBase->mpPlayer; }
...
void cLuxInputHandler::ResetSmoothMousePos() { mlstSmoothMousePos.clear(); }
...
#ifdef USE_GAMEPAD
bool cLuxInputHandler::IsGamepadPresent() { return mpPad != NULL; }
#endif

//-----------------------------------------------------------------------

#ifdef USE_GAMEPAD
void cLuxInputHandler::AppDeviceWasPlugged() { SetUpGamepad(); }
#endif

#ifdef USE_GAMEPAD
void cLuxInputHandler::AppDeviceWasRemoved() { SetUpGamepad(); }
#endif
...
bool cLuxInputHandler::UpdateGamepadUIInput() {
    ...
    int vActionIDs[] = {eLuxAction_UIArrowUp,
...
void cLuxInputHandler::UpdateGameInput() {
    ...
    if (mpPlayer->IsDead() == false && gpBase->mpDebugHandler->GetAllowQuickSave() &&
        gpBase->mbPTestActivated == false) {
    ...
    if (mpInput->BecameTriggerd(eLuxAction_Exit) &&
        gpBase->mpPlayer->GetRandomEscapeFailCount() <= 0) // escape may fail when chased
    {
...
void cLuxInputHandler::UpdateGamePlayerInput() {
    ...
    if (mpPlayer->IsDead()) {
        if (mpInput->BecameTriggerd(eLuxAction_Attack) || mpInput->BecameTriggerd(eLuxAction_Interact) ||
            mpInput->BecameTriggerd(eLuxAction_Ignite) || mpInput->BecameTriggerd(eLuxAction_Jump) ||
    ...
    cVector2f vMouseRelPosFloat = cVector2f(vMouseRelPos.x, vMouseRelPos.y) * mfMouseSensitivity * mfPointerSpeed *
                                  (mpPlayer->IsAtMaxInfectionLevel() ? mfSensitivityDropAtMaxLevel : 1.0f) *
                                  mfEventSensitivityModifier;
    cVector2l vAbsRel = cMath::RoundToInt(vMouseRelPosFloat);
    cVector2f vRelPos =
        cVector2f((float)vAbsRel.x, (float)vAbsRel.y) / (1.7f * mpGraphics->GetLowLevel()->GetScreenSizeFloat().y);
    cVector2f vFinalPos;
    ...
    if (IsGamepadPresent()) {
        ...
        //////////////////////////////////////////
        // Walk
        // cVector2f vAnalogWalkAxis = cVector2f(mpPad->GetAxisValue(eGamepadAxis_0),
        // mpPad->GetAxisValue(eGamepadAxis_1));
        ...
        //////////////////////////////////////////
        // Look / Lean
        cVector2f vAnalogLookAxis =
            cVector2f(mpPad->GetAxisValue(eGamepadAxis_RightX), mpPad->GetAxisValue(eGamepadAxis_RightY));
        ...
        if (mpInput->IsTriggerd(eLuxAction_Lean)) {
            ...
            if (mpInput->IsTriggerd(eLuxAction_Rotate) && mpInput->IsTriggerd(eLuxAction_Interact) &&
                (playerState == eLuxPlayerState_Normal || playerState == eLuxPlayerState_Ladder ||
                 playerState == eLuxPlayerState_UseItem) == false) {
                mpPlayer->Scroll(-gpBase->mpEngine->GetFrameTime() * 6.0f * vAnalogLookAxis.y);

                vAnalogLookAxis =
                    cVector2f(mpPad->GetAxisValue(eGamepadAxis_LeftX), mpPad->GetAxisValue(eGamepadAxis_LeftY)) * 2.0f /
                    1.5f;
            ...
            vAnalogLookAxis -= cVector2f(cMath::Sign(vAnalogLookAxis.x), cMath::Sign(vAnalogLookAxis.y)) *
                               mpPad->GetAxisDeadZoneRadiusValue();
            ...
            cVector2f vGamepadPos =
                (vAnalogLookAxis * vExponent) * mfGamepadLookSensitivity * gpBase->mpEngine->GetStepSize();
...
void cLuxInputHandler::UpdateInventoryInput() {
    ...
    if (gpBase->mpInventory->GetMessageActive()) {
        gpBase->mpEngine->GetGui()->SendMousePos(mpInput->GetMouse()->GetAbsPosition(),
        ...
        if (mpInput->BecameTriggerd(eLuxAction_LeftClick) || mpInput->BecameTriggerd(eLuxAction_MiddleClick) ||
            mpInput->BecameTriggerd(eLuxAction_RightClick) || mpInput->BecameTriggerd(eLuxAction_UIPrimary) ||
...
void cLuxInputHandler::UpdateJournalInput() {
    ...
    } else if (mpInput->BecameTriggerd(eLuxAction_Journal) || mpInput->BecameTriggerd(eLuxAction_QuestLog) ||
               mpInput->BecameTriggerd(eLuxAction_Inventory) || mpInput->BecameTriggerd(eLuxAction_RecentText)) {
#ifdef USE_GAMEPAD
