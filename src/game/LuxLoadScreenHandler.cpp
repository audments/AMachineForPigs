...
void cLuxLoadScreenHandler::SetupLoadText(const tString &asCat, const tString &asEntry, int alRandomNum,
                                          const tString &asImage) {
    if (msLoadTextCat == asCat && msLoadTextEntry == asEntry && msLoadTextImage == asImage &&
...
void cLuxLoadScreenHandler::DrawGameState(float afFrameTime) {
    ...
    mpGuiSet->DrawGfx(mpWhiteGfx, cVector3f(mvGuiSetStartPos.x, mvGuiSetStartPos.y, 40), mvGuiSetSize,
                      cColor(0, 1.0f - mfAlpha));
}
...
void cLuxLoadScreenHandler::DrawGameScreen(cGuiSet *apSet) {
    ...
    if (msCurrentGameScreenTextEntry != "") {
        ...
        mpFontDefault->GetWordWrapRows(mfTextMaxWidth, mvTextFontSize.y, mvTextFontSize,
                                       kTranslate(msLoadTextCat, msCurrentGameScreenTextEntry), &vTextRows);
        ...
        apSet->DrawFont(
            sLoading, mpFontDefault, vPos, mvLoadingFontSize,
            cColor(mLoadingFontColor.r, mLoadingFontColor.g, mLoadingFontColor.b, mLoadingFontColor.a * fLoadAlpha),
            eFontAlign_Center);
        // apSet->DrawFont(sContinue, mpFontDefault, vPos, mvLoadingFontSize,
        // cColor(mLoadingFontColor.r,mLoadingFontColor.g,mLoadingFontColor.b,mLoadingFontColor.a * fContinueAlpha),
        // eFontAlign_Center);
    }
    //////////////////////
    // Draw loading only
