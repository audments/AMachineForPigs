...
void cLuxEnemy_Child::OnSetupAfterLoad(cWorld *apWorld) { mpMover->SetWallAvoidanceActive(false); }
...
bool cLuxEnemy_Child::PlayerIsDetected() { return false; }
...
float cLuxEnemy_Child::GetDamageMul(float afAmount, int alStrength) { return 1.0f; }
...
kBeginSerialize(cLuxEnemy_Child_SaveData, iLuxEnemy_SaveData)
    ...
    kSerializeVar(mPatrolMoveSpeed, eSerializeType_Int32) kSerializeVar(mfWaitTime, eSerializeType_Float32)
...
void cLuxEnemy_Child::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
