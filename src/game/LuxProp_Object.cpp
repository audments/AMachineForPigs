...
cLuxProp_Object_BodyCallback::cLuxProp_Object_BodyCallback(cLuxProp_Object *apObject) { mpObject = apObject; }

cLuxProp_Object_BodyCallback::~cLuxProp_Object_BodyCallback() {}

//-----------------------------------------------------------------------

bool cLuxProp_Object_BodyCallback::OnAABBCollide(iPhysicsBody *apBody, iPhysicsBody *apCollideBody) { return true; }

//-----------------------------------------------------------------------

static inline cVector3f GetCorrectNormal(const cVector3f &avNormal, const cVector3f &avCollidePoint,
                                         const cVector3f &avBodyACenter) {
...
void cLuxProp_Object_BodyCallback::OnBodyCollide(iPhysicsBody *apBody, iPhysicsBody *apCollideBody,
    ...
    if (mpObject->mBreakData.mbActive) {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            cVector3f vTowardsImpactNormal =
                GetCorrectNormal(apContactData->mvContactNormal, apContactData->mvContactPosition, vBodyCenter);
    ...
    if (apCollideBody->IsCharacter()) {
        ...
        if (fSpeed >= mpObject->mfMinHitDamageSpeed) {
            ...
            {
                gpBase->mpPlayer->GiveDamage(mpObject->mfHitDamageAmount,
            mpObject->mlHitDamageStrength,eLuxDamageType_BloodSplat);
            }
            else*/
...
cLuxProp_Object::~cLuxProp_Object() { hplDelete(mpBodyCallback); }
...
kBeginSerialize(cLuxProp_Object_SaveData, iLuxProp_SaveData) kSerializeVar(mfLifeLengthCount, eSerializeType_Float32)
    kSerializeVar(msContainedItem, eSerializeType_String) kSerializeVar(mlStuckState, eSerializeType_Int32)
        kSerializeVar(mfFoodAttractCount, eSerializeType_Float32) kSerializeVar(mbDisableBreakable, eSerializeType_Bool)
            kSerializeVar(mbIsInsanityVision, eSerializeType_Bool)
                kSerializeVar(mfVisionMinInfection, eSerializeType_Float32)
                    kSerializeVar(mbInfectionVisionActive, eSerializeType_Bool) kEndSerialize()
...
void cLuxProp_Object::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
