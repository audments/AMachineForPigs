...
cLuxLampLightConnection::cLuxLampLightConnection(iLight *apLight) { mpLight = apLight; }
    ...
    if (mpMeshEntity) {
        if (mbFlickerActive && mvLights.size() > 0) {
            if (mvLights[0]->GetFlickerFade()) {
                mpMeshEntity->SetIlluminationAmount(
                    mfEffectsAlpha * (mvLights[0]->GetFlickerOn() ? mvLights[0]->GetFadeProgress() : 0.0f));
            } else {
...
void cLuxProp_Lamp::SetFlickerActive(bool abFlickerActive) {
    ...
    if(mpMeshEntity)
    {
        mpMeshEntity->SetIlluminationAmount(mfEffectsAlpha * (mvLights[0]->GetFlickerOn() ?
mvLights[0]->GetFadeProgress() : 0.0f ));
    }
}
else
{
...
void cLuxProp_Lamp::SetupLampLightConnection() {
    if (msConnectionLight != "") {
        ...
        // Iterate all lights and get light with name and with NO parent (otherwise a light connected to an entity is
        // gotten).
    ...
    if (msConnectionLight2 != "") {
        ...
        if (pConnectionLight2) {
            mpMap->AddLampLightConnection(this, pConnectionLight2, mfConnectionLight2Amount,
                                          mbConnectionLight2UseOnColor, mbConnectionLight2UseSpec);
        } else {
            Error("Light '%s' that is connected to lamp '%s' does not exist!\n", msConnectionLight2.c_str(),
                  msName.c_str());
        }
    }
...
kBeginSerialize(cLuxProp_Lamp_SaveData, iLuxProp_SaveData) kSerializeVar(mbLit, eSerializeType_Bool)
    kSerializeVar(mbLightConnectionSetup, eSerializeType_Bool) kSerializeVar(msConnectionLight, eSerializeType_String)
