...
void cLuxPropLoader_Critter_Spider::LoadCritterInstanceVariables(iLuxProp *apProp,
...
float cLuxCritter_Spider::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    return afStartY;
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 1), 12, cColor(1, 1), _W("OnGround: %d Colliding: %d Health: %f"),
                    mbOnGround, mbColliding, mfHealth);
    afStartY += 13;
...
void cLuxCritter_Spider::UpdateVelocity(float afTimeStep) {
    ...
    // Move
        ...
        if (mfCount <= 0) {
            ...
            if (gpBase->mpPlayer->IsDead() == false && fDistToPlayer < mvAttackPlayerDistMinMax.y &&
                ...
                cVector3f vDir = cMath::Vector3Normalize(gpBase->mpPlayer->GetCharacterBody()->GetPosition() +
                                                         cVector3f(0, 0.5f, 0) - mpBody->GetLocalPosition());
    ...
    // Attack 1
        ...
        if (mfCount <= 0 || GetDistanceToPlayer2D() < 1.5f) {
            ...
            Attack(cMath::Vector3Normalize(gpBase->mpPlayer->GetCharacterBody()->GetPosition() -
                                           mpBody->GetLocalPosition()));
            mfCount = 0.5f;
...
void cLuxCritter_Spider::OnShapeCollision(const cVector3f &avPushVec, float afTimeStep) {
    ...
    if (mfNewGroundNormalCount > 0) {
        ...
        // gpBase->mpDebugHandler->AddMessage(_W("Newground count too high: ")+
        // cString::ToStringW(mfNewGroundNormalCount), false);
        return;
    }
    ...
    if (mpRayCallback->GetIntersected()) {
        ...
        {
            // gpBase->mpDebugHandler->AddMessage(_W("Dot only: ")+
            // cString::ToStringW(cMath::Vector3Dot(mvIntersectNormal, mvGroundNormal)), false);
            return;
        }
    } else {
        return;
    ...
    // gpBase->mpDebugHandler->AddMessage(_W("New ground normal!
    // ")+cString::ToStringW(mvGroundNormal.x)+_W(":")+cString::ToStringW(mvGroundNormal.y)+_W(":")
...
kBeginSerialize(cLuxCritter_Spider_SaveData, iLuxProp_CritterBase_SaveData) kSerializeVar(mlState, eSerializeType_Int32)
    kSerializeVar(mfFallingCount, eSerializeType_Float32) kSerializeVar(mfNewGroundNormalCount, eSerializeType_Float32)
        kSerializeVar(mbOnGround, eSerializeType_Bool) kSerializeVar(mfMaxSpeed, eSerializeType_Float32)
            kSerializeVar(mfCount, eSerializeType_Float32) kSerializeVar(mfPlaySoundCount, eSerializeType_Float32)
                kEndSerialize()
...
void cLuxCritter_Spider::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
