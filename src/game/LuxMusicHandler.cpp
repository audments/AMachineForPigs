...
cLuxMusic::cLuxMusic() { Reset(); }

void cLuxMusic::Reset() { msFile = ""; }
...
void cLuxMusicHandler::Play(const tString &asFile, bool abLoop, float afVolume, float afFadeTime, int alPrio,
                            bool abResume, bool abSpecialEffect) {
    if (alPrio > mlMaxPrio)
        alPrio = mlMaxPrio;
...
void cLuxMusicHandler::AddEnemy(eLuxEnemyMusic aType, iLuxEnemy *apEntity) { m_setEnemies[aType].insert(apEntity); }

//-----------------------------------------------------------------------

void cLuxMusicHandler::RemoveEnemy(eLuxEnemyMusic aType, iLuxEnemy *apEntity) { m_setEnemies[aType].erase(apEntity); }
    ...
    if (mbEnemyClosePlaying == false) {
        ...
        if (sMusic != "") {
            ...
            if (mfEnemyCloseCount > 2.0f) {
                ...
                mvGameMusic[5].msFile =
                    ""; // hack to make sure it doesnt loop automatically when all enemies are disabled
    ...
    if (mbEnemyPlaying[aType] == false && m_setEnemies[aType].empty() == false) {
        ...
        if (mfEnemyPlayCount[aType] > 0.1f) {
            ...
            if (sMusic != "") {
                ...
                mvGameMusic[9 + aType].msFile =
                    ""; // hack to make sure the music does not start playing automatically again
    ...
    for (int i = mlMaxPrio; i >= 0; --i) {
        if (mvGameMusic[i].msFile != "") {
            ...
            if (mvGameMusic[i].mbLoop) {
                mpMusicHandler->Play(mvGameMusic[i].msFile, mvGameMusic[i].mfVolume, 0.3f, true,
                                     mvGameMusic[i].mbResume);
