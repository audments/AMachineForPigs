...
void iLuxInstanityEvent::Start() {
    ...
    gpBase->mpProgressLogHandler->AddLog(eLuxProgressLogLevel_Low,
                                         "Starting Insanity event: '" + msName + "'" + " set: '" + msSet + "'");
...
void cLuxInstanityEvent_SoundStream::OnStart() {
    ...
    if (msSubtitleCat != "" && msSubtitleEntry != "") {
        gpBase->mpDefaultFont->GetWordWrapRows(600, 20, 18, kTranslate(msSubtitleCat, msSubtitleEntry),
                                               &mvCurrentTextRows);
    }

    if (msFadeImageFile != "") {
        mpFadeImage =
            gpBase->mpEngine->GetGui()->CreateGfxTexture(msFadeImageFile, eGuiMaterial_Alpha, eTextureType_Rect);
    }
}
...
void cLuxInstanityEvent_SoundStream::OnDraw(float afFrameTime) {
    if (gpBase->mpMessageHandler->ShowEffectSubtitles() && mvCurrentTextRows.empty() == false &&
    ...
    if (mbFadeScreen) {
        if (mpFadeImage)
            gpBase->mpGameHudSet->DrawGfx(mpFadeImage, gpBase->mvHudVirtualStartPos, gpBase->mvHudVirtualSize,
                                          mFadeColor * mfFadeAlpha);
        else
            gpBase->mpGameHudSet->DrawGfx(mpWhiteGfx, gpBase->mvHudVirtualStartPos, gpBase->mvHudVirtualSize,
                                          mFadeColor * mfFadeAlpha + cColor(1, 1) * (1 - mfFadeAlpha));
    }
}
...
cLuxInsanityHandler::~cLuxInsanityHandler() { STLDeleteAll(mvEvents); }
...
void cLuxInsanityHandler::EnableSet(const tString &asSet) { m_setDisabledSets.erase(asSet); }

//-----------------------------------------------------------------------

void cLuxInsanityHandler::DisableSet(const tString &asSet) { m_setDisabledSets.insert(asSet); }
