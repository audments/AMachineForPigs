...
void cLuxPlayerState_InteractGrab::OnEnterState(eLuxPlayerState aPrevState) {
    ...
    ); pCustomMat->SetDepthTest(true); pCustomMat->SetBlendMode(eMaterialBlendMode_Mul);
...
cGuiGfxElement *cLuxPlayerState_InteractGrab::GetCrosshair() { return NULL; }
...
float cLuxPlayerState_InteractGrab::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    cVector3f vCurrentRot =
        cMath::MatrixToEulerAngles(mpCurrentBody->GetLocalMatrix().GetRotation(), eEulerRotationOrder_XYZ);
    ...
    /*apSet->DrawFont(apFont, cVector3f(5,afStartY,0),12,cColor(1,1),_W("Current: %ls"),
    cString::To16Char(vCurrentRot.ToString()).c_str()); afStartY += 13; apSet->DrawFont(apFont,
    cVector3f(5,afStartY,0),12,cColor(1,1),_W("Wanted: %ls"), cString::To16Char(vWantedRot.ToString()).c_str());
    afStartY += 13;
    apSet->DrawFont(apFont, cVector3f(5,afStartY,0),12,cColor(1,1),_W("Error: %ls"),
    cString::To16Char(vRotError.ToString()).c_str()); afStartY += 13;*/
...
kBeginSerialize(cLuxPlayerState_InteractGrab_SaveData, iLuxPlayerState_Interact_SaveData)
    kSerializeVar(m_mtxBodyRotation, eSerializeType_Matrixf) kSerializeVar(mvLocalBodyOffset, eSerializeType_Vector3f)
        kSerializeVar(mfDepth, eSerializeType_Float32) kSerializeVar(mfMaxDistance, eSerializeType_Float32)
            kEndSerialize()
