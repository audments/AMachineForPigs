...
void cLuxCredits::AddKeyPart(int alKey) { mKeyBuffer.AddInt32(alKey); }
...
void cLuxCredits::OnDraw(float afFrameTime) {
        ...
        for (size_t i = 0; i < mvTextRows.size(); ++i) {
            ...
            // Text row
                ...
                if (fY >= -fRowAdd[lSize]) {
                    ...
                    // Normal
                    if (lSize == 0)
                        mpGuiSet->DrawFont(mvTextRows[i], mpFontHeader, cVector3f(400, fY, 10), mvHeaderFontSize,
                                           cColor(0.8f, fAlpha), eFontAlign_Center);
                    // Header
                    else
    ...
    // STATE 1 - SECRET CODE
        ...
        if (mlEndNum >= 0 && mlEndNum <= 2 && gpBase->mbPTestActivated == false) {
            ...
            //	mpGuiSet->DrawFont(_W("Well done, but far from perfect..."), mpFontNormal,
            //cVector3f(400,560,10),11,cColor(0.65f,1),eFontAlign_Center);
        }
    }
}
