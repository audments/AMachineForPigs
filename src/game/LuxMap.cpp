...
cLuxDissolveEntity::cLuxDissolveEntity(cLuxMap *apMap) { mpMap = apMap; }

cLuxDissolveEntity::~cLuxDissolveEntity() { mpMap->mpWorld->DestroyMeshEntity(mpEntity); }
...
void cLuxMap::SetCheckPoint(const tString &asName, const tString &asStartPos, const tString &asCallback,
    ...
    for (int i = gpBase->mpMusicHandler->GetMaxPrio(); i >= 0; --i) {
        ...
        if (pMusic->msFile != "" && pMusic->mbLoop && pMusic->mbSpecialEffect == false) {
            ...
            // When music handler plays music, it multiplies it to get a new volume. Need to change that back when
            // storing.
...
void cLuxMap::LoadCheckPoint() {
    gpBase->mpDebugHandler->AddMessage(_W("Loading check point ") + cString::To16Char(msCheckPointName) +
                                           _W(" Count: ") + cString::ToStringW(mlCheckPointCount),
                                       false);
    gpBase->mpProgressLogHandler->AddLog(eLuxProgressLogLevel_High,
                                         "Loading checkpoint " + msCheckPointName +
                                             " count: " + cString::ToString(mlCheckPointCount));
...
cLuxEntityIterator cLuxMap::GetEntityIterator() { return cLuxEntityIterator(&mlstEntities); }

//-----------------------------------------------------------------------

cLuxEnemyIterator cLuxMap::GetEnemyIterator() { return cLuxEnemyIterator(&mlstEnemies); }
...
void cLuxMap::BroadcastEnemySoundMessage(const cVector3f &avPos, float afVolume, float afMinDist, float afMaxDist,
    ...
    while (it.HasNext()) {
        ...
        // Log("Broadcasted %s\n", asSoundName.c_str());
        // DebugMessage("Sound '"+asSoundName+"' heard by '"+pEnemy->GetName()+"'
        // Vol:"+cString::ToString(fHearVolume,2)+ "");
    }
}
...
bool cLuxMap::CheckCollision(iLuxCollideCallbackContainer *apCollider1, iLuxCollideCallbackContainer *apCollider2) {
    ...
    for (int body1 = 0; body1 < apCollider1->GetBodyNum(); ++body1)
        for (int body2 = 0; body2 < apCollider2->GetBodyNum(); ++body2) {
            ...
            if (mpPhysicsWorld->CheckShapeCollision(pBody1->GetShape(), pBody1->GetLocalMatrix(), pBody2->GetShape(),
                                                    pBody2->GetLocalMatrix(), collideData, 1, false)) {
                return true;
            }
        }
...
cLuxUseItemCallback *cLuxMap::GetUseItemCallback(const tString &asItem, const tString &asEntity) {
    ...
    for (; it != mlstUseItemCallbacks.end(); ++it) {
        ...
        //	pCallback->msName.c_str(), pCallback->msEntity.c_str(), pCallback->msItem.c_str(),
        //pCallback->msFunction.c_str());
...
void cLuxMap::AddCompletionAmount(int alAmount, float afDelay) {
    ...
    // / ")+ 															cString::ToStringW(mlTotalCompletionAmount) + _W(" Percent: ")+ 															cString::ToStringW(fPercent), 															false);
}
