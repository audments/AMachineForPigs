...
kBeginSerializeBaseVirtual(iLuxInteractConnection_SaveData) kSerializeVar(msName, eSerializeType_String)
    kSerializeVar(mbInteractionOnly, eSerializeType_Bool) kSerializeVar(mlPropId, eSerializeType_Int32)
        kSerializeVar(mbInvert, eSerializeType_Bool) kSerializeVar(mlStateUsed, eSerializeType_Int32) kEndSerialize()
    ...
    cLuxInteractConnection_Rope::cLuxInteractConnection_Rope(const tString &asName, iLuxProp *apProp,
                                                             iPhysicsRope *apRope, float afSpeedMul, float afMinSpeed,
...
iLuxInteractConnection *cLuxInteractConnection_Rope_SaveData::CreateConnection(cLuxMap *apMap) {
    ...
    cLuxInteractConnection_Rope *pConnection = hplNew(
        cLuxInteractConnection_Rope, (msName, pProp, pRope, mfSpeedMul, mfMinSpeed, mfMaxSpeed, mbInvert, mlStateUsed));
...
kBeginSerialize(cLuxInteractConnection_Rope_SaveData, iLuxInteractConnection_SaveData)
    kSerializeVar(mlRopeId, eSerializeType_Int32) kSerializeVar(mfSpeedMul, eSerializeType_Float32)
        kSerializeVar(mfMinSpeed, eSerializeType_Float32) kSerializeVar(mfMaxSpeed, eSerializeType_Float32)
            kEndSerialize()
    ...
    cLuxInteractConnection_MoveObject::cLuxInteractConnection_MoveObject(const tString &asName, iLuxProp *apProp,
                                                                         cLuxProp_MoveObject *apMoveObject,
...
iLuxInteractConnection *cLuxInteractConnection_MoveObject_SaveData::CreateConnection(cLuxMap *apMap) {
    //////////////////
    // Get the move object
    cLuxProp_MoveObject *pMoveObject = static_cast<cLuxProp_MoveObject *>(
        apMap->GetEntityByID(mlMoveObjectId, eLuxEntityType_Prop, eLuxPropType_MoveObject));
    if (pMoveObject == NULL) {
    ...
    cLuxInteractConnection_MoveObject *pConn =
        hplNew(cLuxInteractConnection_MoveObject, (msName, pProp, pMoveObject, mbInvert, mlStateUsed));
...
kBeginSerialize(cLuxInteractConnection_MoveObject_SaveData, iLuxInteractConnection_SaveData)
    kSerializeVar(mlMoveObjectId, eSerializeType_Int32) kEndSerialize()
