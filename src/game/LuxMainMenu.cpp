...
cLuxMainMenu::cLuxMainMenu() : iLuxUpdateable("LuxDebugHandler") {
    ...
    mvWindows[eLuxMainMenuWindow_CustomStoryList] =
        hplNew(cLuxMainMenu_CustomStoryList, (mpGuiSet, mpGuiSkin, pCustomStoryWindow));
    ...
    for (int i = 0; i < 2; ++i) {
        ...
        mpBlurProgram[i] = mpGraphics->CreateGpuProgramFromShaders("MainMenuBlur" + cString::ToString(i),
                                                                   ...
                                                                   "mainmenu_screen_blur_frag.glsl", &programVars);
    }
...
void cLuxMainMenu::OnDraw(float afFrameTime) {
    ...
    if (mpScreenGfx) {
        ...
        mpGuiSet->DrawGfx(mpTopBackground, cVector3f(0, vPos.y, 0.5f),
                          cVector2f(mvScreenSize.x, mvScreenSize.y - vPos.y), cColor(1, 0.5f * mfTopMenuAlpha));
    }
    /////////////////////////////////
    // 3D background
    else {
...
void cLuxMainMenu::AppDeviceWasRemoved() { AppDeviceWasPlugged(); }
#endif
//-----------------------------------------------------------------------
...
void cLuxMainMenu::CreateTopMenuGui() {
    ...
    if (gpBase->mbPTestActivated == false && gpBase->mpMapHandler->MapIsLoaded() == false &&
        gpBase->mpSaveHandler->SaveFileExists()) {
    ...
    ///////////////
    // Load game
    if (gpBase->mbPTestActivated == false && gpBase->mpSaveHandler->SaveFileExists() &&
...
bool cLuxMainMenu::PressContinue(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
        _W(""), kTranslate("MainMenu", "Continue old game?"), kTranslate("MainMenu", "Yes"),
        kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedContinuePopup));
...
kGuiCallbackDeclaredFuncEnd(cLuxMainMenu, ClickedContinuePopup)
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
        _W(""), kTranslate("MainMenu", "Start a new game?"), kTranslate("MainMenu", "Yes"),
        kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedStartGamePopup));
...
bool cLuxMainMenu::ClickedLoadGamePopup(iWidget *apWidget, const cGuiMessageData &aData) { return true; }
...
bool cLuxMainMenu::PressExitToMainMenu(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
        _W(""), kTranslate("MainMenu", "Sure you want to exit to main menu?"), kTranslate("MainMenu", "Yes"),
...
bool cLuxMainMenu::PressExitAndSave(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    mpGuiSet->SetDrawFocus(gpBase->mpInputHandler->IsGamepadPresent());
    cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
        _W(""), kTranslate("MainMenu", "Sure you want to exit and save?"), kTranslate("MainMenu", "Yes"),
