...
cSoundEntity *iLuxEntity::PlaySound(const tString &asName, const tString &asFile, bool abRemoveWhenDone,
                                    bool abAttach) {
    if (asFile == "")
        return NULL;
...
void iLuxEntity::AddConnection(const tString &asName, iLuxEntity *apEntity, bool abInvertStateSent, int alStatesUsed,
    ...
    cLuxEntityConnection *pConnection =
        hplNew(cLuxEntityConnection, (asName, apEntity, abInvertStateSent, alStatesUsed, asCallbackFunc));
...
void iLuxEntity::UpdateCheckCollideCallback(float afTimeStep) { CheckCollisionCallback(msName, mpMap); }
...
void iLuxEntity::ConnectionStateChange(int alState) {
    ...
    for (size_t i = 0; i < mvConnections.size(); ++i) {
        ...
        // Log("Send state '%s'%d to '%s'as %d\n", msName.c_str(), alState, pConn->GetEntity()->GetName().c_str(),
        // lState);
        ...
        if (pConn->GetCallbackFunc() != "") {
            ...
            tString sCommand = pConn->GetCallbackFunc() + "(\"" + pConn->GetName() + "\"," + "\"" + msName + +"\"," +
                               "\"" + pConn->GetEntity()->GetName() + "\"," + cString::ToString(lState) + ")";
...
kBeginSerializeBase(cLuxEntityConnection_SaveData) kSerializeVar(msName, eSerializeType_String)
    kSerializeVar(mlEntityId, eSerializeType_Int32) kSerializeVar(mbInvertStateSent, eSerializeType_Bool)
        kSerializeVar(mlStatesUsed, eSerializeType_Int32) kSerializeVar(msCallbackFunc, eSerializeType_String)
            kEndSerialize()
...
kBeginSerializeBaseVirtual(iLuxEntity_SaveData) kSerializeVar(msName, eSerializeType_String)
    kSerializeVar(mbFullGameSave, eSerializeType_Bool) kSerializeVar(mbActive, eSerializeType_Bool)
        kSerializeVar(mbStartsActive, eSerializeType_Bool) kSerializeVar(mlEntityType, eSerializeType_Int32)
            kSerializeVar(mlID, eSerializeType_Int32)
                ...
                kSerializeVar(mfMaxFocusDistance, eSerializeType_Float32)
                    ...
                    kSerializeVar(mbInteractionDisabled, eSerializeType_Bool)
                        ...
                        kSerializeVar(mbIsCriticalEntity, eSerializeType_Bool) kSerializeVar(msCriticalRootName,
                                                ...
                                                kSerializeVar(msLookAtCallback, eSerializeType_String)
                                                    kSerializeVar(mbLookAtCallbackRemove, eSerializeType_Bool)
                                                        kSerializeVar(mbIsLookedAt, eSerializeType_Bool)
                                                            ...
                                                            kSerializeClassContainer(mvConnections,
                                                                                     cLuxEntityConnection_SaveData,
                                                                                     eSerializeType_Class)
                                                                kSerializeClassContainer(mlstCollideCallbacks,
