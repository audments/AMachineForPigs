...
class cLuxMapHelper : public iLuxUpdateable {
  public:
    ...
    bool GetClosestEntity(const cVector3f &avStart, const cVector3f &avDir, float afRayLength, float *afDistance,
                          iPhysicsBody **apBody, iLuxEntity **apEntity);

    bool GetClosestCharCollider(const cVector3f &avStart, const cVector3f &avDir, float afRayLength,
                                bool abCheckDynamic, float *afDistance, cVector3f *avNormal, iPhysicsBody **apBody);

    float GetLightLevelAtPos(const cVector3f &avPos, std::vector<iLight *> *apSkipLightsVec = NULL,
                             float afRadiusAdd = 0);

    void GetLightsAtNode(iRenderableContainerNode *apNode, tLightList &alstLights, const cVector3f &avPos);
    void GetLightsAtNode(iRenderableContainerNode *apNode, tLightList &alstLights, const cVector3f &avPos,
                         float afRadius, bool includeInvisible = false, bool includeBoxLights = true);
