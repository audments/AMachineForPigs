...
bool cLuxProp_SwingDoor::CanInteract(iPhysicsBody *apBody) {
    if (apBody->GetMass() == 0 && mbCanInteractWithStaticBody == false &&
        mpMap->BodyIsInDetachableStickyArea(apBody) == false) {
        return false;
    }
...
void cLuxProp_SwingDoor::SetOpenAmount(float afOpenAmount, float afDuration, bool abGoTowardsMaxAngle) {
    ...
    if (!pHingeJoint || abGoTowardsMaxAngle && pHingeJoint->GetMaxAngle() == 0.0f ||
...
kBeginSerialize(cLuxProp_SwingDoor_SaveData, iLuxProp_SaveData)
    ...
    kSerializeVar(mbLocked, eSerializeType_Bool) kSerializeVar(mbBroken, eSerializeType_Bool)
        kSerializeVar(mbClosed, eSerializeType_Bool) kSerializeVar(mlBrokenEntityID, eSerializeType_Int32)
...
void cLuxProp_SwingDoor::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
