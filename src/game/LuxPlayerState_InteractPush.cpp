...
void cLuxPlayerState_InteractPush::Update(float afTimeStep) {
    ...
    float fDistance = mpPushData->mbPushAtPoint
                          ? cMath::Vector3Dist(pCharBody->GetFeetPosition(), vAttachPos)
    ...
    if (mvPushDir == 0) {
        ...
        cVector3f vBodyVel = mpPushData->mbPushAtPoint ? mpCurrentBody->GetVelocityAtPosition(vAttachPos)
                                                       : mpCurrentBody->GetLinearVelocity();
    ...
    if (mpPlayer->GetCurrentMoveState() == eLuxMoveState_Normal) {
        cLuxMoveState_Normal *pNormalMove =
            static_cast<cLuxMoveState_Normal *>(mpPlayer->GetMoveStateData(eLuxMoveState_Normal));
    ...
    cMatrixf mtxNextTransform =
        cMath::MatrixMul(cMath::MatrixTranslate(mvPushDir * 0.1f), mpCurrentBody->GetLocalMatrix());

    bool bWillCollideWithPlayer = pPhysicsWorld->CheckShapeCollision(
        pCharBody->GetCurrentShape(), cMath::MatrixTranslate(pCharBody->GetPosition()), mpCurrentBody->GetShape(),
        mtxNextTransform, collideData, 1, false);
...
void cLuxPlayerState_InteractPush::PostUpdate(float afTimeStep) { ClearMoveVars(); }
...
bool cLuxPlayerState_InteractPush::OnAddYaw(float afAmount) { return true; }

bool cLuxPlayerState_InteractPush::OnAddPitch(float afAmount) { return true; }

//-----------------------------------------------------------------------

cGuiGfxElement *cLuxPlayerState_InteractPush::GetCrosshair() { return NULL; }
...
float cLuxPlayerState_InteractPush::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("CurrentVel: %ls"),
                    cString::To16Char(vBodyVel.ToString()).c_str());
    afStartY += 13.0f;

    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("WantedVel: %ls"),
                    cString::To16Char(vVelInDir.ToString()).c_str());
    afStartY += 13.0f;
...
kBeginSerialize(cLuxPlayerState_InteractPush_SaveData, iLuxPlayerState_Interact_SaveData) kEndSerialize()
