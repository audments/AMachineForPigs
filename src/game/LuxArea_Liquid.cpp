    ...
    for (size_t i = 0; i < vBodies.size(); ++i) {
        ...
        if (bInsideWater) {
            if (pPhysicsWorld->CheckShapeCollision(pBody->GetShape(), pBody->GetLocalMatrix(), pAreaBody->GetShape(),
                                                   pAreaBody->GetLocalMatrix(), collideData, 1, false) == false) {
                bInsideWater = false;
            }
        }
...
kBeginSerialize(cLuxArea_Liquid_SaveData, iLuxArea_SaveData) kSerializeVar(mfDensity, eSerializeType_Float32)
    kSerializeVar(mfLinearViscosity, eSerializeType_Float32) kSerializeVar(mfAngularViscosity, eSerializeType_Float32)

        kSerializeVar(msPhysicsMaterial, eSerializeType_String)

            kSerializeVar(mbHasWaves, eSerializeType_Bool) kSerializeVar(mnInfectionLevel, eSerializeType_Int32)
                kSerializeVar(mfWaveAmp, eSerializeType_Float32) kSerializeVar(mfWaveFreq, eSerializeType_Float32)
...
iLuxEntity_SaveData *cLuxArea_Liquid::CreateSaveData() { return hplNew(cLuxArea_Liquid_SaveData, ()); }
...
void cLuxArea_Liquid::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
