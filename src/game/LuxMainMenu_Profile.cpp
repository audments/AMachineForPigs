...
cLuxMainMenu_Profile::~cLuxMainMenu_Profile() { STLDeleteAll(mvProfiles); }
...
void cLuxMainMenu_Profile::CreateMainGui() {
    ...
    mpWindow = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_ButtonClose, cVector3f(0, 0, 5), mvWindowSize,
                                            kTranslate("MainMenu", "Profiles"));
    ...
    //////////////////////////
    // List box
    mpListProfiles = mpGuiSet->CreateWidgetListBox(vPos,
                                                   cVector2f(mvWindowSize.x - mfInformationWidth - fBorderSize * 2,
    ...
    // Select
    pButton =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(fButtonLength, 30), kTranslate("MainMenu", "Select"), mpWindow);
    ...
    // Create
    pButton =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(fButtonLength, 30), kTranslate("MainMenu", "Create"), mpWindow);
    ...
    // Delete
    pButton =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(fButtonLength, 30), kTranslate("MainMenu", "Delete"), mpWindow);
...
void cLuxMainMenu_Profile::CreateEnterNameGui() {
    ...
    // Window
    vPos.y += 5;
    mpWindowEnterName = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_ButtonClose, cVector3f(0, 0, 10),
                                                     mvEnterNameWindowSize, kTranslate("MainMenu", "Create Profile"));
    // mpWindowEnterName->AddCallback(eGuiMessage_WindowClose,this, kGuiCallback(WindowCloses));

    //////////////////////////
    // Label
    cWidgetLabel *pLabel =
        mpGuiSet->CreateWidgetLabel(vPos, -1, kTranslate("MainMenu", "Enter name of Profile"), mpWindowEnterName);
    pLabel->SetAutogenerateSize(true);
    vPos.y += pLabel->GetSize().y + fBorderSize;
    //////////////////////////
    // Text Box
    tWString sIllegalChars = _W("*/\\:<>|\"?");
    mpTextEnterName = mpGuiSet->CreateWidgetTextBox(vPos, cVector2f(mvEnterNameWindowSize.x - fBorderSize * 2, 20),
                                                    _W(""), mpWindowEnterName);
    ...
    pButton = mpGuiSet->CreateWidgetButton(vPos, cVector2f(mfEnterNameButtonLength, 30),
                                           kTranslate("MainMenu", "Create"), mpWindowEnterName);
    ...
    pButton = mpGuiSet->CreateWidgetButton(vPos, cVector2f(mfEnterNameButtonLength, 30),
                                           kTranslate("MainMenu", "Cancel"), mpWindowEnterName);
...
bool cLuxMainMenu_Profile::WindowCloses(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    // User config not loaded, pop up quit game message box
        ...
        cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
            _W(""), kTranslate("MainMenu", "Sure you want to quit?"), kTranslate("MainMenu", "Yes"),
            kTranslate("MainMenu", "No"), this, kGuiCallback(ClickedExitPopup));
...
bool cLuxMainMenu_Profile::PressDeleteProfile(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mpListProfiles->GetSelectedItem() < 0) {
        pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu", "No profile to be deleted!"), _W("OK"),
                                                 _W(""), NULL, NULL);
        pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
        return true;
    }
    ...
    pPopUp = mpGuiSet->CreatePopUpMessageBox(_W(""), kTranslate("MainMenu", "Sure you want to delete profile"),
                                             kTranslate("MainMenu", "Yes"), kTranslate("MainMenu", "No"), this,
...
bool cLuxMainMenu_Profile::PressDeleteProfilePopupClose(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (cPlatform::RemoveFolder(gpBase->msBaseSavePath + sFolder, true, true) == false) {
        cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
            _W(""), kTranslate("MainMenu", "Could not remove profile"), _W("OK"), _W(""), NULL, NULL);
        pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
        return true;
    }
...
static unsigned char gvEncStr[54] = {0x2B, 0x15, 0x7,  0x12, 0x5B, 0x47, 0x6C, 0x30, 0x37, 0x26, 0x22, 0x2E, 0x20, 0x2C,
                                     0x2E, 0x2E, 0x36, 0x2D, 0x2A, 0x37, 0x3A, 0x6D, 0x20, 0x2C, 0x2E, 0x6C, 0x22, 0x30,
                                     0x6C, 0x21, 0x2C, 0x36, 0x2D, 0x20, 0x26, 0x6C, 0x27, 0x22, 0x6C, 0x31, 0x28, 0x27,
                                     0x26, 0x30, 0x20, 0x26, 0x6C, 0x2D, 0x37, 0x6C, 0x7C, 0x28, 0x7E, 0};
...
bool cLuxMainMenu_Profile::PressEnterNameCreate(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (gpBase->CreateProfile(sProfileName) == false) {
        cGuiPopUpMessageBox *pPopUp = mpGuiSet->CreatePopUpMessageBox(
            _W(""), kTranslate("MainMenu", "The profile name already exists"), _W("OK"), _W(""), NULL, NULL);
        pPopUp->GetGuiSet()->SetDrawFocus(mpGuiSet->GetDrawFocus());
        return true;
    }
