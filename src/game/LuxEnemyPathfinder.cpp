...
void cLuxEnemyPathfinder::AfterWorldLoad() {
    ...
    mpNodeContainer =
        pWorld->CreateAINodeContainer(sType, "Default", mpEnemy->mpCharBody->GetSize(), false, 2, 6, 5.0f, 0.41f);
...
cAINode *cLuxEnemyPathfinder::GetNodeAtPos(const cVector3f &avPos, float afMinDistance, float afMaxDistance,
                                           bool abGetClosest, bool abPosToNodeFreePathCheck,
                                           bool abEnemyToNodeFreePathCheck, cAINode *apSkipNode, int alFreePathRayNum,
                                           tAIFreePathFlag alFreePathFlags, bool abSkipUsedNodes) {
    if (mpNodeContainer == NULL)
        return NULL;
    ...
    while (startNodeIt.HasNext()) {
        ...
        // Log(" %s dist: %f. pos: (%s)\n", pNode->GetName().c_str(), sqrtf(fDistSqr),
        ...
        if (abEnemyToNodeFreePathCheck &&
            mpNodeContainer->FreePath(mpEnemy->mpCharBody->GetPosition(), pNode->GetPosition(), alFreePathRayNum,
                                      alFreePathRayNum) == false) {
...
const cVector3f &cLuxEnemyPathfinder::GetFinalGoalPos() { return mvMoveGoalPos; }
...
void cLuxEnemyPathfinder::UpdateMoving(float afTimeStep) {
    ...
    tempBV.SetSize(cVector3f(vBodySize.x * pathNodeReachedCheckVolumeXZScaleFactor, vBodySize.y * 1.2f + 0.45f,
                             vBodySize.z * pathNodeReachedCheckVolumeXZScaleFactor));
...
kBeginSerializeBase(cLuxEnemyPathfinder_SaveData)
    ...
    kSerializeVar(mbMoving, eSerializeType_Bool) kSerializeVar(mvMoveGoalPos, eSerializeType_Vector3f)
