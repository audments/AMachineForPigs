...
void cLuxConfigHandler::SaveMainConfig() {
    ...
    gpBase->mpMainConfig->SetFloat("Graphics", "Gamma",
                                   gpBase->mpEngine->GetGraphics()->GetLowLevel()->GetGammaCorrection());
...
bool cLuxConfigHandler::ShowRestartWarning(cGuiSet *apSet, void *apObject, tGuiCallbackFunc apCallback) {
    ...
    if (mbGameNeedsRestart && mbRestartDialogShown == false) {
        ...
        cGuiPopUpMessageBox *pPopUp = apSet->CreatePopUpMessageBox(
            kTranslate("OptionsMenu", "ReqRestartLabel"), kTranslate("OptionsMenu", "ReqRestartMessage"),
            kTranslate("MainMenu", "OK"), _W(""), apObject, apCallback);
