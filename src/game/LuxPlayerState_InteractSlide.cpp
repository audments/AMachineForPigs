...
bool cLuxPlayerState_InteractSlide::OnDoAction(eLuxPlayerAction aAction, bool abPressed) {
    ...
    // Attack
        ...
        // Pressed
        if (abPressed) {
            cVector3f vImpulse = mpCurrentJoint->GetPinDir() *
                                 cMath::Vector3Dot(mpCurrentJoint->GetPinDir(), mpPlayer->GetCamera()->GetForward());
            vImpulse.Normalize();
...
cGuiGfxElement *cLuxPlayerState_InteractSlide::GetCrosshair() { return NULL; }
...
float cLuxPlayerState_InteractSlide::DrawDebug(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    ...
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 0), 12, cColor(1, 1), _W("SideSpeed: %f Force: %ls"), mfSlideSpeed,
                    cString::To16Char(mvLastForce.ToString()).c_str());
    afStartY += 13;
...
void cLuxPlayerState_InteractSlide::RenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    apFunctions->GetLowLevelGfx()->DrawLine(vWorldInteractPos, vWorldInteractPos + vMouseCamDir * 50,
                                            cColor(1, 0, 0, 1));

    apFunctions->GetLowLevelGfx()->DrawLine(vWorldInteractPos, vWorldInteractPos + mpCurrentJoint->GetPinDir(),
                                            cColor(0, 0, 0.3f, 1));
    apFunctions->GetLowLevelGfx()->DrawLine(
        vWorldInteractPos, vWorldInteractPos + mpCurrentJoint->GetPinDir() * fSpeedAdd * 50, cColor(0, 0, 1, 1));
...
kBeginSerialize(cLuxPlayerState_InteractSlide_SaveData, iLuxPlayerState_Interact_SaveData) kEndSerialize()
