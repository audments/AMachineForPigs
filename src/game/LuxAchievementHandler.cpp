...
iLuxAchievementHandler::iLuxAchievementHandler() : iLuxUpdateable("AchievementHandler") { mbRegistered = false; }
...
void iLuxAchievementHandler::Reset() { m_setDebugActivatedAchievements.clear(); }

//-----------------------------------------------------------------------

void iLuxAchievementHandler::RegisterAchievements() { mbRegistered = true; }
...
void iLuxAchievementHandler::OnUnlockedAchievement(int alID) {
    if (GetAchievementUnlocked(alID) == false) {
        gpBase->mpDebugHandler->AddMessage(
            _W("Activated Achievement: ") + cString::To16Char(m_mapAchievements.find(alID)->second), false);
