...
void cLuxHandObject_Melee::ImplementedReset() { mpCollideShape = NULL; }
...
void cLuxHandObject_Melee::UpdateCheckDamageEvent(float afTimeStep) {
    if (mpHands->CheckAnimationEvent(mfRelativeDamageTime)) {
        ...
        bool bHit = gpBase->mpMapHelper->ShapeDamage(
            mpCollideShape, mtxHands, pCam->GetPosition(), mfAttackMinDamage * fDamageMul,
            mfAttackMaxDamage * fDamageMul, mfAttackForce, mfMaxAttackForce, mlAttackStrength, mfAttackHitSpeed,
            eLuxDamageType_BloodSplat, eLuxWeaponHitType_Sword, true, false, true, false, NULL);
