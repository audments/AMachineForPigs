...
cLuxPreMenuSection::~cLuxPreMenuSection() { STLDeleteAll(mlstTextElements); }
...
bool cLuxPreMenuSection::HasTextElements() { return mlstTextElements.empty() == false; }
...
cLuxPreMenu::cLuxPreMenu() : iLuxUpdateable("LuxPreMenu") {
    ...
    // Create gamma widgets
    {
        ...
        // Value label
        mpLGamma = mpGuiSet->CreateWidgetLabel(vPos, cVector2f(300, 25), kTranslate("OptionsMenu", "GammaInstructions"),
                                               mpGGamma);

        mpLGamma->SetVisible(false);
        mpLGamma->SetEnabled(false);

        mpLHelpGamma = mpGuiSet->CreateWidgetLabel(vPos, cVector2f(300, 60),
                                                   kTranslate("OptionsMenu", "GammaInstructions"), mpGGamma);
...
void cLuxPreMenu::UpdateState() {
            ...
            // New Sections
                ...
                if (mpCurrentSection) {
                    ...
                    if (mpCurrentSection->msMusic != "") {
                        ...
                        float fFadeSpeed =
                            mpCurrentSection->mfMusicFadeTime == 0 ? 100.0f : 1.0f / mpCurrentSection->mfMusicFadeTime;
                        pMusHandler->Play(mpCurrentSection->msMusic, mpCurrentSection->mfMusicVolume, fFadeSpeed, true,
                                          false);
                    }
                }
            }
    ...
    case eLuxPreMenuState_ShowPremenuSection: {
        if (mpCurrentSection->HasTextElements() == false && mpCurrentSection->HasGammaSettings() == false &&
            mfTimer <= 0) {
            mCurrentState = eLuxPreMenuState_FadeOut;
        }
    } break;
