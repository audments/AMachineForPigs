...
void cLuxMoveState_Normal_Callback::OnGravityCollide(iCharacterBody *apCharBody, iPhysicsBody *apBody,
...
cLuxMoveState_Normal::cLuxMoveState_Normal(cLuxPlayer *apPlayer) : iLuxMoveState(apPlayer) {
    ...
    mfClimbLedgeCheckDistForward =
        gpBase->mpGameCfg->GetFloat("Player_Movement_ClimbLedge", "ClimbLedgeCheckDistForward", 0);
    mfClimbLedgeCheckDistFromTop =
        gpBase->mpGameCfg->GetFloat("Player_Movement_ClimbLedge", "ClimbLedgeCheckDistFromTop", 0);
    mfClimbLedgeCheckDistTopToEnd =
        gpBase->mpGameCfg->GetFloat("Player_Movement_ClimbLedge", "ClimbLedgeCheckDistTopToEnd", 0);
...
cLuxMoveState_Normal::~cLuxMoveState_Normal() { hplDelete(mpCallback); }
...
bool cLuxMoveState_Normal::GetAllowsInteraction() { return true; }

//-----------------------------------------------------------------------

bool cLuxMoveState_Normal::OnMove(eCharDir aDir, float afMul) { return true; }
bool cLuxMoveState_Normal::OnAddYaw(float afAmount) { return true; }
bool cLuxMoveState_Normal::OnAddPitch(float afAmount) { return true; }
...
void cLuxMoveState_Normal::OnDraw(float afFrameTime) {
    ...
    gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 0), 12, cColor(1, 1),
                                     _W("Bob: %ls (%f). Bounce: %f\n"), cString::To16Char(mvHeadBob.ToString()).c_str(),
                                     cMath::ToDeg(cMath::Wrap(mfHeadBobCount, 0, k2Pif)), mfHeadGroundBounce);
}
...
void cLuxMoveState_Normal::SetCrouch(bool abActive) {
    ...
    // Stand up
        ...
        cVector3f vPosTestAdd[5] = {cVector3f(0, 0.001f, 0), cVector3f(0.01f, 0.001f, 0), cVector3f(-0.01f, 0.001f, 0),
        ...
        if (bFits) {
            ...
            mbCrouching = false;
            pCharBody->SetPosition(pCharBody->GetPosition() + vPosTestAdd[lCurrentFitTest],
                                   true); // Make sure to move the player to the fitting positon!
...
void cLuxMoveState_Normal::ResetJumping() { mbJumping = false; }

//-----------------------------------------------------------------------

float cLuxMoveState_Normal::GetMoveSpeedMul() {
    float fMoveMul = mpPlayer->GetInteractionMoveSpeedMul() * mpPlayer->GetScriptMoveSpeedMul() *
                     mpPlayer->GetEventMoveSpeedMul() * mpPlayer->GetHurtMoveSpeedMul() *
                     mpPlayer->GetInfectionCollapseSpeedMul() * mpPlayer->GetStaminaSpeedMul() *
...
void cLuxMoveState_Normal::UpdateJumpAndGroundCheck(float afTimeStep) {
    ...
    if (mbJumping) {
        ...
        cVector3f vGravity =
            pCharBody->GetCustomGravityActive() ? pCharBody->GetCustomGravity() : pPhysicsWorld->GetGravity();
...
void cLuxMoveState_Normal::UpdateHeadBob(float afTimeStep) {
    ...
    if (mbHeadGroundBounceActive) {
        ...
        if (mfHeadGroundBounce < 0.5f)
            ...
            mvHeadBob.y +=
                (cMath::SmoothCurve((mfHeadGroundBounce - 0.5f) * 2.0f) - 1) * mfGroundBounceSize * mfBounceSizeMul;
...
void cLuxMoveState_Normal::FootSound(eLuxFootSound aType) {
    ...
    gpBase->mpHelpFuncs->PlayGuiSoundData(sSoundDataName, eSoundEntryType_World, 1, eSoundEntityType_Main, true,
                                          &extraData);

    cLuxMap *pMap = gpBase->mpMapHandler->GetCurrentMap();
    pMap->BroadcastEnemySoundMessage(mpPlayer->GetCharacterBody()->GetFeetPosition() + cVector3f(0, 0.1f, 0),
                                     extraData.mfAIVolume, extraData.mfMinDistance, extraData.mfMaxDistance,
                                     sSoundDataName);
}
...
void cLuxMoveState_Normal::FallDamage(float afYSpeed) {
    ...
    if (fDamage > 0) {
        ...
        mpPlayer->GiveDamage(
            fDamage, 100, eLuxDamageType_BloodSplat, false,
            false); // no fall damage in pigs; no time/budget for the nice solution. Still want sound though.
