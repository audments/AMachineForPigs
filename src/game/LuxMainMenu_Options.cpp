...
cLuxMainMenu_Options::~cLuxMainMenu_Options() { STLDeleteAll(mvOptionData); }
...
void cLuxMainMenu_Options::CreateGui() { CreateMainGui(); }
...
void cLuxMainMenu_Options::CreateMainGui() {
    ...
    //////////////////////////
    // Window
    mpWindow = mpGuiSet->CreateWidgetWindow(eWidgetWindowButtonFlag_None, cVector3f(0, 0, 5), mvWindowSize,
                                            kTranslate("MainMenu", "Options"));
    ...
    mpBCancel =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(fButtonWidth, 30), kTranslate("MainMenu", "Cancel"), mpWindow);
    ...
    mpShoulderHint[0] =
        mpGuiSet->CreateWidgetImage("gamepad_lb.tga", vPos, vImageSize, eGuiMaterial_Alpha, false, mpWindow, "LB Tip");

    vPos.x = mpWindow->GetSize().x - fUpperBorderSize * 5.0f;

    mpShoulderHint[1] =
        mpGuiSet->CreateWidgetImage("gamepad_rb.tga", vPos, vImageSize, eGuiMaterial_Alpha, false, mpWindow, "RB Tip");
#endif
...
void cLuxMainMenu_Options::AddGameOptions(cWidgetTab *apTab) {
    ...
    mpCBLanguage = mpGuiSet->CreateWidgetComboBox(vPos + cVector3f(pLabel->GetSize().x + 5, -2, 0), cVector2f(150, 25),
                                                  _W(""), apTab);
    ...
    mpCBFocusIconStyle = mpGuiSet->CreateWidgetComboBox(vPos + cVector3f(pLabel->GetSize().x + 5, -2, 0),
                                                        cVector2f(150, 25), _W(""), apTab);
...
void cLuxMainMenu_Options::AddGraphicsOptions(cWidgetTab *apTab) {
    ...
    for (int i = 0; i < (int)vToggleButtonStrings.size(); ++i) {
        float fStringLength =
            mpBToggleShowGfxOptions->GetDefaultFontType()->GetLength(mpBToggleShowGfxOptions->GetDefaultFontSize(),
...
void cLuxMainMenu_Options::AddBasicGfxOptions(cWidgetDummy *apDummy) {
    ...
    cWidgetGroup *pGroup = mpGuiSet->CreateWidgetGroup(vPos, 0, kTranslate("OptionsMenu", "Screen"), apDummy);
    {
        ...
        mpCBResolution = mpGuiSet->CreateWidgetComboBox(
            pLabel->GetLocalPosition() + cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(175, 25), _W(""), pGroup);
        ...
        mpChBFullScreen = mpGuiSet->CreateWidgetCheckBox(vPosInGroup + cVector3f(0, 2, 0), -1,
                                                         kTranslate("OptionsMenu", "FullScreen"), pGroup);
        SetUpInput(NULL, mpChBFullScreen, true, kTranslate("OptionsMenu", "FullScreenTip"));

        mpChBVSync = mpGuiSet->CreateWidgetCheckBox(vPosInGroup + cVector3f(0, mpChBFullScreen->GetSize().y + 10, 0), 0,
                                                    kTranslate("OptionsMenu", "VSync"), pGroup);
        SetUpInput(NULL, mpChBVSync, false, kTranslate("OptionsMenu", "VSyncTip"));

        mpChBAdaptiveVSync = mpGuiSet->CreateWidgetCheckBox(
            vPosInGroup + cVector3f(mpChBVSync->GetSize().x + 10, mpChBFullScreen->GetSize().y + 10, 0), 0,
            kTranslate("OptionsMenu", "AdaptiveVSync"), pGroup);
    ...
    mpCBTextureSizeLevel = mpGuiSet->CreateWidgetComboBox(
        pLabel->GetLocalPosition() + cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 25), _W(""), apDummy);
    ...
    pLabel = mpGuiSet->CreateWidgetLabel(vPos, -1, kTranslate("OptionsMenu", "Gamma"), apDummy);
    {
        ...
        cWidgetImage *pImg =
            mpGuiSet->CreateWidgetImage("menu_gamma.tga", vLabelPos, -1, eGuiMaterial_Alpha, false, pLabel);
        SetUpInput(NULL, pImg, false, kTranslate("OptionsMenu", "GammaInstructions"));
        vLabelPos.y += pImg->GetSize().y + 5.0f;

        mpSGamma = mpGuiSet->CreateWidgetSlider(eWidgetSliderOrientation_Horizontal, vLabelPos,
                                                cVector2f(pImg->GetSize().x, 20), 0, pLabel);
        ...
        // cWidgetLabel *pLInstr = mpGuiSet->CreateWidgetLabel(vLabelPos, cVector2f(pImg->GetSize().x,27),
        // kTranslate("OptionsMenu","GammaInstructions"), pLabel); pLInstr->SetDrawBackGround(true);
...
void cLuxMainMenu_Options::AddAdvancedGfxOptions(cWidgetDummy *apDummy) {
    ...
    cWidgetFrame *pMainFrame =
        mpGuiSet->CreateWidgetFrame(cVector3f(0, 0, 1), cVector2f(550, 275), false, apDummy, false, true);
    ...
    // Texture
    cWidgetGroup *pGroup =
        mpGuiSet->CreateWidgetGroup(vPos, vGroupSize, kTranslate("OptionsMenu", "Material"), pMainFrame);
    {
        float fBorderSize = 15;
        cVector3f vPosInGroup = cVector3f(fBorderSize, fBorderSize, 0.1f);
        ...
        mpCBTextureFilter = mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(150, 25),
                                                           _W(""), pLabel);
        ...
        mpCBAnisotropy = mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 25),
                                                        _W(""), pLabel);
        ...
        mpCBParallaxQuality = mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0),
                                                             cVector2f(100, 25), _W(""), pLabel);
    ...
    pGroup = mpGuiSet->CreateWidgetGroup(vPos, vGroupSize, kTranslate("OptionsMenu", "Shadows"), pMainFrame);
    {
        ...
        mpCBShadowQuality = mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 25),
                                                           _W(""), pLabel);
        ...
        mpCBShadowRes = mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 25),
                                                       _W(""), pLabel);
        ...
        mpCBParallaxQuality->AddItem(
            kTranslate("Launcher", "On")); // Skipping medium since high and medium is really the same!
    }
    ...
    pGroup = mpGuiSet->CreateWidgetGroup(vPos, vGroupSize, kTranslate("OptionsMenu", "PostEffects"), pMainFrame);
    {
        ...
        // ImageTrail
        mpChBImageTrail =
            mpGuiSet->CreateWidgetCheckBox(vPosInGroup, 0, kTranslate("OptionsMenu", "ImageTrail"), pGroup);
        ...
        // RadialBlur
        mpChBRadialBlur =
            mpGuiSet->CreateWidgetCheckBox(vPosInGroup, 0, kTranslate("OptionsMenu", "RadialBlur"), pGroup);
        ...
        // Insanity
        mpChBColorGrading =
            mpGuiSet->CreateWidgetCheckBox(vPosInGroup, 0, kTranslate("OptionsMenu", "ColorGrading"), pGroup);
    ...
    pGroup = mpGuiSet->CreateWidgetGroup(vPos, vGroupSize, kTranslate("OptionsMenu", "SSAO"), pMainFrame);
    {
        ...
        mpCBSSAOSamples =
            mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(60, 25), _W(""), pLabel);
        ...
        mpCBSSAOResolution =
            mpGuiSet->CreateWidgetComboBox(cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(80, 25), _W(""), pLabel);
    ...
    pGroup = mpGuiSet->CreateWidgetGroup(vPos, vGroupSize, kTranslate("KeyConfig", "Misc"), pMainFrame);
    {
        ...
        // Enabled
        mpChEdgeSmooth =
            mpGuiSet->CreateWidgetCheckBox(vPosInGroup, 0, kTranslate("OptionsMenu", "FullscreenSmooth"), pGroup);
        SetUpInput(NULL, mpChEdgeSmooth, true, kTranslate("OptionsMenu", "EdgeSmoothTip"));

        vPosInGroup.y += mpChEdgeSmooth->GetSize().y + 10;

        mpChBRefraction =
            mpGuiSet->CreateWidgetCheckBox(vPosInGroup, 0, kTranslate("OptionsMenu", "Refraction"), pGroup);
        ...
        mpChBWorldReflection = mpGuiSet->CreateWidgetCheckBox(vPosInGroup, 0,
                                                              kTranslate("OptionsMenu", "WorldReflection") + _W(" (") +
                                                                  kTranslate("OptionsMenu", "Water") + _W(")"),
                                                              pGroup);
...
void cLuxMainMenu_Options::AddInputOptions(cWidgetTab *apTab) {
    ...
    mpSMouseSensitivity = mpGuiSet->CreateWidgetSlider(
        eWidgetSliderOrientation_Horizontal, cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 20), 0, pLabel);
    ...
    SetUpSlider(mpSMouseSensitivity, mfMouseSensitivityMin, mfMouseSensitivityMax, mfMouseSensitivityStep,
                kGuiCallback(MouseSensitivitySlider_OnMove), &mpLMouseSensitivity);

    // mpLMouseSensitivity = mpGuiSet->CreateWidgetLabel(cVector3f(mpSMouseSensitivity->GetSize().x*0.5f,2,1), -1,
    // _W(""), mpSMouseSensitivity); mpLMouseSensitivity->SetTextAlign(eFontAlign_Center);
    ...
    /////////////////////////////
    // Invert Gamepad look
    mpChBGamepadInvertLook =
        mpGuiSet->CreateWidgetCheckBox(vPos, 0, kTranslate("OptionsMenu", "InvertGamepadLook"), apTab);
    ...
    mpSGamepadLookSensitivity = mpGuiSet->CreateWidgetSlider(
        eWidgetSliderOrientation_Horizontal, cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 20), 0, pLabel);
    // mpSGamepadLookSensitivity->AddCallback(eGuiMessage_SliderMove, this,
    // kGuiCallback(MouseSensitivitySlider_OnMove));
    SetUpInput(pLabel, mpSMouseSensitivity, false, kTranslate("OptionsMenu", "GamepadLookSensitivityTip"));
    SetUpSlider(mpSGamepadLookSensitivity, mfGamepadLookSensitivityMin, mfGamepadLookSensitivityMax,
                mfGamepadLookSensitivityStep, kGuiCallback(GamepadLookSensitivitySlider_OnMove),
                &mpLGamepadLookSensitivity);
    ...
    // Key Config Button
    mpBKeyConfig =
        mpGuiSet->CreateWidgetButton(vPos, cVector2f(0, 25), kTranslate("OptionsMenu", "KeyConfigButton"), apTab);
    float fButtonWidth = mpBKeyConfig->GetDefaultFontType()->GetLength(mpBKeyConfig->GetDefaultFontSize(),
                                                                       mpBKeyConfig->GetText().c_str());
...
void cLuxMainMenu_Options::AddSoundOptions(cWidgetTab *apTab) {
    ...
    mpSVolume =
        mpGuiSet->CreateWidgetSlider(eWidgetSliderOrientation_Horizontal,
                                     vPos + cVector3f(0, pLabel->GetSize().y + 5, 0), cVector2f(100, 20), 0, apTab);
...
void cLuxMainMenu_Options::SetInputValues(cResourceVarsObject &aObj) {
    ...
    // Graphics options
    {
        ...
        // Resolution
        {
            ...
            cVideoMode vCurrentRes =
                cVideoMode(aObj.GetVarInt("Display"), cVector2l((int)vCurrentResf.x, (int)vCurrentResf.y), -1, -1);
            ...
            for (size_t i = 0; i < vVidModes.size(); ++i) {
                ...
                if (mode.isFullScreenDesktop()) {
                    sRes = kTranslate("OptionsMenu", "FullScreenDesktop") + _W(" ") +
                           cPlatform::GetDisplayName(mode.mlDisplay);
                } else {
        ...
        // Texture quality and filtering
        {
            /////////////////////////////////
            // Texture Quality
            mpCBTextureSizeLevel->SetSelectedItem(
                (mpCBTextureSizeLevel->GetItemNum() - 1) - aObj.GetVarInt("TextureQuality"), true, false);
    ...
    SetSliderValue(mpSGamepadLookSensitivity, fSensitivity, false, mfGamepadLookSensitivityMin,
                   mfGamepadLookSensitivityMax);
...
void cLuxMainMenu_Options::SetUpInput(cWidgetLabel *apLabel, iWidget *apInput, bool abNeedsRestart,
...
void cLuxMainMenu_Options::PopulateLanguageList() {
    ...
    for (; it != lstLangs.end(); ++it) {
        ...
        tWString sLangEntry =
            cString::ToLowerCaseW(cString::SubW(sLang, 0, cString::GetLastStringPosW(sLang, _W("."))));
...
void cLuxMainMenu_Options::SetUpSlider(cWidgetSlider *apSlider, float afMinValue, float afMaxValue, float afStepValue,
                                       tGuiCallbackFunc apCallback, cWidgetLabel **apValueDisplay) {
    ...
    if (apValueDisplay) {
        cWidgetLabel *pLabel =
            mpGuiSet->CreateWidgetLabel(cVector3f(apSlider->GetSize().x * 0.5f, 2, 1), -1, _W(""), apSlider);
...
void cLuxMainMenu_Options::SetSliderValue(cWidgetSlider *apSlider, float afValue, bool abGenCallback, float afMinValue,
                                          float afMaxValue) {
...
void cLuxMainMenu_Options::SetSliderLabelString(cWidgetLabel *apLabel, float afValue, float afMinValue,
                                                float afMaxValue, const tWString &asMin, const tWString &asMax) {
    tWString sText;
...
void cLuxMainMenu_Options::DumpCurrentValues(cResourceVarsObject &aObj) {
    ...
    // Graphics options
    {
        ...
        /////////////////////////
        // Texture quality and filtering
        aObj.AddVarInt("TextureQuality",
                       (mpCBTextureSizeLevel->GetItemNum() - 1) - mpCBTextureSizeLevel->GetSelectedItem());
...
bool cLuxMainMenu_Options::Window_OnUpdate(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (mbTipWidgetUpdated) {
        ...
        if (mpCurrentTipWidget && mpCurrentTipWidget->GetUserData()) {
            ...
            tWString sRestart =
                pData->mbNeedsRestart ? _W(" (") + kTranslate("OptionsMenu", "ReqRestart") + _W(")") : _W("");
