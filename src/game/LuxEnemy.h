...
class cLuxEnemyPathfinder_SaveData : public iSerializable {
    kSerializableClassInit(cLuxEnemyPathfinder_SaveData) public
...
enum eLuxEnemyPoseType { eLuxEnemyPoseType_Biped = 0, eLuxEnemyPoseType_Quadruped, eLuxEnemyPoseType_LastEnum };

//----------------------------------------------

enum eLuxEnemyMoveType { eLuxEnemyMoveType_Normal, eLuxEnemyMoveType_Flee, eLuxEnemyMoveType_LastEnum };
...
class cEnemyAttackDamageData {
  public:
    cEnemyAttackDamageData()
        : mfMinDamage(0), mfMaxDamage(0), mfForce(0), mfMaxImpulse(0), mlStrength(0), mfHitSpeed(3),
          mDamageType(eLuxDamageType_BloodSplat), mWeaponHitType(eLuxWeaponHitType_Sword), mbCheckPlayer(true),
          mbCheckProps(true), msHitSound("") {}
...
class iLuxEnemy : public iLuxEntity {
  ...
  public:
    ...
    void PlayAnim(const tString &asName, bool abLoop, float afFadeTime, bool abDependsOnSpeed = false,
                  float afSpeedMul = 1.0f, bool abSyncWithPrevFrame = false, bool abOverideMoveState = true,
                  bool abUseMoveAnimWhenCurrentIsOver = true, bool abCanBlend = true, bool abPlayTransition = true);
  ...
  protected:
    //////////////////////////////
    // Update and related
    void AddTransitionAnimation(const tString &asMainAnim, const tString &asTransAnim, const tString &asPrevAnim,
                                float afMinTime = -1, float afMaxTime = -1);
