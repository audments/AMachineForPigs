    ...
    if (apEnemy->GetPathFinder()) {
        for (size_t i = 0; i < mvPatrolNodes.Size(); ++i) {
            ...
            apEnemy->AddPatrolNode(pNode, mvPatrolNodes[i].mfWaitTime, mvPatrolNodes[i].msAnimation,
                                   mvPatrolNodes[i].mbLoopAnimation);
        }
    }
}
...
cLuxSavedGameMap::~cLuxSavedGameMap() { DestroyAll(); }
    ...
    // Particle systems
    {
        ...
        while (psIt.HasNext()) {
            ...
            if (pPS->IsSaved() && pPS->GetEntityParent() == NULL && pPS->GetParent() == NULL &&
                (pPS->IsDying() == false ||
                 pPS->IsActive() == false &&
                     pPS->IsDead() == false)) // Save PS that are either looping or that have not been activated yet
            {
...
template <class T> static void ListToSet(cContainerList<T> &aList, std::set<T> &aSet) {
...
template <class T> static bool ExistsInSet(T aVar, std::set<T> &aSet) {
...
void cLuxSavedGameMap::ToMap(cLuxMap *apMap) {
    ...
    if (msSkyboxTexture == "") {
        ...
        iTexture *pTexture =
            gpBase->mpEngine->GetResources()->GetTextureManager()->CreateCubeMap(msSkyboxTexture, true);
    ...
    // Sounds
    {
        ...
        while (saveSoundIt.HasNext()) {
            ...
            if (saveSound.mlID >= 0)
                ...
                pSound =
                    pWorld->CreateSoundEntity(saveSound.msName, saveSound.msSoundDataName, saveSound.mbRemoveWhenOver);
    ...
    // Entities setting
    {
        std::set<int> setDisabledEntities, setLockedDoors, setLockedLevelDoors, setActiveItems,
...
kBeginSerializeBase(cLuxSavedGameEntity)
    ...
    kSerializeVar(mlID, eSerializeType_Int32)
        ...
        kSerializeClassContainer(mvAttachedProps, cLuxProp_AttachedProp, eSerializeType_Class)
            kSerializeClassContainer(mvInteractConnections, iLuxInteractConnection_SaveData,
            ...
            kSerializeClassContainer(mvPatrolNodes, cLuxEnemyPatrolNode_SaveData, eSerializeType_Class)
                ...
                kEndSerialize()
                    ...
                    kBeginSerializeBase(cLuxSavedGameEntityFunc)
                        ...
                        kSerializeVar(mlID, eSerializeType_Int32) kSerializeVar(msFunc, eSerializeType_String)
    ...
    kBeginSerializeBase(cLuxSavedGameMap)
        ...
        kSerializeVar(msName, eSerializeType_String) kSerializeVar(msDisplayNameEntry, eSerializeType_String)

            kSerializeVar(mlNumberOfQuests, eSerializeType_Int32) kSerializeVar(
                mlTotalCompletionAmount, eSerializeType_Int32) kSerializeVar(mlCurrentCompletionAmount,
                                                                             eSerializeType_Int32)

                kSerializeVar(mbSkyBoxActive, eSerializeType_Bool) kSerializeVar(
                    msSkyboxTexture, eSerializeType_String) kSerializeVar(mSkyBoxColor, eSerializeType_Color)

                    kSerializeVar(mbFogActive, eSerializeType_Bool) kSerializeVar(
                        mbFogCulling, eSerializeType_Bool) kSerializeVar(mfFogStart, eSerializeType_Float32)
                        kSerializeVar(mfFogEnd, eSerializeType_Float32) kSerializeVar(
                            mfFogFalloffExp, eSerializeType_Float32) kSerializeVar(mFogColor, eSerializeType_Color)

                            kSerializeClassContainer(mlstFullEntities, iLuxEntity_SaveData, eSerializeType_ClassPointer)

                                kSerializeClassContainer(
                                    mlstCollideAndConnectEntities, cLuxSavedGameEntity,
                                    eSerializeType_ClassPointer) kSerializeClassContainer(mlstEnemies,
                                                                                          cLuxSavedGameEnemy,
                                                                                          eSerializeType_ClassPointer)
                                    ...
                                    kSerializeClassContainer(mlstTimers, cLuxEventTimer, eSerializeType_Class)
                                        ...
                                        kSerializeClassContainer(
                                            mlstVars, cLuxScriptVar,
                                            eSerializeType_Class) kSerializeClassContainer(mvUseItemCallbacks,
                                                                                           cLuxUseItemCallback,
                                                                                           eSerializeType_Class)

                                            kSerializeClassContainer(mlstPlayerCollideCallbacks,
                                                                     cLuxCollideCallback_SaveData, eSerializeType_Class)

                                                kSerializeClassContainer(
                                                    mlstLights, cEngineLight_SaveData,
                                                    eSerializeType_Class) kSerializeClassContainer(mlstSounds,
                                                                                                   cEngineSound_SaveData,
                                                                                                   eSerializeType_Class)
                                                    kSerializeClassContainer(mlstPS, cEnginePS_SaveData,
                                                                             eSerializeType_Class)

                                                        kSerializeClassContainer(mlstEntityFunc_Look,
                                                                                 cLuxSavedGameEntityFunc,
                                                                                 eSerializeType_Class)
                                                            kSerializeClassContainer(mlstEntityFunc_Interact,
                                                                                     cLuxSavedGameEntityFunc,
                                                                                     eSerializeType_Class)
                                                                kSerializeClassContainer(mlstEntityFunc_Callback,
                                                                                         cLuxSavedGameEntityFunc,
                                                                                         eSerializeType_Class)
                                                                    kSerializeClassContainer(mlstEntityFunc_StateChange,
                                                                                             cLuxSavedGameEntityFunc,
                                                                                             eSerializeType_Class)

                                                                        kSerializeVarContainer(mlstDisabledEntities,
                                                                                               eSerializeType_Int32)
                                                                            kSerializeVarContainer(mlstLockedDoors,
                                                                                                   eSerializeType_Int32)
                                                                                kSerializeVarContainer(
                                                                                    mlstLockedLevelDoors,
                                                                                    eSerializeType_Int32)
                                                                                    kSerializeVarContainer(
                                                                                        mlstActiveItems,
                                                                                        eSerializeType_Int32)
                                                                                        kSerializeVarContainer(
                                                                                            ...
                                                                                            eSerializeType_Int32)
                                                                                            kSerializeVarContainer(
                                                                                                mlstUnbrokenItemContainers,
                                                                                                eSerializeType_Int32)
                                                                                                kSerializeVarContainer(
                                                                                                    mlstBrokenContainersWithActiveItem,
                                                                                                    eSerializeType_Int32)
                                                                                                    kSerializeVarContainer(
                                                                                                        mlstUnlitLamps,
                                                                                                        eSerializeType_Int32)
