...
bool Sort_Complexity(const tLightComplexity &aA, const tLightComplexity &aB) { return aA.first > aB.first; }
...
void cLuxDebugHandler::OnPostRender(float afFrameTime) {
    ...
    if (cRendererDeferred::GetDebugRenderLightComplexity()) {
        ...
        cRendererDeferred *pRenderer =
            static_cast<cRendererDeferred *>(gpBase->mpMapHandler->GetViewport()->GetRenderer());
...
static void PrintContainerNode(iRenderableContainerNode *apNode, int alLevel) {
    ...
    Log("%sNode %d Radius: %f Min: (%s) Max: (%s) Objects: %d\n", GetSpaces(alLevel), apNode, apNode->GetRadius(),
        apNode->GetMin().ToString().c_str(), apNode->GetMax().ToString().c_str(), apNode->GetObjectNum());
...
void cLuxDebugHandler::Update(float afTimeStep) {
    ...
    if (mbFirstUpdateOnMap) // && mlTempCount>0)
    {
        ...
        if (pMap) {
            ...
            // Log("Root objects: %d Root has childs: %d\n",pContainer->GetRoot()->GetObjectNum(),
            // pContainer->GetRoot()->HasChildNodes()?1:0);
        }
    }
    mlTempCount++;
...
void cLuxDebugHandler::OnDraw(float afFrameTime) {
    ...
    if (mbShowFPS) {
        ...
        gpBase->mpGameDebugSet->DrawFont(
            gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
            _W("GameLogic: %05.2fms (Iterations: %03.1f / Frame) RenderingLogic: %05.2fms\n"), fGameLogic, fIterations,
            fRenderingLogic);
        fY += 13.0f;

        if (fGameLogic + fRenderingLogic > 16.6667f) {
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                                             _W("GameLogic + RenderingLogic: %03.1fms > 16.7 ms, the game requires "
                                                "multiple updates per frame and will lag\n"),
    ...
    if (cRendererDeferred::GetDebugRenderLightComplexity()) {
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 0), 14, cColor(1, 1),
                                         _W("Lights Visible: %d"), mvLightComplexity.size());
        fY += 12;

        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 0), 14, cColor(1, 1),
                                         _W("Expensive Light(s):"));
        fY += 12;
        ...
        for (int i = 0; i < mvLightComplexity.size(); ++i) {
            ...
            cColor vColor =
                cColor(0, 1, 0, 1) + (cColor(1, 0, 0, 1) - cColor(0, 1, 0, 1)) * cMath::Min(1.0f, fComplexity * 1.5f);
            vColor = vColor * (1.0f / cMath::Max(cMath::Max(vColor.r, vColor.g), vColor.b));

            ///////////
            // Draw the name with black outline
            gpBase->mpGameDebugSet->DrawFont(
                gpBase->mpDefaultFont, cVector3f(5 + 1, fY + 1, -1), 14, cColor(0, 1), _W("- %ls %g"),
                cString::To16Char(mvLightComplexity[i].second->GetName()).c_str(), fComplexity);
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 0), 14, vColor, _W("- %ls %g"),
                                             cString::To16Char(mvLightComplexity[i].second->GetName()).c_str(),
                                             fComplexity);
    ...
    if (mbShowDebugMessages) {
        ...
        for (tLuxDebugMessageListIt it = mlstMessages.begin(); it != mlstMessages.end(); ++it) {
            ...
            gpBase->mpGameDebugSet->DrawFont(debugMessage.msText, gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14,
                                             cColor(1, 1));
            fY -= 17;
        }
    }
    ...
    if (mbShowPlayerInfo) {
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                                         ...
                                         pCharBody->IsOnGround(), pCharBody->GetMovedLastUpdate(), pCharBody->GetMass(),
                                         pCharBody->GetActiveSize());
        fY += 15.0f;
        gpBase->mpGameDebugSet->DrawFont(
            gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
            _W("MoveSpeed: %f, %f (%f) Mul: %f AvgSpeed: %f\n"), pCharBody->GetMoveSpeed(eCharDir_Forward),
            pCharBody->GetMoveSpeed(eCharDir_Right), pCharBody->GetVelocity(gpBase->mpEngine->GetStepSize()).Length(),
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                                         _W("Infection: %.2f InfectionLevel: %d Tinderboxes: %d"),
                                         pPlayer->GetInfection(), pPlayer->GetInfectionLevel(),
                                         pPlayer->GetTinderboxes());
        fY += 15.0f;

        // gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5,fY,0),12,cColor(1,1),
        //					_W("FlashbackCount: %f Pulse: %f"), pPlayer->GetFlashbackCount() ,
        //pPlayer->GetFlashbackPulseCount()); fY+=13.0f;
        ...
        if (pAnim) {
            gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                                             _W("HandAnim: '%ls' Time: %f/%f\n"), cString::To16Char(sHandAnim).c_str(),
                                             pAnim->GetTimePosition(), pAnim->GetRelativeTimePosition());
            fY += 15.0f;
        }

        eLuxMoveState moveState = pPlayer->GetCurrentMoveState();
        if (moveState == eLuxMoveState_Normal) {
            cLuxMoveState_Normal *pNormalMoveState =
                static_cast<cLuxMoveState_Normal *>(pPlayer->GetMoveStateData(moveState));
        ...
        // gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5,fY,10),14,cColor(1,1),
        //	_W("Sanity Between event time: %f AtLowSanityCount: %f\n"), gpBase->mpInsanityHandler->GetNewEventCount() ,
        //pPlayer->GetHelperSanity()->GetAtLowSanityCount()); fY+=15.0f;

        fY = pPlayer->GetStateData(pPlayer->GetCurrentState())
                 ->DrawDebug(gpBase->mpGameDebugSet, gpBase->mpDefaultFont, fY);
    }
    ...
    if (mbShowSoundPlaying) {
        ...
        if (pMusic) {
            ...
            gpBase->mpGameDebugSet->DrawFont(
                ...
                cString::To16Char(pChannel->GetData()->GetName()).c_str(), pChannel->GetVolume(), pChannel->IsPlaying(),
                pChannel->GetPriority(), pChannel->GetElapsedTime(), pChannel->GetTotalTime(),
        ...
        gpBase->mpGameDebugSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                                         _W("Num of sounds: %d"), vSoundNames.size());
        fY += 15.0f;
        ...
        for (int i = 0; i < (int)vSoundNames.size(); i++) {
            ...
            gpBase->mpGameDebugSet->DrawFont(
                gpBase->mpDefaultFont, cVector3f((float)lCol * 250, fY + (float)lRow * 15, 10), 14, cColor(1, 1),
                _W("%ls%ls%ls(%.2f)(%d) (%.2f/%.2f)"), cString::To16Char(vSoundNames[i]).c_str(),
                pChannel->GetData()->IsStream() ? _W("*st*") : _W(""),
                pChannel->IsBufferUnderrun() ? _W("BUFFER UNDERRUN!!") : _W(""), pChannel->GetVolume(),
                pChannel->GetPriority(), pChannel->GetElapsedTime(), pChannel->GetTotalTime());
    ...
    if (mbInspectionMode && mpInspectMeshEntity) {
        ...
        pSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("EntityName: '%ls'"),
                       cString::To16Char(mpInspectMeshEntity->GetName()).c_str());
        fY += 15;

        pSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1), _W("MaterialFile: '%ls'"),
                       pMaterial->GetFullPath().c_str());
        fY += 15;
        ...
        for (int i = 0; i < eMaterialTexture_LastEnum; ++i) {
            ...
            pSet->DrawFont(gpBase->mpDefaultFont, cVector3f(5, fY, 10), 14, cColor(1, 1),
                           _W(" %d size: %d x %d x %d mem: %.2fmb type: %ls file: '%ls'"), i, pTex->GetSize().x,
                           pTex->GetSize().y, pTex->GetSize().z, fMemSize,
...
void cLuxDebugHandler::RenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    if (mbDrawPhysics) {
        gpBase->mpMapHandler->GetCurrentMap()->GetPhysicsWorld()->RenderDebugGeometry(apFunctions->GetLowLevelGfx(),
                                                                                      cColor(0.5f));
    }
}
...
void cLuxDebugHandler::CheckLineObjectIntersection(iRenderable *apObject, const cVector3f &avStart,
                                                   const cVector3f &avEnd, cBoundingVolume *apBV) {
    ...
    if (cMath::CheckPointInBVIntersection(avStart, *pObjectBV) == false) {
        if (cMath::CheckAABBLineIntersection(pObjectBV->GetMin(), pObjectBV->GetMax(), avStart, avEnd, NULL, &fT) ==
            false)
            return;
        if (fT > gfMinT)
            return;
    ...
    bool bIntersect = cMath::CheckLineTriVertexBufferIntersection(avStart, avEnd, mtxInvModel,
                                                                  apObject->GetVertexBuffer(), NULL, &fT, NULL, true);
...
void cLuxDebugHandler::IterateRenderableNode(iRenderableContainerNode *apNode, const cVector3f &avStart,
                                             const cVector3f &avEnd, cBoundingVolume *apBV) {
    apNode->UpdateBeforeUse();
    ...
    if (apNode->GetParent() != NULL) {
        ...
        if (cMath::CheckPointInAABBIntersection(avStart, apNode->GetMin(), apNode->GetMax()) == false) {
            float fT = 0;
            if (cMath::CheckAABBLineIntersection(apNode->GetMin(), apNode->GetMax(), avStart, avEnd, NULL, &fT) ==
                false)
                return;
            if (fT > gfMinT)
                return;
...
void cLuxDebugHandler::CreateScriptOutputWindow() {
    ...
    ///////////////////////////
    // Frame
    mpScriptOutputFrame = mpGuiSet->CreateWidgetFrame(cVector3f(10, 30, 1), vSize - cVector2f(20, 80), true,
                                                      mpScriptOutputWindow, true, true);
...
void cLuxDebugHandler::CreateGuiWindow() {
    ...
    // Debug texts
    {
        ...
        cWidgetTextBox *pTextBox = mpGuiSet->CreateWidgetTextBox(vGroupPos, cVector2f(70, 25), _W(""), pGroup,
                                                                 eWidgetTextBoxInputType_Numeric, 0.1f, true);
        ...
        pTextBox = mpGuiSet->CreateWidgetTextBox(vGroupPos + cVector2f(75, 0), cVector2f(70, 25), _W(""), pGroup,
                                                 eWidgetTextBoxInputType_Numeric, 0.1f, true);
        pTextBox->SetDecimals(4);
        ...
        pTextBox = mpGuiSet->CreateWidgetTextBox(vGroupPos + cVector2f(150, 0), cVector2f(70, 25), _W(""), pGroup,
                                                 eWidgetTextBoxInputType_Numeric, 0.1f, true);
        pTextBox->SetDecimals(4);
        ...
        pTextBox = mpGuiSet->CreateWidgetTextBox(vGroupPos, cVector2f(70, 25), _W(""), pGroup,
                                                 eWidgetTextBoxInputType_Numeric, 5, true);
        pTextBox->SetDecimals(4);
...
void cLuxDebugHandler::DrawDynamicContainerDebugInfo() {
    iRenderableContainer *pDynContainer =
        gpBase->mpMapHandler->GetCurrentMap()->GetWorld()->GetRenderableContainer(eWorldContainerType_Dynamic);
...
void cLuxDebugHandler::OutputContainerContentsRec(iRenderableContainerNode *apNode, int alLevel) {
    ...
    // apNode->UpdateBeforeUse(); <- skip this as it might remove any bug otherwise (as it changes stuff from how it was
    // rendered)

    Log("%s-- Node %d AABB: (%s)-(%s)  ------\n", GetTab(alLevel).c_str(), apNode, apNode->GetMin().ToString().c_str(),
    ...
    if (apNode->HasObjects()) {
        ...
        for (; it != apNode->GetObjectList()->end(); ++it) {
            ...
            Log("%s %s (%s) AABB: (%s)-(%s)\n", GetTab(alLevel).c_str(), pObject->GetName().c_str(),
                pObject->GetEntityType().c_str(), pBV->GetMin().ToString().c_str(), pBV->GetMax().ToString().c_str());
        }
    }
...
bool cLuxDebugHandler::PressRebuildDynCont(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    cWorld *pWorld = gpBase->mpMapHandler->GetCurrentMap()->GetWorld();
    cRenderableContainer_DynBoxTree *pBoxTree =
        static_cast<cRenderableContainer_DynBoxTree *>(pWorld->GetRenderableContainer(eWorldContainerType_Dynamic));
...
bool cLuxDebugHandler::PressLoadWorld(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    cGuiPopUpFilePicker *pPicker = mpGuiSet->CreatePopUpLoadFilePicker(mvPickedFiles, false, msCurrentFilePath, false,
                                                                       this, kGuiCallback(LoadWorldFromFilePicker));
...
bool cLuxDebugHandler::PressBatchLoad(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    cGuiPopUpFilePicker *pPicker = mpGuiSet->CreatePopUpLoadFilePicker(mvPickedFiles, false, msCurrentFilePath, false,
                                                                       this, kGuiCallback(PressLoadBatchLoadFile));
