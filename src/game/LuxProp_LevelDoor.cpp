...
bool cLuxProp_LevelDoor::CanInteract(iPhysicsBody *apBody) { return true; }
...
tWString cLuxProp_LevelDoor::GetFocusText() {
    ...
    /*if(mbShowStats)
    {
        ...
        if(pSavedMap)
        {
            float fP = ((float)pSavedMap->mlCurrentCompletionAmount / (float)pSavedMap->mlTotalCompletionAmount)
    *100.0f; lPercent = (int)fP;
        }
...
void cLuxProp_LevelDoor::SetLocked(bool abLocked) { mbLocked = abLocked; }
...
kBeginSerialize(cLuxProp_LevelDoor_SaveData, iLuxProp_SaveData)
    ...
    kSerializeVar(msMapFile, eSerializeType_String) kSerializeVar(msStartPos, eSerializeType_String)
        kSerializeVar(msTextEntry, eSerializeType_String)

            kSerializeVar(mbLocked, eSerializeType_Bool) kSerializeVar(msLockedSound, eSerializeType_String)
...
void cLuxProp_LevelDoor::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
