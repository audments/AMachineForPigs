...
cLuxChestMessageCallback::cLuxChestMessageCallback(cLuxProp_Chest *apChest) { mpChest = apChest; }
...
cLuxProp_Chest::~cLuxProp_Chest() { hplDelete(mpMessageCallback); }
...
bool cLuxProp_Chest::CanInteract(iPhysicsBody *apBody) { return mbLocked; }
...
bool cLuxProp_Chest::OnInteract(iPhysicsBody *apBody, const cVector3f &avPos) {
    ...
    sText +=
        kTranslate("Game", "InteractChest03") + _W(" ") + cString::ToStringW(gpBase->mpPlayer->GetCoins()) + _W("\n");
...
void cLuxProp_Chest::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
