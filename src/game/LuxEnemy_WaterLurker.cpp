...
bool cLuxEnemy_WaterLurker::StateEventImplement(int alState, eLuxEnemyStateEvent aEvent, cLuxStateMessage *apMessage) {
    ...
    kLuxOnMessage(eLuxEnemyMessage_FoodInRange) cAINode *pNode =
        mpPathfinder->GetNodeAtPos(apMessage->mvCustomValue, 0, 1.5f, true, true, true, NULL);
    if (pNode) {
    ...
    kLuxState(eLuxEnemyState_Investigate)
        kLuxOnEnter gpBase->mpDebugHandler->AddMessage(_W("Sound Heard! Vol: ") + cString::ToStringW(mfTempVal), false);
...
bool cLuxEnemy_WaterLurker::PlayerIsDetected() { return AbsHeightDistToPlayer() <= mfPlayerDetectionHeight; }
...
kBeginSerialize(cLuxEnemy_WaterLurker_SaveData, iLuxEnemy_SaveData)
    ...
    kSerializeVar(mfPlayerDetectionHeight, eSerializeType_Float32) kSerializeVar(mfRunSpeedMul, eSerializeType_Float32)
...
void cLuxEnemy_WaterLurker::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
