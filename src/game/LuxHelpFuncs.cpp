...
cLuxModelCache::~cLuxModelCache() { Destroy(); }
...
void cLuxHelpFuncs::LoadFonts() { mpFontDefault = LoadFont("game_default.fnt"); }
...
bool cLuxHelpFuncs::PlayGuiSoundData(const tString &asName, eSoundEntryType aDestType, float afVolMul,
    ...
    cSoundEntry *pSound =
        pSoundHandler->PlayGui(sSoundName, false, pSoundData->GetVolume() * afVolMul, cVector3f(0, 0, 1), aDestType);
