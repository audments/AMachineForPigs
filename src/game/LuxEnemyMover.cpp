...
void cLuxEnemyMover::TurnAwayFromPos(const cVector3f &avFeetPos) {
    ...
    float fGoalAngle =
        -cMath::GetAngleFromPoints2D(cVector2f(vStartPos.x, vStartPos.z), cVector2f(avFeetPos.x, avFeetPos.z));
...
void cLuxEnemyMover::OnRenderSolid(cRendererCallbackFunctions *apFunctions) {
    ...
    if (bMoving && mbWallAvoidActive) {
        ...
        apFunctions->GetLowLevelGfx()->DrawLine(mpCharBody->GetPosition(), vGoal + mvSteeringVec * fSteerAmount,
                                                cColor(1, 0, 0));
    ...
    if (mbWallAvoidActive && bMoving) {
        ...
        for (size_t i = 0; i < mvSampleRays.size(); ++i) {
            ...
            apFunctions->GetLowLevelGfx()->DrawLine(
                mpCharBody->GetPosition(), mpCharBody->GetPosition() + mvSampleRays[i] * mfWallAvoidRadius, col);
        }
        ...
        cColor vDebugColors[] = {cColor(1,0,0), cColor(0,1,0), cColor(0,0,1), cColor(1,0,1), cColor(1,1,0),
        cColor(1,1,1)}; for(size_t i=0; i<mvPrecalcSampleDirs.size(); ++i)
        {
            ...
            //apFunctions->GetLowLevelGfx()->DrawLine(mpCharBody->GetPosition(), mpCharBody->GetPosition()+vDir*2,
        vDebugColors[i / lPartCount]); apFunctions->GetLowLevelGfx()->DrawSphere(mpCharBody->GetPosition()+vDir*2,
        0.05f, vDebugColors[i /lPartCount]);
        }*/
    }
}
...
void cLuxEnemyMover::ForceMoveState(eLuxEnemyMoveState aMoveState) {
    ...
    case eLuxEnemyMoveState_Walking: {
        ...
        mpCharBody->Move(eCharDir_Forward, 1.0f);
        mpCharBody->SetLastPosition(mpCharBody->GetPosition() - mpCharBody->GetForward() * mpEnemy->mfForwardSpeed *
                                                                    gpBase->mpEngine->GetStepSize());
        mpEnemy->PlayAnim(aMoveState == eLuxEnemyMoveState_Running ? mpEnemy->GetRunAnimationName()
                                                                   : mpEnemy->GetWalkAnimationName(),
                          true, 0.0f, true, mpEnemy->mfMoveSpeedAnimMul, false, false);
        break;
    }
    }
...
void cLuxEnemyMover::UpdateMoveAnimation(float afTimeStep) {
    ...
    bool bCanJog =
        mpEnemy->mfWalkToJogSpeed > 0 && mpEnemy->mfRunToJogSpeed > 0 && mpEnemy->GetJogAnimationName() != "";
    ...
    if (prevMoveState != mMoveState) {
        ...
        if (mMoveState == eLuxEnemyMoveState_Backward) {
            // Log(" To Backward\n");
            mpEnemy->PlayAnim(mpEnemy->GetBackwardAnimationName(), true, fFadeSpeed, true, mpEnemy->mfMoveSpeedAnimMul,
                              false, false);
        }
        // Stopped
        ...
        // Walking
            ...
            mpEnemy->PlayAnim(mpEnemy->GetWalkAnimationName(), true, fFadeSpeed, true, mpEnemy->mfMoveSpeedAnimMul,
                              bSync, false);
        }
        // Jogging
            ...
            mpEnemy->PlayAnim(mpEnemy->GetJogAnimationName(), true, fFadeSpeed, true, mpEnemy->mfMoveSpeedAnimMul,
                              bSync, false);
        }
        // Running
            ...
            mpEnemy->PlayAnim(mpEnemy->GetRunAnimationName(), true, fFadeSpeed, true, mpEnemy->mfMoveSpeedAnimMul,
                              bSync, false);
        }
    }
...
void cLuxEnemyMover::UpdateWallAvoidance(float afTimeStep) {
    ...
    if (mfWallAvoidCount >= fUpdateTime) {
        ...
        for (size_t i = 0; i < mvSampleRays.size(); ++i) {
            ...
            float fDistance = 0;
            bool bCollide = gpBase->mpMapHelper->GetClosestCharCollider(
                mpCharBody->GetPosition(), vDir, mfWallAvoidRadius, false, &fDistance, NULL, NULL);
...
kBeginSerializeBase(cLuxEnemyMover_SaveData)
    ...
    kSerializeVar(mbTurning, eSerializeType_Bool) kSerializeVar(mfTurnGoalAngle, eSerializeType_Float32)
        kSerializeVar(mfTurnSpeed, eSerializeType_Float32) kSerializeVar(mfTurnBreakAcc, eSerializeType_Float32)
