...
void cLuxJournal_ListEntry::AddWidget(iWidget *apWidget) { mlstWidgets.push_back(apWidget); }
...
void cLuxJournalStateData::Reset() { mfAlpha = 0; }
...
void cLuxJournalStateData::OnDraw(float afFrameTime) {
    ...
    ///////////////////////
    // Note open
    if (mState == eLuxJournalState_OpenNote || mState == eLuxJournalState_OpenHint ||
        mState == eLuxJournalState_OpenDiary || mState == eLuxJournalState_OpenNarratedDiary) {
...
cLuxDiaryContainer::~cLuxDiaryContainer() { STLDeleteAll(mvDiaries); };
...
void cLuxJournal::RemoveAllHints() { STLDeleteAll(mvHints); }
...
void cLuxJournal::SetupNavigationWidgets(eLuxJournalState aState, int alListIndex, int alForwardIndex,
                                         int alBackwardIndex, cWidgetDummy *apRoot) {
    cVector2f vSize(65, 46);
    cVector3f vPos = cVector3f(360 + mfNoteTextWidth / 2 - vSize.x - 80,
    ...
    // Forward
    mpImageForward[aState] = mpGuiSet->CreateWidgetImage("journal/note_forward.tga", vPos + cVector3f(41, 0, 0), vSize,
                                                         eGuiMaterial_Alpha, false, apRoot);
    ...
    // Return
    if (aState == eLuxJournalState_Diaries || aState == eLuxJournalState_Notes || aState == eLuxJournalState_Hints ||
        aState == eLuxJournalState_QuestLog) {
        ...
        cWidgetImage *pImage =
            mpGuiSet->CreateWidgetImage("journal/note_return.tga", 0, -1, eGuiMaterial_Alpha, false, apRoot);
    ...
    mpImageBackward[aState] = mpGuiSet->CreateWidgetImage("journal/note_backward.tga", vPos + cVector3f(41, 0, 0),
                                                          vSize, eGuiMaterial_Alpha, false, apRoot);
...
void cLuxJournal::LoadText(const tWString &asName, const tWString &asText) {
    ...
    for (size_t i = 0; i < sTempRows.size(); ++i) {
        ...
        ///////////////////////////////
        // If first row, max row num is reach or a new page tag found, then make new page and push all coming rows
        // there.
...
void cLuxJournal::SetNoteListPageFromHint(int alHint) {
    ...
    if (lPage > -1) {
        ...
        if (lEntry < mvNoteListPages[3][lPage].mvEntries.size()) {
            iWidget *pWidget = mvNoteListPages[3][lPage].mvEntries[lEntry].mlstWidgets.empty()
                                   ? NULL
                                   : mvNoteListPages[3][lPage].mvEntries[lEntry].mlstWidgets.front();
...
void cLuxJournal::SetNoteListPageFromNote(int alDiary) {
    ...
    if (lPage > -1) {
        ...
        if (lEntry < mvNoteListPages[0][lPage].mvEntries.size()) {
            iWidget *pWidget = mvNoteListPages[0][lPage].mvEntries[lEntry].mlstWidgets.empty()
                                   ? NULL
                                   : mvNoteListPages[0][lPage].mvEntries[lEntry].mlstWidgets.front();
...
void cLuxJournal::CreateMainGui() {
    ...
    // Notes
    pLabel =
        mpGuiSet->CreateWidgetLabel(cVector3f(300, 200, 3), cVector2f(200, 25), kTranslate("Journal", "Notes"), pState);
    ...
    pLabel =
        mpGuiSet->CreateWidgetLabel(cVector3f(300, 250, 3), cVector2f(200, 25), kTranslate("Journal", "Hints"), pState);
    ...
    // pState); SetupLabel(pLabel,fSize,1, eLuxJournalState_Main); pLabel->AddCallback(eGuiMessage_MouseDown,this,
    // kGuiCallback(MainMenuTextClick)); pLabel->AddCallback(eGuiMessage_OnDraw,this, kGuiCallback(MainMenuTextOnDraw));
    // vLabels.push_back(pLabel);
    //
    ////Quest Log
    // pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(300, 300, 3),cVector2f(200, 25),kTranslate("Journal","Quest Log"),
    // pState); SetupLabel(pLabel,fSize,2, eLuxJournalState_Main); pLabel->AddCallback(eGuiMessage_MouseDown,this,
    // kGuiCallback(MainMenuTextClick)); pLabel->AddCallback(eGuiMessage_OnDraw,this, kGuiCallback(MainMenuTextOnDraw));
    // vLabels.push_back(pLabel);
...
void cLuxJournal::CreateNotesGui() {
    ...
    // Has notes
        ...
        for (size_t i = 0; i < mvNotes.size(); ++i) {
            ...
            // cVector2f(20, 20),eGuiMaterial_Alpha, false, pRoot); SetupImage(pImage,(int)i,eLuxJournalState_Notes);
...
void cLuxJournal::CreateHintsGui() {
    ...
    // Header
    pLabel = mpGuiSet->CreateWidgetLabel(cVector3f(250, mfNoteListHeaderY, 3), cVector2f(300, 32),
                                         kTranslate("Journal", "Hints"), pRoot);
    ...
    // Has hints
        ...
        for (int i = (int)mvHints.size() - 1; i >= 0; --i) {
            ...
            // Add label
            pLabel = mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24),
                                                 kTranslate("Journal", pHint->msNameEntry), pRoot);
            ...
            {
                pImage = mpGuiSet->CreateWidgetImage(pHint->msIconFile, vStartPos + cVector3f(-25, 2, -0.05f),
...
void cLuxJournal::CreateDiariesGui() {
    ...
    if (mvDiaryContainers.empty()) {
        vStartPos.y = 270;
        pLabel =
            mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24), kTranslate("Journal", "DiariesEmpty"), pRoot);
    ...
    // Has Diaries
        ...
        for (size_t cont = 0; cont < mvDiaryContainers.size(); ++cont) {
            ...
            for (size_t i = 0; i < pCont->mvDiaries.size(); ++i) {
                ...
                // pImage = mpGuiSet->CreateWidgetImage(pDiary->msIconFile, vStartPos - cVector3f(23,0,1), cVector2f(20,
                // 20),eGuiMaterial_Alpha, false, pRoot); SetupImage(pImage,(int)i,eLuxJournalState_Diaries);
...
void cLuxJournal::CreateQuestNotesGui() {
    ...
    if (bHasActiveQuests == false) {
        vStartPos.y = 270;
        pLabel =
            mpGuiSet->CreateWidgetLabel(vStartPos, cVector2f(300, 24), kTranslate("Journal", "QuestsEmpty"), pRoot);
...
void cLuxJournal::CreateOpenNoteGui() {
    ...
    // Diary and note
    for (int i = 0; i < 4; ++i) {
        eLuxJournalState state =
            i == 0 ? eLuxJournalState_OpenNote
                   : (i == 1 ? eLuxJournalState_OpenDiary
                             : (i == 2 ? eLuxJournalState_OpenHint : eLuxJournalState_OpenNarratedDiary));
...
bool cLuxJournal::MainMenuTextOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    if (apWidget->HasFocus())
        apWidget->SetDefaultFontColor(
            cColor(232.0f / 255.0f, 201.0f / 255.0f, 28.0f / 255.0f, apWidget->GetColorMul().a));
    else
...
bool cLuxJournal::ListTextOnDraw(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if(pData->mfEffectfAlpha >0)
    {
        ...
        if(pLabel->GetTextAlign() == eFontAlign_Center)
        {
            ...
            mpGuiSet->DrawFont(	apWidget->GetText(), apWidget->GetDefaultFontType(), vPos, vSize, cColor(1,0,0, fAlpha),
    eFontAlign_Center);
        }
        //////////////////////
        //Left Aligned
...
bool cLuxJournal::UIListenerArrowPress(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    case eUIArrow_Down:
        ...
        if (gpBase->mpInputHandler->IsGamepadPresent() &&
            gpBase->mpInputHandler->GetGamepad()->ButtonIsDown(eGamepadButton_DpadDown) == false)
            break;
#endif
        if (lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenDiary ||
            lType == eLuxJournalState_OpenHint || lType == eLuxJournalState_OpenNarratedDiary) {
            ExitPressed(false);
            return true;
        }
        break;
...
bool cLuxJournal::UIListenerButtonPress(iWidget *apWidget, const cGuiMessageData &aData) {
    ...
    if (!(lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenDiary ||
          lType == eLuxJournalState_OpenNote || lType == eLuxJournalState_OpenHint ||
