...
bool cLuxSaveHandlerThreadClass::IsRunning() { return mpThread != NULL; }
...
void cLuxSaveHandlerThreadClass::UpdateThread() { ProcessPendingSaves(); }
...
void cLuxSaveHandler::LoadSaveGameData(cLuxSaveGame_SaveData *apSave) {
    ...
    ///////////////////
    // Load new map
    if (pCurrentMap == NULL || msOldMapFolder != gpBase->mpMapHandler->GetMapFolder() ||
...
tWString cLuxSaveHandler::GetSaveName(const tWString &asPrefix) {
    ...
    tWString sFileName =
        asPrefix + _W("_") + cString::To16Char(sMapName) + _W("_") + cString::ToStringW(currentDate.year) + _W("_") +
        cString::ToStringW(currentDate.month + 1) + _W("_") + cString::ToStringW(currentDate.month_day) + _W("_") +
        cString::ToStringW(currentDate.hours) + _W("_") + cString::ToStringW(currentDate.minutes) + _W("_") +
