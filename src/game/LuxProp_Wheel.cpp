...
float cLuxProp_Wheel::OnInteractDebugDraw(cGuiSet *apSet, iFontData *apFont, float afStartY) {
    apSet->DrawFont(apFont, cVector3f(5, afStartY, 5), 12, cColor(1, 1), _W("WheelAngle: %f, Max: %f, Min: %f"),
                    cMath::ToDeg(mfAngle), cMath::ToDeg(mfMaxLimit), cMath::ToDeg(mfMinLimit));
    afStartY += 13.0f;
...
void cLuxProp_Wheel::UpdateAutoRotation(float afTimeStep) {
    ...
    if (mbAutoMoving) {
        if (cMath::Abs(mfAngle - mfAutoMoveGoal) < cMath::ToRad(0.1f)) {
            ...
            // cMath::ToDeg(mfAutoMoveGoal),cMath::Abs(mfAngle-mfAutoMoveGoal), fWantedSpeed);
...
kBeginSerialize(cLuxProp_Wheel_SaveData, iLuxProp_SaveData) kSerializeVar(mfAngle, eSerializeType_Float32)
    kSerializeVar(mfPrevAngle, eSerializeType_Float32)

        kSerializeVar(mfLastToMax, eSerializeType_Float32) kSerializeVar(mfLastToMin, eSerializeType_Float32)

            kSerializeVar(mlCurrentState, eSerializeType_Int32) kSerializeVar(mlStuckState, eSerializeType_Int32)
...
void cLuxProp_Wheel::SetupSaveData(iLuxEntity_SaveData *apSaveData) { super_class::SetupSaveData(apSaveData); }
