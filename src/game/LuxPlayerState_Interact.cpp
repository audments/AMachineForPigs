...
void iLuxPlayerState_Interact::ResetInteractVars() { mpCurrentProp->SetIsInteractedWith(false); }
...
kBeginSerializeVirtual(iLuxPlayerState_Interact_SaveData, iLuxPlayerState_SaveData)
    kSerializeVar(mlCurrentPropId, eSerializeType_Int32) kSerializeVar(mlCurrentBodyId, eSerializeType_Int32)
        kSerializeVar(mvCurrentFocusPos, eSerializeType_Vector3f) kEndSerialize()
...
void iLuxPlayerState_Interact::LoadFromSaveDataBeforeEnter(cLuxMap *apMap, iLuxPlayerState_SaveData *apSaveData) {
    ...
    if (pEntity && pEntity->GetEntityType() == eLuxEntityType_Prop) {
        ...
        if (pFoucsBody == NULL) {
            Error("Could not load body with id %d in prop '%s'\n", pData->mlCurrentBodyId,
                  mpCurrentProp->GetName().c_str());
        }
    } else {
